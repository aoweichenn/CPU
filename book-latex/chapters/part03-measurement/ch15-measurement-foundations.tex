...
\subsection{被测对象：workload、kernel、measurement}
...
\subsubsection{\code{steady_clock}}
...
\subsubsection{\code{do_not_optimize}}
...
\subsubsection{\code{clobber_memory}}
...
\subsection{一个最小 C++20 benchmark harness}
...
\begin{center}
\begin{tabular}{L{0.20\linewidth}L{0.35\linewidth}L{0.35\linewidth}}
\toprule
指标 & 直觉 & 注意 \\
\midrule
...
\subsection{常见 benchmark 反模式}

\subsubsection{反模式 1：把打印放进 timed region}
...
\subsection{实验 15.1：修复一个会被优化器删除的 benchmark}
...
\subsection{实验 15.2：写一个最小可信 sum benchmark}
...
\subsection{实验 15.3：比较 \code{steady_clock} 与 TSC}
...
\subsection{实验 15.4：用 \code{perf stat} 建立硬件证据}
...
\begin{exercisebox}
\subsubsection*{作业 1：benchmark 批判性阅读}
...
\subsubsection*{作业 2：最小 harness 代码评审}

\section{Benchmark 设计：输入、热路径和报告}
...
\subsection{Batch：短 kernel 的必要技巧}
...
\subsection{一个小型 benchmark suite 的数据模型}
...
\subsection{Workload factory：生成输入但不污染计时}
...
\subsection{Kernel API：统一但不过度抽象}
...
\subsection{校验策略：不同 kernel 不同方法}
...
\subsection{样本汇总：从 raw 到 summary}
...
\subsection{派生指标：不要只给时间}

不同 kernel 应输出不同派生指标。

\subsubsection{\code{sum_u64}}
...
\subsection{实验 16.2：加入 \code{copy_u64} 和 \code{fill_u64}}
...
\subsection{实验 16.5：完整 benchmark 报告目录}
...
\begin{exercisebox}
\subsubsection*{作业 1：设计一个 benchmark suite 架构}
...
\subsubsection*{作业 2：输入规模和 cache 假设}
...
\subsubsection*{作业 3：CSV schema 评审}
...
\subsubsection*{作业 4：多 kernel suite 实现}
...
\subsubsection*{作业 5：Benchmark 报告}

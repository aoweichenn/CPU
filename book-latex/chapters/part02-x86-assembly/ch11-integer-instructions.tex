...
\subsection{\instruction{mov}：复制位模式，不是高级赋值}
...
\subsection{清零寄存器：\instruction{xor reg, reg} 和 \instruction{mov reg, 0}}
...
\subsection{加法和减法：结果、flags 和语言规则}

\subsubsection{\instruction{add} 和 \instruction{sub}}
...
\subsubsection{\instruction{inc} 和 \instruction{dec}}
...
\subsection{乘法：\instruction{imul}、常数乘法和 \instruction{lea}}

\subsubsection{常数乘法经常不用乘法指令}
...
\subsubsection{\instruction{imul} 的常见形式}
...
\subsection{符号扩展和零扩展：\instruction{movsx}、\instruction{movzx}、\instruction{movsxd}}
...
\subsubsection{\instruction{movsxd}：32 位到 64 位符号扩展}
...
\subsection{移位：乘除 2 的幂、位域和符号}

\subsubsection{\instruction{shl} 和 \instruction{sal}}
...
\subsubsection{\instruction{shr} 和 \instruction{sar}}
...
\subsection{位运算：mask、alignment 和 flags}

\subsubsection{\instruction{and}、\instruction{or}、\instruction{xor}、\instruction{not}}
...
\subsection{\instruction{cmp} 和 \instruction{test}：只设置 flags 的计算}
...
\subsection{read-modify-write：一条指令不等于一次微操作}
...
\subsection{案例：branchless mask 的第一眼}
...
\subsection{性能注意点：延迟、吞吐、依赖和 flags}
...
\subsection{实验 11.2：\instruction{lea}、乘法和结构体步长}

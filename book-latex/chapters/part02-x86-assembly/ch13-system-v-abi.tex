...
这里故意不把 \code{Pair} 的最终位置写死，因为完整分类需要按 ABI 规则仔细判断字段、padding 和 eightbyte。教材式报告应明确“需要分类”，而不是凭直觉把整个结构体塞进一个寄存器。实际实验可以通过编译小函数、查看调用点和被调用者读取位置来验证。
...
\section{实验 13.2：破坏 callee-saved 寄存器}
...
\section{实验 13.3：栈对齐和 \instruction{movaps}}
...
\section{实验 13.5：变参函数和 \register{al}}

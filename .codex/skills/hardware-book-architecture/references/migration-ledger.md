...
本批次不改变章节、内容单元、练习或答案，只处理成书表现：增强机制图节点边界和手机尺寸辨识度，绑定图与图注，防止主题、习题和解答标题孤悬页底，适度增加表格行距并控制孤行；同时修复第 25 章短代码转义被字体保护命令逐字排出的问题，并分离第 26 章教学机总图右侧过近的状态返回线与控制干线。验收范围覆盖全部含图页面缩略扫描、受影响页面高分辨率复检、全书构建日志与手机导出。

| 增补范围 | 内容单元 | 状态 |
|---|---|---|
| 全书版式 | 图与图注绑定、标题页底保护、表格行距、孤行控制、图形边线对比度 | verified |
| 第 25 章 | 反斜杠与美元符号短代码的正确排印 | verified |
| 第 26 章 | 教学机总图状态返回线与控制干线分离 | verified |

## 2026-07 坐标图范围与读图语义精修批次

本批次不改变章节、正文知识单元、练习或答案，只修正坐标图的范围表达与零基础读图语义。所有曲线都必须在独立绘图区内留出余量，并使用裁剪边界防止控制点越界；负值、对数轴、归一化轴和判决阈值要明确标出。对抽象曲线补充单位、关键刻度、图例、参考线、分区和读图步骤，最终以 PDF 实际页面而不是源码坐标验收。

| 增补范围 | 内容单元 | 状态 |
|---|---|---|
| 第 9 章 | 内存墙半对数曲线的绘图区、网格、端点余量与同年差距读法 | verified |
| 第 11 章 | SRAM 蝴蝶曲线的坐标范围、镜像轴、图例与 SNM 三步读法 | verified |
| 第 11 章 | 负位线波形的负电压范围、零电平与写入窗口 | verified |
| 第 14 章 | SLC/TLC 阈值分布的独立面板、参考线、状态间距与判决语义 | verified |
| 第 22 章 | 阶跃响应的归一化纵轴、超调上界、稳态容差与曲线图例 | verified |

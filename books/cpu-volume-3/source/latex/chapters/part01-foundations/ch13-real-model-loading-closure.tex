...
\subsection{当前 lab 的 tokenizer 合同与 GPT-2 BPE}

当前 \filepath{books/cpu-volume-3/labs/linux\_cpu\_inference} 先把 tokenizer 从“正文要求”推进到可运行 fixture。这个 fixture 不是完整 BPE 或 SentencePiece 实现，而是一个最小合同：BOS/EOS、UNK 策略、chat template hash、词表、prompt fixture 和合同 hash。
...
在这个最小合同之后，lab 进一步新增了 GPT-2 byte-level BPE 路径。GPT-2 的 tokenizer 不是按空格查词表，而是先把 UTF-8 字节映射到可逆 unicode 符号，再按 \code{merges.txt} 的 rank 做 BPE 合并，最后查 \code{vocab.json}。因此真实报告要同时保存 prompt ids 和 detokenize 结果，否则只看生成文本会掩盖 tokenizer 错误。

\begin{lstlisting}[numbers=none,caption={GPT-2 tokenizer smoke 的关键证据}]
...
完整 SentencePiece、tokenizer.json 的所有 normalizer/pre-tokenizer 组合、chat template 和 GGUF 内嵌 tokenizer 仍然是后续工作。当前状态应准确写成：LCQI 已支持 GPT-2 byte-level BPE 的 reference path；还没有支持所有主流 tokenizer 方言。
...
\subsection{当前 lab 的 safetensors manifest gate 与 GPT-2 权重读取}

当前 lab 已经补了一个 safetensors header 解析器。它把真实模型格式最关键的目录事实落到可测试代码：header 长度、\code{\_\_metadata\_\_}、tensor name、dtype、shape、\code{data\_offsets}、payload 边界，以及 dtype/shape 推导出的字节数是否等于 offset 区间。
...
测试会运行时生成一个合法 fixture，并验证两个 tensor 的名称、dtype、shape 和 offset；还会生成 \code{data\_offsets} 超出 payload、dtype/shape 字节数不匹配的坏文件，要求加载器拒绝。新增 GPT-2 路径后，测试还会读取 F32/F16/BF16 tensor，临时写出一个 HuggingFace 风格 tiny GPT-2 \code{model.safetensors}，再通过 loader 读回并对齐 logits。这个 gate 的意义是把“权重格式导入”从口号压到工程边界：name mapping、shape verifier、dtype 转换和 Conv1D 转置都必须可测。若连 offset 范围和字节数都不检查，后端 kernel 读到的只是任意字节流。
...
这样写能保持边界诚实：当前 lab 已经完成 GPT-2 子集的真实权重解释和生成 smoke，但还没有完成 7B 级模型、GGUF 自研解析、分片 safetensors、量化 block、KV cache 优化和外部框架逐 logits 对齐。

\subsection{GPT-2 reference path：为什么要单独实现}

GPT-2 与第三册早期的 LLaMA-like tiny decoder 不同。它使用 LayerNorm，而不是 RMSNorm；使用 learned absolute position embedding，而不是 RoPE；attention projection 在 HuggingFace 权重里是 packed QKV Conv1D；MLP 使用 GELU；\code{lm\_head} 通常与 token embedding 共享权重。若把 GPT-2 硬塞进原 decoder，读者会误以为这些结构差异只是变量名不同。

因此 LCQI 新增 \code{gpt2\_reference.hpp/.cpp}。它的第一目标是正确性，不是吞吐：读取 \code{config.json}，加载 \code{vocab.json} 和 \code{merges.txt}，从 \code{model.safetensors} 读取 F32/F16/BF16 张量，解析两类 HuggingFace 权重名前缀，执行 LayerNorm、绝对位置嵌入、packed QKV、causal attention、GELU、tied lm head 和 greedy generation。

\begin{lstlisting}[numbers=none,caption={运行 LCQI 自研 GPT-2 smoke}]
make cpu3-gpt2-smoke
\end{lstlisting}

脚本下载 \code{openai-community/gpt2} 的标准资产到用户缓存目录，构建 \code{lcqi\_gpt2}，并把报告写入：

\begin{lstlisting}[numbers=none,caption={GPT-2 smoke 报告路径}]
books/cpu-volume-3/results/lcqi-gpt2-smoke.txt
\end{lstlisting}

最近一次本机报告显示：

\begin{lstlisting}[numbers=none,caption={GPT-2 smoke 关键输出}]
prompt_ids 15496 11 616 1438 318
generated_ids 15496 11 616 1438 318 1757
generated_text Hello, my name is John
validation=PASS
...
这条 smoke 的结论也必须写窄。它证明 LCQI 自研 C++ reference path 已经能跑标准 GPT-2 safetensors 的 tokenizer、权重导入、forward 和 greedy generation；它不证明 7B 模型已支持，也不证明所有 HuggingFace 架构都能加载。GPT-2 路径现在同时保留两种生成方式：\code{full\_prefix} 每生成一个 token 就重跑完整前缀；\code{cached\_kv} 在 prefill 阶段把每层 key/value 写进 cache，后续 decode 只计算新 token 并读取历史 KV。这个对照比单独报一个速度数字更有教学价值，因为它把“为什么推理引擎必须有 KV cache”落到了可复跑命令上。

\begin{lstlisting}[numbers=none,caption={运行 GPT-2 full-prefix、cached-KV 和外部引擎对比}]
make cpu3-gpt2-benchmark-compare
\end{lstlisting}

报告写入：

\begin{lstlisting}[numbers=none,caption={GPT-2 benchmark 对比报告路径}]
books/cpu-volume-3/results/lcqi-gpt2-benchmark-compare.txt
\end{lstlisting}

最近一次本机 aarch64/Qualcomm 报告中，LCQI full-prefix 在 GPT-2 F32 上生成 4 个 token 的生成阶段为 \code{2522.81 ms}，约 \code{1.586 token/s}；LCQI cached-KV 同样生成 \code{Hello, my name is John. I'm}，生成阶段为 \code{826.152 ms}，约 \code{4.842 token/s}。cached-KV 的第一个新 token 来自 prefill 最后一步，后续 decode 实际执行 3 个 forward step，耗时 \code{309.524 ms}，约 \code{9.692 step/s}。同机 llama.cpp 使用 SmolLM2-135M-Instruct Q4\_K\_M 的 \code{llama-bench -p 5 -n 4} 作为成熟引擎参照，prefill 为 \code{76.41 token/s}，decode 为 \code{27.83 token/s}。

这个对照不能被误读成同模型质量排名：LCQI 跑的是 GPT-2 F32 reference path，llama.cpp 跑的是 GGUF 量化 small instruct 模型。它说明的差距是工程能力。LCQI 已经消除了“每步重算前缀”的主要算法浪费，但还缺 packed weight、量化 matmul、SIMD/SVE/NEON kernel、线程并行、mmap/lazy load、分页 KV cache、成熟采样器和外部 golden logits 对齐。下一步不是再写一个口头模板，而是把这些差距分别变成可测的 kernel benchmark、logits 对齐报告和内存账本。

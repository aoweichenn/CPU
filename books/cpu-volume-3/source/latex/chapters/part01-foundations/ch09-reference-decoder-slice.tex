...
为了避免每一节换例子，本册使用同一条 TinyTrace-16 作为手算主线：
...
测试入口在 \filepath{books/cpu-volume-3/labs/linux_cpu_inference/tests/lcqi_tests.cpp}。目前它覆盖 RMSNorm、RoPE、KV attention、reference decode end-to-end、packed int8 kernel 与 scalar 对齐。下一步要把 TinyTrace-16 的每个 checkpoint 固化成 golden 文件，而不是只检查最终 \code{predicted\_token}。
...
\subsection{逐张量 checkpoint 应该长什么样}

最终 logits 只能说明端到端错了，不能说明错在哪里。TinyTrace-16 的 checkpoint 应该记录每个阶段的 shape、少量数值摘要和可选完整 tensor。最小格式可以这样设计：

\chapter{张量布局、地址流与第一个矩阵向量内核}

\section{为什么先讲布局再讲算子优化}
...
\begin{lstlisting}[numbers=none,caption={本章先追踪这一层}]
...
\section{layout conversion 也要进入 profiler}
...
\section{从 float32 路径过渡到 int8 路径}
...
\section{把这条账本接回 Transformer}

现在把本章的 \code{matvec} 账本放回 decoder-only Transformer 的一个 decode 步骤。假设当前层输入是一条 hidden 向量 \code{x[hidden]}，这一层至少要做下面几组投影：
...
这里的每个 \code{matvec} 都可以用本章的方法拆开：输入向量地址、权重行地址、accumulator 依赖、输出 store、量化 scale 和 oracle。不同的是，真实模型把尺寸和状态放大了。Q/K/V 的输出还要 reshape 成 \code{[heads, head_dim]}；K/V 不是普通临时输出，而要写入第 \code{layer} 层、第 \code{position} 个 token 的 KV cache；attention 会在后续 token 中反复读取这些 cache 行。
...
prefill 阶段则把 \code{x} 从一条向量扩成多条 token 行。此时 \code{matvec} 账本会变成下一章的 GEMM/GEMV 问题：prompt token 越多，权重复用越明显，packing、tile、SIMD 和 GPU kernel 的价值越容易体现；decode 阶段每次只处理一个或少数 token，权重流和 KV cache 读写又会成为另一种瓶颈。因此后续性能报告必须把 prefill tokens/s、decode tokens/s 和首 token 延迟分开写。

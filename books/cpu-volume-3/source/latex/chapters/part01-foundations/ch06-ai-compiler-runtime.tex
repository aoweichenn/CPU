\section{AI 编译器：从推理图到机器执行计划}

\subsection{推理引擎里为什么会出现编译器}
...
\subsection{IR：图 IR、张量 IR 和循环 IR 要分层}
...
\subsection{内存规划：生命周期分析是推理编译器的核心}
...
\subsection{后端 lowering：CPU 和 GPU 的目标不同}
...
\subsection{runtime 调度器：执行计划怎样变成 token 流}
...
\subsection{plan cache 和 shape bucket}

\chapter{推理运行时与内存规划：typed graph 如何服务一次真实请求}

\section{先从一个会抖动的请求开始}
...
\section{runtime state：请求不是一串独立 kernel}
...
\section{activation arena：把生命周期分析落到地址}
...
\section{packed weight：把文件布局变成内核布局}
...
这一步和传统编译器的后端 lowering 很像。高层 IR 中的 \code{linear(h, Wq)} 是语义；packed weight 是目标机器上的物理表示。语义值相同，不代表物理布局相同。CPU AVX2、AVX-512、ARM NEON 和 GPU tensor core 可能需要完全不同的 packed descriptor；runtime 选择 plan 时必须同时选择对应的 packed weight。

\section{KV cache planner：跨 token 状态不能当 scratch}
...
\section{paged KV allocator：不是 vector push back}
...
\section{prefill 和 decode：两套计划，两套指标}
...
\section{调度边界：segment 比单算子更适合热路径}
...
\section{profiler 和 oracle：运行时要留下证据}

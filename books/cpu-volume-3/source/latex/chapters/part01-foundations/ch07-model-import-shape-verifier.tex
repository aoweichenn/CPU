\chapter{AI 编译器前端：模型导入、manifest 与 shape verifier}

\section{先从一个会悄悄跑错的模型开始}
...
\section{schema resolution：把模型方言翻译成统一角色}
...
\section{一层 decoder 的 shape 推导例子}
...
\section{shape verifier：让每条边都有可证明的形状}
...
\section{dtype verifier：不是每个字节数组都能进 kernel}
...
\section{前端产物：typed graph 必须足够给后端使用}

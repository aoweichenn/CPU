...
\subsection{三种 decode attention 读流：朴素、分组复用和分页}

上一节的表只给出容量和唯一元素下界。真正做后端时，还要把 kernel 的读取顺序写出来。对 GQA 7B，\code{n\_q=32,n\_kv=8}，每个 KV head 服务 4 个 query head。三种实现的字节账本完全不同。

\topic{朴素 per-query-head 扫描。} 最直接的写法是外层遍历 query head，内层扫全部历史位置：

\begin{lstlisting}[numbers=none,caption={朴素 GQA decode attention 读流}]
for qh in 0..n_q:
  kvh = qh / (n_q / n_kv)
  for pos in 0..S:
...
\topic{按 KV head 分组复用。} 更好的 CPU/GPU kernel 会把同一个 KV head 下的 query head 分成一组。每次读取一个历史 K/V 向量后，给 4 个 query head 同时使用。这样唯一 K/V 读流接近下界，但代价是寄存器、accumulator 和 softmax 状态更多：

\begin{lstlisting}[numbers=none,caption={按 KV head 分组的读流}]
for kvh in 0..n_kv:
...
\topic{paged KV attention。} Paged KV 把连续逻辑位置映射到物理 block/page。它解决的是多 session、变长上下文、prefix 共享和内存碎片问题，不会让历史 K/V 本身消失。读流会多一层 block table：

\begin{lstlisting}[numbers=none,caption={paged KV 的额外地址流}]
...
\begin{center}
\small
\begin{tabular}{p{0.23\linewidth}p{0.25\linewidth}p{0.25\linewidth}p{0.15\linewidth}}
\toprule
实现形态 & K/V 主读流 & 额外读写 & 适合场景 \\
\midrule
per-query 扫描 & 接近 \code{n\_q} 重读 & score 临时简单 & 教学 baseline \\
按 KV head 分组 & 接近 \code{n\_kv} 下界 & 多 head softmax 状态 & GQA/MQA decode \\
online softmax & 不物化完整 score & running max/sum/out & 长上下文、显存紧张 \\
paged KV & K/V 仍按历史读取 & block table、page stride & 多 session serving \\
FlashAttention 风格 tile & 减少 score/HBM 往返 & tile 状态和同步 & prefill、大块 attention \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{把 attention 读流写进 profiler schema}

Attention benchmark 只报 \code{tokens/s} 不够。至少要让 profiler 输出下面这些字段，才能区分“模型结构账本”和“kernel 实际读流”：

\begin{lstlisting}[numbers=none,caption={decode attention profile 的关键字段}]
...
若 \code{kv\_estimated\_read\_bytes / kv\_unique\_bytes\_lower\_bound} 接近 \(n_q/n_{kv}\)，说明 GQA 的结构收益没有被 kernel 读流吃到。若 \code{score\_temp\_bytes} 随 \(S\) 很大，说明没有使用 online softmax 或 tile 内归约。若 \code{block\_table\_bytes} 很小但耗时仍高，问题更可能在 K/V 主读流、TLB、NUMA、GPU page locality 或线程组织，而不是 block table 本身。

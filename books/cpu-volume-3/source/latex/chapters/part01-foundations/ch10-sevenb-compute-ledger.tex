...
如果同一配置的带宽上限只有 \code{24 tok/s}，而算力上限是 \code{74 tok/s}，首要问题就不是“FLOPs 不够”，而是每 token 读了太多字节或没有把带宽用起来。CPU micro-kernel 章节里的 \code{4096x4096} 实测会继续把这个差距拆开：同样是 int8 权重，标量、packed scalar 和 AVX2 的有效带宽差异很大。

\chapter{业务服务实战：请求、调度、队列、取消与资源预算}

\section{业务开发不是把 \code{generate()} 暴露成接口}
...
\section{Admission control：先预算，再接纳}
...
\section{调度粒度：一次跑完整请求还是一 token 一调度}
...
\section{prefill 调度和 decode 调度不同}

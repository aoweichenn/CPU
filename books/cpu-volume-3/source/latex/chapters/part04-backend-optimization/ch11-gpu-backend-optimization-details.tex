\chapter{GPU 后端优化细讲：device buffer、stream、kernel launch 与端到端延迟}

\section{GPU 后端不是把 CPU 指针换成 device pointer}
...
\section{host/device 驻留：数据在哪里比函数在哪里更重要}
...
\section{stream 和 event：异步不是自动并行}
...
\section{kernel launch：小算子会被固定开销吞掉}
...
\section{prefill GPU plan：吞吐和 workspace}
...
\section{decode GPU plan：小 batch、KV cache 与同步}

\chapter{GPU decode 端到端：常驻权重、KV 布局、sampler 与同步成本}

\section{GPU 快，不代表端到端一定快}
...
\section{activation 和 workspace}
...
\section{device residency report}
...
\section{prefill 和 decode 的 GPU 策略不同}
...
贯穿项目可以先用 CPU sampler，但必须在 trace 中单独记录 \code{logits\_copy\_ns} 和 \code{sampler\_ns}。这样后续把 sampler 下沉到 GPU 时，收益能被证明。

\section{stream 和 event}

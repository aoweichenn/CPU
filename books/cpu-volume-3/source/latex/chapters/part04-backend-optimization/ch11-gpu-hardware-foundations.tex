\chapter{GPU 硬件基础：SM、warp、显存层级、tensor core 与推理瓶颈}

\section{为什么第三册必须讲 GPU 硬件}
...
\section{warp/wavefront：线程不是完全独立执行}
...
\section{coalescing：连续地址决定一次访存能带回多少有效数据}
...
\section{显存层级：global、shared、register 和 cache}
...
\section{shared memory：它解决的是复用，不是容量}
...
\section{tensor core：不是所有矩阵都天然吃满}
...
\section{用 roofline 判断 GPU 推理瓶颈}
...
\section{prefill 和 decode 的 GPU 瓶颈不同}

\chapter{CPU 后端优化细讲：地址流、SIMD、线程、NUMA 与 C++20 工程边界}

\section{后端优化的第一原则：先解释慢在哪里}
...
\section{GEMV 和 GEMM：prefill 与 decode 的不同账本}
...
\section{SIMD micro-kernel：先设计寄存器账本}
...
\section{线程划分：decode 不一定适合细粒度并行}
...
\section{NUMA、TLB 与大页：长上下文会放大内存事实}
...
\section{KV cache CPU 路径：读流比写入更关键}
...
\section{benchmark harness：测量要能复现}

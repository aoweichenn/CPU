...
repeat=20
f32_matvec_average_us=296.424
q4_f32_input_average_us=258.879
q8_quantize_input_average_us=3.010
q4_q8_matvec_average_us=39.852
q4_q8_total_average_us=42.862
speedup_q4_q8_total_vs_f32=6.916
q4_f32_vs_f32_max_abs_diff=0.000
q4_q8_vs_q4_f32_max_abs_diff=0.011
llama_cpp_pp5_tps=1571.51
llama_cpp_tg4_tps=260.07
\end{lstlisting}

读这组数时要分层。LCQI 的 \code{42.862 us} 是单个真实 \code{Q4\_K} tensor matvec 的耗时，证明量化权重加 \code{Q8\_K} 激活路径已经比“先解成 f32 权重再算”的基线快 \code{6.916x}；llama.cpp 的 \code{pp5=1571.51 tok/s} 和 \code{tg4=260.07 tok/s} 是完整引擎端到端参考，包含模型图、tokenizer、KV cache、线程池、调度和 sampler。二者不能直接相除，但它们共同给出下一步目标：LCQI 还要把这个单算子能力扩展到完整 decoder block，再进入端到端 tokens/s 验收。没有 correctness 对照的 tokens/s，只是吞吐数字，不是 AI Infra 证据。

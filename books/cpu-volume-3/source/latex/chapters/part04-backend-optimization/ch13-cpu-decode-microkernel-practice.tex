...
这些数字的解释要克制。packed scalar 在小 shape 上可能更慢，因为它增加了 block 级循环和 workspace 写入；它的价值是证明 packed 地址流正确。NEON 才是本机 SIMD 证据：它在这些 shape 上输出误差为 0，并且显著快于 scalar。下一步不能再停留在平均耗时，而要补 \code{perf stat}、反汇编摘要、cache miss、branch miss、IPC 和真实系统对照。

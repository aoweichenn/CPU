\chapter{AI 编译器细化：IR、pass、lowering 与 executable plan}

\section{先把 IR 讲清楚：它是可检查的中间账本}
...
\section{Tensor IR：shape、stride、layout 和 dtype 是语义的一部分}
...
\section{RMSNorm + Linear 融合的前置条件}
...
\section{CPU lowering：从 tensor 到 SIMD plan}
...
\section{GPU lowering：把同步和 staging 写进计划}
...
\section{Executable Plan：runtime 执行的是计划，不是重新解释图}

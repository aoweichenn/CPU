...
这会拒绝原本合法的复用。若 memory planner 不知道 oracle materialization，它可能让 \code{\%h\_norm} 覆盖 \code{\%attn}，结果最终 logits 仍可能正确，但中间 dump 和回归定位全部错。可靠的 AI compiler 不是只让模型跑起来，还要让证据点和优化共同存在。

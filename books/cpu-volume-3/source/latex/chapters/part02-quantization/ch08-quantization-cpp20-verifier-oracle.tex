\section{量化工程实现：C++20 descriptor、校准记录、oracle 与回归门禁}

\subsection{为什么量化实现不能只写一个转换脚本}
...
\subsection{QuantDescriptor：量化合同的最小载体}
...
\subsection{CalibrationRecord：校准不是临时日志}
...
\subsection{verifier：把格式错误停在热路径之前}
...
\subsection{IR 中的量化节点：显式边界胜过隐式魔法}
...
\subsection{packing：descriptor 到后端物理布局的转换}
...
\subsection{reference converter：先证明字节解释正确}
...
\subsection{reference decoder layer：把整层链条跑通}
...
\subsection{单 token decode 的 reference 流程}
...
\subsection{把 reference decoder layer 落成 C++20 骨架}
...
\subsection{paged KV、GQA 和量化 KV 的 oracle 闭环}
...
\subsection{一个失败定位例子：不是 softmax，是 scale page 错}
...
\subsection{prefill reference 不能只看最后一列}
...
\subsection{C++20 项目里 oracle 链怎么落地}
...
\subsection{profiler：量化收益要拆成容量、带宽和计算}

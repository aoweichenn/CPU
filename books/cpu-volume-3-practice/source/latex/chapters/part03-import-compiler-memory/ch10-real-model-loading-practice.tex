...
本章新增一个明确边界：tiny fixture 负责手算和逐层 oracle，真实 small 模型负责证明“开源模型资产确实能在本机 CPU 上加载并生成”。二者不能互相冒充。tiny MLP 算对，不等于真实 GGUF 加载器完成；真实 small 模型能输出文本，也不等于回答质量正确或 LCQI 自研后端已经支持完整 GGUF。

\practicesection{一、对应原书问题：概念必须落到对象和命令}

原书讲的是系统边界，本章实践要回答三个问题。第一，哪个代码对象承载这个概念；第二，哪个命令能产生可复查输出；第三，哪个坏输入或缺失字段能证明边界不是空话。这里有两组核心对象。第一组是 \code{tiny_tokenizer.txt}、reference decoder 和 \cmd{lcqi_trace}，用于手算 token 合同和逐 checkpoint 追踪。第二组是 \filepath{tools/run\_smollm2\_small\_smoke.py}，用于下载并校验 SmolLM2-135M-Instruct 的 GGUF 量化文件，再通过 llama.cpp 在 CPU 上实际生成 assistant 文本。
...
\practicesection{三、真实 small 模型 smoke：不用 tiny，先让开源模型实际跑起来}
...
\practicesection{四、实践任务：把观察固定成报告字段}
...
任务三：运行真实 small 模型 smoke。报告不许只贴“命令执行成功”，必须解释脚本为什么校验模型 hash、为什么固定 \code{llama.cpp} commit、为什么使用 \code{-ngl 0}，以及为什么 assistant response 非空只是 smoke，不是质量评测。

任务四：构造反例。围绕本章知识点设计一个坏输入、缺失字段或不满足前提的场景，说明系统应拒绝、降级、fallback 还是继续执行。只给正常路径不合格。真实模型 smoke 的反例可以是：模型 SHA256 不匹配、网络不可用、\code{cmake} 缺失、assistant response 为空、命令超时。每个反例都要写清期望处理方式。

\practicesection{五、验收和递进大作业}

验收标准：报告能从一个具体输入推到工程规则；命令可以在仓库中复跑；输出字段能对应到源码；失败路径说得清；未验证边界不伪装成结论。新增真实模型 smoke 后，最低验收还包括：模型身份有 hash，模型规模不是 tiny，CPU 路径实际运行，报告里有非空 assistant response，并明确声明这不是自研 LCQI 完整 GGUF 支持。

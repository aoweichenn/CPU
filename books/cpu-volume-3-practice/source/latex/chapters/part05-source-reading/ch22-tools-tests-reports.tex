...
\filepath{llama\_gguf.cpp} 是自研 SmolLM2/GGUF decode path。它读取 GGUF metadata 得到 hidden size、层数、GQA 头数、RoPE base、RMSNorm epsilon 和 tokenizer id；再读取 \code{token\_embd.weight}、每层 attention/FFN 权重和 \code{output\_norm.weight}。最新实现把权重表示分成三类：默认启用的 shape 兼容 \code{Q4\_K} linear tensor 保留 GGUF 原始 block bytes，前向时通过 \code{Q8\_K} scratch 调用 \code{matvec\_q4\_k\_q8}；\code{LCQI\_LLAMA\_GGML\_DIRECT=1} 打开的 \code{Q5\_0/Q6\_K/Q8\_0} 实验 tensor 通过 \code{Q8\_0} scratch 调用 GGML direct matvec；其余 tensor materialize 成 f32 fallback。读这份源码时要看三条线：loader 如何记录 \code{q4\_k\_direct\_tensors}、\code{ggml\_direct\_tensors} 与 fallback 字节，\code{linear\_gguf} 如何在三类路径之间分派，\code{LlamaGgufHotspotReport} 如何把 direct/fallback call 和 per-op hotspot 输出到报告。它的意义不是性能已经追上 llama.cpp，而是 LCQI 有了同一个真实 GGUF 文件上的自研端到端 correctness baseline，并且知道哪些 direct path 可以进入默认，哪些必须保留为实验。

\lstinputlisting[language=C++,caption={src/llama\_gguf.cpp}]{../../../cpu-volume-3/labs/linux_cpu_inference/src/llama_gguf.cpp}

\filepath{llama\_cli.cpp} 是 \cmd{lcqi\_llama\_gguf} 的命令行入口。它支持 \code{--ids} 直接给 token id，也支持 \code{--prompt} 从 GGUF tokenizer arrays 构造 chat prompt；\code{--benchmark} 输出 manifest、权重加载、prefill、decode、KV cache、量化权重字节、direct \code{Q4\_K} 字节、实验 \code{ggml\_direct} 字节、fallback f32 字节、direct/fallback tensor 数、per-op hotspot 和 direct/fallback call 数。环境变量 \code{LCQI\_LLAMA\_Q4\_DIRECT=0} 会关闭真实 decoder 里的 Q4 direct path，用于同二进制 A/B；\code{LCQI\_LLAMA\_GGML\_DIRECT=1} 会打开 \code{Q5\_0/Q6\_K/Q8\_0} 实验 direct path，用来证明覆盖率和速度必须分开验收。报告中 \code{decode\_tokens\_per\_second} 只统计真正喂入新 token 后再次预测的 decode step，不能把首 token prefill 的预测误算成续 decode。
...
\filepath{run\_smollm2\_same\_input\_compare.py} 是 SmolLM2 同输入对齐脚本。它构建 LCQI 和固定 commit 的 llama.cpp，先用 \code{LCQI\_LLAMA\_Q4\_DIRECT=0} 跑 LCQI baseline，再跑默认 Q4 direct 路径，再用 \code{LCQI\_LLAMA\_GGML\_DIRECT=1} 跑 \code{Q5\_0/Q6\_K/Q8\_0} 实验 direct 路径，最后跑 \code{llama-simple}、\code{llama-tokenize} 和 \code{llama-bench}。报告必须同时看 \code{same\_input\_prompt\_token\_ids\_match}、LCQI 自身 A/B、llama.cpp 对比和 hotspot 字段。caw 最新报告显示，默认 Q4 direct 把 \code{w\_down} 热点从 \code{91.68 ms} 降到 \code{63.54 ms}，端到端 prefill 提升 \code{1.062x}；实验 GGML direct 把 direct 量化字节覆盖率从 \code{7.68\%} 提高到 \code{70.85\%}，但 prefill 从 \code{370.847 ms} 退化到 \code{1211.710 ms}。这正是源码卷要训练的工程判断：覆盖率、内存、局部 kernel 和端到端速度必须分开证明。

...
\filepath{run\_smollm2\_q4\_k\_benchmark.py} 复用同一个 SmolLM2 GGUF 缓存，构建 LCQI 的 \code{lcqi\_gguf\_q4\_bench}，并可附带 llama.cpp \code{llama-bench} 端到端参考。报告要同时说明模型 SHA-256、tensor 名、shape、量化字节、f32 解码字节、误差、checksum 和成熟引擎参考，避免把不同层级数字混在一起。

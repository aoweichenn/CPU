...
\filepath{llama\_gguf.cpp} 是自研 SmolLM2/GGUF decode path。它读取 GGUF metadata 得到 hidden size、层数、GQA 头数、RoPE base、RMSNorm epsilon 和 tokenizer id；再读取 \code{token\_embd.weight}、每层 attention/FFN 权重和 \code{output\_norm.weight}。最新实现已经把权重表示分成两类：shape 兼容的 \code{Q4\_K} linear tensor 保留 GGUF 原始 block bytes，前向时通过 \code{Q8\_K} scratch 调用 \code{matvec\_q4\_k\_q8}；其他 tensor materialize 成 f32 fallback。读这份源码时要看三条线：loader 如何记录 \code{direct\_quantized\_weight\_bytes} 与 \code{fallback\_dequantized\_weight\_bytes}，\code{linear\_gguf} 如何在 direct/fallback 之间分派，\code{LlamaGgufHotspotReport} 如何把 \code{wq/wk/wv/wo/w\_gate/w\_up/w\_down/lm\_head} 分项耗时输出到报告。它的意义不是性能已经追上 llama.cpp，而是 LCQI 有了同一个真实 GGUF 文件上的自研端到端 correctness baseline，并且第一段量化 block matvec 已经进入真实 decoder。

\lstinputlisting[language=C++,caption={src/llama\_gguf.cpp}]{../../../cpu-volume-3/labs/linux_cpu_inference/src/llama_gguf.cpp}

\filepath{llama\_cli.cpp} 是 \cmd{lcqi\_llama\_gguf} 的命令行入口。它支持 \code{--ids} 直接给 token id，也支持 \code{--prompt} 从 GGUF tokenizer arrays 构造 chat prompt；\code{--benchmark} 输出 manifest、权重加载、prefill、decode、KV cache、量化权重字节、direct \code{Q4\_K} 字节、fallback f32 字节、direct/fallback tensor 数、per-op hotspot 和 direct/fallback call 数。环境变量 \code{LCQI\_LLAMA\_Q4\_DIRECT=0} 会关闭真实 decoder 里的 Q4 direct path，用于同二进制 A/B。报告中 \code{decode\_tokens\_per\_second} 只统计真正喂入新 token 后再次预测的 decode step，不能把首 token prefill 的预测误算成续 decode。
...
\filepath{run\_smollm2\_same\_input\_compare.py} 是 SmolLM2 同输入对齐脚本。它构建 LCQI 和固定 commit 的 llama.cpp，先用 \code{LCQI\_LLAMA\_Q4\_DIRECT=0} 跑 LCQI baseline，再跑当前 Q4 direct 路径，再跑 \code{llama-simple}、\code{llama-tokenize} 和 \code{llama-bench}。报告必须同时看 \code{same\_input\_prompt\_token\_ids\_match}、LCQI 自身 A/B、llama.cpp 对比和 hotspot 字段。caw 最新报告显示，Q4 direct 把 \code{w\_down} 热点从 \code{98.10 ms} 降到 \code{67.50 ms}，但 direct 量化字节只占 \code{7.68\%}，所以端到端 prefill 只提升 \code{1.067x}。这正是源码卷要训练的工程判断：局部 kernel 能赢，不代表完整引擎差距已经解决。

...
\filepath{llama\_gguf.cpp} 是本轮新增的自研 SmolLM2/GGUF reference path。它读取 GGUF metadata 得到 hidden size、层数、GQA 头数、RoPE base、RMSNorm epsilon 和 tokenizer id；再读取 \code{token\_embd.weight}、每层 attention/FFN 权重和 \code{output\_norm.weight}。当前执行方式是加载时统一解到 f32，再走 RMSNorm、normal RoPE、GQA KV cache、SwiGLU 和 tied lm head。它的意义不是性能已经追上 llama.cpp，而是 LCQI 终于有了同一个真实 GGUF 文件上的自研端到端 correctness baseline。

\lstinputlisting[language=C++,caption={src/llama\_gguf.cpp}]{../../../cpu-volume-3/labs/linux_cpu_inference/src/llama_gguf.cpp}

\filepath{llama\_cli.cpp} 是 \cmd{lcqi\_llama\_gguf} 的命令行入口。它支持 \code{--ids} 直接给 token id，也支持 \code{--prompt} 从 GGUF tokenizer arrays 构造 chat prompt；\code{--benchmark} 输出 manifest、权重加载、prefill、decode、KV cache、量化权重字节和解码后 f32 字节。报告中 \code{decode\_tokens\_per\_second} 只统计真正喂入新 token 后再次预测的 decode step，不能把首 token prefill 的预测误算成续 decode。

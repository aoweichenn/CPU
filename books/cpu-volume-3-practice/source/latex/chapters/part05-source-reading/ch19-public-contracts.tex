...
GGML matvec 头文件固定 \code{Q5\_0/Q6\_K/Q8\_0} direct 实验路径的合同。\code{matvec\_ggml\_quantized\_f32} 是 exact oracle：它不先 materialize 整个矩阵，而是直接从 GGUF block bytes 和 f32 input 做点积，用来对照解码后的 f32 权重。\code{quantize\_q8\_0\_input} 把 activation 每 32 个元素临时量化一次，\code{matvec\_ggml\_quantized\_q8\_0} 再执行低比特权重乘 \code{Q8\_0} activation。这个头文件只承诺单算子数学和边界检查，不承诺端到端更快；是否能进入默认路径必须看 caw 同输入 A/B。

\lstinputlisting[language=C++,caption={include/lcqi/ggml\_matvec.hpp}]{../../../cpu-volume-3/labs/linux_cpu_inference/include/lcqi/ggml_matvec.hpp}

LLaMA GGUF 头文件把真实 GGUF 文件接到 decoder 数学层。当前实现有三种权重执行状态：关闭 direct 时是 \code{f32\_dequantized\_reference}；默认是 \code{gguf\_mixed\_q4\_k\_direct}，只让 shape 兼容的 \code{Q4\_K} linear tensor 保留原始 block bytes；显式设置 \code{LCQI\_LLAMA\_GGML\_DIRECT=1} 时才进入 \code{gguf\_mixed\_quantized\_direct\_experimental}，尝试把 \code{Q5\_0/Q6\_K/Q8\_0} 也接入 direct。这个边界很重要，因为它防止把“能跑真实模型”误写成“已经拥有 llama.cpp 同等级的低比特执行引擎”。

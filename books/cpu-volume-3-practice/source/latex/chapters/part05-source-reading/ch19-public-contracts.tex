...
F32 kernel 头文件定义 GPT-2 热路径使用的点积边界。读这里要区分三层：\code{dot\_f32()} 是 checked API，先确认两个 span 长度一致；\code{dot\_f32\_unchecked()} 是热路径入口，由上层 shape validation 保证长度正确；\code{dot\_f32\_scalar\_unchecked()} 是永远可用的正确性基线。AVX2/FMA 是否可用是运行时能力，不是 CMake 把文件编进来就等于一定执行。

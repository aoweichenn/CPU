...
\filepath{include/lcqi/f32_kernels.hpp} 与 \filepath{src/f32_kernels.cpp} & \code{dot\_f32}、\code{dot\_f32\_unchecked}、\code{dot\_f32\_scalar\_unchecked}。 & GPT-2 F32 hot path 的单行点积边界。checked API 负责 size guard，unchecked API 给已经验证过 shape 的热循环使用。 \\
\filepath{src/f32_kernels_avx2.cpp} & \code{dot\_f32\_avx2\_available}、\code{dot\_f32\_avx2\_unchecked}。 & x86 平台优化路径。它只替换一行 dot 的内核，不改变 GPT-2 层结构、线程分片或 greedy tie-breaking。 \\

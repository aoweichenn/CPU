...
这些入口文件回答“代码在哪里”和“怎样运行”。读者进入本卷代码走读章时，先不要直接跳到 \filepath{gpt2_reference.cpp}。先确认 \code{lcqi} 静态库包含 \filepath{model.cpp}、\filepath{inference.cpp}、\filepath{int8_kernels.cpp}、\filepath{f32_kernels.cpp}、\filepath{gguf.cpp}、\filepath{q4_k.cpp}、\filepath{reference_decoder.cpp}、\filepath{tokenizer.cpp}、\filepath{safetensors.cpp} 和 \filepath{gpt2_reference.cpp}；再确认 \code{lcqi_cli}、\code{lcqi_bench}、\code{lcqi_gguf_q4_bench}、\code{lcqi_trace}、\code{lcqi_gpt2}、\code{lcqi_ledger}、\code{lcqi_serving_probe} 和 \code{lcqi_tests} 分别从哪条路径进入。

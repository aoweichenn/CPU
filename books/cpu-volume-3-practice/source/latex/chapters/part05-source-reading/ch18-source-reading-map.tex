...
\filepath{本卷 Makefile} & 本卷的书籍构建入口。 & 它构建 PDF，也提供 \code{test} 目标去构建第三册 LCQI；实践与代码卷不能只排版不验证。 \\
...
这些入口文件回答“代码在哪里”和“怎样运行”。读者进入本卷代码走读章时，先不要直接跳到 \filepath{gpt2_reference.cpp}。先确认 \code{lcqi} 静态库包含 \filepath{model.cpp}、\filepath{inference.cpp}、\filepath{int8_kernels.cpp}、\filepath{f32_kernels.cpp}、\filepath{gguf.cpp}、\filepath{ggml_tensors.cpp}、\filepath{q4_k.cpp}、\filepath{llama_gguf.cpp}、\filepath{reference_decoder.cpp}、\filepath{tokenizer.cpp}、\filepath{safetensors.cpp} 和 \filepath{gpt2_reference.cpp}；再确认 \code{lcqi_cli}、\code{lcqi_bench}、\code{lcqi_gguf_q4_bench}、\code{lcqi_llama_gguf}、\code{lcqi_trace}、\code{lcqi_gpt2}、\code{lcqi_ledger}、\code{lcqi_serving_probe} 和 \code{lcqi_tests} 分别从哪条路径进入。
...
\filepath{include/lcqi/f32_kernels.hpp} 与 \filepath{src/f32_kernels.cpp} & \code{dot\_f32}、\code{linear\_f32\_rows\_unchecked}、\code{linear\_f32\_batch\_rows\_unchecked}、\code{max\_dot\_f32\_rows\_unchecked}。 & F32 fallback 的 kernel 边界。单行 dot 是基础，row-range linear 一次写一段输出行，batch-row linear 在 prompt prefill 中复用同一行权重给多个 token，row-range max 给 tied \code{lm\_head} 用。 \\
\filepath{src/f32_kernels_avx2.cpp} & \code{dot\_f32\_avx2\_available}、8-row row-range fast path、8-token batch-row fast path。 & x86 平台优化路径。decode path 在一段输出行内部把同一份 input load 复用到 8 个输出行；prefill batch path 把同一行权重复用到 8 个 prompt token，并处理 4/3/2/1 token tail；不改变 greedy tie-breaking。 \\
\filepath{include/lcqi/gguf.hpp} 与 \filepath{src/gguf.cpp} & \code{GgufManifest}、\code{GgufTensorInfo}、\code{load\_gguf\_manifest}、\code{read\_gguf\_tensor\_bytes}。 & 读取 GGUF header、metadata、tensor 表、alignment 和真实 tensor payload；它只负责格式边界，不做推理数学。 \\
\filepath{include/lcqi/ggml\_tensors.hpp} 与 \filepath{src/ggml\_tensors.cpp} & \code{dequantize\_ggml\_tensor}、\code{ggml\_f16\_to\_f32}。 & 把 GGUF 中的 \code{F32}、\code{Q8\_0}、\code{Q5\_0}、\code{Q6\_K} 和 \code{Q4\_K} 张量按 GGML block 布局解释出来。它是格式 oracle，不决定某个 tensor 最终走 f32 fallback 还是量化直算。 \\
\filepath{include/lcqi/q4\_k.hpp} 与 \filepath{src/q4\_k.cpp} & \code{dequantize\_q4\_k}、\code{quantize\_q8\_k\_input}、\code{matvec\_q4\_k\_q8}。 & 实现 llama.cpp 同格式的 \code{Q4\_K} block 解码和 \code{Q4\_K x Q8\_K} 单算子路径；单测用手写 block 固定 nibble 顺序、scale/min 解包和误差边界。 \\
\filepath{include/lcqi/q5\_0\_packed.hpp}、\filepath{src/q5\_0\_packed.cpp} 与 \filepath{src/q5\_0\_packed\_avx2.cpp} & \code{pack\_q5\_0\_blocks}、\code{matvec\_q5\_0\_packed\_q8\_0}、batch group path。 & 默认 \code{Q5\_0} decode 路径。loader 先把 GGML 的 high-bit+nibble 布局打包成 32 个 unsigned lane，decode/single-token 前向时用 \code{Q8\_0InputBlock} 和 AVX2 \code{maddubs} 做点积，避免每次 matvec 现场拆 high-bit。低层 batch group path 仍保留并测试，但 caw A/B 证明它不适合作为当前 prompt prefill 默认路径；模型默认 batch prefill 改用同一 Q5 tensor 的 F32 sidecar，通过 \code{q5\_0\_batch\_f32} 字段单独记账。 \\
\filepath{include/lcqi/ggml\_matvec.hpp}、\filepath{src/ggml\_matvec.cpp} 与 \filepath{src/ggml\_matvec\_avx2.cpp} & \code{matvec\_ggml\_quantized\_q8\_0}、\code{matvec\_ggml\_quantized\_q8\_0\_batch\_rows\_unchecked}、\code{max\_dot\_ggml\_quantized\_q8\_0}。 & 普通 \code{Q8\_0} direct、独立 \code{Q6\_K} direct 实验和完整 GGML direct 的共享内核。默认只让 \code{Q8\_0} linear 和 tied lm head 使用 AVX2 \code{Q8\_0 x Q8\_0}；tied lm head 只需要最大 token，所以现在可以用 fused max-dot 直接在 row-range 内归约，不必先写完整 vocab logits 再 argmax。\code{LCQI\_LLAMA\_Q6\_K\_DIRECT=1} 只打开 \code{Q6\_K} A/B；完整 \code{Q5\_0/Q6\_K/Q8\_0} direct 仍需 \code{LCQI\_LLAMA\_GGML\_DIRECT=1} 显式打开。 \\
\filepath{include/lcqi/llama\_gguf.hpp} 与 \filepath{src/llama\_gguf.cpp} & \code{load\_llama\_gguf\_reference\_model}、\code{load\_gpt2\_tokenizer\_from\_gguf}、\code{llama\_gguf\_generate\_greedy}。 & 自研 SmolLM2/GGUF decoder path。它从同一个 GGUF 读取 tokenizer tokens/merges、混合量化权重、RMSNorm、normal RoPE、GQA KV cache、SwiGLU 和 tied lm head；默认执行 \code{Q4\_K} direct、\code{Q5\_0} decode packed、Q5 prompt-batch F32 sidecar、普通 \code{Q8\_0} direct、tied lm head fused max-dot 与 F32 fallback，并在 prompt 阶段用 \code{LlamaBatchWorkspace} 批量推进 prefill。\code{Q6\_K} direct 现在是单独实验存储类型和热点字段，caw A/B 已证明当前实现变慢，所以仍不进入默认路径。 \\
\filepath{src/llama\_cli.cpp} & \code{lcqi\_llama\_gguf}。 & 命令行加载真实 SmolLM2 GGUF，支持 token id prompt 和文本 prompt，输出生成 token、文本、load/prefill/decode 时间、KV 字节、量化权重字节、\code{Q5\_0} packed 字节、\code{Q5\_0} batch F32 sidecar 字节、\code{Q6\_K} direct 实验字节、\code{Q8\_0} direct 字节和 fallback f32 字节。 \\
\filepath{src/q4\_k\_avx2.cpp} & \code{q4\_k\_q8\_avx2\_available}、\code{matvec\_q4\_k\_q8\_avx2\_unchecked}。 & x86 AVX2 路径。它用 \code{maddubs} 把 32 个 unsigned nibble 与 32 个 signed int8 激活先归约成整数和，再套用 \code{Q4\_K} 的 scale/min；报告必须同时给 scalar 与 auto 后端，防止把机器波动写成优化收益。 \\
\filepath{src/gguf_q4_bench.cpp} & \code{lcqi\_gguf\_q4\_bench}。 & 从真实 SmolLM2 GGUF 选择 \code{Q4\_K} 矩阵，比较 f32 解码基线、\code{Q4\_K x f32} 和 \code{Q4\_K x Q8\_K}，输出速度、误差、checksum 和字节账本。 \\
...
kernel 文件要按“可信基线到优化路径”的顺序读。先看 deterministic layer 和 input 怎么生成，保证每次运行输入相同；再看 \code{linear_i8_packed()} 怎样把 int8 权重、scale、bias 和 float 输入算成输出；再看 workspace 路径怎样复用临时缓冲；最后看 AVX2 路径怎样在支持平台上替换内层循环。Q4\_K 路径也遵守同一顺序：先读 scalar 解码，再读 AVX2 \code{maddubs} 子块归约，最后看 benchmark 怎样强制 scalar 和 auto 后端并列输出。测试用 packed 与 scalar 的 \code{max_abs_diff} 对照，benchmark 再比较耗时。
...
文件 & 关键代码 & 这段代码的意思 \\
\hline
\filepath{include/lcqi/gpt2_reference.hpp} & 配置、模型、KV cache、cached decoder、tokenizer 与 forward/generate API。 & 公开 GPT-2 reference path 的合同：配置、权重、KV cache、tokenizer、full-prefix、cached generation，以及 prompt prefill 中只推进 cache、不做无用预测的入口。 \\

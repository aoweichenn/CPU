...
\filepath{src/f32_kernels_avx2.cpp} & \code{dot\_f32\_avx2\_available}、\code{dot\_f32\_avx2\_unchecked}、8-row F32 row-range fast path。 & x86 平台优化路径。它在一段输出行内部把同一份 input load 复用到 8 个输出行，再回退到 4-row 和单行 tail；不改变 GPT-2/LLaMA 层结构、线程分片或 greedy tie-breaking。 \\
...
\filepath{include/lcqi/ggml\_matvec.hpp}、\filepath{src/ggml\_matvec.cpp} 与 \filepath{src/ggml\_matvec\_avx2.cpp} & \code{matvec\_ggml\_quantized\_f32}、\code{matvec\_ggml\_quantized\_q8\_0}、\code{quantize\_q8\_0\_input}。 & \code{Q5\_0/Q6\_K/Q8\_0} direct matvec 的实验路径。它能显著提高量化字节覆盖率，但 caw 证明当前 block/row 组织端到端更慢，所以只用 \code{LCQI\_LLAMA\_GGML\_DIRECT=1} 显式打开。 \\
\filepath{include/lcqi/llama\_gguf.hpp} 与 \filepath{src/llama\_gguf.cpp} & \code{load\_llama\_gguf\_reference\_model}、\code{load\_gpt2\_tokenizer\_from\_gguf}、\code{llama\_gguf\_generate\_greedy}。 & 自研 SmolLM2/GGUF decoder path。它从同一个 GGUF 读取 tokenizer tokens/merges、混合量化权重、RMSNorm、normal RoPE、GQA KV cache、SwiGLU 和 tied lm head；默认执行 \code{Q4\_K} direct 加 f32 fallback，实验开关可接入 \code{Q5\_0/Q6\_K/Q8\_0} direct。 \\
\filepath{src/llama\_cli.cpp} & \code{lcqi\_llama\_gguf}。 & 命令行加载真实 SmolLM2 GGUF，支持 token id prompt 和文本 prompt，输出生成 token、文本、load/prefill/decode 时间、KV 字节、量化权重字节和解码后 f32 字节。 \\

...
第三部分处理模型导入、真实模型加载闭环、memory planner 和 IR lowering。你会把 manifest、shape verifier、tensor role、state edge、workspace、liveness 和 executable plan 写成可检查报告，并显式运行一个 small 级别开源 GGUF 模型 smoke，而不是只停留在 tiny fixture。

...
\section{target 是构建系统里的对象}

学习 CMake 时不要把它当成“命令脚本”。现代 CMake 的核心单位是 target。库、可执行文件、测试程序都是 target；include 路径、编译特性、链接库都挂在 target 上。这样依赖才能沿 target 传播。

反例：

\begin{lstlisting}[language=CMake,caption={全局 include 路径会污染依赖}]
include_directories(include)
add_executable(app src/main.cpp)
\end{lstlisting}

这个写法让后续所有目标都能看到 \code{include}，即使它们不应该依赖这个目录。目标级写法更清楚：

\begin{lstlisting}[language=CMake,caption={目标自己声明公开头文件路径}]
add_library(config_core src/config.cpp)
target_include_directories(config_core PUBLIC include)

add_executable(app src/main.cpp)
target_link_libraries(app PRIVATE config_core)
\end{lstlisting}

\code{app} 通过链接 \code{config_core} 获得它的公开 include 路径。这就是构建层面的接口传播。

\section{PUBLIC、PRIVATE、INTERFACE 怎么判断}
...
可见性 & 含义 & 例子 \\
\midrule
\code{PRIVATE} & 只当前 target 需要 & \code{.cpp} 里用到的库或 include \\
\code{PUBLIC} & 当前 target 和使用者都需要 & 公开头文件里出现的类型所在 include \\
\code{INTERFACE} & 当前 target 不编译，只给使用者传播 & 头文件库、编译选项集合 \\
\bottomrule
\end{longtable}

如果公开头文件里写了 \code{#include <span>} 并在函数签名使用 \code{std::span}，那么 C++20 特性是 PUBLIC；如果某个实现文件内部使用 \code{std::filesystem}，但公开接口没有暴露相关类型，它可能只是 PRIVATE。

\section{ASan 构建不是普通 Debug}

内存错误可以用 AddressSanitizer 辅助发现。一个简单配置：

\begin{lstlisting}[language=bash,caption={ASan 构建命令}]
...
\begin{lstlisting}[caption={低质量失败信息}]
assertion failed
\end{lstlisting}

更好的失败信息：

\begin{lstlisting}[caption={可诊断失败信息}]
active_user_ids_handles_duplicate_ids:
expected [3, 7], actual [3, 3, 7]
\end{lstlisting}
...
\section{本章验收：新机器三条命令能跑}

一个健康小项目，换到新机器后至少能用三条命令跑起来：

\begin{lstlisting}[language=bash,caption={新机器验收命令}]
cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug
cmake --build build --parallel
ctest --test-dir build --output-on-failure
\end{lstlisting}

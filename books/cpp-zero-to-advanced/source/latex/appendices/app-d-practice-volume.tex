...
本卷主线工程是 \filepath{cpp_practice_workbook} 文本分析工作簿。它把 C++ 语法、标准库、资源管理、错误处理、测试和性能测量都接到同一个工具上。每章新增语法时，都要回答“它让这个工具多了什么边界”：配置是否更清楚，解析是否更可靠，缓存是否更稳定，CLI 是否更薄，benchmark 是否更可解释。

\begin{longtable}{p{0.20\linewidth}p{0.30\linewidth}p{0.40\linewidth}}
阶段 & 主线代码 & 学到的工程能力 \\
\hline
数据合同 & 配置与结果类型 & 用值类型描述输入、错误和输出，而不是散落变量 \\
解析器 & \code{parse_text_config} & 一次返回全部诊断，库层不直接退出进程 \\
分析器 & \code{TextAnalyzer} & 字符扫描、归一化、过滤、排序逐步分解 \\
缓存 & \code{AnalysisCache} & 用对象不变量理解容器、迭代器、移动和生命周期 \\
文件与 CLI & 文件入口 & RAII 文件读取和薄命令行入口，不复制业务逻辑 \\
测试与性能 & 测试和基准 & 行为先被 GTest 固定，再用 benchmark 问一个性能问题 \\
\end{longtable}

最后的大作业就是继续完善这个工作簿，而不是新开一堆小程序。读者应该能从第一章的最小程序一路走到一个可配置、可测试、可 benchmark 的文本分析工具。

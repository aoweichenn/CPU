...
本册不是 Linux 内核源码注释，但每个重要机制都应能落到真实源码。本版以 Linux 6.16 文档作为接口与术语核对基线；下面的路径采用 Linux 6.x 主线的常见目录。源码阅读时仍应先 checkout 明确的 tag，并以该源码树为准，因为函数名、文件位置和实现策略都会变化。目标不是背路径，而是知道“这个概念在真实系统里由哪些结构和函数承载”。
...
ELF 装载 & \filepath{fs/binfmt\_elf.c}，\filepath{fs/exec.c}，\filepath{fs/binfmt\_script.c} & \code{load\_elf\_binary}、\code{create\_elf\_tables}、\code{begin\_new\_exec}、\code{install\_exec\_creds}、CLOEXEC 和 binfmt 递归 \\
...
MiniFS-Lab & \filepath{source/latex/listings/minifs\_lab.cpp} & mkfs、mount 自举、显式磁盘编码、create/read/unlink、崩溃注入与 fsck 式回收 \\

...
\node[stage,minimum width=2.2cm] (wait) {等待头部};
\node[stage,minimum width=2.2cm,right=of wait] (recv) {接收载荷};
\node[stage,minimum width=2.2cm,right=of recv] (verify) {验证与建栈};
\node[stage,minimum width=2.2cm,below=13mm of verify] (run) {任务运行};
\node[stage,minimum width=2.6cm,right=of run] (report) {报告结果\\回到等待头部};
\node[stage,minimum width=2.2cm,above=11mm of wait] (reset) {复位初始化};
\draw[flow] (reset)--(wait);
\draw[flow] (wait)--node[above,font=\scriptsize]{合法头部}(recv);
\draw[flow] (recv)--node[above,font=\scriptsize]{载荷到齐}(verify);
\draw[flow] (verify)--node[above,font=\scriptsize]{提交 PC}(run);
\draw[flow] (run)--node[above,font=\scriptsize]{合作返回}(report);
\node[stage,text width=4.3cm,below=13mm of wait,fill=BookWarm] (reject)
  {头部、接收或验证失败：报告拒绝，随后回到等待头部};
\draw[->,>=Latex,thick,draw=BookMuted] (wait.south) -- (wait.south |- reject.north);
...
\end{tikzpicture}
\end{center}
\diagramnote{上排是一次成功传输从左到右的状态变化，下排统一表示任务运行以前的拒绝路径。
进入任务后，唯一正常出边仍是任务主动返回；当前机器没有从运行状态强制回到固件的事件。}

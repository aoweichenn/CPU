...
\node[stage,minimum width=2.7cm,below=12mm of offset] (array) {非易失字节阵列\\\(2^{16}\) 个字节单元};
\node[stage,minimum width=2.55cm,right=of array] (resp) {返回锁存器\\字节与 \code{OK}};
\node[stage,fill=BookWarm,minimum width=4.9cm,below=12mm of check] (error)
...
\draw[->,>=Latex,thick,BookMuted] (check.south) -- (error.north);
\end{tikzpicture}
\end{center}
...
\node[stage,minimum width=2.75cm,below=12mm of lanes] (array) {易失字节阵列\\共享读写端口};
\node[stage,minimum width=2.45cm,right=of array] (resp) {响应锁存器\\数据或错误状态};
\draw[flow] (req) -- (guard);
\draw[flow] (guard) -- (lanes);
\draw[flow] (lanes) -- (array);
\draw[flow] (array) -- (resp);
\node[stage,fill=BookWarm,minimum width=4.9cm,below=12mm of guard] (reject)
  {任一检查失败：返回错误，四条通道均禁止更新};
\draw[->,>=Latex,thick,BookMuted] (guard.south) -- (reject.north);
\end{tikzpicture}
\end{center}

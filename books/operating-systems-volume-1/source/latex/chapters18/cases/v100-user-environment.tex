...
\subsection{从八槽种类分派到开放文件与事件对象}

v1.0 的描述符槽直接保存对象种类和对应句柄，因此一次查表就能进入控制台、
管道或文件路径。现代 VFS 增加 open file description 并不是为了增加层数，
而是因为 dup、fork、独立 open、共享偏移和 close-on-exec 需要不同的身份与
引用边界。

\begin{longtable}{|L{0.20\linewidth}|L{0.23\linewidth}|L{0.30\linewidth}|L{0.17\linewidth}|}
\toprule
v1.0 最小对象 & 当前限制 & 正文与路线图中的扩展 & 保持不变的契约 \\
\midrule
\endhead
八槽 fd 表 & 槽位固定，动态槽直接拥有底层句柄 & 可增长 fd table、bitmap、
close-on-exec 和资源上限 & fd 始终只是进程局部名字 \\\tablerowrule
种类 + 文件句柄 & 再次打开与复制描述符的关系无法表达 & open file description
保存 offset/flags，再引用 vnode/inode & 打开实例与持久对象身份必须分层
\\\tablerowrule
通用 Try/Wait & 每次只等待一个 fd，按种类直接分派 & poll table、epoll item、
边沿/水平触发和取消 & readiness 是可重新检查的条件，不是数据所有权 \\\tablerowrule
fd 0/1/2 控制台 & 单字节 ASCII，没有会话和作业控制 & TTY 行规程、session、
process group 与前台终端 & 外部输入必须经过设备和内核队列才到用户缓冲
\\
\bottomrule
\end{longtable}

这张映射也解释了正文中 \code{epoll} 为什么绑定开放文件对象，而不是只记一个
整数 fd：fd 可以被关闭并复用，事件订阅需要稳定地引用被打开的对象及其等待源。
同理，\code{dup} 共享 offset 是开放文件层的语义，close-on-exec 则属于单个
fd 槽；若仍把所有字段塞在一个表项中，两种生命周期便无法同时表达。

Shell 是第五条演化线：v1.0 的内建命令没有 fork/exec、继承、管线或重定向；
路线图中的 PID1、外部 ELF、spawn/exec/wait、dup 与 pipe 组合将替换这项容量
限制，但“Shell 只能通过公开 ABI 编排资源”仍是用户与内核的边界。

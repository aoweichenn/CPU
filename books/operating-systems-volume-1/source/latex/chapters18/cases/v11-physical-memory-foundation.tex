...
\begin{statebox}
\textbf{状态边界。} 项目当前发布基线仍是 v1.0；v1.1 已接受并实现两项增量。
ADR 0017 建立动态页帧元数据、高半区物理直映和 64 GiB QEMU 容量路径；
ADR 0020 把 64 KiB 高半区单调堆升级为可释放、可合并并能完整校验的通用堆。
当前 \(76/76\) 项回归全部通过，但 CMake 工程元数据仍为 v0.11，buddy、
type cache、KVA、动态内核栈和页表回收仍未闭合。因此本节把这些结果称为
...
\(100000\) 次随机申请与释放，逐步核对对齐、首尾数据和拓扑。三项新增测试使
当前完整套件由 v1.0 的 73 项增长到 76 项。

当前实现仍不是无限增长、并发安全的通用内核分配入口。后备区固定为 64 KiB，
best-fit 线性扫描空闲链，调用者还必须保证启动路径串行；IRQ、NMI、panic 与
多 Thread 分配都不在当前契约内。type cache 负责固定尺寸热对象，buddy 负责
页粒度连续块，KVA 负责虚拟区间；通用堆不能替代这些层次。

\subsection{两项基础怎样继续长成通用内核分配}
...
因此，direct map 不是 \code{kmalloc}，动态页帧账本也不是 buddy，边界标记堆
也不是 type cache。当前四个 PCB、内核栈和描述符容量尚未动态化，堆后备区也
不会按需增长。v1.1 的两项已完成增量分别解除“内核只能看见低 64 MiB”和
“堆对象只能分配、不能归还”两条限制，为后续动态对象表、COW 与按需分页建立
可逐层验收的资源通路。

...
\subsection{从固定磁盘格式回接页缓存、VFS 与日志}

v0.11 不是与正文中的现代文件系统并列的另一种实现。它把每一层压缩到最小容量，
使“对象身份、缓存副本、介质布局和提交状态”能够同时看见；正文随后做的是拆分
职责、扩大索引并增加可恢复协议。

\begin{longtable}{|L{0.20\linewidth}|L{0.23\linewidth}|L{0.30\linewidth}|L{0.17\linewidth}|}
\toprule
v0.11 可见对象 & 当前规模与语义 & 正文中的扩展对象 & 不会消失的事实 \\
\midrule
\endhead
固定文件句柄 & 直接保存 inode、偏移和能力 & fd、open file description、
vnode/inode、dentry 与 mount 分层 & 打开实例的偏移不同于持久文件身份
\\\tablerowrule
八项块缓存 & 以 LBA 为键，LRU 淘汰并整块写回 & page cache、address-space、
folio、writeback 与回写错误 & 内存中的新字节不自动等于介质已持久化 \\\tablerowrule
\bottomrule
\end{longtable}

运行时对象解决“当前打开者看见什么”；磁盘布局与恢复协议还要回答“块属于谁、
哪些修改可以在重启后采用”。这两类问题继续映射如下。

\begin{longtable}{|L{0.20\linewidth}|L{0.23\linewidth}|L{0.30\linewidth}|L{0.17\linewidth}|}
\toprule
v0.11 持久对象 & 当前规模与语义 & 正文中的扩展对象 & 不会消失的事实 \\
\midrule
\endhead
直接块数组 & 文件容量小，块映射可线性检查 & extent、间接树、空洞文件与
延迟分配 & 每个逻辑范围必须有唯一、可验证的块归属 \\\tablerowrule
Dirty/Clean & 能检测半事务，但不能重放或撤销 & journal transaction、
commit record、checkpoint 与恢复扫描 & 恢复只能采用有完整提交证据的状态
\\\tablerowrule
全局文件系统锁 & 所有名称和元数据操作串行化 & inode/dentry/page 锁、
rename 顺序与 RCU 路径读取 & 跨对象更新仍需一条明确的序列化规则 \\
\bottomrule
\end{longtable}

例如，正文中的日志不会取消 superblock、位图、inode 和目录之间的一致性检查；
它只是保存“哪些修改共同构成一次已提交事务”的恢复证据。page cache 也不会
取消设备 flush；它只把缓存单位提升为文件页，并让 \code{read}、\code{write}
和 \code{mmap} 共享同一份可见性账本。这样阅读，现代结构都能回到 v0.11 中
已经出现的具体失败，而不是成为需要孤立记忆的新术语。

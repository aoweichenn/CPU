...
\subsection{从一个具名等待原因到通用等待队列}

项目把等待原因直接保存在 PCB，并在单一管道状态改变后扫描进程槽位。这个设计
足以证明“Blocked 不参加调度”和“唤醒后重新 Try”，但对象与等待者一多，扫描
全局进程表既昂贵，也不能表达独占唤醒、公平性、信号和超时。正文中的等待队列
正是沿这些压力拆出来的索引与并发对象。

\begin{longtable}{|L{0.20\linewidth}|L{0.23\linewidth}|L{0.30\linewidth}|L{0.17\linewidth}|}
\toprule
v0.10 最小对象 & 当前能够证明什么 & 扩展到通用系统时新增什么 & 保持不变的规则 \\
\midrule
\endhead
PCB 中的等待原因 & 被唤醒者能重新检查具体 pipe 与方向 & 对象局部 wait queue、
wait entry 和回调过滤 & 等待原因必须绑定可重新计算的谓词 \\\tablerowrule
单生产者、单消费者 & 满、空、EOF 与 broken pipe 的基本状态机 & 多等待者、
独占/广播唤醒、公平顺序与惊群控制 & 唤醒只给运行资格，不预留数据或空间 \\\tablerowrule
无超时的 Wait & 阻塞能离开 runqueue，条件变化能恢复 & deadline timer、
可中断等待、信号原因与取消清理 & 每种返回都必须重新获得锁并检查谓词 \\\tablerowrule
一把管道自旋锁 & 谓词、环形索引和端点状态同规则提交 & 多核锁竞争、锁顺序、
睡眠锁及内存模型证明 & 检查、入队与释放 CPU 之间不能出现丢失唤醒 \\\tablerowrule
退出时关闭端点 & 异常路径不会留下永久等待的对侧 & 引用计数、fd 复制与最后引用
关闭协议 & 生命周期变化必须触发相关谓词复查 \\
\bottomrule
\end{longtable}

因此，后文讲 mutex、futex、completion、socket wait queue 或 epoll 时，可以
继续使用同一份问题清单：谓词由谁保护，等待者何时可见，事件由谁提交，唤醒
是否独占，信号和超时怎样清理，以及恢复后为什么必须重试。对象变复杂，并不
改变 v0.10 已经验证的 lost-wakeup 边界。

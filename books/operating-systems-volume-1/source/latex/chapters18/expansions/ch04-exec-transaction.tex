...
内存长不小于文件长，BSS 区域清零 \\ \hline
\end{longtable}

表中的 \code{0x555555554000} 不是 x86-64 或 ELF ABI 规定的常量，而是本次执行由
地址空间随机化策略选出的示例装载基址。它位于低半部规范用户地址中，避开低地址空洞，
并按页对齐，使页表映射和逐段权限控制都能从完整页面开始。内核只为整个位置无关映像选择
一次装载偏移 \(B\)；每一段的运行地址都由
\[
  \text{runtime\_address}=B+\texttt{p\_vaddr}
\]
统一推导，不能给三个段分别挑选“看起来空闲”的地址。分别选择会破坏段间相对距离，使
PC-relative 引用、重定位项和调试信息不再描述同一幅映像。这里使用重复的十六进制数字，
只是便于读者辨认“同一个装载区域”；换成另一个不冲突、满足对齐和规范地址约束的基址，
算法仍然成立。
...
这个数同样是布局策略的示例，而不是 ABI 要求。四级页表下，低半部规范地址在
\code{0x0000800000000000} 之前结束；示例把栈顶放在该边界下方并保持页对齐，从而不让
任何栈页跨入非规范地址。栈顶表示可用区间的排他上界，本身不必存放一个字节。实际系统还
会在规范地址边界、栈、动态链接器和其他映射之间保留更大的空洞，并随机下移栈顶。若把栈
直接贴住边界，向上越界时留给检测和随机化的空间更少；若放得过低，又会无谓压缩堆、共享
库和内存映射区。因而真正稳定的约束是“低半部、页对齐、与其他区域留有间隔”，而不是这
串十六进制数字本身。

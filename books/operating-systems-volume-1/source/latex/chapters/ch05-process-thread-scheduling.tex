...
\midrule
地址与数据 & 地址空间、代码、堆和共享映射 & 用户栈、内核栈、栈指针 \\\tablerowrule
文件资源 & 文件描述符表、当前目录 & 正在执行的系统调用及其局部现场 \\\tablerowrule
安全与限制 & 凭据、资源限制、命名空间 & 部分可独立设置的信号屏蔽状态 \\\tablerowrule
处理器执行 & 进程所获得的总体资源环境 & 指令位置、通用寄存器、浮点与向量状态 \\\tablerowrule
...
对象或集合 & 保存的事实 & 必须维持的关系 \\
\midrule
进程 & 地址空间、文件表、凭据、进程关系 & 资源必须在最后一个有效引用消失后回收 \\\tablerowrule
task & 寄存器现场、栈、调度属性、等待原因 & 每个可恢复现场只能处于一个明确状态 \\\tablerowrule
runqueue & 当前已经具备运行条件的 task & 可运行但未占用 CPU 的 task 必须进入且只进入一个运行队列 \\\tablerowrule
wait queue & 等待同一条件的 task 与等待规则 & 条件成立时，唤醒路径能够找到相应等待者 \\\tablerowrule
...
转换 & 触发事件 & 转换后必须成立的条件 \\
\midrule
NEW $\rightarrow$ RUNNABLE & 创建所需状态已经完整初始化 & task 可见于调度器，且只位于一个 runqueue \\\tablerowrule
RUNNABLE $\rightarrow$ RUNNING & 调度器在某个 CPU 上选中 task & task 从 runqueue 移除，并成为该 CPU 的 current \\\tablerowrule
RUNNING $\rightarrow$ RUNNABLE & 时间片到期、主动让出或高优先级任务抢占 & 执行现场已保存，task 重新进入候选集合 \\\tablerowrule
RUNNING $\rightarrow$ BLOCKED & 等待 I/O、锁、定时器或子进程事件 & 等待条件和等待队列已经登记，task 不再位于 runqueue \\\tablerowrule
BLOCKED $\rightarrow$ RUNNABLE & 条件成立、超时或可中断等待收到信号 & 唤醒原因已记录，task 重新进入一个 runqueue \\\tablerowrule
RUNNING $\rightarrow$ EXITED & 执行退出流程 & 不再被调度，运行资源按引用关系释放 \\\tablerowrule
EXITED $\rightarrow$ ZOMBIE & 仍需保留退出状态供父进程读取 & 仅保留 PID、退出码和必要统计 \\\tablerowrule
...
\input{chapters/ch05-task-context-anatomy}
...
\midrule
0 & 创建 A/B & A,B & A & A running \\\tablerowrule
1 & 时间片结束 & B,A & B & A runnable, B running \\\tablerowrule
2 & B 发起 I/O & A & A & B blocked(io) \\\tablerowrule
3 & I/O 完成 & A,B & A & B runnable \\\tablerowrule
...
\midrule
A & 0 & 5 & 0 & 5 \\\tablerowrule
B & 5 & 7 & 4 & 6 \\\tablerowrule
C & 7 & 8 & 5 & 6 \\
\bottomrule
\end{longtable}
...
任务 & 首次运行 & 完成 & 响应时间 & 周转时间 \\
\midrule
A & 0 & 8 & 0 & 8 \\\tablerowrule
B & 2 & 4 & 1 & 3 \\\tablerowrule
C & 4 & 5 & 2 & 3 \\
\bottomrule
\end{longtable}
...
\midrule
阻塞等待 & RUNNING $\rightarrow$ BLOCKED & 登记等待条件并从可运行集合移除 \\\tablerowrule
主动让出 & RUNNING $\rightarrow$ RUNNABLE & 保存现场并重新入队 \\\tablerowrule
时间片到期 & RUNNING $\rightarrow$ RUNNABLE & 结算运行时间，更新策略所需的排序字段 \\\tablerowrule
更高优先级任务被唤醒 & 当前任务可能保持 RUNNING 到安全点 & 标记需要抢占，处理目标 CPU 通知 \\\tablerowrule
...
指标 & 说明 & 常见误判 \\
\midrule
on-CPU time & 真正执行指令的时间 & 把等待 I/O 误认为算法慢 \\\tablerowrule
runnable wait & 可运行但没拿到 CPU 的时间 & 把调度拥塞误认为锁死 \\\tablerowrule
context switches & 切换次数和成本 & 只看次数，不看每次成本 \\\tablerowrule
migration count & 跨 CPU 迁移 & 均衡改善但缓存局部性变差 \\\tablerowrule
...
字段类别 & 保存什么 & 为什么重要 \\
\midrule
执行现场 & 内核栈、保存寄存器、返回路径 & 被切走后还能从同一点继续 \\\tablerowrule
调度状态 & state、policy、prio、vruntime、on\_rq & 判断能否运行、怎样排序、是否已入队 \\\tablerowrule
地址空间 & \code{mm}、\code{active\_mm} & 进程切换可能要切页表和 TLB \\\tablerowrule
资源指针 & files、fs、signal、sighand & fork、exec、exit 要复制、共享或释放 \\\tablerowrule
安全身份 & cred、namespace、cgroup & 权限、限额和可见性都依赖它 \\\tablerowrule
...
动作 & 发生了什么 & 不必同时发生什么 \\
\midrule
进入内核 & 保存用户返回现场，切换到受保护的内核入口与内核栈 & 不一定更换当前 task，也不一定运行调度器 \\\tablerowrule
作出调度决定 & 比较可运行实体，确定是否继续当前 task & 若仍选择当前 task，可以不切换执行现场 \\\tablerowrule
任务上下文切换 & 保存 prev 的内核执行现场，恢复 next 的现场 & 若二者共享地址空间，不一定切换页表根 \\\tablerowrule
...
时刻 & 线程 A & 线程 B 或内核状态 \\
\midrule
1 & A 检查 pipe，发现没有数据 & wait queue 仍为空 \\\tablerowrule
2 & A 还没把自己挂入 wait queue & B 写入数据，调用 wakeup，但没有等待者可唤醒 \\\tablerowrule
3 & A 把自己设为 sleeping 并调用 \code{schedule} & pipe 已经有数据，但没有新的 wakeup 会来 \\\tablerowrule
...
阶段 & 状态变化 & 必须区分的边界 \\
\midrule
\code{recv} 阻塞 & task 从 runqueue 移到 socket wait queue & 它不再竞争 CPU，但内核必须记住等待条件 \\\tablerowrule
包到达 & 数据进入 socket receive queue & 条件变为真，但等待线程还没有运行 \\\tablerowrule
wakeup & task 变成 runnable，进入某个 runqueue & runnable 只表示有资格运行，不表示已经运行 \\\tablerowrule
调度选择 & CPU 在安全点切换到该 task & 可能立即抢占，也可能等当前任务返回或时间片结束 \\\tablerowrule
...
机制 & 它保存的等待事实 & 常见错误 \\
\midrule
mutex & 持有者是谁，等待者排在哪个队列 & 忘记在循环中复查条件，或异常路径漏解锁 \\\tablerowrule
futex & 用户态字值和内核等待队列的绑定 & 只看内核队列，不验证用户态谓词是否仍成立 \\\tablerowrule
condition variable & 等待哪个共享状态变真 & 把 wakeup 当成状态已经满足 \\\tablerowrule
signal & 异步事件和可中断睡眠的交汇点 & 忽略 \code{EINTR}，把部分完成误当失败 \\\tablerowrule
...
\input{chapters/ch05-scheduling-expanded}

...
分析一段调度过程时，可以并列记录三张表。进程表列出 PID、父 PID、状态、退出码、地址空间
和 fd 表引用；线程表列出 TID、所属 PID、状态、等待原因、优先级、最近运行 CPU 和已消耗
tick；runqueue 则列出当前可运行线程的顺序和调度账本。
...
度量调度行为时必须区分三个时间：wall time、runnable wait 和 on-CPU time。wall time 是请求
从开始到结束的总时间；runnable wait 是线程已经可运行但还没取得 CPU 的时间；on-CPU time
是真正执行指令的时间。把三者混在一起，会把调度问题误判成算法问题。

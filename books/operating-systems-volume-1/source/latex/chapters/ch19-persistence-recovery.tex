...
崩溃点 & 恢复策略 \\
\midrule
TempCreated & 删除临时文件或忽略 \\\tablerowrule
DataWritten & 校验失败则删除，不能发布 \\\tablerowrule
DataSynced & 仍未提交，等待 rename 或清理 \\\tablerowrule
RenamedVisible & 若 manifest 未提交，按业务规则决定是否采用 \\\tablerowrule
DirectorySynced & 文件名持久，但业务仍以 manifest 为准 \\\tablerowrule
...
故障 & 例子 & 需要的机制 \\
\midrule
transaction crash & commit 前断电 & WAL/journal/manifest 状态机 \\\tablerowrule
system crash & 内核 panic、掉电 & fsync、flush、replay、fsck \\\tablerowrule
...
级别 & 优点 & 风险 \\
\midrule
RAID0 & 高吞吐、高容量利用 & 任一盘坏全丢 \\\tablerowrule
RAID1 & 简单冗余，读可并行 & 容量减半 \\\tablerowrule
RAID5/6 & 容量效率高，有校验 & 小写放大、write hole、重建压力 \\\tablerowrule
RAID10 & 性能和可靠性较好 & 成本高 \\
\bottomrule
\end{longtable}

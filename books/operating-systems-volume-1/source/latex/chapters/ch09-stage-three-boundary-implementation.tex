...
返回类型 & 例子 & 调用者应该怎样理解 \\
\midrule
非负数量 & \code{read/write} 返回字节数 & 可能短读/短写，不等于请求全部完成 \\\tablerowrule
新句柄 & \code{open/socket/dup} 返回 fd & fd 是进程表索引，不是对象永久身份 \\\tablerowrule
0 表示成功 & \code{close/mprotect/munmap} & 只说明接口语义完成，不说明持久化 \\\tablerowrule
\code{-errno} & 内核内部常见约定 & wrapper 转成 \code{errno} 或直接传递 \\\tablerowrule
...
视角 & 使用对象 & 典型消费者 \\
\midrule
运行时装载 & \code{PT\_LOAD/PT\_INTERP/PT\_PHDR} & 内核 exec、动态链接器 \\\tablerowrule
链接和重定位 & section、symbol、relocation & 链接器、objdump、调试器 \\\tablerowrule
...
路径 & 关键文件或函数 & 读法 \\
\midrule
系统调用入口 & \code{arch/x86/entry/}、\code{kernel/seccomp.c} & 参数、seccomp、返回工作 \\\tablerowrule
用户复制 & \code{arch/x86/lib/usercopy*}、\code{lib/iov\_iter.c} & fault fixup、短拷贝、iov \\\tablerowrule
exec/ELF & \code{fs/exec.c}、\code{fs/binfmt\_elf.c} & 准备新镜像和提交点 \\\tablerowrule
VMA 管理 & \code{mm/mmap.c}、\code{include/linux/mm\_types.h} & maple tree、切分、合并 \\\tablerowrule
缺页 & \code{mm/memory.c}、\code{mm/filemap.c} & anon/file/COW fault 分支 \\\tablerowrule
页表失效 & \code{mm/tlb.c}、架构 TLB 代码 & batch、shootdown、延迟释放 \\\tablerowrule
...
反例 & 构造方法 & 必须观察到 \\
\midrule
用户指针半页坏 & iovec 前半有效后半 unmapped & 短完成或 \code{EFAULT}，内核不崩 \\\tablerowrule
exec 坏解释器 & ELF 的 \code{PT\_INTERP} 指向不存在路径 & exec 失败，旧程序继续 \\\tablerowrule
ELF 溢出段 & \code{p\_vaddr + p\_memsz} 整数溢出 & 拒绝装载 \\\tablerowrule
\code{mprotect} 后旧权限 & RW 改 R 后立即写 & 必须 fault，不能被 TLB 旧权限放行 \\\tablerowrule
fault 等 I/O 时 munmap & 线程 A fault 等页，线程 B unmap & A 醒来后重新验证并失败 \\\tablerowrule
close 与 read 并发 & 一个线程 read 已 fget，另一个 close 并复用 fd & read 用旧 file 引用，fd 数字可复用 \\\tablerowrule

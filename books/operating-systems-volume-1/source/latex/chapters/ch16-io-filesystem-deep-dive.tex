...
对象 & 保存内容 & 生命周期 \\
\midrule
fd slot & 进程局部整数到 file 的引用 & \code{open/dup/close} 改变 \\\tablerowrule
file object & offset、flags、ops、inode 引用 & 引用计数为 0 时释放 \\\tablerowrule
inode & 文件元数据、数据块索引、权限 & link count 和 open count 共同决定回收 \\\tablerowrule
...
阶段 & 崩溃后可能看到 & 正确恢复态度 \\
\midrule
\code{write} 返回后 & 数据只在 page cache，磁盘可能无变化 & 不能宣布业务提交 \\\tablerowrule
\code{fsync(file)} 后 & 文件内容和部分元数据持久 & 若路径是新建文件，还要考虑目录 \\\tablerowrule
\code{rename(tmp, final)} 后 & 名字可能切换，目录项未必持久 & 需要 \code{fsync(parent dir)} \\\tablerowrule
manifest 写入前 & 数据文件可能存在但未被承认 & 读取者忽略孤儿文件 \\\tablerowrule
...
VFS 对象 & 语义 & 例子 \\
\midrule
superblock & 一个挂载文件系统实例 & ext4、tmpfs、procfs 的挂载点 \\\tablerowrule
inode & 文件身份和元数据 & 普通文件、目录、设备节点 \\\tablerowrule
dentry & 名字到 inode 的缓存关系 & \code{/usr/bin/sh} 的路径组件 \\\tablerowrule
file & 一次打开实例 & offset、flags、权限和 ops \\\tablerowrule
...
访问方式 & 数据所在位置 & 一致性关注点 \\
\midrule
buffered read & page cache 到用户 buffer & 热页不访问设备，冷页触发 I/O \\\tablerowrule
buffered write & 用户 buffer 到 page cache & 返回不等于落盘，页变 dirty \\\tablerowrule
MAP\_SHARED mmap & PTE 映射 page cache 页 & 写入变 dirty，回写时机独立 \\\tablerowrule
MAP\_PRIVATE mmap & COW 匿名页 & 写入不改变文件内容 \\\tablerowrule
...
模式 & 写入内容 & 崩溃后保证 \\
\midrule
metadata journal & journal 只记录元数据 & 文件系统结构一致，文件新数据可能旧或空洞 \\\tablerowrule
ordered mode & 数据先写，再提交元数据 journal & 新元数据不会指向未写入垃圾数据 \\\tablerowrule
...
策略 & 优点 & 问题 \\
\midrule
FCFS & 简单，保持提交顺序 & 随机 I/O 性能差，尾延迟不可控 \\\tablerowrule
SSTF & 优先近距离请求，减少寻道 & 远处请求可能饥饿 \\\tablerowrule
SCAN/elevator & 有方向地扫过磁盘 & 参数依赖介质，交互延迟仍需控制 \\\tablerowrule
deadline & 给请求设置过期时间 & 吞吐和延迟需要权衡 \\\tablerowrule
BFQ/权重公平 & 按进程或 cgroup 分配带宽 & 实现复杂，需识别同步/异步流 \\\tablerowrule
...
测试类型 & 方法 & 期望 \\
\midrule
路径错误 & 构造 \code{ENOENT/ENOTDIR/ELOOP/EACCES} & 错误码准确，目标不被破坏 \\\tablerowrule
引用生命周期 & open 后 unlink、dup、fork、close & inode/file 引用计数正确 \\\tablerowrule
page cache & buffered read/write/mmap 混用 & 内容一致，dirty 状态正确 \\\tablerowrule
direct I/O & 非对齐和混用 buffered I/O & 返回明确错误或同步缓存 \\\tablerowrule
写回错误 & 注入设备 EIO & fsync/close 可观察到错误 \\\tablerowrule
崩溃恢复 & 在事务各阶段断电重放 & fsck/journal 恢复到合法状态 \\\tablerowrule

...
方式 & CPU 做什么 & 风险边界 \\
\midrule
PIO & 每次读写设备寄存器搬运数据 & CPU 忙等、吞吐低、长时间占用核心 \\\tablerowrule
DMA & 准备 buffer 和描述符，让设备直接搬运内存 & buffer 生命周期、cache 一致性、IOMMU 映射 \\\tablerowrule
中断 & 设备完成后通知 CPU & 中断风暴、上半部不能睡眠、completion 要能找回请求 \\\tablerowrule
...
步骤 & 驱动和硬件动作 & 不能错的边界 \\
\midrule
保留槽位 & 驱动读取 \code{head/tail}，确认环未满，选择 \code{tail} 槽位 & 满环必须返回或停止上层队列，不能覆盖设备未消费的描述符 \\\tablerowrule
映射 buffer & DMA API 把 packet buffer 映射成设备可见 DMA 地址 & 设备不能拿用户指针或内核虚拟地址 \\\tablerowrule
填写描述符 & 写入地址、长度、flags、tag，最后设置 owned/valid 位 & valid 位不能早于其他字段对设备可见 \\\tablerowrule
发布顺序 & 执行 DMA 写屏障，保证描述符内容先于 doorbell & 少屏障会让设备读到旧字段或半初始化字段 \\\tablerowrule
敲门铃 & 写 MMIO doorbell 或更新 tail，通知设备取新描述符 & doorbell 只是通知，不代表发送完成 \\\tablerowrule
设备 DMA & 网卡读取描述符和数据，经 IOMMU 访问内存，发送到链路 & buffer 在 completion 前不能释放或复用 \\\tablerowrule
...
\midrule
TO\_DEVICE & CPU 写 buffer 后，clean cache，再让设备 DMA 读 & 设备发送旧包、旧磁盘数据或半初始化描述符 \\\tablerowrule
FROM\_DEVICE & 设备 completion 后，invalidate cache，再让 CPU 解析 & CPU 读到旧包内容，协议栈校验失败或数据错乱 \\\tablerowrule
BIDIRECTIONAL & 两个方向都要同步，并严格定义所有权切换 & CPU 和设备同时修改，结果半新半旧 \\\tablerowrule
...
层次 & 保存什么账本 & 不能混淆的边界 \\
\midrule
bio & 一组页、扇区范围、读写方向和完成回调 & bio 完成不一定等于文件系统事务提交 \\\tablerowrule
request & 合并后的设备队列项、tag、超时和状态 & request tag 复用必须等旧 completion 收敛 \\\tablerowrule
hardware queue & CPU/NUMA 亲和的提交队列 & 多队列提高吞吐，也引入负载和顺序问题 \\\tablerowrule
flush/FUA & 写缓存顺序和持久化约束 & 普通写完成不等于掉电安全 \\\tablerowrule
...
类别 & 核心对象 & 典型回调 \\
\midrule
字符设备 & \code{cdev}、\code{file\_operations} & \code{open/read/write/ioctl/poll} \\\tablerowrule
块设备 & \code{gendisk}、\code{request\_queue}、blk-mq & \code{submit\_bio}、queue\_rq、completion \\\tablerowrule

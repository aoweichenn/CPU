...
\midrule
批量计算 & 少量长 CPU burst & 吞吐、平均周转时间、cache 局部性 \\\tablerowrule
交互请求 & 多次短 CPU burst 与网络等待交替 & 首次响应、p95/p99 延迟、唤醒抢占 \\\tablerowrule
后台写回 & 周期性被唤醒，处理一批脏页后再睡眠 & 对前台任务的干扰、I/O 与 CPU 节奏 \\\tablerowrule
...
\midrule
A、B、C & $20/3$ & $34/3$ & 短任务排在长任务之后 \\\tablerowrule
...
\begin{longtable}{|L{0.13\linewidth}|L{0.17\linewidth}|L{0.18\linewidth}|L{0.18\linewidth}|L{0.18\linewidth}|}
\toprule
任务 & 到达 & 完成 & 周转时间 & 等待时间 \\
\midrule
A & 0 & 14 & 14 & $14-8=6$ \\\tablerowrule
B & 1 & 7 & 6 & $6-4=2$ \\\tablerowrule
C & 2 & 4 & 2 & $2-2=0$ \\
\bottomrule
\end{longtable}
...
时间片范围 & 主要收益 & 主要风险 \\
\midrule
相对较长 & 切换较少，计算任务保持局部性 & runnable task 的等待上界增大 \\\tablerowrule
相对较短 & 交互任务更快获得第一次服务 & 调度、中断和 cache 恢复成本上升 \\\tablerowrule
...
任务 & 权重 & 实际运行 & 虚拟运行增量 \\
\midrule
A & 1024 & 20ms & $20\times1024/1024=20$ \\\tablerowrule
...
阶段 & 未采用继承时 & 采用优先级继承时 \\
\midrule
H 请求 mutex & H 阻塞，等待 L & H 阻塞，并把有效优先级传给 L \\\tablerowrule
M 变为 runnable & M 抢占 L & L 以继承后的优先级继续运行 \\\tablerowrule
L 释放 mutex & 释放时刻可能被 M 长期推迟 & L 尽快完成临界区并唤醒 H \\\tablerowrule
...
组织方式 & 主要收益 & 必须额外解决的问题 \\
\midrule
单一全局 runqueue & 所有 CPU 看到同一候选集合，天然均衡 & 锁竞争、cache line 迁移、选择结构扩展性 \\\tablerowrule
...
\midrule
运行队列 & 避免跨队列加锁和通知 & 原 CPU 的 runnable wait 明显更高 \\\tablerowrule
cache/TLB & 上次工作集可能仍在本地 & 空闲 CPU 可立即执行任务 \\\tablerowrule
NUMA & 内存页靠近原 CPU 所在节点 & 任务已经迁移内存或远端等待过长 \\\tablerowrule
CPU capacity & 原核心可能性能更适合当前任务 & 高容量核心被更紧急任务占用 \\\tablerowrule
...
\midrule
先入目标、后出源 & task 同时位于两个 runqueue & 同一内核栈被两个 CPU 恢复，状态遭到破坏 \\\tablerowrule
先出源、丢失引用 & runnable task 不在任何队列 & 线程永久得不到 CPU \\\tablerowrule
双队列锁顺序不一致 & CPU0 等 CPU1 的锁，CPU1 等 CPU0 的锁 & 调度路径死锁 \\\tablerowrule
...
触发方式 & 适用时机 & 需要避免的问题 \\
\midrule
周期均衡 & 定期检查相邻调度域的负载差 & 周期过短导致迁移抖动，过长导致空闲与排队并存 \\\tablerowrule
idle balance & CPU 即将无任务可运行 & 扫描成本超过即将获得的运行收益 \\\tablerowrule
wakeup balance & blocked task 重新变为 runnable & 每次唤醒都跨核迁移，破坏 cache 局部性 \\\tablerowrule
...
阶段 & 可见范围 & 失败时的处理 \\
\midrule
分配 task 与栈 & 仅创建路径持有局部引用 & 释放已经分配的内存 \\\tablerowrule
建立资源引用 & 尚未进入 PID 查找和 runqueue & 按逆序撤销 mm、files、cred 等引用 \\\tablerowrule
建立进程关系 & 父进程与管理结构开始可见 & 在锁保护下解除关系和标识 \\\tablerowrule
...
资源 & 复制后的语义 & 共享后的语义 \\
\midrule
地址空间 & 修改私有映射通常不影响对方 & 任意线程的普通内存写对其他线程可见 \\\tablerowrule
文件描述符表 & 关闭本方 fd 槽不删除对方槽位 & 关闭共享槽位会影响所有引用该表的线程 \\\tablerowrule
文件对象 & 不同 fd 表仍可引用同一打开文件对象 & offset、状态标志按对象规则共同变化 \\\tablerowrule
信号处理配置 & 后续修改各自独立 & 处理函数配置属于线程组共同状态 \\\tablerowrule
...
阶段 & 仍可返回旧程序吗 & 需要保持的条件 \\
\midrule
读取并验证参数 & 可以 & 旧 mm、旧用户栈和调用线程仍有效 \\\tablerowrule
打开并检查可执行文件 & 可以 & 尚未改变线程组的公开执行状态 \\\tablerowrule
构造新 mm 与用户栈 & 通常可以 & 新对象尚未成为当前地址空间 \\\tablerowrule
处理其他线程 & 进入敏感过渡阶段 & 不能再允许其他线程恢复旧用户现场 \\\tablerowrule
...
\midrule
RUNNING/RUNNABLE & 完整执行现场和资源引用 & 无 \\\tablerowrule
EXITING & 退出码、清理进度、必要引用 & 不再接受新的普通工作 \\\tablerowrule
ZOMBIE & PID、父子关系、退出码、必要统计 & 用户地址空间、普通执行资源；不能再次调度 \\\tablerowrule
...
阶段 & 主要状态变化 & 必须保持的不变量 \\
\midrule
创建 & C 从私有构造对象变为 RUNNABLE & 所有必要资源已初始化，C 只进入一个 runqueue \\\tablerowrule
首次调度 & C 从 RUNNABLE 变为 CPU1 的 current & C 已从 runqueue 移除，不在其他 CPU 执行 \\\tablerowrule
阻塞读取 & C 从 RUNNING 变为 BLOCKED & 等待谓词与 wait queue 已登记，C 不再占 CPU \\\tablerowrule
网络唤醒 & C 从 BLOCKED 变为 RUNNABLE & 唤醒原因已记录，只进入一个目标 runqueue \\\tablerowrule
抢占 & C 的现场保存后重新入队 & 状态与 on\_rq 一致，共享进程资源引用不变 \\\tablerowrule
迁移 & C 从 CPU0 队列原子转移到 CPU1 队列 & 迁移期间不会同时属于两队列，也不会丢失 \\\tablerowrule
退出 & C 停止调度并形成退出结果 & 地址空间与文件引用按引用计数释放，退出码仍可读取 \\\tablerowrule

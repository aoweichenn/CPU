...
路径 & 数据流 & 代价 \\
\midrule
page cache hit & cache page 到用户 buffer & copy 成本，低延迟 \\\tablerowrule
page cache miss & storage 到 page cache，再 copy & I/O 等待，后续可复用 \\\tablerowrule
...
上下文 & fault 结果 & 原因 \\
\midrule
用户指令 load/store & signal & 用户程序违反地址空间规则 \\\tablerowrule
内核 copy user & \code{-EFAULT} & 系统调用参数错误，可返回给调用者 \\\tablerowrule
内核普通指针 & oops/panic & 内核自身错误或硬件故障 \\\tablerowrule
...
编号 & 对象 & 状态问题 \\
\midrule
1 & 用户 fd 整数 & 是否仍指向预期 file \\\tablerowrule
2 & \code{struct file} & 引用计数、offset、flags \\\tablerowrule
3 & inode/address space & 文件大小、page cache、权限 \\\tablerowrule
4 & folio/page & uptodate、dirty、locked、writeback \\\tablerowrule
5 & bio/request & 块映射、队列、超时 \\\tablerowrule
6 & DMA buffer/descriptor & map/unmap、所有权、completion \\\tablerowrule

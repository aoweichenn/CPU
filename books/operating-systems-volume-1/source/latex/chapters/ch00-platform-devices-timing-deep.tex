...
\begin{pdfdiagram}[x=0.92cm,y=1cm]
  \node[os state, minimum width=1.55cm] (cause) at (0,0) {device\\cause};
  \node[os control, minimum width=1.35cm] (mask) at (1.9,0) {mask};
  \node[os block, minimum width=1.6cm] (msix) at (3.8,0) {MSI-X\\message};
  \node[os control, minimum width=1.55cm] (apic) at (5.9,0) {local APIC\\IRR/ISR};
  \node[os compute, minimum width=1.55cm] (cpu) at (8.0,0) {CPU\\trap entry};
  \node[os control, minimum width=1.55cm] (idt) at (10.05,0) {IDT\\vector};
  \node[os state, minimum width=1.75cm] (handler) at (12.2,0) {handler\\thread/poll};

  \draw[os strong bus] (cause) -- (mask);
  \draw[os strong bus] (mask) -- (msix);
  \draw[os strong bus] (msix) -- (apic);
  \draw[os strong bus] (apic) -- (cpu);
  \draw[os strong bus] (cpu) -- (idt);
  \draw[os strong bus] (idt) -- (handler);
  \draw[os feedback] (handler.south) .. controls +(0,-1.05) and +(0,-1.05) .. node[os label, below] {clear cause + EOI} (cause.south);
\end{pdfdiagram}
...
\begin{pdfdiagram}[x=1cm,y=1cm]
  \node[os memory, minimum width=1.7cm] (tsc) at (0,0.6) {TSC\\clocksource};
  \node[os compute, minimum width=1.7cm] (tk) at (2.3,0.6) {timekeeper\\ns};
  \node[os state, minimum width=1.7cm] (hr) at (4.6,0.6) {hrtimer\\rb-tree};
  \node[os control, minimum width=1.8cm] (lapic) at (6.95,0.6) {APIC timer\\deadline};
  \node[os control, minimum width=1.7cm] (irq) at (9.25,0.6) {timer IRQ};
  \node[os state, minimum width=1.7cm] (sched) at (11.45,0.6) {scheduler\\tick};

  \draw[os strong bus] (tsc) -- (tk);
  \draw[os bus] (tk) -- (hr);
  \draw[os strong bus] (hr) -- (lapic);
  \draw[os strong bus] (lapic) -- (irq);
  \draw[os strong bus] (irq) -- (sched);
  \node[os label] at (5.7,-0.55) {$deadline = now + \Delta t,\quad fired \Leftrightarrow counter \ge deadline$};
\end{pdfdiagram}
...
\begin{pdfdiagram}[x=0.92cm,y=1cm]
  \node[os block, minimum width=1.55cm] (wire) at (0,0) {wire\\frame};
  \node[os block, minimum width=1.55cm] (mac) at (1.85,0) {PHY/MAC};
  \node[os compute, minimum width=1.55cm] (rss) at (3.7,0) {RSS\\queue};
  \node[os memory, minimum width=1.65cm] (rx) at (5.6,0) {RX buffer\\DMA};
  \node[os state, minimum width=1.55cm] (cq) at (7.55,0) {CQE};
  \node[os control, minimum width=1.55cm] (napi) at (9.4,0) {NAPI\\poll};
  \node[os state, minimum width=1.55cm] (skb) at (11.3,0) {skb};
  \node[os state, minimum width=1.55cm] (sock) at (13.2,0) {socket};

  \draw[os strong bus] (wire) -- (mac);
  \draw[os strong bus] (mac) -- (rss);
  \draw[os strong bus] (rss) -- (rx);
  \draw[os strong bus] (rx) -- (cq);
...
\begin{pdfdiagram}[x=1cm,y=1cm]
  \node[os state, minimum width=1.55cm] (write) at (0,0) {write()};
  \node[os memory, minimum width=1.75cm] (pc) at (2.0,0) {page cache\\dirty};
  \node[os state, minimum width=1.55cm] (bio) at (4.1,0) {bio/request};
  \node[os memory, minimum width=1.75cm] (nvme) at (6.25,0) {NVMe SQ\\DMA};
  \node[os block, minimum width=1.75cm] (ssd) at (8.45,0) {SSD FTL\\cache};
  \node[os block, minimum width=1.75cm] (nand) at (10.65,0) {NAND\\stable};
  \node[os control, minimum width=1.55cm] (fsync) at (5.15,-1.35) {fsync\\flush/FUA};

  \draw[os strong bus] (write) -- (pc);
  \draw[os strong bus] (pc) -- (bio);
  \draw[os strong bus] (bio) -- (nvme);
  \draw[os strong bus] (nvme) -- (ssd);
  \draw[os bus] (ssd) -- (nand);
  \draw[os feedback] (fsync) -- (ssd);
  \node[os label] at (5.6,0.78) {$write\ return \ne media\ persistent$};
\end{pdfdiagram}

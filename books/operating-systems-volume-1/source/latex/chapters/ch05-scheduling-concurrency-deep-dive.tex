...
原因 & 内核状态 & 诊断方向 \\
\midrule
等 CPU & RUNNABLE 但不在 CPU 上 & runnable wait、runqueue 长度、优先级 \\\tablerowrule
等锁 & SLEEPING 在 mutex/futex 等待队列 & 持锁者、锁顺序、临界区长度 \\\tablerowrule
等 I/O & SLEEPING 在 completion 等待队列 & 块层队列、设备延迟、writeback \\\tablerowrule
等页面 & page fault 或内存回收 & major fault、swap、page cache miss \\\tablerowrule
等事件 & poll、epoll、timer、条件变量 & 等待谓词、关闭路径、超时 \\\tablerowrule
...
策略 & 表现 & 问题 \\
\midrule
FCFS & 若 A 先到，C 和 B 长时间等待 & convoy effect，交互体验差 \\\tablerowrule
RR & A 不能长期霸占 CPU，B/C 能穿插运行 & 时间片过小会增加切换成本 \\\tablerowrule
优先级 & B 可以更快响应输入 & 低优先级任务可能饥饿 \\\tablerowrule
MLFQ & 通过行为反馈照顾交互任务 & 参数复杂，可能被任务行为利用 \\\tablerowrule
公平调度 & 长期 CPU 份额接近权重 & 响应延迟需要额外 wakeup/preempt 规则 \\\tablerowrule
...
算法 & 需要的输入 & 失败模式 \\
\midrule
RM & 周期、最坏执行时间、固定优先级 & 高优先级任务过多时低优先级 miss \\\tablerowrule
EDF & 绝对 deadline、剩余执行时间 & 过载时大量任务连续 miss \\\tablerowrule
...
当前状态 & 事件 & 动作 \\
\midrule
open, empty & pop blocking & 消费者入 not\_empty 等待队列 \\\tablerowrule
open, full & push blocking & 生产者入 not\_full 等待队列 \\\tablerowrule
open & close producer & 标记 no\_more\_push，唤醒消费者 \\\tablerowrule
open & close consumer & 标记 no\_more\_pop，唤醒生产者 \\\tablerowrule
closed, empty & pop & 返回 EOF/CLOSED \\\tablerowrule
...
原语 & 适合 & 不适合 \\
\midrule
spinlock & 短临界区、不能睡眠上下文 & I/O、缺页、可能阻塞路径 \\\tablerowrule
mutex & 普通内核对象互斥 & 中断上下文、自旋锁内层 \\\tablerowrule
rwlock & 读多写少且临界区短 & 写频繁或读者长期持有 \\\tablerowrule
semaphore & 资源计数与限流 & 表达复杂对象不变量 \\\tablerowrule
...
字段或对象 & 作用 \\
\midrule
\code{task\_struct.state} & runnable、interruptible sleep、uninterruptible sleep 等状态 \\\tablerowrule
\code{mm/active\_mm} & 用户地址空间和内核线程借用地址空间 \\\tablerowrule
\code{files} & fd table，fork/clone 时按标志共享或复制 \\\tablerowrule
\code{signal/sighand} & 进程级信号状态和 handler 表 \\\tablerowrule
\code{sched\_entity se} & CFS 的 vruntime、权重、红黑树节点 \\\tablerowrule
\code{cpus\_mask} & CPU affinity，限制可运行 CPU 集合 \\\tablerowrule
...
机制 & 调度影响 \\
\midrule
CPU quota & 组用完周期预算后 throttled，即使有 runnable 任务也不能运行 \\\tablerowrule
CPU weight & 多组竞争时按权重分配长期份额 \\\tablerowrule
CPU affinity & 限制任务可运行 CPU，可能改善局部性也可能造成热点 \\\tablerowrule
NUMA balancing & 在任务迁移和页面迁移之间寻找更低远端访问成本 \\\tablerowrule

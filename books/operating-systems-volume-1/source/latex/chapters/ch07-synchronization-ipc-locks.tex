...
字段 & 含义 & 受谁保护 \\
\midrule
capacity & 最大槽数，固定大于 0 & 构造后只读 \\\tablerowrule
head & 下一个可读槽 & mutex \\\tablerowrule
tail & 下一个可写槽 & mutex \\\tablerowrule
size & 当前元素数量 & mutex \\\tablerowrule
...
原语 & 适用场景 & 禁止事项 & 典型对象 \\
\midrule
关中断 & 单核短临界区，中断共享变量 & 多核共享保护，长时间计算 & tick 计数、单核设备寄存器影子状态 \\\tablerowrule
自旋锁 & 多核短临界区，不能睡眠的上下文 & 持锁 I/O、持锁 page fault、持锁等待用户事件 & runqueue、描述符环、引用计数 \\\tablerowrule
mutex & 进程上下文中可能等待的互斥 & 中断上下文获取，持有后进入不可重入路径 & inode 元数据、文件系统目录更新 \\\tablerowrule
读写锁 & 读多写少且临界区短 & 长读者导致写者饥饿，升级锁顺序混乱 & mount 表、路由表快照 \\\tablerowrule
RCU & 读路径极热，读者不阻塞写者 & 立即释放旧对象，读区内睡眠 & fdtable、路径缓存、配置表 \\\tablerowrule
...
\begin{keyidea}
对同步机制的理解应达到以下程度：能够说明每把锁保护的对象，判断自旋锁、mutex 与 RCU 的
上下文限制，写出 lost wakeup 的反例及修复，并用锁顺序图解释死锁风险。
\end{keyidea}

\section{读多写少、无锁读取与锁依赖}
...
Linux 的 qspinlock 比这段伪代码更紧凑：pending bit 处理短竞争，MCS queue 承载较长竞争，
锁路径还要配合抢占控制和虚拟化环境。queued lock 并不因为结构更复杂就必然更快；它是为
高竞争多核场景降低共享缓存行争用而增加的机制。
...
原语 & 优点 & 风险 \\
\midrule
rwlock & 读者并行，路径短 & 长读者或写频繁时收益差，不能睡眠 \\\tablerowrule
rwsem & 可睡眠，适合较长临界区 & 饥饿、公平性、唤醒策略复杂 \\\tablerowrule
optimistic spin & 避免短等待睡眠成本 & 持有者不运行时浪费 CPU \\
\bottomrule
\end{longtable}

读写信号量必须同时维护等待链表、持有者、读者计数和写者交接。若一条路径持有 rwsem 后，
又等待另一项同样需要该 rwsem 的工作完成，就会形成隐藏的等待环。
...
原语 & 语义 \\
\midrule
\code{smp\_rmb} & 约束读-读顺序 \\\tablerowrule
\code{smp\_wmb} & 约束写-写顺序 \\\tablerowrule
\code{smp\_mb} & 完整内存屏障，约束读写重排 \\\tablerowrule
\code{smp\_store\_release} & 发布数据前的写入对获取方可见 \\\tablerowrule
...
管道实现的关键不在函数名称，而在关闭语义：写端全部关闭且缓冲为空时，读操作返回 EOF；
读端全部关闭时，写操作失败并可能触发 \code{SIGPIPE}；非阻塞模式在暂时不能推进时返回
\code{EAGAIN}。这些结果都来自端点引用和缓冲区状态，而不是单纯来自一次读写调用。
...
机制 & 特点 & 适用边界 \\
\midrule
System V semaphore & 内核维护计数和 undo 语义 & 传统系统和跨进程资源计数 \\\tablerowrule
POSIX semaphore & named/unnamed 两类 & 进程间或线程间许可控制 \\\tablerowrule
System V msgqueue & 类型化消息，内核队列 & 需要消息边界但吞吐不极致 \\\tablerowrule
POSIX mq & 描述符化，可通知 & 与 fd/event loop 集成更方便 \\\tablerowrule
...
接口 & 内核语义 & 常见用途 \\
\midrule
\code{eventfd} & 64 位计数器，read 消费，write 增加 & 线程/进程通知、io\_uring、KVM kick \\\tablerowrule
\code{timerfd} & 定时器到期次数可读 & event loop 中处理超时 \\\tablerowrule
\code{signalfd} & 把被 mask 的信号作为 fd 读取 & 避免复杂 async signal handler \\\tablerowrule
...
需求 & 推荐思路 \\
\midrule
少量控制事件 & \code{eventfd}、pipe、socketpair \\\tablerowrule
需要消息边界 & Unix datagram socket、POSIX mq、自定义帧协议 \\\tablerowrule
大块数据同机传输 & shared memory + futex/eventfd 通知 \\\tablerowrule
要跨机器扩展 & TCP/QUIC/RPC，明确超时、重试和幂等 \\\tablerowrule
要把文件数据转发给 socket & \code{sendfile}、\code{splice}、zero-copy 路径 \\\tablerowrule

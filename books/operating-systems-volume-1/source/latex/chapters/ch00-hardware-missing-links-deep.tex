...
\begin{pdfdiagram}[x=0.96cm,y=1cm]
  \node[os title, anchor=east] at (-0.35,1.65) {CPU 提交链};
  \node[os title, anchor=east] at (-0.35,0) {地址/cache 链};
  \node[os title, anchor=east] at (-0.35,-1.65) {设备 DMA 链};

  \node[os control, minimum width=1.35cm] (isa) at (0.8,1.65) {ISA};
  \node[os compute, minimum width=1.55cm] (front) at (2.7,1.65) {fetch\\decode};
  \node[os compute, minimum width=1.55cm] (exec) at (4.6,1.65) {execute};
  \node[os state, minimum width=1.55cm] (retire) at (6.5,1.65) {retire};
  \node[os control, minimum width=1.55cm] (trap) at (8.4,1.65) {trap\\IRQ};
  \node[os control, minimum width=1.55cm] (kernel) at (10.3,1.65) {kernel};

  \node[os bit, minimum width=1.05cm] (va) at (0.8,0) {VA};
  \node[os memory, minimum width=1.4cm] (tlb) at (2.7,0) {TLB};
  \node[os memory, minimum width=1.55cm] (pt) at (4.6,0) {page\\walk};
  \node[os bit, minimum width=1.05cm] (pa) at (6.5,0) {PA};
  \node[os memory, minimum width=1.4cm] (cache) at (8.4,0) {cache};
  \node[os memory, minimum width=1.4cm] (dram) at (10.3,0) {DRAM};

  \node[os control, minimum width=1.45cm] (drv) at (0.8,-1.65) {driver};
  \node[os memory, minimum width=1.45cm] (ring) at (2.7,-1.65) {ring};
  \node[os control, minimum width=1.45cm] (db) at (4.6,-1.65) {doorbell};
  \node[os block, minimum width=1.45cm] (dev) at (6.5,-1.65) {device};
  \node[os memory, minimum width=1.45cm] (iommu) at (8.4,-1.65) {IOMMU};
  \node[os state, minimum width=1.55cm] (comp) at (10.3,-1.65) {completion};
  ...
  \draw[os feedback] (pt.north) .. controls +(0,0.75) and +(-2.4,-0.65) .. (kernel.south);
  \node[os label, fill=white, inner sep=1pt] at (3.65,0.48) {miss};
  \node[os label, fill=white, inner sep=1pt] at (6.95,0.85) {\#PF};
  ...
  \draw[os feedback] (comp.north) .. controls +(0,0.75) and +(1.2,-0.65) .. (kernel.south);
  \node[os label, fill=white, inner sep=1pt] at (9.7,0.48) {MSI-X};
\end{pdfdiagram}
...
\begin{pdfdiagram}[x=1cm,y=1cm]
  \node[os title, anchor=east] at (-0.35,0.95) {事件源};
  \node[os title, anchor=east] at (-0.35,-0.95) {CPU 入口};

  \node[os block, minimum width=1.45cm] (dev) at (0.85,0.95) {device};
  \node[os block, minimum width=1.65cm] (msix) at (2.9,0.95) {MSI-X\\msg};
  \node[os block, minimum width=1.45cm] (timer) at (5.0,0.95) {timer};
  \node[os block, minimum width=1.45cm] (ipi) at (7.0,0.95) {IPI};

  \node[os control, minimum width=1.75cm] (apic) at (0.95,-0.95) {local\\APIC};
  \node[os control, minimum width=1.35cm] (cpu) at (3.05,-0.95) {CPU};
  \node[os state, minimum width=1.35cm] (idt) at (4.9,-0.95) {IDT};
  \node[os control, minimum width=1.55cm] (entry) at (6.85,-0.95) {handler};

  \draw[os strong bus] (dev) -- (msix);
  \draw[os strong bus] (msix.south) -- node[os label, right] {vector} (apic.north);
  \draw[os bus] (timer.south) -- (apic.north east);
  \draw[os bus] (ipi.south) -- (apic.north east);
  \draw[os strong bus] (apic) -- (cpu);
  \draw[os strong bus] (cpu) -- (idt);
  \draw[os strong bus] (idt) -- (entry);
  \draw[os feedback] (entry.south) .. controls +(0,-0.95) and +(0,-0.95) .. node[os label, below] {EOI} (apic.south);
\end{pdfdiagram}

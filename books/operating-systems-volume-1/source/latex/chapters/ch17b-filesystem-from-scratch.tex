...
\begin{keyidea}
页缓存是 Linux \emph{buffered I/O} 的核心设计：普通 \code{read()} 先查缓存，
...
具体字段会随内核版本变化；这里以本书 Linux 6.16 基线附近的 folio 接口说明职责，
源码走读仍须以实际 checkout 的 tag 为准。许多现代文件系统还通过 iomap 复用
buffered I/O、direct I/O 和块映射的公共实现，而不是各自手写整套旧式 page 回调。
...
\begin{keyidea}
Btrfs snapshot 是 COW 的直接收益：旧树根仍然有效，新树根通过 COW 路径逐渐分叉。
共享的数据块通过 extent tree 的引用记录管理，最后一个引用消失后才具备回收条件。
创建时不复制全部文件数据，这是应当记住的复杂度优势；真实完成时间还受事务提交、
元数据更新和系统负载影响。
\end{keyidea}
...
\filepath{fs/btrfs/transaction.c}）用 COW 树、generation、校验和与超级块

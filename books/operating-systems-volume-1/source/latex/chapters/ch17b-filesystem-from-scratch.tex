...
\begin{statebox}
\textbf{项目里程碑 v0.11。} 文件系统占用磁盘 LBA 2048..3071 的 1024 个
512 字节块，固定保存 superblock、两张位图、32 个 inode、64 字节目录项和
1013 个数据块。八项 LRU 写回缓存连接 ATA PIO；修改以 Dirty/Clean 协议提交，
同盘两次全新 QEMU 证明跨启动持久化，损坏超级块的第三次启动必须在 Ring 3 前拒绝。
\end{statebox}
...
\input{chapters18/cases/v011-persistent-filesystem}
...
\begin{fspdfdiagram}[x=1mm,y=1mm]
% 用真实比例的矩形表达连续区间；文字不再参与盒宽计算，避免最小块被标签撑大后互相覆盖。
\draw[draw=BookAmber,fill=BookAmberTint,rounded corners=1.5pt] (-48,32) rectangle (48,42);
\node[os label,text=BookInk] at (0,37) {64};
\node[os label,anchor=east] at (-52,37) {初始空闲区};

\draw[draw=BookAmber,fill=BookAmberTint,rounded corners=1.5pt] (-48,18) rectangle (0,28);
\draw[draw=BookAccent,fill=BookBlueTint,rounded corners=1.5pt] (0,18) rectangle (48,28);
\node[os label,text=BookInk] at (-24,23) {32};
\node[os label,text=BookInk] at (24,23) {32};

\draw[draw=BookAmber,fill=BookAmberTint,rounded corners=1.5pt] (-48,4) rectangle (-24,14);
\draw[draw=BookAccent,fill=BookBlueTint,rounded corners=1.5pt] (-24,4) rectangle (0,14);
\node[os label,text=BookInk] at (-36,9) {16};
\node[os label,text=BookInk] at (-12,9) {16};

\draw[draw=BookAmber,fill=BookAmberTint,rounded corners=1.5pt] (-48,-10) rectangle (-36,0);
\draw[draw=BookAccent,fill=BookBlueTint,rounded corners=1.5pt] (-36,-10) rectangle (-24,0);
\node[os label,text=BookInk] at (-42,-5) {8};
\node[os label,text=BookInk] at (-30,-5) {8};

\draw[draw=BookTeal,fill=BookGreenTint,rounded corners=1.5pt] (-48,-24) rectangle (-42,-14);
\draw[draw=BookAmber,fill=BookAmberTint,rounded corners=1.5pt] (-42,-24) rectangle (-36,-14);
\node[font=\scriptsize\sffamily,text=BookInk] at (-45,-19) {4};
\node[font=\scriptsize\sffamily,text=BookInk] at (-39,-19) {4};
\node[os label,text=BookTeal,anchor=west] at (-32,-19) {左块交给请求，右块留在空闲链};
\end{fspdfdiagram}
...
\draw[os feedback] (daemon.east) -- ++(20mm,0) |- (app.east);
\node[os label,anchor=west] at ([xshift=23mm]fuse.east) {结果返回};
\node[os label,anchor=east] at ([xshift=-5mm]fuse.west) {请求下行};
\end{fspdfdiagram}
...
\chapter{缓存、写回与崩溃一致性}
\begin{statebox}
\textbf{项目里程碑 v0.11 的恢复边界。} 修改前先持久化 Dirty superblock，
再写回数据、inode、目录项与位图，最后持久化 Clean 和 CRC32。该协议能够检测
中途断电并拒绝挂载，却没有日志重放或回滚；正文由这个“检测并停止”的最小边界
继续推导 journal、checkpoint 与 COW 根切换。
\end{statebox}
\begin{keyidea}
...
\begin{fspdfdiagram}[x=1mm,y=1mm]
% 三组使用独立等宽网格；V/X 放在固定页格中，避免文字把相邻页格撑到一起。
\node[os small block,minimum width=30mm] (s0) at (-48,24) {stream 0\\metadata};
\node[os small block,minimum width=30mm] (s1) at (0,24) {stream 1\\hot data};
\node[os small block,minimum width=30mm] (s2) at (48,24) {stream 2\\cold data};

\foreach \x/\name in {-60/b00,-50/b01,-40/b02,-30/b03}
  \node[os bit,os state] (\name) at (\x,7) {V};
\node[os bit,os state] (b10) at (-12,7) {V};
\node[os bit,os control] (b11) at (-4,7) {X};
\node[os bit,os state] (b12) at (4,7) {V};
\node[os bit,os control] (b13) at (12,7) {X};
\foreach \x/\name in {36/b20,44/b21,52/b22,60/b23}
  \node[os bit,os control] (\name) at (\x,7) {X};

\draw[os bus] (s0.south) -- (b01.north);
\draw[os bus] (s1.south) -- (b11.north);
\draw[os bus] (s2.south) -- (b21.north);

\node[os label] at (0,-6) {stream 2 全部无效：GC 回收时复制 0 页};
\node[os label] at (0,-13) {\textcolor{BookMuted}{V = valid，X = invalid}};
\end{fspdfdiagram}
...
\begin{fspdfdiagram}[x=1mm,y=1mm]
\node[os label] at (-54,28) {t=0: write(4K)};
\node[os label] at (-22,28) {t=1: write(4K)};
\node[os label] at (10,28) {t=2: write(4K)};
\node[os label] at (58,28) {t=3: writeback};

\node[os memory,minimum width=24mm] (c1) at (-54,14) {page A\\dirty};
\node[os memory,minimum width=24mm] (c2) at (-22,14) {page B\\dirty};
\node[os memory,minimum width=24mm] (c3) at (10,14) {page C\\dirty};
\node[os compute,minimum width=40mm] (a) at (58,14)
  {ext4\_mballoc\\一次分配 3 个连续块};

\coordinate (m1) at (-54,-1);
\coordinate (m2) at (-22,-1);
\coordinate (m3) at (10,-1);
\draw[os guide] (c1.south) -- (m1);
\draw[os guide] (c2.south) -- (m2);
\draw[os guide] (c3.south) -- (m3);
\draw[os guide] (m1) -- (m3);
\draw[os strong bus] (m3) -- ++(14,0) |- (a.west);
\node[os label] at (-22,-8) {三次调用只预留内存空间，尚未分配物理块};

\node[os small block,minimum width=40mm] (d) at (58,-12) {disk: [A|B|C] contiguous};
\draw[os strong bus] (a.south) -- (d.north);
\end{fspdfdiagram}

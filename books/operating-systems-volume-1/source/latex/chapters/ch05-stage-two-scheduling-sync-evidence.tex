...
现象 & 内核解释 & 主要证据 \\
\midrule
CPU 忙但请求慢 & runnable 竞争、锁自旋、软中断占用 & \code{perf top}、\code{perf sched}、runqueue \\\tablerowrule
CPU 不忙但请求慢 & 线程 sleeping 等事件 & off-CPU profile、等待队列、I/O 统计 \\\tablerowrule
周期性尖峰 & cgroup throttle、定时批处理、GC/回收 & cgroup \code{cpu.stat}、tracepoint \\\tablerowrule
永久等待 & lost wakeup、死锁、关闭未唤醒 & 等待图、锁依赖、线程栈 \\\tablerowrule
低优先级长期得不到运行机会 & 高优先级或实时任务长期占用 & 调度类、优先级、runtime 统计 \\\tablerowrule
...
指标 & 解释 & 风险 \\
\midrule
nr\_running & 可运行任务数量 & 高表示 CPU 竞争 \\\tablerowrule
voluntary cs & 主动阻塞切换 & I/O/锁/等待事件多 \\\tablerowrule
involuntary cs & 被抢占切换 & 时间片、优先级、竞争 \\\tablerowrule
migrations & CPU 迁移次数 & 可能损失 cache/NUMA 局部性 \\\tablerowrule
throttled time & cgroup 配额等待 & 容器周期性延迟 \\\tablerowrule
...
注入点 & 目的 & 期望 \\
\midrule
fork 第 N 步失败 & 验证资源回滚 & 无泄漏、无半成品可见 \\\tablerowrule
exec commit 前失败 & 验证旧程序保留 & 返回错误，旧地址空间仍可用 \\\tablerowrule
wait 与 exit 并发 & 验证 zombie 回收 & 只回收一次 \\\tablerowrule
唤醒发生在入队前 & 验证无 lost wakeup & 等待者不永久睡眠 \\\tablerowrule
锁顺序反转 & 验证 deadlock 检测 & 报告等待环 \\\tablerowrule
高优先级等低优先级锁 & 验证优先级继承 & 持锁者被提升并释放锁 \\\tablerowrule
cgroup budget 耗尽 & 验证 throttle/unthrottle & 周期刷新后恢复运行 \\\tablerowrule

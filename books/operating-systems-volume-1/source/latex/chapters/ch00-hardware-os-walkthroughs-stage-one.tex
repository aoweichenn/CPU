...
地址空间切换可能影响 TLB。若硬件支持 ASID/PCID，旧地址空间的 TLB 项可以带标签保留；若没有标签或必须失效，切换成本更高。FPU/SIMD 状态很大，早期系统常用 lazy save，但安全问题和实现复杂性让许多系统转向 eager 策略。调度器的“公平”算法只是上层，底层切换成本会反过来影响线程数量和时间片选择。
...
页表根/ASID & 不同进程地址空间不同 & PCID/ASID 降低 TLB 刷新 \\
...
进程/进程 & 地址空间、ASID/TLB & VMA、PTE、COW、信号权限 \\

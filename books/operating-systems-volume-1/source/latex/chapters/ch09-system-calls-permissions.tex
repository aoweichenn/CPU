...
片段 & 复制时发生什么 & 结果 \\
\midrule
页 A 尾部 100 字节 & 页表权限允许读，直接复制 & \code{copied = 100} \\
...
时刻 & 动作 & 对象身份 \\
\midrule
1 & 受害者 \code{stat("/tmp/x/data.txt")} & 检查的是旧 dentry/inode \\
2 & 攻击者 \code{rename} 替换路径分量 & 路径字符串不变，指向对象已变 \\
3 & 受害者 \code{open("/tmp/x/data.txt")} & 解析得到新 dentry/inode \\
...
用户内存 & 外层结构、内层指针和长度何时复制、何时 pin 页 & \code{EFAULT} 变 panic，TOCTOU，整数溢出 \\

...
证据 & 典型来源 & OS 需要回答的问题 \\
\midrule
fault address & CR2 & 这个地址属于哪个 VMA，是否 canonical \\\tablerowrule
error code.P & page fault error code & 是 PTE 不存在，还是 present 页权限拒绝 \\\tablerowrule
error code.W/R & page fault error code & 是读、写还是取指触发 \\\tablerowrule
error code.U/S & page fault error code & 是 Ring 3 访问，还是内核路径访问 \\\tablerowrule
...
路径 & 角色 & 阅读重点 \\
\midrule
\filepath{arch/x86/mm/fault.c} & x86-64 page fault 入口 & 读取 CR2、解释 error code、区分用户/内核上下文 \\\tablerowrule
\filepath{arch/x86/entry/entry\_64.S} & 异常入口汇编 & trap frame、寄存器保存、进入 C 函数前的状态 \\\tablerowrule
\filepath{include/linux/mm\_types.h} & mm 核心结构 & \code{mm\_struct}、\code{vm\_area\_struct}、页表相关对象 \\\tablerowrule
\filepath{mm/mmap.c} & VMA 创建与查找 & \code{mmap}、\code{mprotect}、VMA 合并拆分 \\\tablerowrule
\filepath{mm/memory.c} & 通用 fault 主干 & \code{handle\_mm\_fault}、匿名 fault、文件 fault、COW \\\tablerowrule
\filepath{mm/rmap.c} & 反向映射 & 页被哪些映射引用，回收和 COW 需要的关系 \\\tablerowrule
\filepath{mm/filemap.c} & 文件页 fault & page cache、文件映射、读入文件页 \\\tablerowrule
...
硬件现象 & 内核可能动作 & 不能误判成什么 \\
\midrule
not-present 读匿名页 & 分配零页，安装 PTE，重试指令 & 程序错误 \\\tablerowrule
not-present 读文件页 & 查 page cache，必要时发起 I/O & 简单 malloc \\\tablerowrule
present 写只读 PTE & 若 VMA 允许写且 PTE 是 COW，则复制或提升 & 普通权限失败 \\\tablerowrule
present 写只读 VMA & 发送信号或返回错误 & COW \\\tablerowrule
...
同一个坏地址，在用户指令里访问和在系统调用参数复制时访问，结果不同。用户程序自己执行 \asmcode{mov rax, qword ptr [rdi]} 访问坏地址，通常应得到信号。若用户把坏指针传给 \code{read} 或 \code{write}，内核在 \code{copy\_to\_user} 或 \code{copy\_from\_user} 中 fault，则系统调用应返回 \code{-EFAULT}。
...
\midrule
\code{/proc/<pid>/maps} & VMA 区间、权限、文件映射 & 物理页是否已分配 \\\tablerowrule
\code{/proc/<pid>/smaps} & RSS、dirty、shared/private 统计 & 每一次 fault 的源码分支 \\\tablerowrule
\code{perf stat -e page-faults} & page fault 总量 & major/minor 和 fault 原因细节 \\\tablerowrule
\code{perf stat -e minor-faults,}\\\tablerowrule
\code{major-faults} & 缺页是否涉及 I/O & COW 与匿名零页的区别 \\\tablerowrule

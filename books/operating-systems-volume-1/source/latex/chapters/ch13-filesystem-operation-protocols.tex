\section{从数据结构到操作协议：一次修改为何不能随便排序}

前面已经知道 create 会改位图、inode、目录项和数据块。但“需要改哪些块”只回答了
静态结构问题，还没有回答三个动态问题：两个线程同时改怎么办，中途失败怎样撤销，
掉电后哪一个版本算已经完成。要回答它们，先给出三个不混用的词。
...
于是，\code{write} 返回通常建立的是新字节对进程的可见性；\code{rename} 可以建立
名字切换的原子可见性；\code{fsync} 请求建立持久性。三个边界有关联，却不是同一个
边界。把它们统称为“原子写”会掩盖真正需要证明的性质。

\subsection{锁、事务和写序分别解决什么}
...
\emph{写序（write ordering）}规定某个写在另一个写获得持久性之前必须先获得持久性。
...
事务，而不一定是在磁盘上逐步执行传统意义的“反向撤销”。两者不要因为都叫恢复就混为一谈。

\begin{longtable}{@{}L{0.16\linewidth}L{0.28\linewidth}L{0.26\linewidth}L{0.22\linewidth}@{}}
\toprule
机制 & 首先防止的问题 & 生效时间 & 单独做不到什么 \\
\midrule
锁 & 线程并发交错破坏内存状态 & 系统运行期间 & 不能跨断电保存 \\
运行期回滚 & 系统调用中途报错泄漏资源 & 错误返回之前 & 不能修复已经掉电的机器 \\
日志事务 & 多块元数据只持久了一部分 & mount 恢复与 checkpoint & 不自动保证普通数据内容原子 \\
写序/flush/FUA & 设备重排或易失缓存破坏依赖 & I/O 提交到持久化完成 & 不能决定两个线程谁先修改目录 \\
fsck 式扫描 & 已有结构不满足全局不变量 & 离线或受控修复期间 & 通常不知道应用原本想完成什么 \\
\bottomrule
\end{longtable}

\subsection{create：名字是最后的“发布边”}
...
create(parent, "x"):
  ...
  6. insert dirent "x" -> inode          // 发布边：名字开始可达
...
第 6 步叫发布边，因为在它之前，新 inode 即使已经存在，也没有从根目录出发的路径；
在它之后，路径解析可以找到它。发布边之前出错，资源应被回滚。发布边之后若向用户
返回失败，调用者也不能武断地认为名字不存在：网络文件系统、I/O 错误或崩溃边界都
可能造成“操作已经生效但答复丢失”，稳健程序要重新查询状态。

“先预留日志空间”也不是性能小技巧。若先把目录项发布，最后才发现日志已满，就会
进入既不能安全提交、又不能简单撤销的困境。预留是在修改前证明：最坏情况下有足够
空间记录使结构一致所需的元数据。

\subsection{create 的掉电矩阵：结构一致不等于用户意图完成}
...
再写 inode 与数据后 & 完整数据仍没有名字可达 & 目录项这个发布边尚未持久 \\
只持久目录项、inode 未持久 & 名字指向无效或旧 inode & 发布发生在被发布对象之前 \\
全部写完后 & 结构可能正确 & 仍要确认设备是否兑现写序与持久化命令 \\
\bottomrule
\end{longtable}

日志的价值现在可以精确描述：把相关元数据版本连同事务号写入日志，完整写出 commit
record 后才把事务视为可重放；没有完整提交证据的尾部事务被忽略。ordered data 模式
还建立“数据先于使其可达的元数据提交”的顺序。日志不是让掉电不发生，而是把恢复时
无限多种半完成组合压缩成“重放已提交事务、丢弃未提交事务”两类可判定选择。

\subsection{unlink：删除名字为什么不等于删除正在打开的文件}
...
永久泄漏。孤儿在这里不是“找不到就猜”，而是删除协议主动留下的待办记录。

\subsection{rename：原子可见性与持久发布是两回事}
...
关系规则；跨目录 rename 还必须防止把目录移动进自己的后代，从而制造目录环。锁顺序
不是实现细枝末节，而是“目录树始终是一棵无环层级结构”这一不变量的并发证明。

\subsection{truncate：改变长度为何会牵动 page cache、块映射和 mmap}
...
的旧内容。因此 truncate 的核心不是“把 size 赋成 5000”，而是撤销 EOF 之后所有层次
对旧数据的引用，并与仍可能使用这些引用的并发路径建立同步屏障。
...
\subsection{fsync：到底同步了文件、名字还是整个文件系统}
...
\begin{warningbox}
持久性证明必须写出对象集合和顺序，而不是只写一句“调用 fsync”：哪些文件的数据，
哪些 inode 元数据，哪些父目录项，先后顺序怎样，每次调用是否检查错误，设备是否具有
正确的持久化语义。应用若需要跨多个文件的原子提交，通常还要用 WAL、manifest 或
数据库事务；单个文件的 fsync 不会自动把多个文件变成一个事务。
\end{warningbox}

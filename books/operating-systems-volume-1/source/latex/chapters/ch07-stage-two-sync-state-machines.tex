...
问题 & 适合的原语 & 原因 \\
\midrule
连接池最多 N 个并发连接 & semaphore & 许可数量就是核心状态 \\\tablerowrule
队列不空/不满 & condition variable & 谓词依赖多个字段 \\\tablerowrule
等待某个请求完成一次 & completion/future & 一次性状态转移 \\\tablerowrule
等待多个 fd readiness & epoll/wait queue & 等待集合是 fd 对象 \\\tablerowrule
...
状态 & 事件 & 期望 \\
\midrule
empty open & blocking pop & 消费者睡眠 \\\tablerowrule
empty open & producer push & 消费者被唤醒并取得 item \\\tablerowrule
full open & blocking push & 生产者睡眠 \\\tablerowrule
full open & consumer pop & 生产者被唤醒并写入 \\\tablerowrule
empty closed & pop & 返回 CLOSED/EOF \\\tablerowrule
full closed & push & 返回 CLOSED/EPIPE \\\tablerowrule
waiting & timeout & 等待节点移除，返回 TIMEOUT \\\tablerowrule

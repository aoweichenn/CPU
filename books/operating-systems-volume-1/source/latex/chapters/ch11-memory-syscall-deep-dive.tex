...
结论 & 条件 & 处理 \\
\midrule
合法缺页 & 地址属于 VMA，权限允许，只是 PTE 不存在 & 分配页、读文件页或建立 COW 页 \\\tablerowrule
权限错误 & 地址属于 VMA，但访问类型不允许 & 发送 SIGSEGV 或返回 \code{EFAULT} \\\tablerowrule
...
错误 & 典型原因 & 调用者策略 \\
\midrule
\code{EBADF} & fd 不存在或模式不允许 & 程序 bug 或句柄生命周期错误 \\\tablerowrule
\code{EFAULT} & 用户指针不可访问 & 程序 bug，不能重试同一指针 \\\tablerowrule
\code{EINTR} & 阻塞调用被信号打断 & 根据已完成字节数决定是否重试 \\\tablerowrule
\code{EAGAIN} & 非阻塞对象暂时不能推进 & 等 readiness 后重试 \\\tablerowrule
\code{ENOMEM} & 内存不足或限制触发 & 降级、释放资源或失败返回 \\\tablerowrule
\code{EIO} & 设备或底层 I/O 错误 & 上报、切换副本、进入恢复流程 \\\tablerowrule
...
对象 & 回答的问题 & 典型变化 \\
\midrule
VMA & 这段地址是否合法，权限和来源是什么 & \code{mmap/munmap/mprotect} 改变 \\\tablerowrule
PTE & 这一页现在是否可由硬件访问 & 缺页、换出、COW、unmap 改变 \\\tablerowrule
...
\subsection{TLB、PCID 和 shootdown 成本}
...
操作 & 必要性 & 成本来源 \\
\midrule
local invalidate & 当前 CPU 旧翻译不能继续用 & pipeline/TLB 扰动 \\\tablerowrule
remote shootdown & 其他 CPU 旧翻译不能继续用 & IPI、等待 ack、跨核同步 \\\tablerowrule
batch unmap & 合并多个失效请求 & 延迟释放与复杂生命周期 \\\tablerowrule
...
分配层 & 管理对象 & 典型问题 \\
\midrule
page allocator & 物理页和连续页块 & 外部碎片、NUMA、本地性 \\\tablerowrule
slab allocator & inode、task、dentry 等固定对象 & 对象复用、构造析构、缓存热度 \\\tablerowrule
vm allocator & 内核虚拟连续区域 & 页表开销、TLB、地址空间碎片 \\\tablerowrule
...
测试 & 触发方式 & 期望 \\
\midrule
坏用户读指针 & \code{write(fd, bad, len)} & 返回 \code{EFAULT}，内核不崩溃 \\\tablerowrule
坏用户写指针 & \code{read(fd, bad, len)} & 返回 \code{EFAULT} 或短读语义清楚 \\\tablerowrule
COW 写入 & fork 后父子分别写同页 & 内容分离，引用计数正确 \\\tablerowrule
权限缺页 & 写只读映射或执行 NX 页 & 信号或错误，不能绕过权限 \\\tablerowrule
munmap 竞态 & 复制期间另线程 unmap & 返回错误或短完成，不 use-after-free \\\tablerowrule
TLB 失效 & mprotect 后旧权限访问 & 旧权限不能继续生效 \\\tablerowrule

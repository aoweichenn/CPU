...
任务位置 & 含义 & 调度器能否直接运行 \\
\midrule
CPU current & 正在运行或在内核入口中 & 已经在运行 \\\tablerowrule
runqueue & runnable，等待 CPU & 可以被选择 \\\tablerowrule
wait queue & 条件不满足，睡眠中 & 不能，必须被唤醒 \\\tablerowrule
完成请求 & 等设备或异步结果 & 不能，completion 路径唤醒 \\\tablerowrule
...
调度类 & 目标 & 风险 \\
\midrule
stop/deadline & CPU 停止、deadline 保证 & 最高优先级路径错误会冻结系统 \\\tablerowrule
real-time FIFO/RR & 固定优先级实时响应 & 饥饿普通任务，需要限额 \\\tablerowrule
fair & 权重公平与通用吞吐 & 不能保证硬实时 \\\tablerowrule
...
证据 & 回答的问题 & 层次 \\
\midrule
\code{/proc/<pid>/sched} & 任务运行、等待、迁移等统计 & 单任务 \\\tablerowrule
\code{perf sched} & 调度事件时间线 & 系统级 \\\tablerowrule
cgroup cpu.stat & 是否被 quota throttle & 服务组 \\\tablerowrule
tracepoint sched\_* & wakeup、switch、migrate 事件 & 内核事件 \\\tablerowrule

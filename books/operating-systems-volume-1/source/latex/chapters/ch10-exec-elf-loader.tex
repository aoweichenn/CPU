...
对象 & 成功后状态 & 失败回滚 \\
\midrule
旧 mm & 被新 mm 替换并释放引用 & 保持可运行 \\\tablerowrule
新 mm & 成为 current 的地址空间 & 释放所有 VMA 和页 \\\tablerowrule
文件引用 & 可执行文件可关闭或保留映射引用 & put inode/file 引用 \\\tablerowrule
fd table & 关闭 \code{FD\_CLOEXEC} 条目 & 不应改变 \\\tablerowrule
信号处理 & 捕获处理器重置，忽略语义按规则保留 & 不应改变 \\\tablerowrule
...
\midrule
复制用户参数 & 内核缓冲区里的 path、argv、envp 副本 & 返回 \code{EFAULT/E2BIG}，旧程序继续 \\\tablerowrule
路径解析 & 指向 \code{/usr/bin/grep} 的 inode/file 引用 & 返回 \code{ENOENT/EACCES}，释放引用 \\\tablerowrule
ELF 验证 & ELF header 和 program header 副本 & 返回 \code{ENOEXEC}，旧 mm 不变 \\\tablerowrule
临时装载 & 新 mm、VMA、栈页、解释器信息 & 释放临时 mm，旧程序继续 \\\tablerowrule
...
时刻 & 状态 & 关键边界 \\
\midrule
exec 提交 & 新 mm 已安装，代码段只有 VMA 或部分 PTE & 旧程序不能继续运行 \\\tablerowrule
第一次取指 & 入口页 PTE 不 present，CPU 产生缺页异常 & faulting 指令还没执行 \\\tablerowrule
缺页处理 & 根据 VMA 找到 ELF 文件和页偏移 & 必须检查 VMA 可执行权限 \\\tablerowrule
读入页面 & page cache miss 时等待存储 I/O & 当前 task 可睡眠，调度器运行别人 \\\tablerowrule
安装 PTE & PTE 标成 user、present、read、exec、not writable & 权限不能超过 ELF 段权限 \\\tablerowrule
...
\midrule
地址空间 & 新 \code{mm}、VMA、栈、brk、mmap base、VDSO & 提交前失败必须保留旧 \code{mm}，提交后不能再回旧代码 \\\tablerowrule
文件与路径 & 可执行文件、解释器、脚本原文件、mount flags & 权限检查必须绑定稳定 inode/file，而不是路径字符串 \\\tablerowrule
fd 继承 & 普通 fd 继承，\code{FD\_CLOEXEC} fd 关闭 & 不能在 exec 失败时提前关闭旧程序 fd \\\tablerowrule
凭据与安全 & setuid、file capability、LSM、no\_new\_privs、dumpable & 提权 exec 要清理危险环境和调试关系 \\\tablerowrule
线程组 & 成功后只保留调用 exec 的线程 & 旧用户态锁和其他线程栈不能进入新映像 \\\tablerowrule
...
字段或类型 & 含义 & 装载用途 \\
\midrule
\code{PT\_LOAD} & 可映射段 & 建立 VMA，设置 R/W/X 权限 \\\tablerowrule
\code{PT\_INTERP} & 动态解释器路径 & 装载 \code{ld-linux.so} 并把入口交给它 \\\tablerowrule
\code{PT\_PHDR} & program header 自身位置 & 传给用户态动态链接器 \\\tablerowrule
\code{PT\_GNU\_STACK} & 栈权限提示 & 决定用户栈是否可执行 \\\tablerowrule
...
机制 & exec 阶段输入 \\
\midrule
ASLR & 随机选择 mmap、stack、brk、VDSO、PIE 基址 \\\tablerowrule
stack canary & \code{AT\_RANDOM} 提供随机字节 \\\tablerowrule
NX stack & \code{PT\_GNU\_STACK} 和默认策略决定栈可执行性 \\\tablerowrule

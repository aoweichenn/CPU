...
\section{从一次文件保存认识操作系统}

编辑器里点“保存”，表面上只是把一段字节写进名为 \code{data} 的文件，再告诉用户“保存成功”。这一句话看起来很小，但它实际包含三个承诺：用户给出的内容被系统接收了；旧文件不会因为半路失败遭到非预期破坏；如果程序或机器随后出错，系统还能说明哪个版本可信。
...
\midrule
用户传入了内容 & 这段地址属于当前进程，且内核有权读取 & 可能读到非法地址，甚至泄漏内核数据 \\\tablerowrule
系统接收了请求 & 当前执行流、文件引用和目标对象都被记录下来 & 设备慢时只能忙等，或完成后找不到发起者 \\\tablerowrule
写入推进了 & 数据进入了哪个对象：内存缓存、设备队列还是持久介质 & \code{write} 返回后掉电，用户以为保存了，其实磁盘仍是旧内容 \\\tablerowrule
失败能报告 & 错误发生在参数、权限、地址、设备、持久化还是目录发布 & 只能返回模糊失败，无法修复或恢复 \\\tablerowrule
完成能证明 & 哪个提交点之后，崩溃恢复可以相信这个结果 & 重启后无法区分成功版本和半成品 \\
\bottomrule
\end{longtable}

在继续引入 task、页表和文件对象之前，还要回答一个更基础的问题：这些工作为什么不能由
每个应用程序各自完成？假设编辑器、备份程序和日志程序都需要写同一块设备。最初可以给
三个程序各放一份设备访问库，让它们在写入之前调用 \code{acquire}，写完之后调用
\code{release}。只要三个程序都正确、都使用同一套库，这种协作可以维持。

困难在于，库只能为愿意调用它的代码提供帮助，不能约束绕过它的代码。一个程序可能因为
缺陷忘记释放设备，另一个程序可能直接访问设备寄存器，第三个程序还可能把不属于自己的
内存地址交给设备。此时，问题不在于库函数写得是否精巧，而在于它没有高于应用程序的
控制权。
...
\midrule
约定每个程序主动让出 CPU & 失控程序是否会遵守约定 & 一个死循环可以长期占用处理器 \\\tablerowrule
约定写设备前先取得软件锁 & 程序是否绕过库直接写寄存器 & 两个请求可能同时改动设备状态 \\\tablerowrule
约定只访问分配给自己的内存 & 任意机器指令是否遵守地址范围 & 一个错误指针可能破坏其他程序 \\\tablerowrule
每个程序维护自己的空闲块表 & 多份记录是否始终一致 & 同一磁盘块可能被重复分配 \\\tablerowrule
程序自行判断访问权限 & 被拒绝者是否愿意接受判断 & 权限规则无法对恶意或错误代码生效 \\
\bottomrule
\end{longtable}

要使约束成立，系统需要一个应用程序不能绕过的共同管理者。处理器必须能够区分普通程序和
这个管理者的执行权限；敏感寄存器、页表和中断控制不能由普通程序直接改写；应用程序若要
使用设备、创建地址映射或访问文件，只能通过受控入口提出请求。这个受保护的核心称为
\emph{内核（kernel）}。

内核不是因为代码规模更大而特殊，而是因为机器赋予了它不同的权限和入口。普通程序运行在
受限制的执行级别，只能访问页表允许的地址；系统调用把请求交给内核；异常让内核处理当前
指令无法自行解决的问题；中断让内核在设备完成或定时器到期时重新取得控制。这样，管理规则
才从“请各程序自觉遵守”变成“机器和内核共同执行”。

\begin{longtable}{|L{0.2\linewidth}|L{0.34\linewidth}|L{0.38\linewidth}|}
\toprule
所需能力 & 由谁提供 & 在保存请求中的作用 \\
\midrule
限制敏感操作 & 处理器特权级与内核入口 & 应用不能直接改写磁盘队列或页表 \\\tablerowrule
限制地址访问 & 页表检查与地址空间管理 & 编辑器的 buffer 不能指向其他进程的私有页面 \\\tablerowrule
周期性收回 CPU & 硬件定时器与调度器 & 一个程序失控时，其他程序仍有运行机会 \\\tablerowrule
统一记录资源归属 & 内核对象与全局分配状态 & fd、页面和设备请求不会由各应用分别解释 \\\tablerowrule
接收异步结果 & 中断、完成队列与等待关系 & 设备结束工作后，结果能交还给正确请求 \\
\bottomrule
\end{longtable}

“操作系统”和“内核”也需要区分。内核是以特权方式运行、直接维护隔离和资源状态的核心；
操作系统还包括系统库、后台服务、命令工具和管理配置。系统库可以把复杂接口整理得更易用，
后台服务可以实现名称解析、日志收集或设备管理，但最终涉及地址隔离、处理器调度和设备授权
时，仍要依赖内核提供的强制边界。

同一项管理还应区分\emph{机制（mechanism）}与\emph{策略（policy）}。机制说明系统
“怎样做到”，例如怎样保存执行现场、怎样把任务放入等待队列、怎样拒绝无权限的页访问；
策略说明系统“选择什么”，例如先运行哪个任务、允许哪个主体打开文件、脏页积累到什么程度
开始写回。若没有机制，策略无法执行；若策略被固定在每个底层动作里，系统又难以适应不同
负载。后续章节会在每个子系统中分别说明两者。

至此，操作系统的起点已经可以从具体条件中得到：多个程序共享处理器、内存和设备，而程序
可能出错、延迟或不合作；系统因而需要一个不能被普通程序绕过的内核，由它建立统一对象、
执行隔离规则、安排等待关系，并把硬件事件转换成可检查的结果。
...
判断一个 OS 机制是否讲清楚，要看四类可观察证据：用户程序提出了什么请求；进入内核后新增了哪些对象状态；硬件用什么事件、寄存器或权限检查支撑这些状态；失败时外部能看到什么结果。比如 \code{write(fd, buf, 4)} 这一行，在用户层只是一个函数调用，在内核层要解释 fd、buffer 和长度，在硬件层可能触发 page fault、DMA 和中断，在证据层则只返回字节数或错误码。
...
\midrule
0 ms & not ready & 读一次状态寄存器 & 发现不能写 \\\tablerowrule
1 ms & not ready & 再读一次状态寄存器 & 没有任何新进展 \\\tablerowrule
2 ms & not ready & 继续读状态寄存器 & 其他程序得不到 CPU \\\tablerowrule
...
要让执行流可靠地停下并恢复，需要引入 \code{task}。\code{task} 是内核能暂停、恢复、选择运行的一条执行流。它不是用户写的一个普通函数；它必须保存运行位置、可运行状态以及当前等待条件。
...
\midrule
局部变量 \code{i} & 当前复制进度、请求偏移、已完成字节数 & 函数可能睡眠，醒来后要知道从哪里继续 \\\tablerowrule
一次函数调用 & \code{task} 和保存的执行现场 & 当前执行流可能被切走，稍后从同一位置恢复 \\\tablerowrule
一个布尔判断 & 谓词和受保护状态 & 条件可能被中断路径或另一个 CPU 修改 \\\tablerowrule
...
词 & 在本例中的含义 & 反例 \\
\midrule
\code{BLOCKED} & 当前 task 不能继续运行，因为它在等设备 ready & 如果还把它放在可运行队列里，调度器可能又选中它，结果还是忙等 \\\tablerowrule
\code{wait queue} & 等同一个条件的一组 task，例如都在等 \code{dev.ready} & 如果没有队列，设备完成时不知道该叫醒谁 \\\tablerowrule
...
状态 & 含义 & 在保存例子里的位置 \\
\midrule
\code{running} & 正在某个 CPU 上执行 & 保存函数正在跑 \code{device\_ready()} 或复制数据 \\\tablerowrule
\code{runnable} & 条件已经满足，只是暂时没轮到 CPU & 设备已经 ready，task 被唤醒，等调度器选中 \\\tablerowrule
...
真实状态名 & 对应教学状态 & 含义 \\
\midrule
\code{TASK\_RUNNING} & running 或 runnable & 在 CPU 上，或已在 runqueue 排队；两者共用一个状态名，靠 \code{on\_rq} 字段区分 \\\tablerowrule
\code{TASK\_INTERRUPTIBLE} & blocked & 睡眠等待条件，可被信号打断 \\\tablerowrule
\code{TASK\_UNINTERRUPTIBLE} & blocked & 睡眠等待条件（多为 I/O），信号打不断 \\\tablerowrule
\code{\_\_TASK\_STOPPED} & blocked 的一种 & 被信号或调试器停住 \\\tablerowrule
...
\midrule
1 & 检查 \code{dev.ready == false} & 还没发生 \\\tablerowrule
2 & 准备把自己睡下去 & 设备变成 ready \\\tablerowrule
3 & 还没入队 & 中断处理调用 \code{wake\_up(&dev.waiters)}，但队列为空 \\\tablerowrule
...
错误理解 & 正确理解 & 后果 \\
\midrule
wakeup 是一条消息 & wakeup 只是提示条件可能变化 & 醒来后必须重新检查谓词 \\\tablerowrule
入队和检查可以分开写 & 检查谓词和登记等待者必须受同一规则保护 & 否则完成事件会从缝隙里丢失 \\\tablerowrule
睡眠只是暂停函数 & 睡眠会让出 CPU，并改变 task 的调度状态 & blocked task 不能继续留在 runqueue 上抢 CPU \\\tablerowrule
...
词 & 在本例中是什么意思 & 为什么必须要它 \\
\midrule
谓词 & 一个能回答“现在能不能继续”的条件，这里是 \code{dev.ready} & wakeup 不是消息本身，task 醒来后还要重新检查条件 \\\tablerowrule
...
路径 & 它做什么 & 不能破坏的关系 \\
\midrule
等待路径 & 检查 \code{dev.ready}，不满足就把当前 task 放入 wait queue & 检查条件和入队不能被完成路径插到中间 \\\tablerowrule
完成路径 & 设备中断到来，更新请求状态，再唤醒等待者 & 必须先写完成状态，再唤醒；否则等待者醒来仍看不到结果 \\\tablerowrule
调度路径 & \code{schedule} 选择另一个 runnable task 运行 & blocked task 不能还留在 runqueue 里抢 CPU \\\tablerowrule
...
初始判断 & 忽略的问题 & 完整追问 \\
\midrule
指针不为空 & 非空地址仍可能没有映射 & 它是否落在当前进程允许的地址区间内 \\\tablerowrule
起点在用户空间 & 后续字节可能跨到非法页 & 整个范围是否逐页可读 \\\tablerowrule
页表现在有翻译 & 复制中可能发生缺页、换入或权限变化 & fault 能否修复，修复后是否需要重试 \\\tablerowrule
...
词 & 含义 & 保存例子里的判断 \\
\midrule
虚拟地址 & 用户程序看到的地址编号，不直接等于物理内存位置 & \code{0x1000} 是用户给出的虚拟地址 \\\tablerowrule
VMA & virtual memory area，一段地址区间的“使用意图” & 这段地址是不是用户栈、堆或 mmap 文件，是否允许读 \\\tablerowrule
PTE & page table entry，一页地址的“当前翻译事实” & 这一页现在是否在内存里，是否允许用户态读 \\\tablerowrule
...
步骤 & 内核检查到什么 & 下一步 \\
\midrule
第一页 & 地址落在 VMA 内，VMA 允许读，PTE present & 复制第一页的数据 \\\tablerowrule
第二页 & 地址落在 VMA 内，但 PTE not-present & 触发 page fault，能补页就重试 \\\tablerowrule
第二页另一种情况 & 找不到覆盖它的 VMA & 停止复制，返回部分写入或 \code{-EFAULT} \\\tablerowrule
...
用户传入的情况 & 内核看到什么 & 合理结果 \\
\midrule
buffer 全部合法且页已在内存 & VMA 允许读，PTE present & 复制成功，继续写入路径 \\\tablerowrule
buffer 合法但页未装入 & VMA 允许读，PTE not-present & 触发 page fault，分配或读入页面后重试 \\\tablerowrule
buffer 前半段合法，后半段非法 & 前几页复制成功，后一页找不到 VMA & 返回已写字节数或 \code{-EFAULT}，不能越界读 \\\tablerowrule
buffer 指向内核地址 & 用户态无权访问内核专用页 & 拒绝，不能把内核内存泄漏给用户 \\\tablerowrule
...
这里没有出现“内核无条件崩溃”，也没有出现“从地址 \code{0x1} 读出无效数据”。发生的是一条受控错误路径：CPU 在内核复制用户数据时发现地址访问不成立，进入 page fault；内核发现这次 fault 发生在 \code{copy\_from\_user} 这类允许恢复的位置，于是把它翻译成 \code{-EFAULT} 返回给系统调用。内核里通常会为这类可恢复访问准备异常表。异常表记录特定访存指令及其修复入口：若这些指令发生 fault，内核不按普通缺陷处理，而是进入修复路径，设置错误结果并返回。

\begin{longtable}{|L{0.2\linewidth}|L{0.34\linewidth}|L{0.38\linewidth}|}
\toprule
观察到的现象 & 背后的边界 & 说明 \\
\midrule
\code{0x1} 能作为参数传入 & 系统调用入口只接收整数值 & 入口接收参数不代表参数语义有效 \\\tablerowrule
\code{write} 返回 \code{EFAULT} & 地址复制边界失败 & 错误发生在读取用户 buffer，而不是 fd 或磁盘 \\\tablerowrule
进程继续运行 & fault 被内核修复路径接住 & 这类 fault 是可报告错误，不是必须 panic 的内核 bug \\\tablerowrule
...
层次 & 在本例中的含义 & 它回答的问题 \\
\midrule
fd 整数 & 用户态拿到的号码，例如 3 & 用户后续用哪个号码引用这次打开得到的对象 \\\tablerowrule
fd table & 当前进程的号码表 & 3 号槽是否存在，指向哪个内核对象 \\\tablerowrule
打开文件对象（file object） & 一次打开文件形成的内核对象 & 当前 offset、读写模式、引用计数、具体操作函数 \\\tablerowrule
...
动作 & 共享 offset & 文件内容 \\
\midrule
\code{write(a,"A",1)} & 0 变成 1 & \code{A} \\\tablerowrule
\code{write(b,"B",1)} & 1 变成 2 & \code{AB} \\\tablerowrule
\code{lseek(a,0)} & 2 变成 0 & \code{AB}，只是位置变了 \\\tablerowrule
\code{write(b,"C",1)} & 0 变成 1 & \code{CB}，第一个字节被覆盖 \\
\bottomrule
\end{longtable}

这比口头说“fd 是引用”更有力。用户手里确实有两个整数，但内核中只有一个共享的打开文件对象；offset 属于这个对象，不属于整数变量本身。若代码需要两个互不影响的文件位置，就不能用 \code{dup} 得到第二个位置，而要重新 \code{open}，让内核创建另一个打开文件对象。
...
\begin{longtable}{|L{0.2\linewidth}|L{0.35\linewidth}|L{0.37\linewidth}|}
\toprule
现象 & 如果把 fd 当文件本体会怎样误解 & 用对象状态怎样解释 \\
\midrule
\code{dup} 后共享 offset & 以为两个整数各写各的 & 两个 fd 槽指向同一个打开文件对象 \\\tablerowrule
\code{close(a)} 后 \code{b} 还能写 & 以为文件已经关闭 & 只减少一个引用，打开对象仍被 \code{b} 持有 \\\tablerowrule
\code{fork} 后子进程继承 fd & 以为复制了整个文件 & 父子 fd table 槽可引用同一打开对象 \\\tablerowrule
...
写法 & 内核对象关系 & offset 结果 \\
\midrule
\code{dup(a)} & 两个 fd 槽指向同一个打开文件对象 & \code{a} 写完后，\code{b} 接着往后写 \\\tablerowrule
重新 \code{open} & 两个打开文件对象指向同一个 inode & \code{a} 的 offset 不影响 \code{b} 的 offset \\\tablerowrule
...
边界 & 此时数据在哪里 & 还不能承诺什么 \\
\midrule
\code{copy\_from\_user} 结束 & 内核 buffer 或 page cache 中 & 目标文件名未必已经安全发布 \\\tablerowrule
\code{write} 返回 & 接口语义接受了若干字节 & 设备未必见过这批数据 \\\tablerowrule
块请求提交 & request 和 DMA 映射交给驱动/设备 & completion 未到，buffer 不能释放 \\\tablerowrule
设备 completion & 某次设备请求完成或失败 & 介质持久性仍可能依赖 flush/FUA/日志 \\\tablerowrule
...
完成点 & 说明 & 反例 \\
\midrule
系统调用完成 & \code{write} 返回，内核已经按接口语义接受了一部分数据 & 机器立刻掉电，dirty page 还没写到设备 \\\tablerowrule
设备请求完成 & 块层或驱动收到 completion，某次 I/O 已结束 & 设备缓存还没 flush，断电后介质上未必有数据 \\\tablerowrule
...
\code{write} 几十微秒就返回，因为它只是把数据复制进 page cache 并标上 dirty；\code{fsync} 必须等设备真正确认，第一次甚至花了 3 毫秒多——完成点 1 和完成点 3 之间隔着一到两个数量级。中间各层仍在维护 page cache、块层队列、驱动请求和设备固件状态；本章后半部分的每一条路径，都是在解释这段距离里发生了什么。
...
\midrule
\code{write} 返回后、\code{fsync} 前 & 内存里有 dirty page，磁盘旧内容还在 & \code{write} 的契约不是持久化契约 \\\tablerowrule
\code{fsync(fd)} 中途 & 部分块已写，错误可能稍后报告 & I/O error 需要被记录并返回给调用者 \\\tablerowrule
\code{rename} 后、目录未 \code{fsync} & 文件内容可能在，但名字更新未必可恢复 & 目录项本身也是元数据，也需要提交边界 \\\tablerowrule
日志 commit 前 & 日志里有准备信息，但不能当成功事务 & 恢复时必须丢弃未 commit 的更新 \\\tablerowrule
...
\midrule
当前正在执行的函数 & task & 它能否运行，睡在哪里，怎样从同一位置恢复 \\\tablerowrule
用户传入的指针 & VMA、PTE、page & 这个地址是否合法，读写执行权限是什么，fault 能否修复 \\\tablerowrule
一个整数句柄 & fd table、打开文件对象 & 这个 fd 指向谁，是否可写，关闭或 exec 时怎样处理 \\\tablerowrule
设备内部队列 & request、descriptor、completion & 哪个请求完成了，buffer 归谁，错误怎样返回 \\\tablerowrule
缓存中的文件页 & page cache、inode & 哪些字节已修改，何时写回，谁能看到 \\\tablerowrule
落盘顺序 & journal、commit record & 崩溃后哪些更新可信，哪些必须丢弃或重放 \\
\bottomrule
\end{longtable}

OS 对象把函数调用期间无法完整承载的状态保存到内核中。对象建立后，还要配套权限、
...
本章叫法 & 运行时对象与职责 \\
\midrule
task & 任务对象保存执行现场、调度状态以及指向地址空间和文件表的引用 \\\tablerowrule
wait queue & 等待队列头与等待节点保存“谁在等待哪个条件” \\\tablerowrule
VMA & 虚拟内存区域记录一段地址范围的用途与访问策略 \\\tablerowrule
PTE & 页表项记录当前映射、权限和处理器可见状态 \\\tablerowrule
fd table & 文件描述符表保存整数句柄到打开文件对象的映射 \\\tablerowrule
打开文件对象 & 保存当前偏移、打开标志、引用计数和操作方法 \\\tablerowrule
page cache & 文件映射对象与缓存页共同保存文件内容的内存副本 \\\tablerowrule
request & 块请求对象保存设备操作、范围、缓冲区和完成身份 \\\tablerowrule
completion & 完成对象保存异步操作的结果并连接等待者 \\\tablerowrule
...
\code{strace} 只能看到用户态和系统调用边界，不能直接告诉你 page cache、DMA 或文件系统日志里发生了什么。但它给了第一层证据：保存不是一个内部过程完全不可见的函数，而是一串明确的内核入口。每一行返回 \code{0} 或正数，只说明对应系统调用自己的契约成立；它不能自动证明后续边界已经成立。

\begin{longtable}{|L{0.18\linewidth}|L{0.34\linewidth}|L{0.4\linewidth}|}
\toprule
观察到的入口 & 这行真正越过的边界 & 还没有自动保证什么 \\
\midrule
\code{openat} & 路径解析、权限检查、fd 分配成功 & 后续写入一定成功 \\\tablerowrule
\code{write} & 内核接受了 6 个字节，并推进到目标对象的写路径 & 数据已经在断电后可恢复 \\\tablerowrule
\code{fsync(fd)} & 文件数据和必要文件元数据被要求推进到持久化边界 & 目录项名字更新已经可恢复 \\\tablerowrule
\code{close} & fd 槽位释放，并可能暴露延迟错误 & 另一个 fd 不再引用同一个打开文件对象 \\\tablerowrule
\code{renameat} & 目录项从临时名字发布到正式名字 & 崩溃后一定能看到新名字 \\\tablerowrule
...
阶段 & 状态对象 & 关键边界 & 失败证据 \\
\midrule
入口 & task、入口现场 & 用户态切到内核态，保存返回位置 & 错误码、审计记录 \\\tablerowrule
授权 & fd、打开文件对象、inode & fd 是否存在，目标是否可写 & \code{-EBADF}、\code{-EACCES} \\\tablerowrule
复制 & VMA、PTE、page & 用户 buffer 是否可读 & \code{-EFAULT}、部分写入 \\\tablerowrule
缓存 & page cache、inode & dirty 页只是内存状态 & dirty/writeback 计数、回写错误 \\\tablerowrule
提交 I/O & request、DMA map & buffer 在 completion 前不能释放 & timeout、I/O error、completion status \\\tablerowrule
...
\midrule
\code{O\_TRUNC} & 旧文件长度先变成 0 & 后续写失败时，旧内容已经丢失 \\\tablerowrule
一次 \code{write} & 可能只复制部分字节，或停在 page cache & 返回短写时，新文件是半成品 \\\tablerowrule
...
\midrule
\code{open(... O\_TRUNC)} 之后 & 长度已经变成 0 & 还没写任何新数据，旧版本已经被破坏 \\\tablerowrule
\code{write} 短写之后 & 可能只有 \code{"ne"} 这样的前缀 & 系统调用允许部分推进，不能假设一次写完 \\\tablerowrule
\code{write} 返回、未 \code{fsync} & page cache 里可能是新内容，磁盘上可能仍是旧内容或部分更新 & 函数返回和持久化不是同一个边界 \\\tablerowrule
...
设计选择 & 它来自哪个失败 & 边界含义 \\
\midrule
不用 \code{O\_TRUNC} 直接改正式文件 & 写到一半失败会破坏旧版本 & 旧文件在新版本准备好前保持可用 \\\tablerowrule
使用同目录临时文件 & 跨文件系统 \code{rename} 不能保证同样语义 & 临时文件和正式文件处在同一个目录事务范围内 \\\tablerowrule
循环 \code{write\_all} & 一次 \code{write} 可能短写或被信号打断 & 未写完整就不能进入发布阶段 \\\tablerowrule
先 \code{fsync(tmpfd)} & page cache 中的新内容不等于持久内容 & 新文件内容先越过文件持久化边界 \\\tablerowrule
再 \code{rename} & 正式名字只能指向旧版本或新版本 & 发布动作集中到目录项切换 \\\tablerowrule
最后 \code{fsync(dirfd)} & 目录项本身也是元数据 & 崩溃后正式名字更新有恢复证据 \\
\bottomrule
\end{longtable}

先展开 \code{write\_all}。它不是一般性的防御写法，而是因为 \code{write} 的基本契约就是“返回本次实际推进了多少字节”。只要返回值小于请求长度，调用者就不能把完整数据视为已经进入保存路径。
...
\midrule
第 1 次 \code{write} & 2 & 0 变成 2 & 继续写剩余 4 字节 \\\tablerowrule
第 2 次 \code{write} & \code{-1/EINTR} & 仍是 2 & 不能报成功，重试同一段剩余数据 \\\tablerowrule
第 3 次 \code{write} & 4 & 2 变成 6 & 才能进入 \code{fsync(tmpfd)} \\
\bottomrule
\end{longtable}

\code{EINTR}、\code{EAGAIN} 和短写看似只是接口细节，但它们对应三种不同边界。\code{EINTR} 表示调用被信号路径打断；\code{EAGAIN} 表示对象现在不能推进，调用者要等待可写条件；短写表示系统已经接受了一部分数据，下一次必须从新 offset 继续。若保存程序不记录 \code{off}，就会重复写前缀、漏写后缀，或者把半成品当完整版本发布。
...
逐行分析它的状态推进会更清楚。打开目录并非多余动作，因为最后要同步目录项；创建临时文件不是形式需要，而是为了让新内容有独立承载对象；\code{write\_all} 不是普通循环，而是在处理“接口允许短推进”这个契约；\code{fsync(tmpfd)} 把临时文件内容向持久化推进；\code{close(tmpfd)} 还可能暴露延迟错误；\code{renameat} 才把正式名字切到新文件；\code{fsync(dirfd)} 则让这个名字变化本身在崩溃后可恢复。

\begin{longtable}{|L{0.19\linewidth}|L{0.35\linewidth}|L{0.38\linewidth}|}
\toprule
处理阶段 & 此刻系统必须记住什么 & 如果失败该怎么解释 \\
\midrule
\code{open(dir)} & 目录对象和权限 & 目录打不开，保存根本没有进入准备阶段 \\\tablerowrule
\code{open\_temp\_at} & 临时 inode、临时目录项、fd 引用 & 只清理临时对象，不影响正式文件 \\\tablerowrule
\code{write\_all} & 已写入字节数、dirty page、可能的短写 & 未完整写入，不允许发布 \\\tablerowrule
\code{fsync(tmpfd)} & 数据和必要元数据的提交结果 & 返回错误说明新版本不可信 \\\tablerowrule
\code{renameat} & 目录项从旧 inode 切到新 inode & 失败时正式名字仍应指向旧版本 \\\tablerowrule
...
动作 & 它保护什么 & 崩溃后恢复含义 \\
\midrule
同目录临时文件 & 保证 rename 不跨文件系统，旧名字暂时不变 & 旧文件仍可作为最后成功版本 \\\tablerowrule
\code{write\_all} & 处理短写、\code{EINTR}、\code{EAGAIN} 等推进问题 & 临时文件要么完整写入，要么不发布 \\\tablerowrule
\code{fsync(tmpfd)} & 推进临时文件数据和必要元数据 & 新内容有机会在崩溃后被读回 \\\tablerowrule
\code{rename} & 原子切换目录项指向 & 名字层面看到旧版本或新版本，不应看到半个名字 \\\tablerowrule
...
\midrule
临时文件创建前 & \code{data} 仍是旧版本 & 保存没有开始影响正式文件 \\\tablerowrule
\code{write\_all} 中途 & \code{data} 仍是旧版本，可能残留半个临时文件 & 临时文件不能发布，清理后重试 \\\tablerowrule
\code{fsync(tmpfd)} 失败 & \code{data} 仍是旧版本，新内容不可信 & 必须返回失败，不能继续 rename \\\tablerowrule
\code{renameat} 前 & \code{data} 仍指向旧版本，临时文件可能完整 & 新版本还没有发布给正式名字 \\\tablerowrule
\code{renameat} 后、目录未同步 & 运行时能看到新名字，但崩溃恢复后名字更新可能没有证据 & 不能把目录项持久化当成已经完成 \\\tablerowrule
...
对象 & 它记住什么 & 没有它会怎样 \\
\midrule
task & 当前执行流是否 running、runnable、blocked，睡在哪里 & 设备慢时只能忙等，或者睡了以后无法恢复 \\\tablerowrule
wait queue & 哪些 task 在等同一个谓词 & 设备完成后不知道该叫醒谁，容易 lost wakeup \\\tablerowrule
VMA/PTE & 地址区间的意图和单页翻译事实 & 用户任意传入一个指针，内核不知道该复制、补页还是拒绝 \\\tablerowrule
fd table/打开文件对象 & 整数 fd 指向哪个打开对象，offset 和读写权限是什么 & \code{dup}、\code{close}、共享 offset 都解释不清 \\\tablerowrule
page cache & 文件内容在内存中的页面，哪些是 dirty & \code{write} 返回后无法说明数据停在哪里 \\\tablerowrule
请求/完成记录 & 哪次设备 I/O 已提交，哪次已经完成 & 中断来了也不知道对应哪个保存请求 \\\tablerowrule
...
\midrule
地址 & 指针值，例如 \code{0x1000} & VMA/PTE 和权限 & 页表翻译、物理地址、设备寄存器地址 \\\tablerowrule
可读 & \code{read} 不会立刻阻塞 & buffer/队列中有数据或条件满足 & 中断、completion、DMA 写入 \\\tablerowrule
写完成 & 系统调用返回字节数 & page cache 或 request 状态更新 & 设备接受请求，介质 flush 可能还没发生 \\\tablerowrule
上下文 & 用户调用链和线程身份 & task、保存的返回位置、内核栈 & 程序计数器、栈指针、寄存器 \\\tablerowrule
...
\input{chapters/ch01-save-object-ledger}

\section{异步事件如何推动任务前进}
...
\begin{pdfdiagram}
  ...
  \node[os title, align=center] at (64mm,-21mm) {内核决策\\同一套对象状态\\task、VMA、fd\\request、wait queue};
...
\end{pdfdiagram}
\diagramnote{四条路径入口不同、结果不同，但中间经过的是同一套内核对象状态。理解其中任何一条，都要能说清它读写了哪些共享状态——这正是后面每一章的展开方式。}
...
阶段 & 关键检查 & 失败表达 \\
\midrule
系统调用入口 & syscall number 是否存在，当前上下文是否允许 & \code{-ENOSYS}、拒绝或信号 \\\tablerowrule
fd 查找 & fd 是否打开，是否有写权限 & \code{-EBADF}、\code{-EPERM} \\\tablerowrule
用户 buffer & 地址格式是否有效、VMA 是否允许读、复制中是否 fault & \code{-EFAULT} 或部分写入 \\\tablerowrule
目标对象 & 文件、pipe 或 socket 是否可写、是否阻塞 & \code{-EAGAIN}、睡眠、信号中断 \\\tablerowrule
...
硬件看到 & 内核进一步区分 & 结果 \\
\midrule
present=0 & 地址属于匿名 VMA & 分配零页，minor fault \\\tablerowrule
present=0 & 地址属于文件映射 & page cache 命中或发起磁盘 I/O \\\tablerowrule
write denied & PTE 标记 shared-copy & 复制页并恢复写权限 \\\tablerowrule
execute denied & NX 或 VMA 不可执行 & 发送信号，安全拒绝 \\\tablerowrule
...
动作 & 需要保存什么 & 保存错误的现象 \\
\midrule
中断进入 & 用户返回位置、栈指针和寄存器 & 返回后用户程序寄存器随机坏 \\\tablerowrule
调度统计 & 当前任务运行时间和状态 & CPU 时间不公平或饥饿 \\\tablerowrule
切换内核上下文 & 内核栈指针、必须保留的寄存器 & 任务从错误调用链恢复 \\\tablerowrule
...
\end{pdfdiagram}
\diagramnote{完成顺序不等于提交顺序：B 先成功、A 后失败是合法结果。completion entry 里的 tag 把设备事实重新接回正确的 request 对象，两条红色虚线表示按请求身份匹配结果。}

\begin{longtable}{|L{0.2\linewidth}|L{0.36\linewidth}|L{0.36\linewidth}|}
\toprule
阶段 & 硬件/驱动看到什么 & 内核必须维护什么 \\
\midrule
填 descriptor & buffer 地址、长度、操作类型、tag & request 对象仍然活着，buffer 不能释放 \\\tablerowrule
写 doorbell & 设备被通知去取 descriptor & descriptor 字段必须已经完整可见 \\\tablerowrule
设备 DMA & 设备直接读写内存 buffer & 页面要被固定或映射，防止被回收复用 \\\tablerowrule
写 completion & completion entry 带回 tag 和 status & 用 tag 找回 request，记录成功或错误 \\\tablerowrule
...
还有一种更容易被忽略的情况：等待者已经不愿继续等待，设备却仍在工作。例如，线程等待
一次写请求时收到信号，或者超时时间已经到达。系统调用可以先返回，但这只改变了“线程是否
继续等待”，并不自动撤销已经交给设备的命令。

\begin{longtable}{|L{0.2\linewidth}|L{0.34\linewidth}|L{0.38\linewidth}|}
\toprule
发生的动作 & 它实际改变的状态 & 不能据此推断什么 \\
\midrule
等待超时 & 等待者停止阻塞，准备取得一个超时结果 & 设备已经停止 DMA \\\tablerowrule
提出取消 & 请求被标记为希望取消，并尝试通知下层 & 下层一定来得及接受取消 \\\tablerowrule
设备完成 & completion 已经带回最终状态 & 原等待线程仍在原处等待 \\\tablerowrule
线程收到信号 & 系统调用可能提前返回或稍后重启 & 已提交 I/O 没有产生任何效果 \\
\bottomrule
\end{longtable}

这四个事实必须分别记录。若把“线程不等了”直接等同于“请求不存在了”，内核可能立即释放
request 和 buffer；迟到的设备完成随后仍会使用旧 tag 或旧地址，结果可能是重复完成、
访问已释放内存，甚至把完成状态写给后来复用同一槽位的新请求。
...
\code{CANCEL\_REQUESTED} 不是终态。它表示取消意图已经登记，但设备仍可能报告“取消成功”、
“请求早已完成”或普通 I/O 错误。只有收到确定的下层结果，或者驱动已经通过复位等协议证明
设备不会再访问该请求，内核才可以进入 \code{COMPLETED}，随后在引用全部释放后进入
\code{REAPED}。

取消与完成还可能同时发生。正确处理需要一个共同的状态转换规则，保证只有一条路径取得
“最终完成权”。完成路径负责写入最终状态和字节数，取消路径只能在成功取得相应状态转换后
宣布取消；失败的一方读取已经存在的最终结果，而不能再次释放对象或再次唤醒同一完成事件。

\begin{longtable}{|L{0.22\linewidth}|L{0.34\linewidth}|L{0.36\linewidth}|}
\toprule
竞争顺序 & 合法结果 & 必须保持的不变量 \\
\midrule
取消先被设备接受 & completion 返回取消状态 & buffer 在取消完成前仍保持映射 \\\tablerowrule
完成先到达 & 取消请求返回“已完成” & 原 I/O 结果不能被取消状态覆盖 \\\tablerowrule
线程先超时返回 & request 留在内核等待迟到 completion & 调用者离开不导致 request 提前释放 \\\tablerowrule
队列槽位准备复用 & 旧请求已经完成并解除设备访问权 & 旧 completion 不能命中新请求 \\
\bottomrule
\end{longtable}

最后一行通常还需要代际编号或不会过早复用的 tag。只有队列位置而没有代际信息时，旧请求
的迟到完成可能与新请求使用同一个编号；内核必须能识别“这是上一代请求的结果”，将其隔离
或报告为设备协议错误。

保存操作也遵守这一规则。若等待 \code{fsync} 时线程被信号打断，应用程序不能只凭
“系统调用提前返回”判断介质上一定没有新数据；写回可能继续并最终成功，也可能在稍后记录
错误。应用需要依据接口定义决定重试、查询错误还是放弃发布，而内核必须继续维护请求，直到
下层不再引用相关页面和对象。超时、取消和完成的分离，是所有异步 I/O、网络请求和设备驱动
共同遵守的生命周期边界。
...
路径 & 进入内核时的主要证据 & 后续处理 \\
\midrule
系统调用 & 调用号、参数寄存器、用户返回现场 & 分发、参数验证、权限检查与返回 \\\tablerowrule
缺页异常 & 故障地址、访问类型、当前页表状态 & VMA 查找、权限判断、建立映射或报告错误 \\\tablerowrule
定时器中断 & 当前任务、运行时间记录和抢占状态 & 计时、调度决策与可能的任务切换 \\\tablerowrule
...
硬件条件 & 操作系统抽象 & 关键问题 \\
\midrule
程序计数器、寄存器和栈可保存 & task 上下文 & 切走后如何从同一位置恢复 \\\tablerowrule
特权级限制敏感指令 & 用户态/内核态 & 用户如何安全请求内核服务 \\\tablerowrule
异常入口能保存出错位置 & page fault、signal、debug & 错误能否修复并返回原指令 \\\tablerowrule
定时器可强制中断 & 抢占式调度 & 不合作任务如何被收回 CPU \\\tablerowrule
MMU 检查页表权限 & 地址空间 & 进程如何互相隔离又能共享 \\\tablerowrule
cache/TLB 需要维护 & 性能和一致性策略 & 为什么迁移、锁和取消映射有成本 \\\tablerowrule
设备寄存器控制设备 & 驱动模型 & 寄存器副作用和顺序如何管理 \\\tablerowrule
...
用户语义 & 内核对象 & 硬件支点 & 关键边界 \\
\midrule
process & task、地址空间、fd table & 寄存器、页表、定时器 & 身份、地址空间、权限分离 \\\tablerowrule
thread & task、内核栈、共享地址空间 & 寄存器、栈指针 & 可调度上下文，不复制全部资源 \\\tablerowrule
fd & fd table、打开文件对象 & 设备队列、page cache、DMA & 整数句柄不是文件本体 \\\tablerowrule
signal & 待处理信号、用户返回现场 & 保存的异常/中断现场 & 异步交付但必须恢复用户现场 \\\tablerowrule
socket & socket 对象、wait queue & 网卡队列、中断、DMA & 协议状态和设备完成分离 \\\tablerowrule
...
\midrule
函数忙等设备 & 改成状态机或中断 & 阻塞、等待队列、completion \\\tablerowrule
任务靠自觉返回 & 任务表和显式 yield & 线程和协作式调度 \\\tablerowrule
任务可能不合作 & 加硬件定时器 & 抢占和时间片 \\\tablerowrule
全局变量被中断处理函数打断 & 关中断临界区 & 锁、原子操作、内存屏障 \\\tablerowrule
所有代码同权 & 约定模块边界 & CPU 特权级和系统调用 \\\tablerowrule
所有内存同权 & 手工内存布局 & MMU、地址空间、页表权限 \\\tablerowrule
...
问题 & 保存路径中的内容 \\
\midrule
状态对象 & 进程、线程、页、fd、inode、socket、设备请求、日志记录 \\\tablerowrule
所有者 & 用户态对象、内核对象、硬件寄存器、设备控制器 \\\tablerowrule
入口 & 系统调用、异常、中断、内核线程、后台回收 \\\tablerowrule
权限 & 特权级、页表权限、文件权限、资源能力、命名空间、引用计数 \\\tablerowrule
等待 & runqueue、futex、page fault、I/O completion、锁、条件变量 \\\tablerowrule
提交 & 状态更新、引用释放、写入 page cache、落盘、目录项发布生效 \\\tablerowrule
...
机制 & 正常路径 & 必须补的反例 \\
\midrule
系统调用 & 参数合法，返回字节数或结果 & 用户指针跨页且后一页不可读，返回 \code{-EFAULT} 或部分成功 \\\tablerowrule
调度 & runnable 任务被选中运行 & 任务睡在 wait queue 中，不能因为优先级高就运行 \\\tablerowrule
锁 & 持锁者修改共享状态后释放 & 两把锁顺序反过来，形成 wait-for 环 \\\tablerowrule
缺页 & VMA 合法，分配或读入页面 & 地址不属于任何 VMA，必须发信号而不是分配页 \\\tablerowrule
DMA & completion 到达后回收 buffer & completion 前释放 buffer，设备可能写坏别人的内存 \\\tablerowrule
...
层次 & 必须回答的问题 & 例子 \\
\midrule
原始失败 & 没有这个机制时，哪个具体动作会坏 & 死循环霸占 CPU；用户指针越界；写一半掉电 \\\tablerowrule
状态对象 & 内核新增哪本账，记录哪些字段 & task state、runqueue、VMA、PTE、request、journal record \\\tablerowrule
状态转移 & 谁能修改状态，入口是什么 & timer interrupt 改调度状态；page fault 安装 PTE；completion 完成 request \\\tablerowrule
不变量 & 哪些关系不能被破坏 & sleeping 任务不能在 runqueue；DMA 完成前 buffer 不能释放 \\\tablerowrule
...
维度 & 追问 & 例子 \\
\midrule
身份 & 对象如何被引用 & pid、tid、fd、inode number、socket 四元组、DMA tag \\\tablerowrule
权限 & 谁能做什么 & 页表读写执行位、fd 标志、资源能力、命名空间 \\\tablerowrule
生命周期 & 何时创建和释放 & fork/exit/wait、open/dup/close、page refcount、request completion \\\tablerowrule
等待 & 无法立即推进时停在哪里 & runqueue、wait queue、page fault、I/O queue、timer \\\tablerowrule
...
关口 & 要解决的问题 & 保存案例中的落点 \\
\midrule
机器能保存状态 & CPU、寄存器、栈、时钟和设备寄存器如何形成可恢复现场 & task 能从睡眠处回来，设备 ready 位能被观察 \\\tablerowrule
机器能被打断 & 中断、异常和定时器怎样把外部事件交给内核 & 设备完成能唤醒等待者，死循环能被抢占 \\\tablerowrule
地址能被检查 & MMU、页表、VMA 和 PTE 怎样区分合法缺页和非法访问 & 用户 buffer 跨页时，内核知道复制、补页还是拒绝 \\\tablerowrule
资源能被引用 & fd、打开文件对象、inode、socket、request 怎样保存一次操作的状态 & 整数 fd 找到文件对象，I/O completion 找回请求 \\\tablerowrule
...
失败 & 需要的机制 & 新增代价 \\
\midrule
固定电路不通用 & 存储程序 CPU & 指令编码、取指译码、异常语义 \\\tablerowrule
单核状态无法被安全共享 & 多核一致性和原子操作 & cache line 迁移、内存序、锁竞争 \\\tablerowrule
程序直接用物理地址 & MMU 和虚拟地址 & 页表、TLB、缺页成本 \\\tablerowrule
CPU 搬运所有 I/O 数据 & DMA 和描述符队列 & buffer 生命周期、IOMMU、completion \\\tablerowrule
轮询浪费 CPU & 中断 & 异步共享状态 \\\tablerowrule
任务死循环霸占 CPU & 定时器抢占 & 上下文保存和调度开销 \\\tablerowrule
任务互相覆盖内存 & 地址空间和权限 & 页表、缺页、TLB 成本 \\\tablerowrule
用户可直接改写设备 & 系统调用和特权级 & 入口检查和复制成本 \\\tablerowrule
I/O 等待拖住计算 & 阻塞队列、异步和 completion & 请求身份和 buffer 生命周期 \\\tablerowrule
...
\item 崩溃点推演：在完成点时间线上任选一点假设“断电”，写出重启后用户、内核、设备各自能看到什么，并说明哪一层的持久记录足以支撑恢复结论。
\end{enumerate}

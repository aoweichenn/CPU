...
缺页异常 & \code{exc\_page\_fault}（\filepath{arch/x86/mm/fault.c}）、\code{handle\_mm\_fault}（\filepath{mm/memory.c}） & 第 7 章 \\
定时器中断 & \code{scheduler\_tick}、\code{\_\_schedule}（\filepath{kernel/sched/core.c}） & 第 4 章 \\
设备完成 & 驱动 irq handler、\code{complete}（\filepath{kernel/sched/completion.c}）、\code{wake\_up} & 第 9 章 \\
\bottomrule
\end{longtable}

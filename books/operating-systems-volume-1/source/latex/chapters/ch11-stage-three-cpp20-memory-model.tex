...
对象 & 责任 & 不做什么 \\
\midrule
\code{PhysicalMemory} & 分配页框、维护引用计数、复制页 & 不模拟 NUMA、cache、DMA \\\tablerowrule
\code{AddressSpace} & 管理 VMA 和页表 & 不模拟多级页表的每级页表页 \\\tablerowrule
\code{PageTableEntry} & 保存页框、权限、COW 标志 & 不模拟 accessed/dirty 的硬件置位细节 \\\tablerowrule
...
测试 & 步骤 & 期望 \\
\midrule
未映射地址 & 不建 VMA，访问地址 & 返回段错误结果 \\\tablerowrule
懒分配 & 建匿名 VMA，首次读 & 分配零页并 resolved \\\tablerowrule
写只读 VMA & VMA 只读，执行写 fault & 返回权限错误结果 \\\tablerowrule
COW refcount>1 & 父子共享页，子写 & 子获得新 frame，旧 frame refcount 减一 \\\tablerowrule
COW refcount=1 & 单独持有 COW 页后写 & 不复制，只恢复 writable \\\tablerowrule
mprotect 收缩 & RW 改 R 后写 & 写失败，旧权限不能生效 \\\tablerowrule

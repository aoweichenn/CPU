...
\midrule
VMA & 记录虚拟区间、文件、offset 0、共享可写 & 不变 \\\tablerowrule
PTE & 两个虚拟页通常都还没有 present 映射 & 第二个虚拟页 present，指向 page cache 页 \\\tablerowrule
page cache & 可能没有对应页，也可能已有别的进程留下的页 & \code{(inode, index=1)} 有缓存页 \\\tablerowrule
文件系统 I/O & 通常没有立即读 8192 字节 & 若 cache miss，提交读第 1 个文件页 \\\tablerowrule
...
对象 & 保存什么 & 不变量 \\
\midrule
VMA & 虚拟区间、权限、flags、file、offset & VMA 不重叠，权限不超过文件打开模式 \\\tablerowrule
page table entry & 虚拟页到物理页、权限、dirty/accessed & PTE 权限不能超过 VMA 权限 \\\tablerowrule
page cache page & inode、文件页索引、物理页、dirty、refcount & 同一 inode+index 对应一个缓存页 \\\tablerowrule
anon page & 私有 COW 后的页面 & 不再回写到文件 \\\tablerowrule
...
\midrule
CPU/PTE & A 的页表项允许写；硬件 dirty 位可能被置位 \\\tablerowrule
page cache & \code{(inode,0)} 对应页内容已变，页面需要写回 \\\tablerowrule
文件系统 & 还未必分配或提交所有元数据事务 \\\tablerowrule
块层/设备 & 还未必收到写请求，更未必完成持久化 \\\tablerowrule
...
\begin{rubricbox}
完成本章后，读者应能画出 VMA、PTE、page cache 与 inode 的关系，解释 private/shared
映射的写故障差异，并区分 \code{msync}、\code{fsync} 和业务提交的边界。
\end{rubricbox}

\subsection{文件映射与持久化语义的衔接}
...
边界 & 已经保证什么 & 仍未保证什么 \\
\midrule
\code{memcpy} 到映射区 & 当前进程地址空间已改变 & 文件页未必写回，私有映射不改文件 \\\tablerowrule
其他进程读到新字节 & 共享 page cache 可见 & 掉电后未必存在 \\\tablerowrule
\code{msync(MS\_SYNC)} & 指定映射脏页写回并等待结果 & 目录项、rename、其他文件未必提交 \\\tablerowrule
\code{fsync(file)} & 文件数据和必要元数据推进到持久层 & 多文件事务顺序未必正确 \\\tablerowrule
...
VMA 的权限与行为由一组标志共同表达。\code{VM\_READ/WRITE/EXEC/SHARED/MAYWRITE/LOCKED/HUGEPAGE}
等状态共同决定缺页处理、权限修改、锁页、回收和写回行为，不能只根据映射时传入的一个
保护参数推断后续结果。
...
XArray 替代较早的 radix tree，支持稀疏索引、并发查找和特殊 entry。理解 page cache 时
不必先掌握 XArray 的全部内部细节，但必须知道它的索引来自文件偏移，而不是进程虚拟地址。
...
模式 & 优点 & 风险 \\
\midrule
buffered I/O & page cache、预读、合并、易用 & 双重缓存、fsync 边界易误解 \\\tablerowrule
\code{O\_DIRECT} & 减少缓存污染，数据库可控 & 对齐、短 I/O、混用一致性复杂 \\\tablerowrule
...
接口 & 语义重点 \\
\midrule
\code{msync} & 映射页写回，可按地址范围控制 \\\tablerowrule
\code{fsync} & 文件数据和必要元数据持久化 \\\tablerowrule
\code{fdatasync} & 文件数据和读取必需元数据，不一定同步所有时间戳 \\\tablerowrule
...
动作 & 它推进了哪层状态 & 仍未保证什么 \\
\midrule
写 mmap 内存 & 改了 page cache 或私有匿名页 & 未必到块设备 \\\tablerowrule
\code{msync(MS\_SYNC)} 数据页 & 等待指定映射脏页写回 & 未必同步目录项或其他文件 \\\tablerowrule
\code{fsync(data\_fd)} & 推进 data 文件数据和必要元数据 & 不代表 manifest 已提交 \\\tablerowrule
\code{rename(tmp, manifest)} & 原子切换目录项视图 & 目录项持久化仍要看目录 fsync \\\tablerowrule

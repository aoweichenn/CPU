...
攻击动作 & 若没有边界 & 哪层机制应当挡住 \\
\midrule
读取 \code{/etc/shadow} & 泄露宿主口令哈希 & DAC、capability、mount namespace、只读根文件系统 \\
打开容器引擎 socket & 创建特权容器控制宿主 & mount namespace 不暴露 socket，文件权限和 LSM 拒绝 \\
调用 \code{ptrace} 注入同机服务 & 横向控制其他进程 & uid 边界、Yama/LSM、缺少 \code{CAP\_SYS\_PTRACE}、seccomp \\
无限 \code{fork} & 占满 pid 和调度资源 & pids cgroup、rlimit \\

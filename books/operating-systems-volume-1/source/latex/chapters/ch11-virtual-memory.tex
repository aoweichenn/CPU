...
输入 & 计算 \\
\midrule
page size & 4096 \\\tablerowrule
virtual address & \code{0x401234} \\\tablerowrule
virtual page number & \code{0x401} \\\tablerowrule
offset & \code{0x234} \\\tablerowrule
physical page number & 查页表得到 \\\tablerowrule
...
\midrule
load & yes & yes & no & no & 成功 \\\tablerowrule
store & yes & yes & no & no & 权限错误 \\\tablerowrule
fetch & yes & yes & no & no & NX 错误 \\\tablerowrule
fetch & yes & yes & no & yes & 成功 \\\tablerowrule
load & no & - & - & - & 缺页或非法 \\
\bottomrule
\end{longtable}
...
时刻 & 父进程 PTE & 子进程 PTE \\
\midrule
fork 前 & \code{0x7000 -> P42, writable} & 不存在 \\\tablerowrule
fork 后 & \code{0x7000 -> P42, read-only, COW} & \code{0x7000 -> P42, read-only, COW} \\\tablerowrule
子进程写入前 & \code{P42.refcount = 2} & store 触发写保护 fault \\\tablerowrule
子进程 fault 后 & 仍指向 \code{P42, read-only, COW} & 指向新页 \code{P99, writable} \\\tablerowrule
...
阶段 & 必须完成的事 & 如果提前进入下一步 \\
\midrule
清 PTE & 后续 page walk 不能再得到旧映射 & 新 TLB 项仍可能被硬件装入 \\\tablerowrule
本地失效 & 当前 CPU 不再使用旧翻译 & 当前线程可能继续访问已撤销页 \\\tablerowrule
远端 IPI & 其他 active CPU 执行 \code{invlpg} 或等价操作 & 远端 CPU 可能写入已释放物理页 \\\tablerowrule
等待 ack & 确认旧翻译不可再被使用 & 释放页会造成 use-after-free 到物理内存 \\\tablerowrule
...
阶段 & 内核状态 & 应用看到什么 \\
\midrule
\code{malloc} 成功 & VMA/堆区扩展，物理页可能未分配 & 返回非空指针 \\\tablerowrule
首次写前几页 & 匿名页缺页，分配物理页并清零 & store 正常完成，只是可能变慢 \\\tablerowrule
内存压力上升 & 回收 page cache、写回脏页、触发 swap & 延迟抖动，RSS 上升 \\\tablerowrule
无法回收 & OOM 在进程或 cgroup 内选择受害者 & 某进程被杀，或 fault 返回失败语义 \\\tablerowrule
...
场景 & 内核主要动作 & 关键风险 \\
\midrule
匿名首次写 & 分配物理页、清零、安装 writable PTE & overcommit 后在首次触碰时 OOM \\\tablerowrule
COW 写 fault & 检查引用、复制页面、替换 PTE、flush TLB & 父子进程互相污染或复制后权限错误 \\\tablerowrule
文件映射 fault & 查 page cache、必要时发起读 I/O、安装映射 & 把 I/O 等待误判成 CPU 慢 \\\tablerowrule
\code{mprotect} & VMA 切分、改权限、更新页表、失效 TLB & 旧 TLB 保留过宽权限 \\\tablerowrule
...
分配器 & 适合的对象 & 验收问题 \\
\midrule
页分配器 & 页表页、大块缓冲、匿名页和 page cache 页 & 压力下是否能回收、写回或明确失败 \\\tablerowrule
SLUB/slab & 大量同类型小对象 & 析构、引用计数、越界和 use-after-free 是否可检测 \\\tablerowrule
\code{vmalloc} & 大型内核虚拟连续缓冲 & 不能交给要求物理连续 DMA 的设备 \\\tablerowrule
...
标志 & 语义 \\
\midrule
present & PTE 可用于地址翻译，否则触发缺页 \\\tablerowrule
writable & store 是否允许 \\\tablerowrule
user & ring 3 是否可访问 \\\tablerowrule
accessed & CPU 访问后置位，用于回收近似 LRU \\\tablerowrule
dirty & CPU 写入后置位，用于写回和 COW 判断 \\\tablerowrule
NX & 禁止取指执行 \\
\bottomrule
\end{longtable}

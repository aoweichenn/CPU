...
对象 & 含义 & 关键字段 \\
\midrule
bio & 上层的一段块 I/O 意图 & sector、len、page list、op、flags、endio \\\tablerowrule
request & 设备队列中的可派发请求 & 合并后的 bio 链、方向、优先级、deadline \\\tablerowrule
request queue & 块设备的软件队列 & pending、inflight、depth、scheduler \\\tablerowrule
hardware queue & 多队列设备的硬件提交队列 & doorbell、completion、CPU 亲和 \\\tablerowrule
...
分析多队列块层时，应沿着 request 的完整生命周期观察：取得空闲 tag，进入 in-flight，
由设备完成，再释放 tag。任何错误或超时路径都必须保证同一 tag 不会同时代表两个请求。
...
指标 & 解释注意点 \\
\midrule
read/write IOPS & 请求次数，受合并影响，不等于应用调用次数 \\\tablerowrule
sectors & 数据量，可和吞吐换算 \\\tablerowrule
await/latency & 队列等待加服务时间，受队列深度影响 \\\tablerowrule
...
参数 & 含义 & 影响 \\
\midrule
\code{bs} & 每次 I/O 大小 & 合并、吞吐和延迟 \\\tablerowrule
\code{iodepth} & in-flight 请求数量 & 设备并行度和排队 \\\tablerowrule
\code{direct} & 是否绕过 page cache & 缓存干扰和对齐要求 \\\tablerowrule
\code{fsync} & 是否等待持久化 & flush 延迟和事务成本 \\\tablerowrule
...
\section{fd、file、dentry 与 inode}
...
标志 & 作用范围 & 边界 \\
\midrule
\code{O\_CLOEXEC} & fd 表项 & exec 提交时关闭该 fd，不影响其他 fd 的标志 \\\tablerowrule
\code{O\_NONBLOCK} & open file description & dup 后共享，影响 read、write、accept、connect 的等待行为 \\\tablerowrule
\code{O\_APPEND} & open file description & 每次写前在内核内原子移动到文件尾 \\\tablerowrule
\code{O\_DIRECT} & file I/O 路径 & 对齐、页 pin、缓存一致性和短 I/O 更复杂 \\\tablerowrule
...
\midrule
查第 0 页 & miss，分配 cache 页并标记 locked & 不能直接返回，需要等待 I/O \\\tablerowrule
提交 I/O & 块层生成 request，设备 DMA 填页 & 线程 sleeping，不占 CPU \\\tablerowrule
completion & 页标记 uptodate，清 locked，唤醒等待者 & 线程变 runnable，不一定立即运行 \\\tablerowrule
复制用户 buffer & 从 page cache 页复制对应片段 & 可能再次遇到用户页 fault \\\tablerowrule
...
路径 & 优点 & 代价 \\
\midrule
buffered read & 命中 page cache 很快，可共享缓存 & 多一次复制，缓存污染可能影响其他任务 \\\tablerowrule
buffered write & 快速标脏，后台批量写回 & write 返回不代表持久化 \\\tablerowrule
direct read/write & 减少 page cache 干扰，适合自管缓存 & 对齐、pin 页、短 I/O 和并发一致性复杂 \\\tablerowrule
...
\midrule
请求未派发 & cancel CQE 表示取消成功，原请求不会再完成 & 可以释放 buffer，但仍要消费 cancel 结果 \\\tablerowrule
请求已派发 & cancel 可能失败或标记取消中，原请求仍会完成 & buffer 继续保持有效，直到看到原请求 CQE \\\tablerowrule
请求已完成 & cancel 返回未找到或已完成 & 根据原 CQE 的结果处理，不重复释放 \\\tablerowrule
...
第二项分析是写出短写循环。任何 \code{write} 都可能只写入一部分字节，尤其是 pipe、socket、
非阻塞 fd 或被信号打断时。正确代码必须记录已完成字节数，继续写剩余部分，遇到
\code{EINTR} 重试，遇到 \code{EAGAIN} 进入等待，不能把短写当成完整成功。

第三项分析用于区分 readiness 和 message。\code{epoll} 告诉进程 fd 可能可读或可写，
并不表示协议帧已经完整。网络程序仍要维护用户态 receive buffer、解析状态机和背压策略；
把 fd readiness 当成完整请求，会破坏事件驱动程序的消息边界。
...
对象 & 保存什么 & 关键不变量 \\
\midrule
dentry & 名字到 inode 的缓存关系 & rename/unlink 后缓存不能让旧名字错误复活 \\\tablerowrule
inode & 文件身份、权限、大小、块映射、时间戳 & 多个路径和 fd 可引用同一 inode \\\tablerowrule
file & 一次打开实例、offset、status flags、ops & \code{dup} 共享，独立 \code{open} 分离 \\\tablerowrule
address\_space & 文件页缓存和写回状态 & read/write/mmap 必须看到同一份 page cache 账本 \\\tablerowrule

...
\begin{enumerate}
\tightlist
\item \code{mov rbx, 0x2000} 把地址常量放进寄存器 \code{rbx}。
\item \code{mov rax, qword ptr [rbx]} 把 \code{rbx} 当地址，从内存读 8 字节到 \code{rax}。
\item \code{add rax, 1} 使用 ALU，让 \code{rax} 加 1。
\item \code{mov qword ptr [rbx], rax} 把 \code{rax} 写回 \code{rbx} 指向的内存。
\end{enumerate}

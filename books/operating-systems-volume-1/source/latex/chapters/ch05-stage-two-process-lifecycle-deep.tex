...
\midrule
地址空间 & 线程可共享，exec 可替换 & 引用计数、COW、TLB shootdown \\\tablerowrule
文件表 & fork 可共享，dup 共享 open file & close-on-exec、并发 close/read \\\tablerowrule
信号处理 & 进程级和线程级状态交织 & handler、mask、pending queue \\\tablerowrule
凭据 & 安全检查频繁读取，变更较少 & RCU 发布、setuid/exec 转换 \\\tablerowrule
命名空间 & 容器内多个进程共享视图 & 引用计数、权限边界 \\\tablerowrule
...
\midrule
分配 task & task 内存和内核栈 & 释放栈和 task 对象 \\\tablerowrule
复制 files & fd 表引用增加 & put 每个 open file 引用 \\\tablerowrule
复制 mm & 页表页、PTE 引用、COW 标记 & 释放页表页、撤销引用 \\\tablerowrule
分配 pid & pid 映射占用 & 释放 pid，不能留下僵尸映射 \\\tablerowrule
链接父子 & 父进程 child list 可见 & 从列表摘除，避免 wait 看到半成品 \\\tablerowrule
...
\midrule
地址空间 & 初始 COW，之后可分离 & 共享同一 \code{mm} \\\tablerowrule
fd 表 & 可复制或共享，取决于语义 & 通常共享，close 影响其他线程 \\\tablerowrule
用户栈 & 子进程有复制视图 & 每线程独立栈 \\\tablerowrule
信号 mask & 每线程独立 mask & 进程 pending 与线程 pending 并存 \\\tablerowrule
...
测试 & 关键断言 & 风险 \\
\midrule
fork 基本路径 & 父返回 child pid，子返回 0 & 寄存器帧设置错误 \\\tablerowrule
fork 失败注入 & 每个阶段失败后引用数归零 & 资源泄漏或半成品可见 \\\tablerowrule
fork COW & 父子写入互不污染 & PTE/TLB 权限错误 \\\tablerowrule
exec 成功 & PID 保留，地址空间替换 & fd/signal/cred 规则错误 \\\tablerowrule
exec 失败 & commit 前失败旧程序继续 & 旧 mm 被提前破坏 \\\tablerowrule
exit/wait & zombie 被 wait 回收一次 & 重复回收或泄漏 \\\tablerowrule
多线程 close/read & 共享 fd 表语义一致 & use-after-close 或引用泄漏 \\\tablerowrule

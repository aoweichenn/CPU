...
事件 & 必要证据 & 避免 \\
\midrule
权限拒绝 & uid、capability、对象类型、操作 & 打印敏感文件内容 \\\tablerowrule
OOM & cgroup、进程、内存统计、victim & 只写 killed without reason \\\tablerowrule
seccomp 拒绝 & syscall number、action、pid & 静默杀进程无诊断 \\\tablerowrule
...
边界 & 测试 & 期望 \\
\midrule
页表隔离 & 用户态读内核地址 & page fault 或进程终止 \\\tablerowrule
文件权限 & 无权限 open/write & 返回拒绝且目标不变 \\\tablerowrule
capability & drop 后执行特权操作 & 返回 \code{EPERM} \\\tablerowrule
namespace & 访问不可见路径或进程 & 不可见或返回不存在 \\\tablerowrule
cgroup & 超过内存或进程限制 & OOM/失败限制在 cgroup 内 \\\tablerowrule
seccomp & 调用禁止 syscall & 按策略返回 errno 或杀进程 \\\tablerowrule
...
能力类型 & 能做什么 & 风险 \\
\midrule
\code{CAP\_NET\_BIND\_SERVICE} & 绑定低端口 & 可在启动后丢弃 \\\tablerowrule
\code{CAP\_SYS\_ADMIN} & 大量系统管理操作 & 过大，接近半个 root \\\tablerowrule
\code{CAP\_SYS\_PTRACE} & 调试或观察其他进程 & 可读敏感内存 \\\tablerowrule
\code{CAP\_DAC\_OVERRIDE} & 绕过文件权限检查 & 可读写本不该访问的文件 \\\tablerowrule
...
namespace & 隔离内容 & 仍然共享 \\
\midrule
mount & 文件系统挂载视图 & 同一内核 VFS 和底层设备风险 \\\tablerowrule
pid & 进程编号和父子视图 & 调度器、内核内存、系统调用实现 \\\tablerowrule
net & 网卡、路由、端口空间 & 协议栈代码和内核漏洞面 \\\tablerowrule
user & uid/gid 映射和部分能力 & 内核仍必须正确限制能力转换 \\\tablerowrule
...
资源 & 没有限制的风险 & 期望证据 \\
\midrule
CPU & 忙等拖慢其他服务 & throttled time、quota hit 计数 \\\tablerowrule
memory & OOM 扩散到宿主 & cgroup OOM 事件、victim、统计 \\\tablerowrule
pids & fork bomb 填满进程表 & pids.current/max、fork 失败 \\\tablerowrule
I/O & 大量写回拖慢共享磁盘 & io latency、队列深度、权重 \\\tablerowrule
...
框架 & 适用对象 & 指标例子 \\
\midrule
USE & CPU、内存、磁盘、网卡、锁、队列 & CPU busy、runqueue、EIO、drop、reclaim \\\tablerowrule
...
手段 & 适合回答 & 局限 \\
\midrule
计数器 & 是否发生、发生多少次 & 难以解释单个请求 \\\tablerowrule
直方图 & 延迟分布和尾部 & 需要合理 bucket \\\tablerowrule
事件 trace & 谁在何时做了什么 & 数据量大，需过滤 \\\tablerowrule
CPU sampling & on-CPU 热点在哪里 & 看不到 sleeping 时间 \\\tablerowrule
...
错误类型 & 处理 & 证据 \\
\midrule
用户错误 & 返回 errno 或发信号 & syscall、pid、地址、errno \\\tablerowrule
设备错误 & 重试、reset、上报 EIO & 设备状态、请求 id、超时 \\\tablerowrule
资源不足 & ENOMEM/OOM/限流 & cgroup、分配大小、victim \\\tablerowrule

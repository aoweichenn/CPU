...
领域 & 中间状态 & 恢复规则 \\
\midrule
网络请求 & received、processing、committed、replied & committed 后重复请求返回旧结果 \\\tablerowrule
文件输出 & temp、synced、renamed、manifest & manifest 后才被读取者信任 \\\tablerowrule
...
领域 & 错误 & 处理 \\
\midrule
socket & \code{EAGAIN} & 等可写/可读事件后继续 \\\tablerowrule
socket & connection reset & 丢弃连接状态，决定请求是否可重试 \\\tablerowrule
socket & short write & 保存剩余 buffer，下次继续发送 \\\tablerowrule
文件 & short write & 循环写或上报部分完成 \\\tablerowrule
文件 & \code{fsync} failure & 业务提交失败，不能发布 manifest \\\tablerowrule
...
协议 & 提供 & 不提供 \\
\midrule
UDP & 数据报边界、端口复用、校验和 & 重传、排序、拥塞控制、连接状态 \\\tablerowrule
TCP & 可靠有序字节流、流量控制、拥塞控制 & 应用消息边界、请求幂等、业务提交 \\\tablerowrule
...
事件 & socket 读写表现 & 应用含义 \\
\midrule
peer FIN & \code{recv} 返回 0 & 对方正常关闭发送方向 \\\tablerowrule
local shutdown write & 本端发送 FIN & 告诉对方本端不再发送 \\\tablerowrule
RST & \code{ECONNRESET} 等错误 & 连接状态丢弃，业务是否完成未知 \\\tablerowrule
...
队列 & 堵塞表现 & 对策 \\
\midrule
SYN 队列 & 新连接握手失败或重传 & SYN cookie、限流、扩容 \\\tablerowrule
accept 队列 & 连接已建立但应用未 accept & 更快 accept、多进程、backlog 调整 \\\tablerowrule
worker 队列 & 已接收请求等待处理 & 背压、负载均衡、降级 \\\tablerowrule
send buffer & 响应写不出去 & 非阻塞写、限速慢客户端 \\\tablerowrule
...
日志类型 & 写入顺序 & 恢复规则 \\
\midrule
redo & 日志 intent/data 先持久，commit 后主数据可稍后写 & 有 commit 则 redo，无 commit 忽略 \\\tablerowrule
undo & 旧值日志先持久，再覆盖主数据 & 无 commit 则 undo，有 commit 保留 \\\tablerowrule
...
状态 & 持有资源 & 失败处理 \\
\midrule
RECEIVING & socket buffer、解析缓冲区 & 连接断开则丢弃未完整请求 \\\tablerowrule
VALIDATED & 请求对象、权限结果 & 拒绝时不改变持久状态 \\\tablerowrule
EXECUTING & 内存中业务变更或临时文件 & 失败则回滚临时资源 \\\tablerowrule
LOGGED & WAL/manifest 候选记录 & 未 commit 崩溃后忽略或 undo \\\tablerowrule
COMMITTED & 持久 commit record & 重试返回同一结果 \\\tablerowrule
...
测试 & 注入故障 & 期望 \\
\midrule
TCP 分片 & 每次只发送 1 字节 & parser 仍能组帧 \\\tablerowrule
短写 & send 只接受部分响应 & 状态机保存剩余字节 \\\tablerowrule
连接 reset & commit 前/后分别 reset & 前者不提交，后者可幂等查询 \\\tablerowrule
重复 request id & 响应丢失后重试 & 返回同一 committed 结果 \\\tablerowrule
fsync 失败 & 日志提交阶段返回 EIO & 请求失败，不能发布 commit \\\tablerowrule
崩溃恢复 & commit 前/后断电 & 前者忽略，后者 redo/可查询 \\\tablerowrule

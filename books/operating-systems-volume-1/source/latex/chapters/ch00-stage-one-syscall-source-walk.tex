...
源码路径 & 先看什么 & 读法 \\
\midrule
\filepath{arch/x86/entry/entry\_64.S} & \code{entry\_SYSCALL\_64}、返回用户态路径 & 看寄存器怎样保存、栈怎样切换、何时走慢路径 \\\tablerowrule
\filepath{arch/x86/entry/common.c} & \code{do\_syscall\_64}、exit work & 看系统调用号、seccomp、tracing、信号、调度检查 \\\tablerowrule
\filepath{arch/x86/entry/syscalls/syscall\_64.tbl} & 系统调用号表 & 看 ABI 如何把数字绑定到实现名 \\\tablerowrule
\filepath{include/linux/syscalls.h} & \code{SYSCALL\_DEFINE*} 声明 & 看 C 实现怎样接入统一表项 \\\tablerowrule
\filepath{arch/x86/include/asm/ptrace.h} & \code{struct pt\_regs} & 看 trap frame 在 C 层怎样表示 \\\tablerowrule
...
\midrule
\code{rax} & 系统调用号，返回时放返回值 & 调度到哪个 \code{sys\_*} 实现 \\\tablerowrule
\code{rdi} & 第 1 参数 & 例如 \code{write(fd,...)} 的 fd \\\tablerowrule
\code{rsi} & 第 2 参数 & 例如用户 buffer 指针 \\\tablerowrule
\code{rdx} & 第 3 参数 & 例如长度 \\\tablerowrule
\code{r10} & 第 4 参数 & \code{rcx} 被硬件占用，不能放参数 \\\tablerowrule
\code{r8/r9} & 第 5/6 参数 & 六参数系统调用使用 \\\tablerowrule
\code{rcx} & 用户返回 RIP & \code{sysretq} 快速返回需要 \\\tablerowrule
...
\midrule
通用寄存器 & 用户态执行 \code{syscall} 前的寄存器 & 提取参数、保存用户可见状态 \\\tablerowrule
\code{orig\_ax} & 原始系统调用号 & tracing/seccomp/restart syscall 需要原号 \\\tablerowrule
\code{ip} & 用户返回 RIP & 信号、ptrace、返回路径验证 \\\tablerowrule
\code{sp} & 用户 RSP & 信号帧构造和调试输出 \\\tablerowrule
\code{flags} & 用户 RFLAGS & 单步、方向标志、中断相关状态 \\\tablerowrule
...
返回条件 & 快路径倾向 & 慢路径原因 \\
\midrule
普通系统调用，无 pending work & 可快速恢复寄存器并返回 & 无 \\\tablerowrule
有信号待投递 & 不适合直接快返 & 需要构造用户信号帧 \\\tablerowrule
需要调度 & 不适合直接快返 & 返回前要切换任务 \\\tablerowrule
ptrace、seccomp、audit & 不适合直接快返 & 需要通知观察者或改写结果 \\\tablerowrule
...
命令或文件 & 能观察什么 & 如何判读 \\
\midrule
\code{strace -e trace=write ./a.out} & 用户态发起的系统调用和 errno & 只能看到边界结果，看不到内核内部等待 \\\tablerowrule
\code{perf trace -e syscalls:sys\_enter\_*} & syscall enter/exit tracepoint & 能看到系统调用号、参数和返回时序 \\\tablerowrule
\code{grep syscall /proc/kallsyms} & 内核符号是否可见 & 受权限和 kallsyms 设置影响 \\\tablerowrule
\code{cat /proc/<pid>/syscall} & 线程当前 syscall 和参数 & 只在任务正在系统调用中时有意义 \\\tablerowrule

...
\begin{pdfdiagram}[x=1cm,y=1cm]
  \node[os control, minimum width=2.2cm] (isa) at (0,1.2) {ISA/ABI\\指令、寄存器、调用约定};
  \node[os state, minimum width=2.2cm] (core) at (3.0,1.2) {CPU core\\流水线、ROB、执行单元};
  \node[os memory, minimum width=2.2cm] (cache) at (6.0,1.2) {cache/TLB\\line、tag、PTE 副本};
  \node[os memory, minimum width=2.2cm] (dram) at (9.0,1.2) {DRAM/NUMA\\channel、bank、page};
  \node[os control, minimum width=2.2cm] (os) at (0,-0.65) {OS kernel\\调度、页表、驱动};
  \node[os block, minimum width=2.2cm] (bus) at (3.0,-0.65) {互联/总线\\PCIe、APIC、CXL};
  \node[os block, minimum width=2.2cm] (dev) at (6.0,-0.65) {设备\\NVMe、NIC、GPU};
  \node[os memory, minimum width=2.2cm] (iommu) at (9.0,-0.65) {IOMMU/DMA\\IOVA、domain、ring};

  \draw[os strong bus] (isa) -- node[os label, above] {提交状态} (core);
  \draw[os strong bus] (core) -- node[os label, above] {load/store} (cache);
  \draw[os strong bus] (cache) -- node[os label, above] {miss/fill} (dram);
  \draw[os strong bus] (os) -- node[os label, above] {配置寄存器} (core);
  \draw[os strong bus] (os) -- node[os label, above] {MMIO/IRQ} (bus);
  \draw[os strong bus] (bus) -- node[os label, above] {事务} (dev);
  \draw[os strong bus] (dev) -- node[os label, above] {DMA} (iommu);
  \draw[os strong bus] (iommu) -- node[os label, right] {内存访问} (dram);
  \draw[os feedback] (cache) -- node[os label, right] {\#PF/TLB miss} (os);
  \draw[os feedback] (dev) -- node[os label, right] {MSI-X/completion} (os);
\end{pdfdiagram}

...
\begin{pdfdiagram}[x=0.94cm,y=1cm]
  \node[os title, anchor=east] at (-0.35,1.75) {CPU 路径};
  \node[os title, anchor=east] at (-0.35,0) {内核契约};
  \node[os title, anchor=east] at (-0.35,-1.75) {设备路径};

  \node[os control, minimum width=1.9cm] (isa) at (0.75,1.75) {ISA/ABI\\指令/寄存器};
  \node[os state, minimum width=1.9cm] (core) at (3.0,1.75) {CPU core\\流水线/ROB};
  \node[os memory, minimum width=1.9cm] (cache) at (5.25,1.75) {cache/TLB\\line/tag/PTE};
  \node[os memory, minimum width=1.9cm] (dram) at (7.5,1.75) {DRAM/NUMA\\channel/bank};

  \node[os control, minimum width=2.0cm] (os) at (0.75,0) {OS kernel\\调度/页表/驱动};
  \node[os memory, minimum width=2.0cm] (pt) at (3.0,0) {页表/属性\\PTE/权限};
  \node[os block, minimum width=2.0cm] (bus) at (5.25,0) {互联/总线\\PCIe/APIC/CXL};
  \node[os state, minimum width=2.0cm] (event) at (7.5,0) {事件证据\\IRQ/fault/log};

  \node[os block, minimum width=1.9cm] (dev) at (0.75,-1.75) {设备\\NVMe/NIC/GPU};
  \node[os block, minimum width=1.9cm] (doorbell) at (3.0,-1.75) {doorbell\\MMIO/queue};
  \node[os memory, minimum width=1.9cm] (iommu) at (5.25,-1.75) {IOMMU/DMA\\IOVA/domain};
  \node[os state, minimum width=1.9cm] (complete) at (7.5,-1.75) {completion\\状态/中断};
  ...
  \draw[os feedback] (event.south west) .. controls +(0,-0.9) and +(1.4,-0.9) .. (os.south east);

  \node[os label, anchor=west, text width=2.65cm] at (8.7,1.05) {TLB 填充/失效\\\#PF/TLB miss};
  \node[os label, anchor=west, text width=2.65cm] at (8.7,-0.05) {事务/DMA 访问\\进入内核状态机};
  \node[os label, anchor=west, text width=2.65cm] at (8.7,-1.15) {MSI-X\\completion};
\end{pdfdiagram}

...
的 C++ 或汇编源文件，约 \(14968\) 行非空、非纯注释目标代码。代码量不是完成
...
在这条已发布基线之上，项目先完成 v1.1 的两项资源增量。ADR 0017 接受
...
对象和当前占用都回到零。ADR 0021 又统一了变量、函数、命名空间和自研
C++/NASM 边界符号的命名，并用 Clang AST 与词法检查共同建立回归门禁；当前
完整回归通过 \(77/77\) 项。

在这些已接受增量之上，ADR 0022 又加入按 order 分裂、合并和精确释放的
buddy 页帧分配器。它为 Free 与 Allocated 块分别建立逐阶位图，在全部启动保留
完成后一次性初始化，并以 order 3 高端连续块完成首尾页写读、释放和全量一致性
校验。新增的单元、生命周期与十万步固定种子随机测试把完整回归推进到
\(80/80\) 项。

ADR 0023 随后在通用堆上加入固定尺寸类型缓存：一个后备块同时保存活动位图、
对齐槽与槽内 LIFO 空闲索引链，取得和归还保持常数时间，耗尽、内部指针、
重复释放与活动对象销毁都会明确失败。目标自检完整耗尽 32 个 64 字节对齐槽，
再完成模式写回、交错归还和复用；单元、三缓存生命周期与十万步固定种子随机
测试把完整回归推进到 \(83/83\) 项。

ADR 0024 又把 \code{0xFFFFC90000000000..0xFFFFE90000000000} 的 32 TiB
高半区交给独立 KVA 所有权分配器。256 个有序描述符只记录 allocation 与
reservation，空闲缝隙由相邻区间隐式推导；目标自检取得六页、保持首尾 guard
未映射，把中间四页接到 buddy order 2 物理块并真实写读，最后按页表、物理块、
虚拟区间的逆序归还。单元、跨页表生命周期与十万步逐页参考模型把完整回归推进
到 \(86/86\) 项。

README、roadmap、架构、需求、测试和调试文档已经与 ADR 0024 一致接受上述五项
资源增量，并把完整回归更新为 86 项。CMake 工程元数据仍为 v0.11，动态内核栈
和空中间页表回收等 v1.1 余下工作尚未闭合。因此本书把 KVA 标作
\textbf{已接受的 v1.1 增量}，把整个版本标作\textbf{v1.1 正在实现}，不提前
标作发布能力。ADR 0019 把第二周期冻结为五个依赖
...
这几个数并不属于同一种决策。\code{0xFFFFFFF0} 是 x86 复位契约，项目只能把
有效字节映射到那里，不能另选一个“更整齐”的入口；这项历史兼容契约使既有平台
固件与处理器仍能互相启动。ROM 基址则由镜像容量和顶端映射共同算出：
\(\code{0x100000000}-\code{0x20000}=\code{0xFFFE0000}\)。因此
\code{0x1FFF0} 只是同一字节在 128 KiB ROM 文件中的偏移，CPU 从未拿这个文件
偏移取指。

复位向量到 4 GiB 边界只剩 16 字节，放不下串口和磁盘初始化。项目把
\code{0xFFFFF000} 选成页对齐的 ROM 入口，并用三字节近跳抵达它；近跳保留复位
时特殊的 CS 隐藏基址，只改 IP，因而不必先建立一套新的段状态。换成别的 ROM
内地址并非不可能，但链接脚本、跳转编码和可用代码空间必须同时重新证明；把完整
初始化放进 16 字节复位窗口则无法成立。
...
\code{0x0500} 的描述符缓冲位于实模式可直接访问的低端 RAM，并避开
\code{0x0000..0x04FF} 的传统中断向量表与平台数据区；\code{0x7000} 作为向下
增长的固件栈顶，给随后从 \code{0x8000} 开始的 Stage 1 留出一个完整 4 KiB
间隔。Stage 1 的 \code{0x8000} 也是项目布局而非处理器要求：它可写成整齐的
\code{0x0800:0x0000}，仍在常规低端 RAM 内，并允许最多 64 个 512 字节扇区
恰好占用半开区间 \([\code{0x8000},\code{0x10000})\)。若把加载点抬高却不移动
栈和页表，栈增长或最大负载就可能覆盖下一阶段状态。
...
三张表分别放在 \code{0x10000}、\code{0x11000} 和 \code{0x12000}：它们从
Stage 1 最大负载的开区间末端开始，每张表又恰好占一个 4 KiB 对齐页。32 个
2 MiB 叶项给出 \(32\times2\text{ MiB}=64\text{ MiB}\)，覆盖后续加载区、
暂存区、BootInfo 和内核栈，却不为尚未需要的全部 RAM 建表。这里选择恒等映射，
是为了让打开分页前后的当前指令、栈和指针保持同值；它减少第一次模式切换的变量，
并不表示虚拟地址和物理地址本来就是同一概念。
...
ELF 文件先进入物理区间
\([\code{0x03E00000},\code{0x03F00000})\)。这个 1 MiB 暂存窗口位于 64 MiB
恒等映射之内，又紧接在 PT\_LOAD 目标上界
\code{0x03E00000} 之后，因此源文件与目标段不会互相覆盖。Stage 1 仍要用 E820
证明整段确实是可用 RAM；“数值低于 64 MiB”不能替代平台所有权检查。内核入口
\code{0x00100000} 则避开低 1 MiB 启动工作区，并满足 4 KiB 段对齐。当前
早期页表采用恒等映射，所以项目暂时要求 \code{p\_vaddr==p\_paddr}；这是降低
首次装载复杂度的阶段策略，不是 ELF 的普遍规则。
...
写保护故障镜像让 Ring 0 写入
\code{0xFFFF800000100000} 的 present、read-only 高半区映射。这个地址位于
64 KiB 早期堆基址之后 1 MiB，保持页对齐却不会落入堆对象；因此可以在同一
高半区页表子树中安装一张独立的只读测试叶页。预期页故障错误码为 \code{0x3}，
CR2 为同一精确测试地址。这个失败同时证明页存在、
...
\code{0x40000000} 是 1 GiB 边界，正好从 PML4[0] 下的 PDPT[1] 开始。低端
PDPT[0] 继续保存 supervisor 启动映射，用户程序则使用可由每个进程独立重建的
下一棵子树；这个选择使“共享内核、隔离同址用户程序”能落到明确页表所有权。
用户栈另放在 PML4[255] 顶端附近，避免与向高地址增长的程序窗口相撞。栈顶
\code{0x00007FFFFFFF0000} 距低半 canonical 上界仍有 64 KiB，并保持页对齐；
它是半开区间边界，不是一张已映射的叶页。
...
\code{0x30000000} 不是随意挑选的坏指针：它是 canonical、页对齐的低半地址，
位于 \code{0x40000000} 用户程序窗口之前，也不属于高端用户栈。因而访问必然
检验“合法用户指令触发 not-present 页故障并被内核收束”这条路径，而不会先因
非 canonical 地址产生 \#GP，也不会误碰真实 ELF 段。
...
\begin{statebox}
\textbf{状态边界。} v1.1 的动态物理内存、可回收通用内核堆、双位图 buddy、
固定尺寸类型缓存与 KVA 已经分别由 ADR 0017、0020、0022、0023 和 0024
接受，README、roadmap 及专题文档也已收口，当前完整回归为 \(86/86\) 项。
动态内核栈、空中间页表回收和资源快照尚未共同闭合，因此
整个 v1.1 仍处于实现中。
v1.2 至 v1.18 以及 v2.0 都是路线图中的\textbf{规划阶段}。v1.0 的完整回归
仍是正式完成基线；64 GiB 高端页读写、64 KiB 堆生命周期与 buddy
连续块、类型缓存和 KVA 自检只分别证明对应增量，不能提前代表整个第二周期
已经实现。
\end{statebox}
...
\endhead
v1.1（实现中） & buddy、固定尺寸类型缓存与 KVA 已完成实现、文档接受与目标
自检，但 64 KiB 堆和单块缓存仍是固定后备区，四份静态内核栈仍限制对象生命周期。 &
当前已管理全部 E820 可用页、建立高半区物理直映，完成可释放、可合并的
通用内核堆，以两族逐阶位图闭合连续页分裂与合并，并以活动位图和槽内空闲链
闭合固定对象存储，再以有序有主区间区分 KVA reservation、allocation 与
隐式空洞；后续补齐动态 guard 栈、空中间页表回收和资源快照，旧四进程通路
继续承担回归。
\\\tablerowrule

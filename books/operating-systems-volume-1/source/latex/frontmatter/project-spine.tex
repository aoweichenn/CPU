...
在这条已发布基线之上，项目主线已经完成 v1.1 的第一项增量：ADR 0017 接受
64 GiB 容量验证规格、动态页帧状态存储与
\code{0xFFFF888000000000} 高半区物理直映。当前容量路径使用 64 GiB，
新增带 1 GiB 地址洞的页帧集成测试、4 GiB 以上范围分配随机测试，并在目标机
输出物理地址宽度、受管上限、元数据尺寸、2 MiB/4 KiB 直映页数和高端页读写
自检结果；本轮仍通过 \(73/73\) 项回归。虽然第一增量已经进入主线，README 与
roadmap 的最新完成标记仍为 v1.0，CMake 工程元数据仍为 v0.11，且 v1.1
余下工作尚未闭合。因此本书把这些主线修改标作\textbf{v1.1 正在实现}，而不
提前标作整个版本的发布能力。最新接受的 ADR 0019 又把第二周期冻结为五个依赖
...
\begin{statebox}
\textbf{状态边界。} v1.1 的动态物理内存基础已经进入主线，但可回收分配器、
动态内核栈、对象生命周期和资源基线尚未共同闭合，因此整个 v1.1 仍处于实现中。
v1.2 至 v1.18 以及 v2.0 都是路线图中的\textbf{规划阶段}。v1.0 的完整回归
仍是正式完成基线；64 GiB 高端页读写与回收只证明 v1.1 已落地增量，不能提前
代表整个第二周期已经实现。
\end{statebox}
...
\endhead
v1.1（实现中） & 低 64 MiB、64 KiB 单调堆、四份静态内核栈和固定容量限制了对象生命周期。 &
当前已管理全部 E820 可用页并建立高半区物理直映；后续补齐可分裂、合并和回收
的 buddy 页分配、内核堆与对象缓存、KVA、动态 guard 栈、页表回收和资源快照；
旧四进程通路继续承担回归。
\\\tablerowrule

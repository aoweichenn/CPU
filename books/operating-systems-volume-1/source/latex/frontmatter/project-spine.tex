...
案例当前已经完成 v0.0 至 v0.8，共有 \(71\) 个进入目标系统的 C++ 或汇编
源文件，约 \(6653\) 行非空、非纯注释目标代码。代码量不是完成证据；更重要的
事实是，机器已经能够从 \code{0xFFFFFFF0} 取到自研指令，最终在内核中接收
真实 PIT 和键盘中断，用独立的 ATA PIO 驱动重新读取启动盘，并进入
Ring 3 执行自研 ELF64 用户程序。进程调度、同步、文件系统和 Shell 属于
v0.9 以后的完成里程碑，不能把后文的完整
设计误写成当前代码已经实现的功能。

v0.8 已经形成 README、roadmap、ADR 和 release 共同确认的发布边界。
项目建立共享系统调用编号和错误结果，能链接三个位于
\code{0x40000000} 起始区域的用户 ELF，验证用户 PT\_LOAD、映射带
\code{userAccessible} 权限的页面、建立四页用户栈以及检查跨页用户指针。
GDT 中已经出现 Ring 3 代码/数据选择子，IDT 提供 DPL3 的
\code{INT 0x80} 入口；汇编入口开始保存系统调用现场、以 \code{iretq} 进入
用户态，并把退出或用户异常收束为内核可检查的执行结果。项目也新增了正常退出、
用户态非法指令、用户态缺页和截断 ELF 的系统测试入口。

最近一次完整验证通过 \(55/55\) 项测试：用户
ELF 单元测试覆盖二十项格式与事务条件，集成测试核对特权帧、用户栈、地址边界
和 ABI，固定种子随机测试完成 8192 轮范围验证，三份用户 ELF 还分别通过目标
文件布局审计。正常
启动要求用户程序拒绝非法指针和未知系统调用、从 Ring 3 输出日志、以零退出，
内核核对六次系统调用后收束执行结果并继续输出 \code{READY}；三份专用镜像
分别要求用户态非法指令和缺页只终止当前用户执行，以及截断 ELF 在进入
Ring 3 前被拒绝。v0.8 因而不再只是结构已经写入，而是具有构造、采用、失败和
跨层证据的已完成里程碑。
...
内核取得控制权以后，后四个里程碑把临时启动环境逐步替换成内核长期拥有的
控制结构：
...
v0.8 & 内核执行受硬件隔离的用户程序 & 用户 ELF、U/S 页、TSS.RSP0、\code{INT 0x80} 与用户故障收束 & \code{HELLO_FROM_RING3}、退出与隔离测试 \\
\bottomrule
\end{longtable}
...
这个里程碑仍只有一个同步用户执行单元，所有用户映像共用当前内核页表根，结束
后尚未回收用户页，也没有 PID、runqueue 或等待状态。v0.9 必须把这条已验证
的保护边界装进具有独立生命周期的进程对象，再引入定时器抢占和上下文切换。

\section*{实现边界怎样推动后续章节}

v0.8 结束时，机器已经有内核地址空间、异常、硬件 IRQ、页帧、早期堆、单调
tick、同步磁盘读，以及一个能进入、调用、故障并返回内核的 Ring 3 执行单元。
它仍然不能保存多个独立用户对象，也不能在一个执行流阻塞或长期占用 CPU 时切换
到另一个执行流；因此 v0.9 必须建立进程状态与调度边界。
...
表中的 v0.9 至 v1.0 是后续路线，不是已经完成的代码声明。本册会把这些结构
讲到现代系统所需的深度，但每章开头都回到项目当时
拥有的机器：先做出最小可运行结构，再增加规模、并发、延迟和故障条件。这样既
不会用项目当前的简化实现限制原理深度，也不会把理论上的完整机制冒充成项目已经
完成的功能。

\section*{怎样用案例阅读后面的深入章节}

重排后的正文首先跟随已实现的复位、模式转换、ELF 交接、异常、内存和设备
里程碑；随后沿 v0.9 至 v1.0 的开发边界推导进程、同步、文件系统、网络、
...
\node[os state, text width=3.5cm, right=10mm of kernel] (roadmap)
  {开发/路线 v0.9--v1.0\\进程、同步与 IPC\\文件与用户环境};

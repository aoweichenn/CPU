...
README、roadmap、架构、需求、测试和调试文档已经与 ADR 0022 一致接受 buddy
增量，并把完整回归更新为 80 项。CMake 工程元数据仍为 v0.11，type cache、
KVA、动态内核栈和页表回收等 v1.1 余下工作尚未闭合。因此本书把 buddy 标作
...
\begin{statebox}
\textbf{状态边界。} v1.1 的动态物理内存、可回收通用内核堆与双位图 buddy
已经分别由 ADR 0017、0020 和 0022 接受，README、roadmap 及专题文档也已收口，
当前完整回归为 \(80/80\) 项。type cache、KVA、动态内核栈、页表回收和资源快照
尚未共同闭合，因此
整个 v1.1 仍处于实现中。
v1.2 至 v1.18 以及 v2.0 都是路线图中的\textbf{规划阶段}。v1.0 的完整回归
仍是正式完成基线；64 GiB 高端页读写、64 KiB 堆生命周期与 buddy
连续块自检只分别证明对应增量，不能提前代表整个第二周期已经实现。
\end{statebox}
...
\endhead
v1.1（实现中） & buddy 已完成实现、文档接受与目标自检，但 64 KiB 堆仍是
固定后备区，四份静态内核栈和固定容量仍限制对象生命周期。 &
当前已管理全部 E820 可用页、建立高半区物理直映，完成可释放、可合并的
通用内核堆，并以两族逐阶位图闭合连续页分裂与合并；后续补齐 type cache、
KVA、动态 guard 栈、页表回收和资源快照，旧四进程通路继续承担回归。
\\\tablerowrule

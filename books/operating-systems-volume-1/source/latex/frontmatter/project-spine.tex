...
在这条已发布基线之上，项目主线提交 \code{89de15e} 已经完成 v1.1 的第一项
增量：ADR 0017 接受 64 GiB QEMU 主规格、动态页帧状态存储与
\code{0xFFFF888000000000} 高半区物理直映。当前实现把主正常路径改为 64 GiB，
...
\begin{statebox}
\textbf{状态边界。} v1.1 的动态物理内存第一增量已经由主线提交
\code{89de15e} 落地，但整个
v1.1 尚未发布；v1.2 至 v2.0 仍是
项目写入路线图的\textbf{规划阶段}。v1.0 的 73 项测试是正式完成基线，v1.1
当前的 64 GiB 回归只构成实现证据。后续版本只有在代码、失败路径、自动化验证、
版本元数据与发布提交同时闭合后，才会进入上面的“已实现主线”。
\end{statebox}
...
\textbf{阶段状态} & \textbf{要解除的 v1.0 限制} & \textbf{建立或预期建立的契约} \\\tablerowrule
\endhead
v1.1（实现中） & 低 64 MiB、64 KiB 单调堆、四份静态内核栈和固定容量限制了对象生命周期。 &
当前先管理全部 E820 可用页并建立高半区物理直映；后续仍需完成可释放内核分配、
动态 guard 栈、至少 64 个 PCB 和 64 槽描述符，并闭合引用计数与失败回滚。
\\\tablerowrule
...
的提前推导；对 v1.1 使用“正在实现”，对更后阶段使用“规划”，都不写成已经发布。

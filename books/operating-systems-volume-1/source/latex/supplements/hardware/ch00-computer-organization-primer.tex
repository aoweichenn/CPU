...
上一节先规定了“硬件对象怎么读”。这一节按那套读法走一遍最小主线：先看状态如何在电路里保存，再看指令如何控制数据通路，最后看异常、中断、页表和设备怎样把控制权交给 OS。
...
\begin{keyidea}
操作系统不是脱离硬件单独存在的一层规则。它是运行在受保护 CPU 模式下的一段程序，利用中断、异常、页表、定时器、总线、设备寄存器和 DMA，把原始硬件状态组织成进程、地址空间、文件和 socket。
\end{keyidea}
...
访存控制 & 是否访问数据内存或 MMIO & load/store、栈 push/pop \\
...
特权检查 & 当前 CPL 是否允许该动作 & 用户态执行特权指令触发异常 \\
\bottomrule
\end{longtable}
...
global/PCID & TLB 保留和地址空间标记 & 降低上下文切换成本 \\
\bottomrule
\end{longtable}
...
接下来把 CPU 这条链拆细：从布尔逻辑、ALU、寄存器、控制信号、流水线和精确异常一步步走到 OS 能接管的入口。

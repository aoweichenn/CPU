...
\begin{pdfdiagram}[x=0.92cm,y=1cm]
  \node[os title, anchor=east] at (-0.35,1.95) {CPU 提交链};
  \node[os title, anchor=east] at (-0.35,0) {地址/cache 链};
  \node[os title, anchor=east] at (-0.35,-1.95) {设备 DMA 链};

  \node[os control, minimum width=1.3cm] (isa) at (0.8,1.95) {ISA};
  \node[os compute, minimum width=1.5cm] (front) at (2.75,1.95) {fetch\\decode};
  \node[os compute, minimum width=1.5cm] (exec) at (4.7,1.95) {execute};
  \node[os state, minimum width=1.5cm] (retire) at (6.65,1.95) {retire};
  \node[os control, minimum width=1.5cm] (trap) at (8.6,1.95) {trap\\IRQ};
  \node[os control, minimum width=1.5cm] (kernel) at (10.55,1.95) {kernel};

  \node[os bit, minimum width=1.05cm] (va) at (0.8,0) {VA};
  \node[os memory, minimum width=1.35cm] (tlb) at (2.75,0) {TLB};
  \node[os memory, minimum width=1.5cm] (pt) at (4.7,0) {page\\walk};
  \node[os bit, minimum width=1.05cm] (pa) at (6.65,0) {PA};
  \node[os memory, minimum width=1.35cm] (cache) at (8.6,0) {cache};
  \node[os memory, minimum width=1.35cm] (dram) at (10.55,0) {DRAM};

  \node[os control, minimum width=1.4cm] (drv) at (0.8,-1.95) {driver};
  \node[os memory, minimum width=1.4cm] (ring) at (2.75,-1.95) {ring};
  \node[os control, minimum width=1.4cm] (db) at (4.7,-1.95) {doorbell};
  \node[os block, minimum width=1.4cm] (dev) at (6.65,-1.95) {device};
  \node[os memory, minimum width=1.4cm] (iommu) at (8.6,-1.95) {IOMMU};
  \node[os state, minimum width=1.5cm] (comp) at (10.55,-1.95) {completion};
  ...
  \draw[os feedback] (pt.north) -- ++(0,0.62) -| (kernel.south);
  \node[os label] at (3.75,0.55) {miss};
  \node[os label] at (7.55,0.78) {\#PF};
  ...
  \draw[os feedback] (comp.north) -- ++(0,0.62) -- ++(0.95,0) |- (kernel.east);
  \node[os label] at (11.25,0.55) {MSI-X};
\end{pdfdiagram}
...
\begin{pdfdiagram}[x=1cm,y=1cm]
  ...
  \node[os control, minimum width=1.75cm] (apic) at (3.0,-0.95) {local\\APIC};
  \node[os control, minimum width=1.35cm] (cpu) at (5.1,-0.95) {CPU};
  \node[os state, minimum width=1.35cm] (idt) at (6.95,-0.95) {IDT};
  \node[os control, minimum width=1.55cm] (entry) at (8.9,-0.95) {handler};

  \draw[os strong bus] (dev) -- (msix);
  \draw[os strong bus] (msix.south) -- ++(0,-0.42) -| node[os label, pos=0.22, right] {vector} (apic.north west);
  \draw[os bus] (timer.south) -- ++(0,-0.62) -| ($(apic.north west)!0.58!(apic.north east)$);
  \draw[os bus] (ipi.south) -- ++(0,-0.92) -| ($(apic.north west)!0.84!(apic.north east)$);
  \draw[os strong bus] (apic) -- (cpu);
  \draw[os strong bus] (cpu) -- (idt);
  \draw[os strong bus] (idt) -- (entry);
  \draw[os feedback] (entry.south) -- ++(0,-0.62) -| node[os label, pos=0.25, below] {EOI} (apic.south);
\end{pdfdiagram}

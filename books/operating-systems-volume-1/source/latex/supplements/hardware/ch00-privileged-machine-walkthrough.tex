...
前面已经把普通指令、编码、栈和 ABI 讲清楚。现在把边界提高一级：同样是执行指令，为什么用户态不能改页表、不能直接碰设备，为什么系统调用和中断能把 CPU 受控地交给内核。

本节把前面的硬件拼起来，走读一台带 OS 的机器如何处理用户程序。目标不是堆 x86-64 细节，而是建立完整路径：用户指令、异常、中断、系统调用、页表、设备、调度如何互相连接。

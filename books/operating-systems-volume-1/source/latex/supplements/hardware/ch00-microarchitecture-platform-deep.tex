...
上一节已经把内存、总线、MMIO、descriptor、DMA 和 IOMMU 连成 I/O 路径。本节推进到实现粒度。学习操作系统最容易卡住的地方，是知道“有 cache、有中断、有 DMA”，但不知道这些机制内部怎样改变状态，为什么内核要按某种顺序写代码。本节按实现粒度讲：CPU 如何提交指令，cache line 如何在核心之间转移，TLB 如何失效，PCIe/NVMe 如何完成一次事务，IOMMU 如何把设备也放进地址空间。

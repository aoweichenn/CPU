...
\begin{pdfdiagram}[x=1cm,y=1cm]
  \node[os title, anchor=east] at (-0.35,1.15) {设备侧};
  \node[os title, anchor=east] at (-0.35,-1.05) {CPU 侧};

  \node[os state, minimum width=1.55cm] (cause) at (0.9,1.15) {device\\cause};
  \node[os control, minimum width=1.35cm] (mask) at (3.0,1.15) {mask};
  \node[os block, minimum width=1.6cm] (msix) at (5.15,1.15) {MSI-X\\message};

  \node[os control, minimum width=1.65cm] (apic) at (0.9,-1.05) {local APIC\\IRR/ISR};
  \node[os compute, minimum width=1.6cm] (cpu) at (3.0,-1.05) {CPU\\trap entry};
  \node[os control, minimum width=1.55cm] (idt) at (5.15,-1.05) {IDT\\vector};
  \node[os state, minimum width=1.8cm] (handler) at (7.35,-1.05) {handler\\thread/poll};

  \draw[os strong bus] (cause) -- (mask);
  \draw[os strong bus] (mask) -- (msix);
  \draw[os strong bus] (msix.south) -- ++(0,-0.72) -| node[os label, pos=0.25, right] {vector} (apic.north);
  \draw[os strong bus] (apic) -- (cpu);
  \draw[os strong bus] (cpu) -- (idt);
  \draw[os strong bus] (idt) -- (handler);
  \draw[os feedback] (handler.south) -- ++(0,-0.78) -- ++(-7.45,0) |- (cause.west);
  \node[os label] at (3.35,-2.05) {clear cause + EOI};
\end{pdfdiagram}
...
\begin{pdfdiagram}[x=1cm,y=1cm]
  ...
  \draw[os bus] (tk) -- (hr);
  \draw[os strong bus] (hr.south) -- ++(0,-0.52) -| node[os label, pos=0.25, right] {$now+\Delta t$} (lapic.north);
...
\begin{pdfdiagram}[x=1cm,y=1cm]
  \node[os title, anchor=east] at (-0.35,1.15) {硬件接收};
  \node[os title, anchor=east] at (-0.35,-1.05) {内核交付};

  \node[os block, minimum width=1.55cm] (wire) at (0.85,1.15) {wire\\frame};
  \node[os block, minimum width=1.55cm] (mac) at (3.0,1.15) {PHY/MAC};
  \node[os compute, minimum width=1.55cm] (rss) at (5.15,1.15) {RSS\\queue};
  \node[os memory, minimum width=1.7cm] (rx) at (7.3,1.15) {RX buffer\\DMA};

  \node[os state, minimum width=1.55cm] (cq) at (0.85,-1.05) {CQE};
  \node[os control, minimum width=1.55cm] (napi) at (3.0,-1.05) {NAPI\\poll};
  \node[os state, minimum width=1.55cm] (skb) at (5.15,-1.05) {skb};
  \node[os state, minimum width=1.55cm] (sock) at (7.3,-1.05) {socket};

  \draw[os strong bus] (wire) -- (mac);
  \draw[os strong bus] (mac) -- (rss);
  \draw[os strong bus] (rss) -- (rx);
  \draw[os strong bus] (rx.south) -- ++(0,-0.7) -| node[os label, pos=0.25, right] {completion} (cq.north);
...
\begin{pdfdiagram}[x=1cm,y=1cm]
  \node[os title, anchor=east] at (-0.35,1.15) {软件写入};
  \node[os title, anchor=east] at (-0.35,-1.05) {设备落点};

  \node[os state, minimum width=1.55cm] (write) at (0.85,1.15) {write()};
  \node[os memory, minimum width=1.8cm] (pc) at (3.15,1.15) {page cache\\dirty};
  \node[os state, minimum width=1.6cm] (bio) at (5.45,1.15) {bio/request};

  \node[os memory, minimum width=1.8cm] (nvme) at (0.85,-1.05) {NVMe SQ\\DMA};
  \node[os block, minimum width=1.8cm] (ssd) at (3.15,-1.05) {SSD FTL\\cache};
  \node[os block, minimum width=1.8cm] (nand) at (5.45,-1.05) {NAND\\stable};
  \node[os control, minimum width=1.65cm] (fsync) at (7.75,-1.05) {fsync\\flush/FUA};

  \draw[os strong bus] (write) -- (pc);
  \draw[os strong bus] (pc) -- (bio);
  \draw[os strong bus] (bio.south) -- ++(0,-0.68) -| node[os label, pos=0.25, right] {提交请求} (nvme.north);
  \draw[os strong bus] (nvme) -- (ssd);
  \draw[os bus] (ssd) -- (nand);
  \draw[os feedback] (fsync.south) -- ++(0,-0.42) -| (ssd.south);
  \node[os label] at (3.4,-2.05) {$write\ return \ne media\ persistent$};
\end{pdfdiagram}
...
硬件深讲到这里结束。下一章把这些事实收束成内核对象：task 保存 CPU 现场，地址空间管理页表和 fault，driver request 绑定设备队列和 completion，buffer ownership 约束 DMA 生命周期。换句话说，下一章回答的是：硬件只给低层状态，OS 为什么必须把它们包装成可检查、可等待、可恢复的对象。

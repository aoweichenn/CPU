...
上一节给出了组成原理主线，本节把最容易被教材压缩的部分展开：为什么要有逻辑门，为什么寄存器必须跟时钟绑定，为什么 CPU 不是一个巨大函数，为什么指令要编码，为什么流水线会带来异常和安全问题。读完本节，后面讲“保存上下文”“系统调用入口”“页表异常”时，读者应能把它们落回具体硬件状态。
...
到这里，读者应该已经知道 CPU 为什么需要 ALU、寄存器、控制信号、流水线和精确异常。下一节换一种方式巩固：搭一台很小的教学机器，手算它怎样取指、改寄存器、访问内存、进入中断和切换特权。

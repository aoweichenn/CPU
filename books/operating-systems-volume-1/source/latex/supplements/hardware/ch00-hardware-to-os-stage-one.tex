...
这段路径需要三个对象。第一是 task：内核必须有地方保存每个执行流的寄存器、栈、状态和调度账本。第二是 runqueue：内核必须知道哪些 task 现在可运行。第三是 scheduler：当定时器或阻塞事件改变状态时，内核必须选择下一个 task。
...
这一步要求系统提供 TLB shootdown、内存屏障、cache flush、DMA 同步、原子操作和锁。很多 OS 难点并不在“怎样表达一个状态”，而在“怎样让所有硬件观察者在正确时刻看到同一个状态”。

...
这里开始的是第二章的深讲部分。正文已经给出硬件主线，接下来先统一读法：以后遇到任何硬件名词，都要同时问它的结构、时序、接口、OS 后果和失败证据。这个读法定下来，后面的电路、CPU、内存、I/O 和平台设备才能连成一条线。
...
\begin{keyidea}
单独问“cache 是什么”“DMA 是什么”还不够。更有用的问题是：cache 的下层是 SRAM 和一致性互联，上层是 load/store 和内核数据结构；DMA 的下层是设备总线事务和内存控制器，上层是驱动描述符、IOMMU 映射和 completion。每个硬件对象都要这样贯通上下层。
\end{keyidea}

\begin{pdfdiagram}[x=0.9cm,y=1cm]
  \node[os title, anchor=east] at (-0.45,2.05) {CPU 路径};
  \node[os title, anchor=east] at (-0.35,0) {内核契约};
  \node[os title, anchor=east] at (-0.45,-2.05) {设备路径};

  \node[os control, minimum width=1.85cm] (isa) at (0.8,2.05) {ISA/ABI\\指令/寄存器};
  \node[os state, minimum width=1.85cm] (core) at (3.1,2.05) {CPU core\\流水线/ROB};
  \node[os memory, minimum width=1.85cm] (cache) at (5.4,2.05) {cache/TLB\\line/tag/PTE};
  \node[os memory, minimum width=1.85cm] (dram) at (7.7,2.05) {DRAM/NUMA\\channel/bank};

  \node[os control, minimum width=1.95cm] (os) at (0.8,0) {OS kernel\\调度/页表/驱动};
  \node[os memory, minimum width=1.95cm] (pt) at (3.1,0) {页表/属性\\PTE/权限};
  \node[os block, minimum width=1.95cm] (bus) at (5.4,0) {互联/总线\\PCIe/APIC/CXL};
  \node[os state, minimum width=1.95cm] (event) at (7.7,0) {事件证据\\IRQ/fault/log};

  \node[os block, minimum width=1.85cm] (dev) at (0.8,-2.05) {设备\\NVMe/NIC/GPU};
  \node[os block, minimum width=1.85cm] (doorbell) at (3.1,-2.05) {doorbell\\MMIO/queue};
  \node[os memory, minimum width=1.85cm] (iommu) at (5.4,-2.05) {IOMMU/DMA\\IOVA/domain};
  \node[os state, minimum width=1.85cm] (complete) at (7.7,-2.05) {completion\\状态/中断};

  \draw[os strong bus] (isa) -- (core);
  \draw[os strong bus] (core) -- (cache);
  \draw[os strong bus] (cache) -- (dram);

  \draw[os strong bus] (os) -- (pt);
  \draw[os strong bus] (pt) -- (bus);
  \draw[os strong bus] (bus) -- (event);

  \draw[os strong bus] (dev) -- (doorbell);
  \draw[os strong bus] (doorbell) -- (iommu);
  \draw[os strong bus] (iommu) -- (complete);

  \draw[os bus] (pt.north) -- (cache.south);
  \draw[os bus] (bus.south) -- (iommu.north);
  \draw[os bus] (bus.north east) -- ++(0.42,0.7) -| (dram.south);
  \draw[os feedback] (cache.south east) -- ++(0,-0.58) -| (event.north);
  \draw[os feedback] (complete.north) -- (event.south);
  \draw[os feedback] (event.south west) .. controls +(0,-1.1) and +(1.5,-1.1) .. (os.south east);

  \node[os label, anchor=west, text width=2.35cm] at (8.75,1.18) {TLB 填充/失效\\\#PF/TLB miss};
  \node[os label, anchor=west, text width=2.45cm] at (8.75,0.1) {事务/DMA 访问\\进入内核状态机};
  \node[os label, anchor=west, text width=2.45cm] at (8.75,-1.2) {MSI-X\\completion};
\end{pdfdiagram}
...
有了这套读法，下一步才回到最底层：电路怎样从开关、逻辑门和寄存器长成一台能运行 OS 的机器。

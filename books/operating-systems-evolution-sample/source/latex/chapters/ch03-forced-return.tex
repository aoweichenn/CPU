...
\node[stage,minimum width=2.55cm,right=of compare] (pending) {\code{pending}\\置位后保持};
...
\node[stage,minimum width=2.6cm,below=11mm of compare] (control)
...
\node[stage,minimum width=2.4cm,right=of frame] (select) {入口 PC/SP 选择\\切到固定入口};
...
\node[stage,fill=BookWarm,minimum width=7.8cm,below=12mm of frame] (return)
  {事件返回译码器：检查帧格式，再恢复 PC、SP 与标志并重新开放事件};

\section{把八条助记符落实为内存中的三十二个字节}

汇编形式便于人阅读，但处理器取回的是字节。若不把两者对应起来，“固件把代码装入 RAM”
仍然缺少可核对对象。下面为本章用到的操作码规定确切数值：
...
\code{0x11} & \code{ADD} & 操作码，目标寄存器，源寄存器，保留字节 \\ \hline
...
\code{0x00100008} & \code{11 04 05 00} & \code{ADD r4,r5} \\ \hline

...
\node[stage,minimum width=2.5cm] (fetch) {\code{FETCH}\\发起取指};
\node[stage,minimum width=2.5cm,right=of fetch] (decode) {\code{DECODE}\\保存并解释字段};
\node[stage,minimum width=2.5cm,right=of decode] (execute) {\code{EXECUTE}\\计算候选效果};
\node[stage,minimum width=2.5cm,below=13mm of execute] (memory) {\code{MEMORY}\\等待外部响应};
\node[stage,minimum width=2.5cm,left=of memory] (commit) {\code{COMMIT}\\更新可见状态};
\node[stage,minimum width=2.5cm,left=of commit,fill=BookWarm] (halt) {\code{HALT}\\保留故障事实};
\draw[flow] (fetch) -- (decode);
\draw[flow] (decode) -- (execute);
\draw[flow] (execute) -- node[right,font=\scriptsize]{需要访存} (memory);
\draw[flow] (memory) -- node[below,font=\scriptsize]{成功} (commit);
\draw[flow] (execute.west) -| node[above,font=\scriptsize,pos=0.25]{无访存} (commit.north);
\draw[flow] (commit) -- (fetch);
\draw[->,>=Latex,thick,BookMuted] (decode) |- (halt);
\draw[->,>=Latex,thick,BookMuted] (memory) -- (halt);
\end{tikzpicture}
\end{center}

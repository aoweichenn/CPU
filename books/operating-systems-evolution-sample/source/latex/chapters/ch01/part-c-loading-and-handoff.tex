...
接收线上的一个电平不能直接写进 \code{RXDATA}。采样状态先在约定时刻取得每一位，移位
寄存器按位次保存它们，位计数器在收满八位时产生“字节完成”。只有这个事件才允许接收槽
替换旧值并设置 \code{rx\_ready}。发送方向按相反顺序工作：处理器写入发送槽，移位状态
逐位取出，位计数归零后才重新设置 \code{tx\_ready}。

\begin{center}
\begin{tikzpicture}[node distance=10mm and 7mm]
\node[stage,minimum width=2.65cm] (rxline) {外部接收线\\逐位电平};
\node[stage,minimum width=2.8cm,right=of rxline] (sample) {采样与位计数\\确定第 0--7 位};
\node[stage,minimum width=2.85cm,right=of sample] (rxshift) {接收移位寄存器\\暂存未满八位的数据};
\node[stage,minimum width=3.1cm,right=of rxshift] (rxslot) {\code{RXDATA} 接收槽\\\code{ready/overrun}};
\draw[flow] (rxline) -- (sample);
\draw[flow] (sample) -- (rxshift);
\draw[flow] (rxshift) -- (rxslot);
\node[stage,minimum width=2.65cm,below=14mm of rxline] (txline) {外部发送线\\逐位输出};
\node[stage,minimum width=2.8cm,right=of txline] (txshift) {发送移位寄存器\\移出当前最低位};
\node[stage,minimum width=2.85cm,right=of txshift] (txcount) {发送位计数\\八位后置空闲};
\node[stage,minimum width=3.1cm,right=of txcount] (txslot) {\code{TXDATA} 发送槽\\\code{tx\_ready}};
\draw[flow] (txslot) -- (txcount);
\draw[flow] (txcount) -- (txshift);
\draw[flow] (txshift) -- (txline);
\node[font=\scriptsize,BookMuted,below=4mm of rxslot,align=center]
  {处理器读：返回槽中字节\\并清除 \code{rx\_ready}};
\node[font=\scriptsize,BookMuted,below=4mm of txslot,align=center]
  {处理器写：仅在空闲时\\替换发送槽};
\end{tikzpicture}
\end{center}
\diagramnote{上排把外部位流提交为一个软件可读字节，下排把一个软件提交的字节展开成位流。
接收槽和发送槽是两个独立所有权边界，不能用一根“串口缓冲”线概括。}
...
\code{0x40000000} 是本书教学样机的地址互连策略，不是 UART 标准规定的通用地址。
样机把低地址留给 RAM 与任务映像，从 \(1\,\mathrm{GiB}\) 边界开始保留一个
4 KiB、页对齐的 UART 窗口 \code{[0x40000000, 0x40001000)}。这样既能用高位
地址做简单译码，也为以后按页设置设备访问属性留下边界。换一个不与 RAM 和其他设备
重叠的基址同样可行，但互连译码、固件常量与验证程序必须一起修改；只改固件地址会访问
空洞或另一设备。

三个寄存器都是 32 位，所以依次放在 \(+0\)、\(+4\) 与 \(+8\)；分开状态、接收槽
和发送槽，能够明确读写方向与“读后消费”等副作用。对这些地址执行读写会触发控制器
采样或更新内部状态，并不会先把寄存器内容复制到普通 RAM。MMIO 地址由此只是设备
事务的入口编号，不是设备内部存储字节的搬运目的地。

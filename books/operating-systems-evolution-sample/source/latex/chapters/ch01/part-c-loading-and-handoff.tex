...
\node[stage,minimum width=2.8cm,below=13mm of verify,fill=BookWarm] (reject) {拒绝并报告错误};
...
\draw[->,>=Latex,thick,draw=BookMuted] (header.south) |- (reject.west);
\draw[->,>=Latex,thick,draw=BookMuted] (recv.south) -- (reject.north);
\draw[->,>=Latex,thick,draw=BookMuted] (verify.south) |- (reject.east);
\end{tikzpicture}
\end{center}

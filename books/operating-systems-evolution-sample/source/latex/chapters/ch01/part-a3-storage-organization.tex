...
\node[stage,minimum width=2.85cm,right=of command] (registers)
  {请求寄存器\\地址、方向、写数据};
\node[stage,minimum width=2.75cm,below=12mm of registers] (control)
  {事务控制器\\保持请求直到响应};
\node[stage,minimum width=2.7cm,left=of control] (adapter)
  {RAM 端口适配\\请求与响应信号};
...
\draw[flow] (readback.north) -- (host.south);
\end{tikzpicture}
\end{center}

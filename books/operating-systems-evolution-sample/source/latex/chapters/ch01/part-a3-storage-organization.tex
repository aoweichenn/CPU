...
\foreach \x/\lab in {0/{\(A\)},1/{\(A+1\)},2/{\(A+2\)},3/{\(A+3\)}} {
  \draw[fill=BookTint,draw=BookRule] (\x*1.7,1.8) rectangle ++(1.3,0.8);
  \node[font=\footnotesize] at (\x*1.7+0.65,2.2) {\lab};
  \draw[->,>=Latex,thick,BookAccent] (\x*1.7+0.65,1.8) -- (3.2+0.35*\x,0.8);
}
\draw[rounded corners=2pt,fill=BookAccent!12,draw=BookRule] (2.4,0.1) rectangle (5.1,0.8);
\node[font=\footnotesize,align=center] at (3.75,0.45) {小端组合器\\形成 32 位返回值};
\end{tikzpicture}
\end{center}
...
\subsection{调试夹具怎样写 RAM 而不与处理器争用端口}

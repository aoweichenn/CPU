...
\node[stage,minimum width=3.0cm,below=13mm of sel] (target) {目标请求端口\\只使能 RAM 通道};
\node[stage,minimum width=2.6cm,below=10mm of target] (tresp) {目标响应端口\\仅接收 RAM 响应};
\node[stage,minimum width=2.65cm,left=of tresp] (mux) {响应选择器\\核对等待目标};
\node[stage,minimum width=2.55cm,left=of mux] (out) {响应锁存器\\返回处理器};
\node[stage,fill=BookWarm,minimum width=2.7cm,below=10mm of cmp] (err) {零匹配或多匹配\\本地错误响应};
\draw[flow] (req) -- (cmp);
\draw[flow] (cmp) -- (sel);
...
\node[stage,minimum width=2.7cm,below=12mm of shape] (divide) {分频状态\\按固定比率选取边沿};
...
\node[stage,minimum width=2.35cm,below=12mm of hold] (sync) {同步释放级\\复位保持或释放};
...
\node[stage,minimum width=3.1cm,below=12mm of bus] (rom) {启动存储\\返回四个字节};

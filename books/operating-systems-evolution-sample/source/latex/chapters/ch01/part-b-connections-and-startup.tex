...
\node[stage,minimum width=3.5cm,right=of req] (dec) {比较两个地址范围};
\node[stage,minimum width=3.0cm,above right=8mm and 16mm of dec] (rom) {启动存储\\不匹配};
\node[stage,minimum width=3.0cm,below right=8mm and 16mm of dec] (ram) {RAM\\匹配};
\draw[flow] (req) -- (dec);
\draw[->,>=Latex,draw=BookMuted] (dec.east) -| (rom.west);
\draw[flow] (dec) -- node[below,font=\footnotesize]{唯一转发} (ram);
\end{tikzpicture}
\end{center}
...
\node[font=\scriptsize] at (3.5,2.48) {\code{0x00000100}};

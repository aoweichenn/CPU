...
\node[unit,right=of owner,minimum width=2.95cm] (ram)
  {RAM\\事务检查、字节通道\\易失字节阵列};
\node[unit,above=of ram,minimum width=2.95cm] (boot)
  {启动存储\\范围检查、地址下标\\非易失字节阵列};
\node[unit,below=of owner,minimum width=2.75cm,fill=BookWarm] (fixture)
...
\draw[request] ([yshift=3mm]link.east) |- ([yshift=3mm]boot.west);
\draw[response] ([yshift=-3mm]boot.west) -| ([yshift=-3mm]link.east);
\draw[request] ([yshift=3mm]link.east) -- ([yshift=3mm]owner.west);
\draw[response] ([yshift=-3mm]owner.west) -- ([yshift=-3mm]link.east);
\draw[request] ([yshift=3mm]owner.east) -- ([yshift=3mm]ram.west);
\draw[response] ([yshift=-3mm]ram.west) -- ([yshift=-3mm]owner.east);
\draw[prep] ([xshift=-4mm]fixture.north) -- ([xshift=-4mm]owner.south);
\draw[prep,<-] ([xshift=4mm]fixture.north) -- ([xshift=4mm]owner.south);
\node[font=\scriptsize,BookMuted,above=2mm of boot] {启动地址范围};
\node[font=\scriptsize,BookMuted,below=2mm of fixture] {只在处理器保持复位时取得 RAM 端口};

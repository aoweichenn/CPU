...
cmake -S "${ROOT_DIR}" -B "${BUILD_DIR}" \
    ...
    -DCPU_VOLUME_1_BUILD_LABS=ON \
    -DCPU_VOLUME_1_ENABLE_COVERAGE=ON
...
"${BIN_DIR}/lab00_bench" --help > /dev/null
"${BIN_DIR}/lab00_bench" --list > /dev/null
"${BIN_DIR}/lab00_bench" \
    --sizes 16 \
    --warmup 1 \
    --iterations 1 \
...
if "${BIN_DIR}/lab00_bench" --unknown-option > /dev/null 2>&1; then
    echo "expected --unknown-option to fail" >&2
    exit 1
fi

if "${BIN_DIR}/lab00_bench" --csv > /dev/null 2>&1; then
    echo "expected --csv without value to fail" >&2
    exit 1
fi

if "${BIN_DIR}/lab00_bench" --sizes 8,0 > /dev/null 2>&1; then
...
"${BIN_DIR}/lab00_bad_benchmarks" > /dev/null

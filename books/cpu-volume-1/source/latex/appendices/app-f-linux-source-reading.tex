...
Linux 源码阅读不是额外装饰，而是把 C/C++、二进制、运行时和可信测量接到真实操作系统实现上。前面内容讲进程、虚拟地址空间、ELF 加载、系统调用、\filepath{/proc}、调度噪声和 \code{perf} 时，如果只停留在概念层，读者仍然会把操作系统当成黑盒；若能读几条窄而清楚的 Linux 源码路径，就能知道用户态程序和内核之间的边界究竟在哪里。
...
\begin{itemize}
  ...
  \item 当前运行内核版本，可用 \code{uname -a} 记录。
  \item 阅读工具和搜索命令。
...
程序到进程章节已经讲过：\code{main} 通常不是进程第一条指令，可执行文件也不只是机器码。本案例把这个结论接到 Linux 源码中，回答：
...
工具章节要求用 \filepath{/proc} 观察进程。本案例追问：\filepath{/proc/self/maps} 的每一行到底由谁生成？它为什么能显示虚拟地址区间、权限、偏移和文件路径？
...
内存章节把内存先看成按字节寻址的巨大数组，但真实程序访问虚拟地址时，可能触发缺页异常。第一册不深入页表和内存回收，但应该能回答：
...
测量章节讲 benchmark 样本会受 OS 调度、中断、迁核和后台任务影响。源码阅读不要求掌握完整调度器，但应知道用户态线程并不是独占 CPU。问题可以写成：

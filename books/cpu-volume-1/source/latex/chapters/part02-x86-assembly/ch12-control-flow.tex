...
基本块不是画图装饰，而是编译器、反汇编人员和性能工程师共同使用的最小结构单位。原因在于：如果一段指令中间没有其他入口，也没有提前出口，那么只要执行进入这段指令的第一条，后面的指令就会按顺序执行到块尾。这样，块内部主要是数据流问题，块与块之间才是控制流问题。
...
基本块还有一个实用价值：它能暴露不完整分析。一个块如果有两个入口，说明切分错了；一个条件跳转如果只画了一条边，说明漏掉 fallthrough；一个循环如果没有回边，说明可能只看到了展开后的部分代码或把 latch 误删了。证据记录应允许别人根据块表复查路径，而不是只能相信文字结论。
...
这三种形态都可能来自同一段源码。分析时要证明它们等价。下标形态里地址是 \code{base + i * 4}；指针形态里 \code{cur} 初始为 \code{base}，每轮加 4，因此第 k 轮 \code{cur = base + k * 4}；剩余计数形态里可能同时维护当前指针和剩余元素数，退出时 count 归零。证明时写出初始值、每轮递推和退出条件，就能恢复源码含义。
...
如果每个 case 只是返回常量，编译器可能生成数据表而不是代码跳转表。为了看清跳转表，可以让每个分支调用不同函数。代码可以改成如下形式：
...
这里不要求穷尽所有降低策略，但要建立一个原则：\code{switch} 是多路选择的语义，机器实现可以是比较链、比较树、跳转表、数据表、位测试，甚至被完全代数化。报告应描述实际观察到的结构，而不是套用“switch 等于跳转表”的模板。
...
尾调用还受 ABI 约束。参数数量、栈上传参、栈对齐、返回类型、调用约定、可变参数函数、异常处理元数据，都可能影响能否复用当前栈帧。关键事实是：\instruction{jmp callee} 作为尾调用，意味着当前函数不再需要自己的返回点，目标函数将直接返回到更上层调用者。
...
从系统安全角度，返回地址是攻击和防御的核心对象。栈 canary 试图在返回前发现栈帧被覆盖；不可执行栈阻止把栈上数据当代码执行；ASLR 让代码地址难预测；CET shadow stack 用硬件维护一份受保护返回地址副本；CFI 检查间接控制流目标是否属于合法集合。这些机制各自复杂，但共同保护的是“控制数据不能被普通数据写坏”这一条边界。
...
\instruction{call} 还和 ABI 紧密相连。执行 \instruction{call} 前，调用者必须按 ABI 放好参数并维护栈对齐；执行 \instruction{call} 后，被调用者入口看到的是返回地址位于 \register{rsp} 指向的位置，参数位于寄存器或返回地址上方的栈槽。这里先抓住控制流事实：返回地址是调用点下一条指令地址，它是 \instruction{ret} 能回来的原因。
...
\section{为什么 CFG 是编译器和人工分析的共同语言}
...
CFG 的价值在于它迫使分析过程说明路径条件。看到一个 load，不能只说“这里读取内存”，还要问到达这个 load 的路径上是否检查过指针非空、下标范围、对象生命周期和权限。看到一个除法，不能只说“这里除以寄存器”，还要问除数在所有前驱路径上是否可能为零。看到一个 \instruction{cmov}，要问两个候选值是否都已经安全计算。路径条件是正确性证明的一部分。
...
固定证据结构能防止两类错误。第一类是只翻译指令，不证明路径条件。第二类是只谈分支性能，不证明优化合法。控制流分析的核心就是把这两件事放在一起。
...
综合案例可以选择一个带长度检查、空指针检查、循环、早返回和 \code{switch} 的函数。先画源码 CFG，再看 \code{-O3} 汇编恢复机器 CFG，最后比较两者。报告应指出哪些源码块消失了，哪些路径合并了，哪些保护关系仍然成立，哪些输入分布可能影响性能。

这个案例能把本章所有主题连起来：flags、条件码、基本块、循环、switch、call、间接跳转、branchless 和预测。若能完成这样的分析，说明控制流已经从语法知识变成工程判断。
...
本章收束时，应能拿一段优化汇编，画出基本块和边，写出路径条件，解释 flags，恢复循环边界，判断 switch 表类型，区分直接和间接调用，并说明输入分布如何影响预测。若只能把 \instruction{jle} 翻译成“小于等于跳转”，说明控制流还停留在逐行翻译层面。

收束标准不要求预测每个 CPU 的精确分支代价，但要求清楚哪些证据缺失。能说“当前只能证明路径形态，不能证明性能，因为缺少输入分布和测量”，也是合格能力。
...
能够把未解释路径主动列出来，说明分析已经不再只是翻译跳转指令，而是在审查程序路径。这是进入 ABI、互操作和 benchmark 的重要门槛。
...
从性能工程角度，控制流不是孤立问题。它和输入分布、数据布局、内存访问、向量化、代码布局、前端带宽和预测器状态交织在一起。放到 System V AMD64 ABI 里，函数调用还包括参数、返回值、寄存器保存、栈对齐和结构体传递规则；\instruction{call} 和 \instruction{ret} 因此不只是跳转，而是二进制接口的一部分。

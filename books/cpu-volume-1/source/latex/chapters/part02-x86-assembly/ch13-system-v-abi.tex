...
ABI 规则不能只停在表格里。下面把一个最小函数生成 x86-64 目标文件，并保留汇编、符号和重定位证据。这个证据包可以在原生 x86-64 Linux 上运行；也可以在其他架构机器上用 clang 交叉生成 \code{x86\_64-linux-gnu} 目标文件。后者只能证明二进制产物形状，不能证明目标环境的执行性能。
...
\subsection{x86-64 动态链接 evidence pack：PLT、GOT 和 JUMP\_SLOT}

上一组证据停在目标文件层：调用点有 \code{R\_X86\_64\_PLT32}，链接器以后会修正。真实共享库调用还要再跨一层：可执行文件启动时不一定已经知道 \code{add\_i32} 在共享库中的最终运行地址。最终可执行文件会保留动态段、PLT、GOT 和运行时重定位，让动态链接器在启动或第一次调用时绑定符号。

下面的最小样例故意不用 C 运行时入口，而用一个很小的 \code{\_start}。这样证据只集中在“按 ABI 放参数、经过 PLT 调用共享库、用 \instruction{syscall} 退出”这条路径上：

\begin{lstlisting}[language=C++,caption={共享库导出的函数}]
// libadd.cpp
extern "C" int add_i32(int a, int b)
{
    return a + b;
...
\begin{lstlisting}[numbers=none,caption={x86-64 \_start 调用共享库函数}]
...
构建命令要把目标平台写死，否则在非 x86-64 机器上可能生成其他目标的证据，不能拿来证明 x86-64 ABI：

\begin{lstlisting}[language=bash,caption={交叉生成 x86-64 共享库调用样例}]
clang++ --target=x86_64-linux-gnu -O2 -fPIC -c libadd.cpp -o libadd.o
clang++ --target=x86_64-linux-gnu -fuse-ld=lld -nostdlib \
  -shared libadd.o -o libadd.so

clang --target=x86_64-linux-gnu -c start.S -o start.o
clang --target=x86_64-linux-gnu -fuse-ld=lld -nostdlib start.o \
  -L. -ladd -Wl,-rpath,'$ORIGIN' \
  -Wl,--dynamic-linker,/lib64/ld-linux-x86-64.so.2 \
  -o app
\end{lstlisting}

先确认文件身份。没有这一行，后面的反汇编和 relocation 都可能被误读成当前主机架构：

\begin{lstlisting}[numbers=none,caption={ELF header 证明目标是 x86-64}]
Class:                             ELF64
Type:                              EXEC (Executable file)
Machine:                           Advanced Micro Devices X86-64
Entry point address:               0x201330
\end{lstlisting}

动态段说明这个可执行文件依赖 \filepath{libadd.so}，运行时搜索路径是 \code{\$ORIGIN}，并且有 PLT/GOT 相关信息：

\begin{lstlisting}[numbers=none,caption={readelf -d 中的动态链接字段}]
0x000000000000001d (RUNPATH) Library runpath: [$ORIGIN]
0x0000000000000001 (NEEDED)  Shared library: [libadd.so]
0x0000000000000017 (JMPREL)  0x200318
0x0000000000000002 (PLTRELSZ) 24 (bytes)
0x0000000000000003 (PLTGOT)  0x203440
\end{lstlisting}

真正把符号和 GOT slot 联系起来的是 \code{.rela.plt}：

\begin{lstlisting}[numbers=none,caption={readelf -r 看到的动态调用重定位}]
Relocation section '.rela.plt' contains 1 entry:
  Offset      Type              Sym. Name + Addend
  0000203458  R_X86_64_JUMP_SLOT add_i32 + 0
\end{lstlisting}

\code{Offset=0x203458} 不是函数地址，而是 GOT 中等待被填充的槽位地址。符号表中 \code{add\_i32} 仍是 undefined，因为它要从共享库解析：

\begin{lstlisting}[numbers=none,caption={可执行文件符号表中的 undefined 动态符号}]
Num: Value Size Type Bind   Ndx Name
  1: 0     0    FUNC GLOBAL UND add_i32
\end{lstlisting}

反汇编把这条链路连起来：

\begin{lstlisting}[numbers=none,caption={\_start 到 add\_i32@plt 的 x86-64 控制流}]
0000000000201330 <_start>:
  201330: bf 07 00 00 00      mov    edi,0x7
  201335: be 05 00 00 00      mov    esi,0x5
  20133a: e8 21 00 00 00      call   201360 <add_i32@plt>
  20133f: 89 c7               mov    edi,eax
  201341: b8 3c 00 00 00      mov    eax,0x3c
  201346: 0f 05               syscall

0000000000201360 <add_i32@plt>:
  201360: ff 25 f2 20 00 00   jmp QWORD PTR [rip+0x20f2] # 203458
  201366: 68 00 00 00 00      push 0x0
  20136b: e9 e0 ff ff ff      jmp 201350 <.plt>
\end{lstlisting}

这段输出有四个关键点。第一，\code{\_start} 仍然按 System V AMD64 函数 ABI 把两个 \code{int} 参数放进 \register{edi} 和 \register{esi}。第二，调用目标不是共享库真实地址，而是本可执行文件里的 \code{add\_i32@plt}。第三，PLT 第一条指令从 GOT slot \code{0x203458} 间接跳转；这个 slot 正是 \code{R\_X86\_64\_JUMP\_SLOT} 要修补的位置。第四，若该 slot 还没绑定，后续 \instruction{push 0x0} 和跳回 PLT 头部会进入动态链接器解析流程；若已经绑定，第一条间接跳转会直接到共享库函数。

可以把 lazy binding 看成一个小状态机：
...
状态 & GOT slot 含义 & 证据位置 \\
\midrule
Unresolved & 指向 PLT resolver 路径 & \code{add\_i32@plt} 后半段仍可到达 \\
Resolving & 动态链接器查找符号和库 & \cmd{LD\_DEBUG=bindings} 或动态链接器 trace \\
Patched & GOT slot 写入共享库函数地址 & 运行后内存中的 GOT 内容变化 \\
Bound & 后续调用直接间接跳到函数 & 热路径只走 PLT 第一条 \instruction{jmp} \\
\bottomrule
\end{tabular}
\end{center}

\code{LD\_BIND\_NOW=1} 或链接选项 \code{-Wl,-z,now} 会把许多绑定提前到加载阶段；\code{-fno-plt} 可能让编译器生成不同的 GOT 间接调用形态；静态链接则不会走普通共享库 PLT 路径。因此，动态链接证据不能只贴一段 \instruction{call foo@plt}。报告必须写清：

\begin{lstlisting}[numbers=none,caption={动态链接 evidence pack 的最小字段}]
target_triple: x86_64-linux-gnu
link_model: dynamic
pie: yes | no
bind_policy: lazy | now
needed_libraries: readelf -d
runpath_or_rpath: readelf -d
plt_disassembly: objdump -d
jump_slot_relocations: readelf -r .rela.plt
loaded_library_path: LD_DEBUG / ldd / proc maps
call_site_abi: argument registers and return register
\end{lstlisting}

这组证据的边界也要写清。上面的交叉构建可以静态证明 x86-64 ELF、动态段、PLT/GOT 和 relocation 形态；若要证明运行时真实绑定路径，还需要在 x86-64 主机或可靠模拟器上执行，并保存 \cmd{LD\_DEBUG=bindings}、\filepath{/proc/<pid>/maps} 或调试器中 GOT slot 的前后值。没有运行证据时，只能说“该二进制将通过 \code{R\_X86\_64\_JUMP\_SLOT} 支持动态绑定”，不能声称已经观测到某次运行的 lazy binding。
...
\subsection{浮点 ABI 不是浮点数学：位置、格式和环境要分开}

ABI 只规定浮点值在调用边界怎样交接，不规定这个值在数值上是否满足你的算法误差边界。一个 \code{double} 参数进入 \register{xmm0}，只说明调用者和被调用者约定用这个寄存器低 64 位传递二进制浮点位模式；它不说明表达式是否允许 FMA，不说明舍入模式是否默认，不说明 NaN 是否可能出现，也不说明 MXCSR 的 FTZ/DAZ 是否打开。

因此，浮点 ABI 报告要分三层写：

\begin{lstlisting}[numbers=none,caption={浮点 ABI 报告的三层字段}]
abi_layer:
  double b -> xmm0
  float d  -> xmm1 low 32 bits
  return double -> xmm0 low 64 bits

instruction_layer:
  cvtsi2sd / cvtss2sd / addsd / movapd
  scalar low lane is the business value
...
numeric_layer:
  ...
  tolerance_or_bit_exact:
\end{lstlisting}

这三层不能互相替代。反汇编证明 \register{xmm0} 被写入，不等于证明结果数值正确；单元测试证明普通输入正确，也不等于证明 NaN、Inf、signed zero、subnormal 和舍入边界都正确。尤其是手写汇编和 intrinsic kernel，接口文档必须同时写 ABI 合同和数值合同。

\subsection{float、double 和 long double 不是同一种 ABI 问题}

\code{float} 和 \code{double} 在常见 System V AMD64 调用中走 SSE 通道，分别使用 \register{xmm} 低 32 位和低 64 位。它们的返回值通常也在 \register{xmm0} 的低位 lane。读汇编时，\instruction{movss}、\instruction{addss}、\instruction{cvtss2sd} 常提示单精度；\instruction{movsd}、\instruction{addsd}、\instruction{cvtsi2sd} 常提示双精度。但指令名只能作为证据之一，最终仍要回到函数声明和调用点。

\code{long double} 在 x86-64 平台上更容易成为坑。某些 System V 环境中它可能是 80-bit extended precision，并涉及 x87 或内存传递；Windows x64 上常见 MSVC \code{long double} 与 \code{double} 等价。跨平台 ABI 文档如果写了 \code{long double}，就必须明确编译器、平台和大小。公开 ABI 中通常应避免直接暴露 \code{long double}，除非你愿意把这套平台差异写进合同和测试。

\begin{warningbox}
不要把“浮点参数走 \register{xmm}”泛化到所有浮点类型和所有平台。至少要区分 \code{float}、\code{double}、\code{long double}、向量类型和聚合里的浮点字段。
\end{warningbox}

\subsection{浮点变参：\register{al} 和默认提升同时存在}

变参调用是浮点 ABI 最容易露馅的地方。对固定原型函数，编译器知道每个参数类型，能按整数通道和浮点通道分别放置。对 C 风格变参，被调用者要用 \code{va_list} 在运行时取参数，因此调用者还要提供额外约定。

System V AMD64 中，调用变参函数时，\register{al} 常用于告诉被调用者用了多少个向量寄存器传递参数。与此同时，C 默认参数提升仍然生效：\code{float} 作为变参会提升为 \code{double}，小整数会提升为 \code{int} 或 \code{unsigned int}。所以：

\begin{lstlisting}[language=C++]
std::printf("%f\n", float_value);
\end{lstlisting}

调用点传的不是 \code{float} ABI 形态，而是提升后的 \code{double}，通常放进某个 \register{xmm} 寄存器，并设置对应的 \register{al} 元信息。若手写汇编调用 \code{printf} 打印浮点数，只把 \register{xmm0} 放好但忘记 \register{al}，在某些环境中就会读取错误的寄存器保存区。

报告中分析变参浮点调用，必须写：

\begin{itemize}
  \item 源码中的实参类型。
  \item 变参默认提升后的实际 ABI 类型。
  \item 使用了哪些 \register{xmm} 寄存器。
  \item 调用前 \register{al} 或 \register{eax} 如何设置。
  \item 格式串是否和提升后的类型一致。
\end{itemize}

\subsection{浮点 ABI evidence pack：用同一例子锁定寄存器}

可以用一个小函数固定训练：

\begin{lstlisting}[language=C++,caption={浮点和整数混合 ABI 样例}]
extern "C" double fp_mix(float a, double b, int c, float d)
{
    return double(a) + b + double(c) + double(d);
}
\end{lstlisting}

System V AMD64 下，入口推导应是：

\begin{lstlisting}[numbers=none,caption={fp\_mix 的入口状态}]
xmm0 low f32 = a
xmm1 low f64 = b
edi          = c
xmm2 low f32 = d
return low f64 in xmm0
\end{lstlisting}

注意 \code{c} 是第一个整数通道参数，仍在 \register{edi}；\code{d} 是第三个浮点通道参数，在 \register{xmm2}，不是 \register{xmm3}。一份合格 evidence pack 至少保存：

\begin{lstlisting}[language=bash,caption={浮点 ABI evidence pack 命令}]
clang++ -std=c++20 -O2 -S -masm=intel fp_mix.cpp -o fp_mix.s
clang++ -std=c++20 -O2 -c fp_mix.cpp -o fp_mix.o
objdump -drwC -Mintel fp_mix.o
nm -C fp_mix.o
\end{lstlisting}

反汇编中可能出现 \instruction{cvtss2sd} 把 \code{float} 扩成 \code{double}，\instruction{cvtsi2sd} 把 \code{int} 转成 \code{double}，再用 \instruction{addsd} 累加。若编译器使用不同寄存器临时值，不要逐字背输出；关键是入口 ABI 位置、低 lane 语义和返回 \register{xmm0} 必须能解释。
...
栈对齐错误常表现得不稳定。一个错误函数可能在某个实验环境、当前优化级别、当前 libc 版本下运行正常，因为被调用路径没有使用对齐敏感指令，或者未对齐访问被硬件以较高代价处理了。换一台机器、换一个编译器版本、打开不同优化选项、链接到不同库实现，错误就可能变成崩溃或数据错误。
...
本章以 Linux 上常见的 System V AMD64 ABI 为主。这个选择是为了让第一册有一个具体、可实验的目标，而不是因为所有 x86-64 平台都这样工作。Windows x64、Linux syscall ABI、macOS 平台约定、RISC-V ABI 和其他 ABI 都可能在参数寄存器、栈空间、红区、向量寄存器、异常展开、名字修饰和系统调用入口上不同。
...
\subsection{Windows x64 ABI 的最小模型}

Windows x64 ABI 的核心规则可以先压缩成四句话。第一，前四个参数位置按参数序号统一分配：整数/指针使用 \register{rcx}、\register{rdx}、\register{r8}、\register{r9}，浮点参数使用对应序号的 \register{xmm0}、\register{xmm1}、\register{xmm2}、\register{xmm3}。第二，调用者在每次调用前预留 32 字节 shadow space，也叫 home space，供被调用者保存这四个寄存器参数或作为 ABI 规定的栈区域。第三，Windows x64 没有 System V 那种 red zone。第四，callee-saved 集合和 unwind 元数据规则与 System V 不同，不能直接复用 Linux 汇编序言/尾声。
...
\midrule
整数/指针参数 & \register{rdi}, \register{rsi}, \register{rdx}, \register{rcx}, \register{r8}, \register{r9} & \register{rcx}, \register{rdx}, \register{r8}, \register{r9} \\
标量浮点参数 & 独立 SSE 通道，\register{xmm0}--\register{xmm7} & 前四个参数位置对应 \register{xmm0}--\register{xmm3} \\
栈调用约定 & 调用前 16 字节对齐；有 red zone & 调用者预留 32 字节 shadow space；无 red zone \\
栈参数起点 & 入口 \code{[rsp]} 是返回地址，后续看溢出参数 & 入口 \code{[rsp]} 是返回地址，\code{[rsp+8..+39]} 是 shadow space \\
向量寄存器保存 & \register{xmm0}--\register{xmm15} 通常 caller-saved & 部分 XMM 寄存器低 128 位属于 nonvolatile，需按 Windows 规则处理 \\
\bottomrule
\end{tabular}
\end{center}

这张表的重点不是背更多寄存器，而是防止混用。System V 的“整数和浮点分别计数”与 Windows x64 的“前四个参数槽位”不是同一种思路。看下面这个函数：

\begin{lstlisting}[language=C++]
extern "C" double cross(double a, int b, double c, int d);
\end{lstlisting}
...
\begin{lstlisting}
xmm0 = a        ; parameter slot 1 is floating
edx  = b        ; parameter slot 2 is integer
xmm2 = c        ; parameter slot 3 is floating
r9d  = d        ; parameter slot 4 is integer
\end{lstlisting}

注意 Windows 这里没有把 \code{c} 放进 \register{xmm1}，也没有把 \code{d} 放进 \register{r8d}。因为寄存器编号跟参数槽位走，而不是按浮点/整数各自独立压缩计数。把这两个 ABI 混起来，是跨平台汇编最常见的参数错位。

\subsection{shadow space：调用者预留，被调用者可以使用}

Windows x64 要求调用者在调用前预留 32 字节 shadow space。即使被调用函数只有一个参数，调用者通常也要预留这块区域。它位于返回地址之上：被调用者入口处 \code{[rsp]} 是返回地址，\code{[rsp + 8]} 到 \code{[rsp + 39]} 是四个 8 字节 home slots。

一个简化时间线：

\begin{lstlisting}
caller before call on Windows x64:
    reserve 32-byte shadow space
    place first four register arguments
    rsp aligned as required before call
...
callee entry:
    [rsp + 0]  = return address
    [rsp + 8]  = home slot for arg0
    [rsp + 16] = home slot for arg1
    [rsp + 24] = home slot for arg2
    [rsp + 32] = home slot for arg3
\end{lstlisting}

这不是 System V 的栈参数区，也不是当前函数自己的局部变量区。它是 Windows ABI 明确给被调用者的一块 home area。很多编译器会把寄存器参数 spill 到这里，方便调试、取地址或支持某些调用形态。手写汇编若忘记调用前预留 shadow space，可能在被调用者写 home slots 时覆盖调用者自己的数据。

\subsection{Windows x64 的浮点和变参边界}

Windows x64 中，前四个参数槽位按序号分配；槽位类型决定使用通用寄存器还是 XMM 寄存器。普通固定原型函数里，浮点实参走对应 \register{xmmN}，整数实参走对应通用寄存器。对于变参和未原型化函数，规则还会额外要求让浮点值在对应整数寄存器中也有可用表示，便于被调用者按运行时类型访问。不同编译器和文档细节要以目标平台 ABI 为准。

第一册要掌握的底线是：不要把 System V 的 \register{al} 规则照搬到 Windows x64，也不要把 Windows 的 shadow space 和参数槽位规则照搬到 System V。两边的变参支持都服务同一个目标：让被调用者能按约定找到变参；但它们的机制不同。

\subsection{Windows x64 的保存责任和 unwind 不是 Linux 序言}

Windows x64 的 nonvolatile 寄存器集合与 System V 不同。通用寄存器里常见 nonvolatile 包括 \register{rbx}、\register{rbp}、\register{rdi}、\register{rsi}、\register{r12}--\register{r15}，而 \register{rcx}、\register{rdx}、\register{r8}、\register{r9}、\register{r10}、\register{r11} 等通常 volatile。向量寄存器也有 Windows 特有保存规则，尤其要注意某些 \register{xmm} 寄存器的低 128 位 nonvolatile。

更重要的是 unwind。Windows 异常处理和栈展开依赖特定的 unwind metadata。手写 Windows x64 汇编不能只写一段能返回的机器码，还要让调试器、异常处理和 profiler 能恢复栈。Linux/GAS 里的 \code{.cfi\_*} 思路和 Windows \code{.seh\_*} 或 MASM unwind 描述不是同一套语法。跨平台汇编文件若只复制 Linux 序言，Windows 上即使计算正确，也可能在异常、崩溃回溯或采样 profiler 中不可用。

\subsection{跨 ABI evidence pack：同一声明生成两份报告}

训练跨平台 ABI，最好的方法是同一 C++ 声明生成 System V 和 Windows 两份目标证据：

\begin{lstlisting}[language=C++,caption={跨 ABI 对照样例}]
extern "C" double cross(double a, int b, double c, int d)
{
...
\begin{lstlisting}[language=bash,caption={生成 System V 和 Windows x64 对照汇编}]
clang++ --target=x86_64-linux-gnu -std=c++20 -O2 \
  -S -masm=intel cross.cpp -o cross.sysv.s

clang++ --target=x86_64-windows-msvc -std=c++20 -O2 \
  -S -masm=intel cross.cpp -o cross.win64.s
\end{lstlisting}

报告不要只贴两份汇编。要列出：

\begin{lstlisting}[numbers=none,caption={跨 ABI 对照报告字段}]
declaration:
  extern "C" double cross(double, int, double, int)
...
windows_x64:
  arg_locations:
    ...
    d: r9d
  stack_model:
    shadow_space: 32 bytes reserved by caller
    red_zone: none

conclusion:
  same C++ declaration has different binary entry state
  one assembly implementation cannot be blindly reused
\end{lstlisting}

这份报告的结论很硬：同一个 C++ 函数声明，在两个 x86-64 平台上可以拥有不同入口寄存器、不同栈合同、不同保存责任和不同 unwind 元数据。跨平台工程的正确做法不是用宏把一份汇编“凑到能编”，而是为每个 ABI 写清独立入口合同，用 C++ wrapper 调度，并用对应平台证据验证。

\subsection{Windows x64 参数槽位：不是两条独立通道}

System V AMD64 最容易记成“两条队列”：整数/指针参数进通用寄存器队列，浮点参数进 SSE 队列。Windows x64 不能这样记。Windows x64 更适合记成“四个参数槽位”：第 1、2、3、4 个参数各有一个通用寄存器位置和一个 XMM 位置，具体使用哪个由该参数类型决定。超过四个参数后，后续参数走栈。

看这个声明：

\begin{lstlisting}[language=C++]
extern "C" double slot_demo(int a, double b, float c, long long d);
\end{lstlisting}

Windows x64 的入口位置应按槽位推导：

\begin{lstlisting}[numbers=none,caption={Windows x64 参数槽位推导}]
slot 0: int a       -> ecx
slot 1: double b    -> xmm1 low f64
slot 2: float c     -> xmm2 low f32
slot 3: long long d -> r9
return double       -> xmm0 low f64
\end{lstlisting}

这和 System V 完全不同。System V 会把 \code{a} 放 \register{edi}，\code{b} 放 \register{xmm0}，\code{c} 放 \register{xmm1}，\code{d} 放 \register{rsi}。如果你用 System V 的“浮点独立队列”理解 Windows，第二个参数 \code{b} 会被错写成 \register{xmm0}，第三个参数 \code{c} 会被错写成 \register{xmm1}。这种错误在手写汇编里不会被 CPU 拦住，只会表现为结果错、NaN、随机值或偶发崩溃。

\begin{keyidea}
Windows x64 先按参数序号占槽，再按类型选择 RCX/RDX/R8/R9 或 XMM0--XMM3。System V 先按类型分类，再分别压入整数队列和 SSE 队列。二者不是同一套规则的两个名字。
\end{keyidea}

\subsection{shadow space 的真实栈图}

shadow space 不能只背“32 字节”。它的关键是所有权和时间点。Windows x64 调用者在执行 \instruction{call} 前预留 32 字节；\instruction{call} 再压入 8 字节返回地址；被调用者入口处，\register{rsp} 指向返回地址，shadow space 位于 \code{[rsp+8]} 到 \code{[rsp+39]}。

调用者视角：

\begin{lstlisting}[numbers=none,caption={Windows x64 调用者准备一次调用}]
; before nested call, rsp must satisfy the call alignment rule
sub rsp, 40              ; 32-byte shadow space + 8-byte alignment fix
mov ecx, 1               ; arg0
mov edx, 2               ; arg1
mov r8d, 3               ; arg2
mov r9d, 4               ; arg3
...
\begin{lstlisting}[numbers=none,caption={Windows x64 callee entry stack map}]
rsp + 00: return address
rsp + 08: home slot for arg0 / shadow space byte 0..7
rsp + 16: home slot for arg1 / shadow space byte 8..15
rsp + 24: home slot for arg2 / shadow space byte 16..23
rsp + 32: home slot for arg3 / shadow space byte 24..31
rsp + 40: stack argument 4, if any
rsp + 48: stack argument 5, if any
\end{lstlisting}

这里最容易错的是把 \code{[rsp+8]} 当成第一个栈上传参。对 Windows x64 来说，\code{[rsp+8]} 首先属于 shadow space；第五个参数才通常在 \code{[rsp+40]}。对 System V 来说，没有这块固定 shadow space，第七个整数类参数常从 \code{[rsp+8]} 读。二者刚好容易形成灾难性错位。

\subsection{Windows x64 的五个参数：第五个为什么在 \code{[rsp+40]}}

用一个固定例子训练：

\begin{lstlisting}[language=C++]
extern "C" long long sum5(long long a, long long b, long long c,
...
\begin{lstlisting}[numbers=none,caption={sum5 的 Windows x64 入口状态}]
rcx        = a
rdx        = b
r8         = c
r9         = d
[rsp + 00] = return address
[rsp + 08] = shadow slot 0
[rsp + 16] = shadow slot 1
[rsp + 24] = shadow slot 2
[rsp + 32] = shadow slot 3
[rsp + 40] = e
\end{lstlisting}

对应手写汇编的核心计算可以是：

\begin{lstlisting}[numbers=none,caption={Windows x64 sum5 栈参数读取}]
sum5 PROC
    lea rax, [rcx + rdx]
    add rax, r8
    add rax, r9
...
这段代码如果放到 System V 上完全不对。System V 的第五个整数类参数在 \register{r8}，而不是 \code{[rsp+40]}；第一个参数也不是 \register{rcx}，而是 \register{rdi}。同一个源函数签名，手写汇编必须按目标 ABI 重写入口。

\subsection{Windows x64 的 leaf 和 non-leaf：shadow space 仍由调用者提供}

leaf function 是不调用其他函数的函数。Windows x64 leaf function 可以非常短，例如：

\begin{lstlisting}[numbers=none,caption={Windows x64 leaf function}]
add2 PROC
    lea eax, [ecx + edx]
    ret
add2 ENDP
...
这个 leaf function 自己没有调用别人，所以它不需要为了 nested call 再预留 shadow space。但它的调用者在调用它之前仍然必须预留 shadow space。被调用者可以选择不用这 32 字节，但调用者不能因为“我猜它不用”就省掉。ABI 约定的是调用边界，不是某次实现的内部习惯。

non-leaf function 要调用 helper，就必须作为调用者满足 Windows x64 规则。比如一个 wrapper 调用 \code{helper}：

\begin{lstlisting}[numbers=none,caption={Windows x64 non-leaf 调用形态}]
wrapper PROC
    sub rsp, 40          ; 32 shadow + alignment
    ; arrange helper arguments in rcx, rdx, r8, r9 / stack
    call helper
    add rsp, 40
    ret
wrapper ENDP
...
如果 wrapper 还要保存 nonvolatile 寄存器，或者分配局部变量，\code{sub rsp, N} 要一起覆盖 shadow space、局部空间和对齐需求。不要机械认为永远是 40。正确报告应写清每一段栈空间的用途。

\subsection{Windows x64 的返回值：小整数、浮点和聚合}

Windows x64 常见标量返回规则与直觉接近：整数、指针返回通常在 \register{rax}；\code{float}/\code{double} 返回通常在 \register{xmm0} 低 lane。但聚合类型不能只凭大小猜，尤其要区分“能放进寄存器的简单小对象”和“需要调用者提供返回存储的对象”。

工程上可以先记一个保守规则：跨 Windows x64 手写汇编边界时，公开接口尽量避免直接返回复杂 C++ 对象；用整数、指针、浮点或显式输出指针更清楚。比如：
...
extern "C" void make_pair64(std::uint64_t x, Pair64* out);
\end{lstlisting}

这比直接写 \code{extern "C" Pair64 make_pair64(std::uint64_t);} 更容易稳定，因为输出地址显式占一个普通参数位置。直接返回聚合时，MSVC ABI、MinGW、Clang 目标和类型性质都可能影响实际返回方式；若你没有保存反汇编和调用点证据，后续维护很容易错。

\subsection{Windows x64 的结构体传参：不要把 System V eightbyte 分类搬过去}

System V AMD64 有一套复杂的 eightbyte 分类规则，小结构体可能拆成通用寄存器和 SSE 寄存器组合。Windows x64 的规则不是这套。许多不符合简单寄存器传递条件的聚合会通过地址传递，调用者在临时存储中放对象，再把地址作为参数传给被调用者。

因此，下面这种接口在跨 ABI 时风险很高：
...
System V 可能把它拆到一个通用寄存器和一个 XMM 寄存器；Windows x64 可能使用完全不同的形态。正确做法不是凭记忆猜，而是生成目标 triple 的反汇编，保存调用点和入口证据。公开 ABI 中更稳妥的设计是：

\begin{lstlisting}[language=C++]
extern "C" double use_mixed_ptr(Mixed const* x);
\end{lstlisting}

指针 ABI 形态简单，布局仍需 \code{static_assert} 保护，但至少调用边界不再涉及聚合分类。

\subsection{Windows x64 变参：不要套用 System V 的 \register{al}}

System V 变参调用常见 \register{al} 元信息；Windows x64 不是这条规则。Windows x64 的变参和未原型化函数为了让被调用者能从统一位置取值，会让前四个参数的寄存器槽位和 home slots 扮演不同角色；浮点值在某些调用形态下还需要在对应通用寄存器中有副本。具体细节要以 Windows x64 ABI 文档和编译器输出为准。

第一册要训练的是边界意识：

\begin{itemize}
  \item 分析 Windows x64 \code{printf} 调用，不要寻找 System V 的 \register{al} 规则。
  \item 看浮点变参时，检查 XMM 寄存器、对应通用寄存器、shadow space 和栈参数。
  \item 手写汇编调用变参函数时，优先让 C++ wrapper 处理；若必须手写，按 Windows ABI 逐项摆放并保存反汇编证据。
\end{itemize}

\subsection{Windows x64 保存寄存器：XMM6--XMM15 也要进入清单}

Windows x64 常见 nonvolatile 通用寄存器包括 \register{rbx}、\register{rbp}、\register{rdi}、\register{rsi}、\register{r12}--\register{r15}。这和 System V 不同：System V 下 \register{rdi}、\register{rsi} 是参数寄存器并且通常 caller-saved；Windows x64 下它们属于 nonvolatile，函数若修改就要恢复。

向量寄存器也不同。Windows x64 中 \register{xmm6}--\register{xmm15} 的低 128 位通常是 nonvolatile。一个手写 AVX/SSE kernel 如果把 \register{xmm8} 当临时寄存器随便改，System V 下可能没问题，Windows x64 下就可能破坏调用者状态。若使用 \register{ymm6}--\register{ymm15}，还要注意 ABI 对低 128 位保存的要求和高位状态的处理边界。

Windows x64 汇编函数报告至少要列：

\begin{lstlisting}[numbers=none,caption={Windows x64 保存责任清单}]
modified_gpr:
  volatile:
  nonvolatile_saved_restored:

modified_xmm:
  xmm0_xmm5_volatile:
  xmm6_xmm15_nonvolatile_saved_restored:

stack:
  shadow_space_for_nested_calls:
  local_stack_size:
  alignment_at_each_call:
\end{lstlisting}

\subsection{SEH unwind 元数据：能返回不等于能展开}

Windows x64 对函数序言、尾声和 unwind metadata 有严格要求。调试器、异常处理、栈回溯和 profiler 需要这些元数据恢复调用链。一个函数如果只写：
...
它从 CPU 执行角度可能能返回，但 Windows 异常展开器未必知道你保存了 \register{rbx}、移动了 \register{rsp}。MASM 里常见 \code{PROC}、\code{ENDP}、\code{.allocstack}、\code{.pushreg}、\code{.endprolog} 一类机制；Clang/GAS 风格也有面向 Windows SEH 的指令。它们和 Linux 上的 \code{.cfi\_*} 不是同一套。

第一册不要求你掌握全部 SEH 语法，但要知道验收标准：Windows x64 手写汇编不能只通过功能测试，还要通过 unwind 证据。至少要能回答：

\begin{itemize}
  \item 这个函数是否可能出现在异常展开路径或 profiler 采样栈里。
  \item 是否保存了 nonvolatile 寄存器，unwind metadata 是否描述了保存位置。
  \item 是否分配了栈空间，unwind metadata 是否描述了栈大小。
  \item 是否有多个返回路径，每条尾声是否符合 unwind 规则。
\end{itemize}

\subsection{MSVC、MinGW 和 Clang target triple 都要写进报告}

“Windows x64 ABI”不是说所有工具输出都长得一样。MSVC、Clang targeting \code{x86\_64-windows-msvc}、Clang targeting \code{x86\_64-w64-windows-gnu}、MinGW-w64 工具链，在符号、异常模型、C++ ABI、库边界和汇编语法上都可能不同。C 函数调用核心约定很接近，但 C++ 对象、异常、name mangling 和运行时库边界不能混说。

跨平台报告必须写：

\begin{lstlisting}[numbers=none,caption={Windows x64 目标身份字段}]
target_os: windows
target_abi: msvc / gnu
target_triple:
compiler:
assembler_syntax: masm / gas-intel / llvm
object_format: COFF
exception_unwind: SEH / other
runtime_library:
\end{lstlisting}

没有这些字段，看到 \code{.seh\_*}、COFF 符号、MSVC name mangling 或 MinGW 导入库时，就很容易把工具链差异误判成 ABI 规则差异。

\subsection{Windows x64 错误案例：把 \code{[rsp+8]} 当第五个参数}

错误实现：

\begin{lstlisting}[numbers=none,caption={错误：把 shadow slot 当第五个参数}]
; extern "C" long long sum5(long long,long long,long long,long long,long long)
bad_sum5 PROC
    lea rax, [rcx + rdx]
    add rax, r8
    add rax, r9
    add rax, qword ptr [rsp + 8]   ; wrong on Windows x64
    ret
bad_sum5 ENDP
\end{lstlisting}

为什么错？因为 \code{[rsp+8]} 是 shadow space 的第一个 home slot，不是第五个参数。第五个参数通常在 \code{[rsp+40]}。这个 bug 可能在某些调用点上“看起来有值”，因为编译器可能把 \register{rcx} spill 到 home slot；但那只是实现细节，不是第五个参数。正确实现应读取 \code{[rsp+40]}。

这类错误很适合写成实验：先用 C++ reference 计算 \code{1+2+3+4+5=15}，再调用错误汇编，观察它把 home slot 当参数导致结果不稳定；最后修复偏移，并用反汇编确认调用者确实预留了 shadow space。

\subsection{Windows x64 错误案例：忘记为嵌套调用预留 shadow space}

错误 wrapper：

\begin{lstlisting}[numbers=none,caption={错误：调用 helper 前未预留 shadow space}]
bad_wrapper PROC
    ; rcx already holds first argument
    call helper      ; wrong: no 32-byte shadow space reserved here
    ret
bad_wrapper ENDP
\end{lstlisting}

如果 \code{helper} 在入口把参数保存到 home slots，例如写 \code{[rsp+8]}、\code{[rsp+16]}，它会覆盖 \code{bad\_wrapper} 栈上的未知位置。程序可能立刻崩溃，也可能污染某个返回地址附近的数据，稍后才坏。正确 wrapper 至少要在调用前分配调用所需栈空间，并保持对齐：

\begin{lstlisting}[numbers=none,caption={正确方向：为嵌套调用准备调用帧}]
good_wrapper PROC
    sub rsp, 40
    call helper
    add rsp, 40
...
实际工程里如果还保存 nonvolatile 寄存器或分配局部变量，栈大小要重新计算，不能机械复制 40。

\subsection{Windows x64 最小实验：生成 COFF 目标文件并审计}

即使在 Linux 主机上，也可以用 Clang 生成 Windows x64 汇编和 COFF 目标文件，训练静态证据：

\begin{lstlisting}[language=bash,caption={生成 Windows x64 ABI 静态证据}]
clang++ --target=x86_64-windows-msvc -std=c++20 -O2 \
  -S -masm=intel winabi.cpp -o winabi.win64.s

clang++ --target=x86_64-windows-msvc -std=c++20 -O2 \
  -c winabi.cpp -o winabi.win64.obj

llvm-objdump -d --x86-asm-syntax=intel winabi.win64.obj
llvm-nm -C winabi.win64.obj
\end{lstlisting}

这个实验能证明：目标是 COFF/Windows x64，参数寄存器如何出现，函数是否使用 shadow space，符号如何命名。它不能证明：该函数已经在 Windows 上运行，SEH unwind 已经被真实异常路径验证，或者 MSVC runtime 交互完全正确。运行证据必须在 Windows 主机、Wine、CI runner 或可靠目标环境中补上。

\subsection{Windows x64 ABI 报告模板}

一份合格 Windows x64 ABI 报告至少包含：

\begin{lstlisting}[numbers=none,caption={Windows x64 ABI report}]
windows_x64_abi_report:
  target:
    triple:
    compiler:
    object_format: COFF
  ...
  parameters:
    ...
    slot2:
    slot3:
    stack_args_from_rsp_plus_40:

  stack_contract:
    caller_shadow_space:
    callee_entry_rsp_map:
    nested_call_rsp_adjustment:
    red_zone: none
  ...
  failure_cases_checked:
    ...
    missing_shadow_space:
    wrong_stack_arg_offset:
    missing_nonvolatile_restore:
\end{lstlisting}

这份模板比“Windows 第一个参数在 RCX”厚得多。它要求读者把目标身份、参数槽位、栈图、保存责任、unwind 和证据边界全部写出来。做到这一步，Windows x64 ABI 才从“被提到”变成“可用知识”。
...
调用者也有责任记录库身份。性能或崩溃报告中，应保存实际加载库路径和版本。若系统升级后共享库实现变了，同一可执行文件也可能表现不同。动态库边界把“程序身份”延伸到了运行环境，这是程序身份和 ABI 边界相互连接的地方。
...
从高性能计算和 AI 算子开发角度，ABI 决定了 kernel 函数边界的成本和正确性：参数如何进入寄存器，哪些寄存器可以自由使用，调用 helper 前要保存什么，返回 tensor metadata 或小结构体会不会产生隐藏内存写。可测试、可链接、可维护的手写汇编工程，都要把这些规则作为接口合同。

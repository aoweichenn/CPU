...
本章要把这个过程讲清楚。性能优化会反复依赖这些基础：cache line 为什么按固定粒度搬运数据，SIMD load 为什么偏好连续布局，结构体字段顺序为什么影响性能，未定义行为为什么会扩大编译器优化空间，AoS 和 SoA 为什么差异显著，GEMM packing 为什么本质上是在重排内存访问。

\section{本章问题链：从一个字节走到一个 kernel}
...
float read_z(Particle const* p, std::size_t i)
{
    return p[i].z;
}
\end{lstlisting}

\textbf{bit 和 byte} 解决“机器最小保存什么”。\code{float} 和 \code{std::uint8_t} 最终都要落成若干 bit，内存按 byte 编址。\textbf{对象表示} 解决“一个 \code{Particle} 对象占哪些字节”。它包含字段字节，也可能包含 padding 字节。padding 是编译器为了满足对齐和数组步长插入的空隙；它占空间，但不一定有稳定语义值。\textbf{类型规则} 解决“这些字节能按什么类型读写”。\code{p[i].z} 可以按 \code{float} 读取 \code{z} 字段，但不能随便把整个对象强转成另一个不兼容结构体再读。

\textbf{地址和指针} 解决“目标字节在哪里”。如果 \code{sizeof(Particle)} 是 16，\code{z} 的 offset 是 8，那么 \code{p[i].z} 的地址就是：

\begin{lstlisting}[numbers=none]
addr(p[0]) + i * 16 + 8
\end{lstlisting}

\textbf{x86-64 寻址模式} 解决“这个地址怎样进入指令”。因为 x86-64 的 scale 只能直接取 1、2、4、8，\code{i * 16} 可能先通过移位或 \instruction{lea} 算出来，再用于 load。\textbf{cache line} 解决“取回一条缓存线后用了多少有效数据”。如果循环只读所有粒子的 \code{z}，每个 16 字节对象只用 4 字节，cache line 中其余字段对这个循环就是带宽浪费。SIMD 和 kernel 优化再进一步问：能否把所有 \code{z} 放成连续数组，让一次向量 load 读到更多有用数据。

这一段还暴露两个常见陷阱。第一，\code{sizeof(Particle)} 不是字段大小简单相加的口算结果，必须考虑 padding 和对齐。第二，有一个地址数值不等于对象生命周期存在；释放后的堆地址、越界后的地址、未构造存储上的地址，都不能因为“看起来像地址”就合法访问。后面所有内存推导都要同时问字节位置和 C++ 对象规则。
...
如果只是记住“int 通常 4 字节”“double 通常 8 字节”，本章价值很有限。更关键的是建立一条稳定链路：看到任意 C++ 表达式，能回答它访问哪些字节；看到任意内存操作汇编，能反推它可能来自什么数据布局；看到一个慢循环，能判断它是否受布局、步长、对齐或别名影响。
...
\begin{enumerate}
  ...
  \item \codeesc{struct V \{ float x, y, z, w; \}; V* v; v[i].z} 访问的字节地址如何计算？
...
注意 \term{word}{字} 是一个容易混淆的词。在 x86 历史语境里，word 常指 16 bit，dword 是 32 bit，qword 是 64 bit；在体系结构语境中，word 有时指机器自然处理宽度；在内存系统里，cache line 又是另一种粒度。读文档时必须看上下文。
...
如果这些前提只存在于作者脑子里，编译器和调用者都无法可靠利用。高质量 kernel 通常会把前提写进 wrapper、断言、文档、类型、span、参数块或专门函数名里。C++ 与汇编互操作章节，会把这种思想用于手写汇编 kernel 的边界设计。
...
\begin{enumerate}
  ...
  \item \codeesc{struct Pair \{ int a; int b; \}; Pair* p; p[i].b}
  \item \code{double a[4][8]; a[i][j]}
  \item \codeesc{struct Node \{ int key; Node* next; \}; Node* p; p->next}
\end{enumerate}
...
这四项比“把大的放前面”更接近工程现实。分析小例子时先学会手算 padding；处理生产代码时，要把 padding 分析放进访问模式和兼容性分析里。
...
这张表不是绝对规则。真正判断要看：一次循环访问哪些字段，是否顺序访问，是否需要 SIMD，是否多线程写入，是否跨 ABI，是否要和外部库共享数据。布局判断应从地址公式和 cache line 利用率出发，而不是背“布局 A 一定比布局 B 快”。
...
\code{20} 是字段 \code{d} 的 offset。这里的关键是：不要看到最后一条里只有 \code{rcx*8} 就误判结构体大小是 8；\register{rcx} 本身已经被前一条 \instruction{lea} 变成了 \code{i*3}。
...
这就是为什么“我在机器上试过能跑”不是 C++ 合法性的证据。未定义行为可能在 \code{-O0} 下看起来正常，在 \code{-O3} 下被优化器利用前提后失败。底层分析必须同时处理机器字节和语言规则，否则代码可能只在当前构建中偶然工作。
...
这份清单看起来严格，但它能防止许多高成本错误。内存 bug 的危险在于它们常常不稳定：Debug 下不复现，Release 下偶发；小输入不复现，大输入崩溃；一个实验环境不复现，换编译器后出现。审查越早把边界写清楚，后面用工具定位的成本越低。
...
\subsection{malloc/new 到内核内存的完整路径}

\code{new} 和 \code{malloc} 不是直接向 CPU 要内存，也不一定每次进入内核。C++ \code{new T} 通常分成两层：先调用 \code{operator new} 获得足够且满足对齐要求的 storage，再在这段 storage 中构造 \code{T} 对象。\code{malloc} 只返回未类型化 storage，不负责 C++ 对象构造。标准库容器又会在这些接口之上批量申请 capacity，并管理元素生命周期。

\begin{lstlisting}[numbers=none,caption={堆分配的分层路径}]
std::vector<T>::reserve
  -> allocator_traits<Alloc>::allocate
      -> operator new / malloc-like allocator
          -> size class / thread cache / arena
              -> existing free block
              -> brk/sbrk heap growth
              -> mmap large allocation
                  -> first touch page fault
\end{lstlisting}

这条链说明为什么“分配 1 MiB”不等于“立刻获得 1 MiB 物理内存”。分配器可能只在虚拟地址空间中保留一段范围，真正物理页可能等到第一次写入时才通过缺页建立。也说明为什么 \code{malloc} 时间和后续初始化时间要分开测：前者可能主要是分配器元数据和系统调用，后者可能混入 page fault、清零、cache/TLB 填充和带宽成本。
...
层次 & 它负责什么 & 常见误判 \\
\midrule
C++ 对象层 & 构造、析构、异常安全、生命周期 & 以为 \code{malloc} 创建了 \code{T} 对象 \\
分配器层 & 切块、复用、size class、arena/cache & 以为每次分配都 syscall \\
虚拟内存层 & VMA、\code{brk}、\code{mmap}、权限 & 以为有地址就有物理页 \\
缺页处理层 & 首次触页、清零、建立页表 & 把 first touch 算成算法成本 \\
硬件层 & cache、TLB、带宽、对齐 & 把分配策略和访问成本混成一个数 \\
\bottomrule
\end{tabular}
\end{center}

\subsection{size class、thread cache 和 arena}

通用分配器通常不会为每个小对象单独向内核申请页面。更常见的做法是从操作系统拿到较大的区域，再按大小类别切成块。小对象可能进入 size class；释放后的块可能进入 thread-local cache 或 arena 的 free list；大对象可能直接走 \code{mmap}。不同分配器实现细节不同，但工程判断的重点相同：分配成本取决于大小、线程、复用状态、对齐、是否跨 arena、是否触发向内核要新页面。

\begin{lstlisting}[numbers=none,caption={分配器状态机的简化模型}]
Request(bytes, align):
  normalize to size class

FastReuse:
  pop a block from thread cache or arena free list

ArenaSlowPath:
  lock or coordinate with arena metadata
  split or coalesce blocks

SystemMemory:
  grow heap with brk or map region with mmap

FirstTouch:
  user code writes page, kernel handles minor fault

Free:
  return to cache/free list, maybe release large mapping
\end{lstlisting}

这解释了三种常见现象。第一，同样的 \code{malloc(64)}，第一次可能慢，后续可能快，因为分配器和页面都热了。第二，多线程分配可能在低竞争时很好，但某些大小或跨线程释放会进入共享元数据路径。第三，释放不等于内存立刻还给操作系统；进程 RSS、虚拟地址空间和分配器可复用内存是不同指标。

\subsection{ptmalloc 和 tcache：glibc malloc 的最小心智模型}

Linux 上常见的 glibc \code{malloc} 实现属于 ptmalloc 家族。版本细节会变化，本书不要求背内部结构体字段，但必须建立一张足够工程化的地图：ptmalloc 用 arena 减少多线程全局锁争用，用 chunk 元数据描述分配块，用 bin 保存可复用空闲块，用 top chunk 表示当前堆顶剩余空间，用 \code{brk} 和 \code{mmap} 从内核取得虚拟内存范围。现代 glibc 还引入 tcache，让每个线程缓存一小批常见大小的空闲块，降低小对象分配释放的锁成本。

\begin{lstlisting}[numbers=none,caption={ptmalloc 的压缩版路径}]
malloc(bytes):
  normalize request to chunk size
  if tcache for this size has an entry:
      pop from per-thread tcache
  else if fastbin/smallbin/unsorted bin can satisfy:
      detach reusable chunk from arena bins
  else if top chunk has enough space:
      split from top chunk
  else if request is large:
      mmap a separate region
  else:
      grow heap with brk and retry

free(ptr):
  recover chunk header from user pointer
  if tcache accepts this size:
      push to per-thread tcache
  else:
      return to arena bin, maybe coalesce neighbors
  if large mmap chunk:
      munmap region
\end{lstlisting}

这段伪代码省略了大量安全检查、加锁、合并和版本差异，但足以解释现象。小对象反复分配释放可能完全命中 tcache，几乎看不到系统调用；tcache 满了或跨线程释放，路径会回到 arena；大对象可能直接 \code{mmap}，释放时可能 \code{munmap}；很多中等对象释放后留在进程里供后续复用，RSS 不一定下降。
...
组件 & 作用 & 性能或调试后果 \\
\midrule
tcache & 线程本地小块缓存 & 快，但会让释放后复用更隐蔽 \\
arena & 一组堆和 bin 元数据 & 多线程降低全局锁，但跨 arena 复杂 \\
fastbin/smallbin & 保存常见大小空闲 chunk & 分配快，碎片和合并有策略差异 \\
unsorted bin & 新释放块的过渡区 & 后续分配可能重用或整理 \\
top chunk & 当前 heap 顶部剩余空间 & 不够时可能触发 \code{brk} 增长 \\
\code{mmap} chunk & 大块独立映射 & 释放时更可能还给内核 \\
\bottomrule
\end{tabular}
\end{center}

不要把这张表当作 glibc ABI。真实分配器内部不是稳定接口，调试时要以当前 glibc 版本、源码和工具输出为准。工程上真正需要的是判断路径：这次分配是否命中线程缓存，是否进入 arena 锁，是否向内核申请新映射，是否只是复用旧块，是否因为大小类别变化导致行为突变。

\subsection{\code{brk} 和 \code{mmap}：两种向内核要虚拟内存的方式}

分配器向内核扩展地址空间常见有两类路径。\code{brk}/\code{sbrk} 调整传统 heap 的末端，把一段连续虚拟地址范围交给分配器管理；\code{mmap} 创建新的虚拟内存映射，常用于大块分配、匿名映射、文件映射和特殊对齐需求。二者拿到的是虚拟地址范围，不等价于物理页已经全部分配。

\begin{lstlisting}[language=bash,caption={观察 brk/mmap 和地址空间变化}]
strace -e trace=brk,mmap,mmap2,munmap ./alloc_probe
cat /proc/$(pidof alloc_probe)/maps
cat /proc/$(pidof alloc_probe)/smaps_rollup
\end{lstlisting}

观察分配器时，\code{strace} 有一个常见陷阱：没有看到 \code{brk} 或 \code{mmap}，不代表没有发生分配；它可能命中 tcache、bin 或已有 arena。反过来，看到 \code{mmap} 也不代表用户代码已经触碰了所有页面；真正的 page fault 可能发生在后续初始化循环。报告应把“取得虚拟范围”“分配器返回指针”“用户首次写入”“RSS 增长”分成四个时间点。
...
现象 & 可能解释 & 需要补的证据 \\
\midrule
\code{malloc} 很快但初始化慢 & 只拿到地址，首次触页在写入时发生 & page faults、触页循环对照 \\
\code{free} 后 RSS 不降 & 块留在 tcache/arena/bin 中复用 & smaps、后续复用实验 \\
大块释放后 RSS 下降 & 大块走独立 \code{mmap}/\code{munmap} & strace、maps diff \\
多线程分配抖动 & arena 锁、跨线程释放、NUMA 位置 & 线程绑定、allocator 指标 \\
\bottomrule
\end{tabular}
\end{center}

\subsection{碎片：有空闲内存但不一定有合适内存}

碎片不是“内存泄漏”的同义词。内存泄漏是对象或块不再需要却仍然不可释放；碎片是总空闲空间存在，但形状不适合未来请求，或者分散在不能合并的位置。内部碎片来自 size class 向上取整，例如请求 33 字节却占用 48 或 64 字节 chunk；外部碎片来自空闲块分散，中间夹着仍活跃的块，无法合并成大块。虚拟地址空间、RSS、allocator retained memory 和业务 live bytes 因此会分离。

\begin{lstlisting}[numbers=none,caption={碎片的最小例子}]
allocate A: 64 KiB
allocate B: 64 KiB
allocate C: 64 KiB
free A
...
total free is 128 KiB, but no adjacent 128 KiB block exists
unless allocator can grow, move, or use another mapping.
\end{lstlisting}

C/C++ 分配器通常不能移动已经返回给用户的对象，因为用户持有原地址。于是它不能像某些移动式 GC 那样紧凑内存。工程上减少碎片的方法包括：按生命周期分 arena，按大小分 pool，避免把长寿命对象和短寿命对象混在同一分配域，批量释放同生命周期对象，避免跨线程频繁释放所有权不清的对象。第三册的 activation arena 和 KV cache allocator 本质上也是在控制碎片形状。

\begin{lstlisting}[numbers=none,caption={碎片报告字段}]
fragmentation_report:
  live_bytes
  requested_bytes_total
  allocated_chunk_bytes_total
  rss_bytes
  virtual_mapped_bytes
  largest_observed_allocation
...
这份报告可以防止一个常见误判：进程 RSS 高，不一定是泄漏；也可能是 allocator 保留、碎片、page cache、mmap 文件、线程栈或 JIT/共享库映射。反过来，RSS 稳定也不证明没有泄漏，小对象泄漏可能被其它释放掩盖。要判断分配器问题，必须同时看业务 live set、分配模式、释放模式、映射和页错误。

\subsection{free 之后发生了什么}

\code{free(p)} 或 \code{delete p} 的语言后果是对象生命周期结束，storage 归还给对应分配器。它不保证字节被清零，不保证虚拟地址取消映射，不保证下一次不会返回同一个地址。分配器可能把块放入缓存，稍后复用；也可能对大块执行 \code{munmap}；也可能为了调试或安全填充特殊模式。程序不能依赖任何一种偶然行为。

\begin{lstlisting}[language=C++,caption={地址复用不是对象复活}]
auto* a = new int(1);
auto old = reinterpret_cast<std::uintptr_t>(a);
delete a;

auto* b = new int(2);
if (reinterpret_cast<std::uintptr_t>(b) == old) {
    // 地址可能相同，但这是另一个对象生命周期。
}
\end{lstlisting}

这条规则直接连接对象模型和调试工具。ASan 的 quarantine 会延迟复用释放块，所以 use-after-free 更容易被抓到；Release 分配器可能很快复用同一地址，所以 bug 表现成数据偶发变化；某些安全分配器会清零或填充释放块，所以调试现象又不同。工具改变分配器状态，因此功能诊断结论不能直接变成性能结论。

\subsection{分配器如何影响 benchmark}

如果被测目标是算法热循环，分配和释放通常应放在 setup/teardown，而不是 timed region。否则你测到的是算法、allocator、page fault、cache 状态和构造析构的混合物。反过来，如果目标就是比较分配策略，就要把 workload 写清楚：对象大小分布、分配/释放比例、线程数、跨线程释放、对象生命周期分布、是否触页、是否调用构造析构。

\begin{lstlisting}[numbers=none,caption={allocator benchmark 必须说明的字段}]
allocator_workload:
  allocator_name
  object_size_distribution
  alignment
  allocate_free_pattern
  threads
  cross_thread_free
  touch_policy: no_touch / first_byte / every_page / full_init
  constructors_destructors
  warmup_iterations
  rss_and_fault_counters
\end{lstlisting}

仅报告 \code{ops/s} 不够。小对象分配可能受 thread cache 命中率支配；大对象可能受 \code{mmap}/\code{munmap} 和 VMA 管理影响；初始化大 buffer 可能主要受写带宽和首次触页影响；释放后 RSS 不降可能是分配器保留内存以便复用。若报告没有把这些因素拆开，就无法判断是算法变慢、分配器变慢，还是测量边界错误。

\subsection{分配器证据包}

分配器问题要用多层证据，而不是只看一条崩溃栈。最小证据包可以包括：

\begin{lstlisting}[numbers=none,caption={分配器证据包}]
allocator_evidence:
  source_call_site: vector reserve / new / malloc / custom arena
  requested_bytes
  requested_alignment
  object_lifetime: constructed / raw storage / destroyed
  allocation_pattern
  deallocation_owner
  strace_mmap_brk_munmap
  proc_maps_or_smaps_snapshot
  page_faults
  rss_before_after
  sanitizer_or_debug_allocator_result
  benchmark_boundary: setup / timed / teardown
\end{lstlisting}

这份证据包会在后两册反复出现。第二册讨论线程池和队列时，allocator 可能成为锁争用和尾延迟来源；第三册讨论张量 runtime 时，activation arena、workspace、packed weight cache 和 KV cache 都是特殊分配器问题。第一册只要把分层路径讲清，读者就不会再把“有一个指针”误认为“对象、物理内存、性能成本都已经确定”。
...
ASan 能把这类错误变得更容易观察，因为它会在释放后标记内存状态，并在访问时报告。但 ASan 改变内存布局和性能，不应把 ASan 下的时间数字当成发布性能。功能诊断和性能测量要分开。这正是工具边界和测量边界的交叉点。
...
这类案例不能只靠口号决定。证据应包含结构体大小、字段 offset、cache line 有效载荷估算、关键循环汇编和 benchmark。若 SoA 版本快，要说明是减少无用字节、改善向量化、降低 stride，还是其他原因。若 AoS 版本更快，也要解释是否因为转换成本、预取、对象封装或输入规模较小。布局选择的核心不是永远选择某一种形式，而是能按访问形状推导。
...
工程报告应先写合法性，再写性能。先证明指针、范围、生命周期和类型成立；再讨论地址模式、工作集和 cache。若合法性不成立，性能结论停止。若合法性成立但性能差，再进入布局和访问模式分析。这条顺序同样适用于汇编审计和 benchmark 报告。
...
\begin{center}
\begin{tabular}{p{0.17\linewidth}p{0.17\linewidth}p{0.14\linewidth}p{0.15\linewidth}p{0.16\linewidth}}
\toprule
...
\section{对象模型、生命周期和优化边界}

只会计算字段 offset 还不够。C++ 程序访问内存时，编译器并不是只看到“某个地址上的若干字节”。它还利用对象类型、生命周期、别名规则、对齐和未定义行为来判断哪些 load/store 可以重排、删除、合并或向量化。第一册要把这层讲清，否则后面第二册的 SIMD、第三册的 tensor buffer 和量化 kernel 都会缺一块地基。

\subsection{storage、object、value 不是同一个概念}

一段内存可以先只是 storage，还没有某个类型对象的生命周期。对象生命周期开始后，这段 storage 才能按该对象类型访问。对象的 value 是语言层的值；object representation 是它在内存中的字节表示。三者不能混用。
...
概念 & 含义 & 常见错误 \\
\midrule
storage & 一段可用字节区域 & 以为有字节就一定有对象 \\
object & 某个类型对象的生命周期已经开始 & 用错误类型解引用破坏别名规则 \\
value & 语言语义中的值 & 把 padding 字节当作值的一部分比较 \\
representation & 对象落在内存中的字节 & 用 \code{memcmp} 判断含 padding 对象相等 \\
\bottomrule
\end{tabular}
\end{center}

例如 \code{std::byte buffer[sizeof(T)]} 只是 storage。只有通过合适的构造方式开始 \code{T} 的生命周期后，才能把它当作 \code{T} 访问。反过来，对象生命周期结束后，旧指针的数值可能仍然等于原地址，但它不再代表一个活着的 \code{T} 对象。无锁结构里的 ABA、对象池复用和 tensor arena 复用，本质都绕不开这条规则。

\subsection{别名规则是优化器的内存依赖模型}

若两个指针可能指向同一对象，编译器必须保守处理 load/store 顺序。若类型规则证明它们不能别名，编译器就可以把 load 提前、把 store 延后、把值保存在寄存器里，甚至向量化循环。别名规则不是“编译器小心眼”，而是它构建内存依赖图的依据。

\begin{lstlisting}[language=C++,caption={别名信息改变优化空间}]
...
若 \code{x} 和 \code{y} 可能重叠，编译器要考虑当前写 \code{y[i]} 是否会影响未来读 \code{x[j]}。若接口或运行时检查证明区间不破坏性重叠，循环就更容易向量化。第三册的 tensor descriptor 也一样：如果两个 view 共享同一底层 buffer，memory planner 和 fusion pass 必须知道 alias；如果一个输出覆盖输入，kernel 合同必须说明是安全 in-place 还是禁止重叠。

\subsection{UB 为什么会改变机器码}

未定义行为的危险不只是“程序可能崩”。更准确地说，一旦程序路径触发未定义行为，C++ 标准不再约束结果；编译器可以基于“合法程序不会触发 UB”的前提做优化。最常见的例子是有符号整数溢出、越界访问、错误类型解引用、违反对齐要求和悬垂指针。

\begin{lstlisting}[language=C++,caption={溢出前提影响循环判断}]
bool increasing_after_add(int x)
{
    return x + 1 > x;
}
\end{lstlisting}

在数学整数里这恒为真；在二进制补码回绕机器上，\code{INT\_MAX + 1} 会变成负数；但在 C++ 有符号溢出是 UB，所以编译器可以假设合法执行中 \code{x + 1} 不溢出，从而把函数优化成返回 true。若想要回绕语义，应使用无符号类型或显式溢出检查。性能工程必须尊重这条边界：不能用“我知道硬件会回绕”去反驳 C++ 优化器。

\begin{warningbox}
机器事实和语言事实不是同一层。x86-64 加法指令会产生固定宽度结果和 flags；C++ 有符号整数表达式若溢出，语言层已经越界。编译器先按语言规则优化，再生成机器指令。混淆这两层，是很多低层 bug 的根源。
\end{warningbox}

\subsection{对齐既是语义边界，也是性能边界}

对象类型通常有对齐要求。若把一个未按 \code{alignof(T)} 对齐的地址强行当作 \code{T*} 解引用，C++ 层可能已经未定义；某些硬件还可能触发异常或产生额外微架构成本。即使硬件允许未对齐访问，跨 cache line 或跨页访问也可能增加 load/store 数量和 page fault 风险。
...
访问形态 & 语言层问题 & 机器层问题 \\
\midrule
自然对齐 \code{T*} & 合法且最清楚 & 通常一条 load/store 覆盖 \\
未对齐但用字节复制 & 合法，需再解释表示 & 可能多次字节访问 \\
未对齐 \code{T*} 解引用 & 可能违反对齐要求 & 可能拆成多次访问或 fault \\
跨 cache line 向量 load & 语言可合法 & 更多 cache line 和端口压力 \\
跨页 speculative overread & 语言可能越界 & 可能 page fault \\
\bottomrule
\end{tabular}
\end{center}

这解释了为什么高性能代码常把 alignment 写进接口：\code{aligned\_allocator}、tensor arena 的 64 字节对齐、SIMD kernel 的 aligned/unaligned 路径、GPU buffer 的对齐要求，都不是风格问题，而是把语言合法性和机器效率同时固定下来。

\subsection{arena 和 placement 的正确边界}

推理 runtime 常用 arena 分配 activation buffer。arena 不是“一个大 \code{char*} 随便切”。每个切片都需要满足大小、对齐、生命周期、别名和析构规则。对平凡字节缓冲，\code{std::byte} 和 \code{std::span<std::byte>} 可以表达 storage；对真正构造对象，必须清楚生命周期开始和结束。

\begin{lstlisting}[language=C++,caption={arena 设计要记录的不只是 offset}]
TensorBufferSlice:
  base_address
  offset_bytes
  size_bytes
...
第三册 memory planner 复用 activation 时，不能只看两个 tensor 字节范围是否相同，还要看生命周期是否重叠、是否被 oracle dump 要求保留、是否有 view alias、是否跨状态边。C++ 对象模型在这里变成 AI runtime 的 correctness 规则。

\section{展开：C++ 抽象机器为什么敢不同于真实机器}

硬件只执行 load、store、add、branch；C++ 抽象机器还规定对象生命周期、类型访问、求值顺序、异常边界和未定义行为。优化器利用的是 C++ 抽象机器，不是“我眼里的汇编会怎么跑”。这就是很多底层 bug 难以理解的原因：在机器层看起来只是读了几个字节，在语言层却可能已经失去对象、类型或 provenance。

\subsection{pointer provenance：地址值相同不代表访问权相同}

工程上可以先把 provenance 理解成“这个指针值从哪个对象或分配结果来”。两个指针的整数值相同，不一定都拥有合法访问同一对象的语言层依据。把指针转成整数、做复杂运算、再转回指针，在某些实现上可能碰巧工作，但它把优化器可依赖的信息变得模糊。安全底线是：用指针表达对象内移动，用 \code{std::uintptr\_t} 只做观测或哈希，不把整数当作创建新访问权的办法。

\begin{lstlisting}[language=C++,caption={地址数值相同不等于语言访问仍合法}]
int* p = new int(7);
std::uintptr_t raw = reinterpret_cast<std::uintptr_t>(p);
delete p;

int* q = reinterpret_cast<int*>(raw); // 数值像原地址
// *q 不是合法访问：原 int 对象生命周期已经结束。
\end{lstlisting}

这个例子和无锁 ABA、对象池复用、tensor arena reuse 是同一种形状：地址可能重复出现，但对象历史不同。性能库越接近手写 allocator，越要把“地址稳定”和“对象仍然存在”分开证明。

\subsection{implicit lifetime 和 placement new：什么时候字节变成对象}

现代 C++ 对某些隐式生命周期类型提供了更友好的规则，但工程上仍应保守：原始字节 buffer 不是任意类型对象；创建非平凡对象需要构造；结束生命周期后需要析构；复用存储后旧指针可能需要重新取得对象指针。对平凡张量数据，通常把 storage 当作 \code{std::byte} 区域，再通过明确的 typed view 建立访问合同；对含构造/析构语义的 C++ 对象，不要把它们塞进 arena 后跳过生命周期。
...
场景 & 更安全的表达 & 风险 \\
\midrule
搬运 float 数据 & \code{std::span<float>} 指向已创建数组 & 越界、对齐、别名 \\
观察任意对象字节 & \code{std::span<std::byte const>} & 不能任意改类型解释 \\
构造非平凡对象 & placement new + 显式析构 & 忘析构、复用后旧指针 \\
复用 arena 切片 & 记录 lifetime begin/end & 活跃对象重叠、oracle 延寿 \\
\bottomrule
\end{tabular}
\end{center}

第三册 runtime 可以把 activation、workspace、packed weight cache 和 KV cache 都放进内存系统，但它们不是同一种生命周期。activation 可能每层复用，packed weight 跟模型生命周期一致，KV cache 跟 session 生命周期一致，workspace 跟 kernel 调用一致。若只用 \code{void* + size} 描述它们，迟早会把生命周期不同的对象混在一起。

\subsection{effective type 和 strict aliasing：优化器如何删掉你以为必要的 load}

别名规则的核心不是“C++ 禁止奇技淫巧”，而是让优化器能建立内存依赖图。若一个 \code{float*} 写入不可能改变一个 \code{int} 对象，编译器就可以把 \code{int} load 缓存在寄存器里。错误类型解引用破坏的是这个证明系统。

\begin{lstlisting}[language=C++,caption={错误类型访问可能让优化器保留旧值}]
int read_after_float_write(int* ip, float* fp)
{
    int before = *ip;
    *fp = 0.0f;
    int after = *ip;
...
若 \code{ip} 和 \code{fp} 按合法程序规则不能指向同一个 \code{int} 对象，优化器可能认为 \code{after == before}。若调用者通过强转让两者指向同一地址，就是调用者破坏了语言合同，不是优化器“错误”。需要观察或复制位模式时，用 \code{std::memcpy}、\code{std::bit\_cast} 或字节视图；需要共享 storage 时，用清楚的 union/variant/对象生命周期设计，而不是随手 \code{reinterpret\_cast}。

\subsection{未定义行为释放了哪些优化自由}

UB 的关键后果是：优化器可以假设合法程序不会走到 UB。下面几类 UB 对性能代码影响最大：

\begin{center}
\begin{tabular}{p{0.24\linewidth}p{0.34\linewidth}p{0.28\linewidth}}
\toprule
UB 来源 & 优化器可能利用的假设 & 常见误判 \\
\midrule
有符号溢出 & 算术满足数学整数单调性 & 以硬件回绕反驳优化结果 \\
越界访问 & 指针只在对象或 one-past 范围内移动 & speculative overread 越过对象边界 \\
悬垂指针 & 生命周期结束后不再访问 & 对象池复用后旧句柄仍可用 \\
违反对齐 & \code{T*} 满足 \code{alignof(T)} & 未对齐 load 只是慢一点 \\
错误类型访问 & 不相关类型不别名 & 强转改对象位模式 \\
\bottomrule
\end{tabular}
\end{center}

这些自由会进入真实机器码。例如边界检查可能被消掉，循环条件可能被简化，两个 load 可能合并，store 可能被删除。调试时若看到 \code{-O0} 正常、\code{-O3} 异常，不应先怪优化器；要先查 UB、别名、生命周期、数据竞争和未初始化值。

\subsection{反例：fast memcpy 也要有语言边界}

高性能代码常写宽 load/store，一次复制 16、32 或 64 字节。若它读取对象末尾之后的字节，即使没有跨页 fault，语言层也可能越界。库实现可以依赖实现内部特权、目标平台和严格证明；普通项目不能照抄“多读一点没事”。安全做法是把主循环限制在完整块内，tail 用合法范围内的标量或 mask 路径。

\begin{lstlisting}[numbers=none,caption={宽复制的边界账本}]
main vector loop:
  only reads bytes [0, n - vector_width] in full chunks

tail path:
  reads exactly remaining bytes
  or uses masked load with documented no-fault guarantee

never assume:
  reading past object end is fine because hardware usually tolerates it
\end{lstlisting}

这条规则会在第二册 SIMD 和第三册 GEMV kernel 中反复出现。高性能实现不是忽略边界，而是把边界拆成可证明的主循环和 tail path。

\subsection{工业取舍：内部布局可以激进，外部合同必须保守}

成熟系统会在内部做 packed layout、arena reuse、SIMD 对齐、object pool、tagged handle 和 lazy materialization，但对外接口保持保守：明确 owner、lifetime、alias、alignment、shape 和 dtype。这样优化可以在内部替换，外部调用者不需要理解每个微架构细节。

\begin{lstlisting}[numbers=none,caption={内存对象的工程描述字段}]
BufferContract:
  owner
  lifetime
  base_pointer
  ...
  mutable_or_const
  alias_set
  external_format_or_internal_layout
  debug_name
\end{lstlisting}

这个合同比裸指针繁琐，但它能直接回答事故问题：谁释放，谁还在读，是否允许 in-place，是否能给 SIMD，是否能跨线程，是否能写入文件格式，是否能被 memory planner 复用。底层工程的“高级感”不在于少写字段，而在于每个字段都对应一条正确性或性能边界。

\section{状态机：storage、lifetime、type 和 alias}

对象模型最容易被讲成若干规则。更有效的方式是把一块内存的状态写成状态机。因为工程事故通常不是“某条规则忘了”，而是对象从一个状态转到另一个状态时，旧指针、旧 view 或旧类型假设还在被使用。
...
状态 & 合法操作 & 典型错误 \\
\midrule
RawStorage & 作为字节观察、按对齐切片、等待构造 & 直接当作 \code{T*} 解引用 \\
ObjectAlive & 按对象类型访问、读写值、创建合法 view & 错误类型访问、越界、破坏性别名 \\
ObjectEnded & 释放资源、复用 storage、保留调试记录 & 旧指针继续读写，旧 span 进入 kernel \\
StorageReused & 新对象开始生命周期，旧身份失效 & 地址相同就认为对象相同 \\
BytesCopied & 表示被复制，目标是否成对象取决于类型规则 & 把 memcpy 当成任意构造机制 \\
\bottomrule
\end{tabular}
\end{center}

这个状态机把 C++ 规则和第三册 runtime 连接起来。activation arena 中，一个 buffer slice 可以从 \code{RawStorage} 进入某个 tensor view 的 \code{ObjectAlive}，在 layer 结束后进入 \code{ObjectEnded}，再被 memory planner 分配给另一个 tensor。若 oracle dump 要求保留中间值，它的生命周期就被延长；若 view alias 仍存在，复用就是 bug。memory planner 的 correctness 不是只看字节区间，而是看状态转移是否互斥。

\subsection{数学模型：区间不重叠还不够}

可以把一个 buffer view 表示成：

\[
V = (base, offset, bytes, align, type, lifetime, alias\_set)
\]

两个 view 可以复用同一段 storage，至少要满足：

\[
[offset_a, offset_a + bytes_a) \cap [offset_b, offset_b + bytes_b) = \emptyset
\]

或者在字节区间重叠时：

\[
lifetime_a \cap lifetime_b = \emptyset
\]

但这还不够。若 \code{alias\_set} 表示二者来自同一个外部输入、debug dump、KV state 或 view 链，memory planner 还要拒绝覆盖。形式化写法是：

\[
\begin{aligned}
reuse(a,b) \Rightarrow{}&
  (no\_byte\_overlap(a,b) \lor no\_lifetime\_overlap(a,b))\\
& \land no\_state\_edge(a,b)\\
& \land no\_oracle\_hold(a,b)
\end{aligned}
\]

这条公式比“节省内存”更重要。它说明了为什么优秀 runtime 会有 liveness analysis、alias analysis、state edge 和 materialization point，而不是靠一个 bump allocator 随便复用。

\subsection{反例：地址相同导致的错误复用}

\begin{lstlisting}[numbers=none,caption={同一地址、不同对象身份的错误复用}]
step 1:
  tensor A uses arena[0..4096)
  debug dump stores span<float> pointing to A

step 2:
  planner marks A dead after op 7
  tensor B reuses arena[0..4096)

step 3:
  oracle compares saved span for A
  it now reads B's bytes
\end{lstlisting}

这个错误在小模型里可能不暴露，因为 A 和 B 数值偶然接近，或者 debug 只在失败时打开。生产事故会表现为“打开诊断后结果变化”“只在某些 batch size 崩溃”“ASan 下正常但 release 错”。根因不是 allocator 慢，而是对象状态机没有把 oracle 持有的 view 算进生命周期。

\subsection{工业取舍：内部 unsafe 必须被描述器包住}

工业系统不会完全避免 \code{reinterpret\_cast}、手工对齐和 packed layout，但它们会把危险操作关在边界内：外部看到的是 \code{TensorDesc}、\code{BufferContract}、\code{Layout} 和 \code{DType}；内部 kernel 拿到的是已经验证过的指针、stride、alignment 和 tail policy。这样做的代价是描述器字段多，收益是错误可定位。

\begin{lstlisting}[numbers=none,caption={对象状态机实验的检查项}]
for every buffer reuse:
  print old tensor name, new tensor name
  print byte interval and alignment
  print lifetime begin/end op
  print alias_set and state_edge flag
  print oracle_hold flag
  reject reuse if any invariant fails
\end{lstlisting}

\subsection{习题：把对象模型用于 AI runtime}

\begin{exercisebox}
习题一：给三个 tensor \code{hidden0}、\code{qkv}、\code{attn\_out} 设计 lifetime 区间。判断哪些可以复用同一 arena 区间，哪些因为 debug dump 或 residual 依赖不能复用。

习题二：写出一个错误示例：\code{std::span<float>} 在对象生命周期结束后仍被保存。说明为什么地址打印看起来正常，语义仍然错误。

习题三：为 packed int8 weight cache 写 \code{BufferContract}。至少包含原始权重、packed 权重、scale、alignment、owner、生命周期和是否可写。

习题四：比较 \code{std::memcpy}、\code{std::bit\_cast}、placement new 和 \code{reinterpret\_cast}。分别说明它们改变的是“字节”“值”“生命周期”还是“类型解释”。
\end{exercisebox}

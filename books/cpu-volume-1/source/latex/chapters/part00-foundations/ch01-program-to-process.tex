...
如果读者已经熟悉 Linux，这两行很自然；如果不熟，后面所有工具命令都会变成黑盒。先把这两行拆开。

\term{终端}{terminal} 是你输入命令、看到文本输出的界面。终端本身不负责理解 \code{g++} 是什么，它通常启动一个 \term{Shell}{命令解释器}，比如 \code{bash}。Shell 读取你输入的一行文本，把它拆成命令名、参数、重定向、管道和环境变量，然后请求操作系统创建进程执行。

例如：

\begin{lstlisting}[language=bash,caption={一行编译命令的拆解}]
...
\code{g++} 收到的是参数数组，不是自然语言。“把 \code{main.cpp} 编译成 \code{app}”只是人对这组参数的解释。\code{-o app} 的意思是把输出文件命名为 \code{app}；如果忘了 \code{-o app}，默认输出可能叫 \code{a.out}。如果路径写错，编译器找不到文件；如果参数顺序或空格写错，工具收到的输入就不是你以为的输入。

运行：

\begin{lstlisting}[language=bash,caption={运行当前目录下的程序}]
./app
\end{lstlisting}

\code{./} 表示当前目录。只写 \code{app} 时，Shell 会去 \code{PATH} 环境变量列出的目录里找命令，通常不会自动找当前目录。初学者经常遇到两类问题：一种是 \code{app: command not found}，因为当前目录不在 \code{PATH}；另一种更危险，是机器上另一个目录里正好有同名 \code{app}，结果你运行的不是刚编译出的程序。

每条命令结束后都有退出状态。通常 \code{0} 表示成功，非 \code{0} 表示失败。可以用：

\begin{lstlisting}[language=bash,caption={查看上一条命令是否成功}]
echo $?
\end{lstlisting}

这件事和性能实验直接相关。如果编译失败，但旧的 \code{app} 文件还留在目录里，后续 \code{./app} 仍可能运行旧程序。于是你以为测的是新源码，其实测的是旧二进制。严谨脚本会在失败时停止，常见写法是：

\begin{lstlisting}[language=bash,caption={脚本中尽早暴露失败}]
set -euo pipefail
\end{lstlisting}

第一册后面大量命令都站在这个模型上：Shell 负责启动工具进程；工具读取文件和参数；工具输出文件或文本；退出状态说明工具是否成功。先把这层看清，后面的 \code{objdump}、\code{readelf}、\code{gdb} 和 \code{perf} 才不是一堆不可解释的单词。

\begin{warningbox}
底层实验的第一个校验不是看汇编，而是确认“我在哪个目录、运行了哪个命令、命令是否成功、产物文件是不是刚生成的”。否则后面所有观察都可能指向错误对象。
\end{warningbox}
...
后端把中间表示映射到具体目标。不同目标机器有不同寄存器数量、指令格式、寻址模式、调用约定和指令代价。即使源码和中间表示相同，面向 x86-64、RISC-V 或其他目标的后端也会做出不同选择。以后讨论性能时，必须把“语言层算法复杂度”和“目标机器上的实现代价”分开。
...
\section{六个阶段工具：输入、输出和常见误解}

这一章会反复出现预处理器、编译器、汇编器、链接器、加载器和运行时。它们不是同义词，也不是“编译器内部几个随便的步骤”。每一层都接收一种输入，产生一种输出，并留下不同证据。

\begin{center}
\small
\begin{tabular}{p{0.16\linewidth}p{0.20\linewidth}p{0.22\linewidth}p{0.28\linewidth}}
\toprule
阶段 & 输入 & 输出 & 常见误解 \\
\midrule
预处理器 & \code{.cpp} 加头文件和宏 & 展开后的翻译单元，常见为 \code{.i/.ii} & 以为头文件会单独编译后再合并 \\
编译器前端 & 预处理后的 C++ 文本 & AST、类型信息、诊断、IR 初稿 & 以为前端负责生成最终机器码 \\
优化器 & IR 和语言语义信息 & 改写后的 IR & 以为它必须保留源码变量和循环形状 \\
后端 & 优化后 IR、目标机器信息 & 汇编文本或机器 IR & 以为所有目标 CPU 都生成同样指令 \\
汇编器 & 汇编文本 \code{.s} & 目标文件 \code{.o}、机器码字节、符号、重定位 & 以为目标文件已经知道所有最终地址 \\
链接器 & 多个 \code{.o}、库、启动文件 & 可执行文件或共享库 & 以为链接只是把文件粘在一起 \\
加载器/动态链接器 & ELF 文件、共享库、环境 & 进程地址空间、初始栈、共享库映射 & 以为运行就是把整个文件复制进内存 \\
\bottomrule
\end{tabular}
\end{center}
...
这些阶段会被驱动程序串起来，读者看不到中间产物。学习时要有意识地停在中间：

\begin{lstlisting}[language=bash,caption={把一条构建链拆开观察}]
g++ -E main.cpp -o main.ii        # 只看预处理结果
g++ -S -O2 main.cpp -o main.s     # 只生成汇编文本
g++ -c -O2 main.cpp -o main.o     # 只生成目标文件
g++ main.o -o app                 # 链接成可执行文件
\end{lstlisting}

四个输出文件回答四类问题。\code{main.ii} 说明编译器前端真正看到哪些文本；\code{main.s} 说明后端选择了哪些汇编动作；\code{main.o} 说明汇编器已经编码出哪些机器码、符号和重定位；\code{app} 说明链接器安排后的最终二进制大概是什么样。以后遇到问题，要先判断应该停在哪一层看证据。

比如“宏为什么把表达式算了两次”要看 \code{-E} 的输出；“函数为什么没有调用开销”要看 \code{-S} 或反汇编，因为它可能被内联；“undefined reference” 要看目标文件和链接命令；“共享库从哪里加载”要看 \code{ldd}、\code{readelf -d} 和运行时环境；“第一次调用为什么慢”可能要看动态链接、首次触页和初始化，而不是只看 C++ 函数体。

\begin{keyidea}
工具链阶段就是排错地图。先判断问题发生在文本展开、语言语义、优化、机器码生成、符号链接、加载启动还是运行测量，再选对应工具。
\end{keyidea}
...
这张表里有几个新工具名，先把它们的最低含义说清楚。\code{nm} 看符号表，适合回答“二进制里有没有这个名字、这个名字是定义还是未定义”。\code{objdump} 看机器码和反汇编，适合回答“某个函数的字节被解释成哪些指令”。\code{readelf} 看 ELF 结构，适合回答“这个文件头、section、program header、符号、重定位和动态链接信息是什么”。它们都读取二进制文件，但问题不同。

例如你想确认 \code{add_i32} 是否真的进入目标文件，可以先用 \code{nm -C add.o} 看符号名。若看到 \code{T add_i32}，说明目标文件的 text 代码区里有一个全局函数符号。接着用 \code{objdump -drwC -Mintel add.o} 看这个符号下面的机器码字节和指令。若调用别的文件里的函数，再用 \code{readelf -r add.o} 看是否有重定位记录。这个顺序比“打开一大段反汇编从头读到尾”更稳。

\begin{warningbox}
\code{nm}、\code{objdump} 和 \code{readelf} 都不会告诉你“这段代码热不热”。它们只说明二进制形态。要证明运行时频率或耗时，还需要 benchmark、profiling 或计数器证据。
\end{warningbox}
...
先用一个具体问题把“前端”这个词放稳：下面这句代码如果写在函数里，编译器要怎样知道它是否合法？

\begin{lstlisting}[language=C++]
int x = a + 3;
\end{lstlisting}

它不能只看字符。字符层只知道有十来个字母和符号；编译器必须先把字符切成 token，再把 token 组织成语法结构，最后查符号表和类型规则。只有前端确认 \code{a} 已经声明、\code{a + 3} 可以转换成 \code{int}、初始化 \code{x} 合法，后端才有资格谈“生成哪几条机器指令”。如果 \code{a} 没声明，程序会在前端语义分析阶段失败，根本不会进入汇编器和链接器。
...
Token 不是最终语义。它只说明“这段字符可以分成哪些词类”。此时编译器还不知道 \code{a} 是不是变量、是不是函数、是不是宏展开后的名字，也不知道 \code{a + 3} 的类型是否能给 \code{x}。所以看到“词法分析”不要把它理解成“编译器已经懂了程序”。它只是把原始文本变成后续 parser 能消费的词流。
...
AST 的重点不是“树画得好看”，而是把嵌套关系固定下来。例如 \code{int x = a + 3 * b;} 不是从左到右随便算，而是乘法节点先成为加法节点的右子树。AST 让编译器知道哪些表达式属于同一个语句、哪个名字是声明出来的新变量、哪个名字是被引用的旧名字、哪个表达式是函数调用实参。没有这一步，编译器无法进行名字查找、重载解析、模板实例化和类型检查。

可以用 Clang 观察这个层次：

\begin{lstlisting}[language=bash,caption={观察前端语法和语义视角}]
clang++ -std=c++20 -Xclang -ast-dump -fsyntax-only main.cpp \
  > main.ast.txt
\end{lstlisting}

这份输出不适合当小说从头读到尾。正确读法是先定位目标函数名，再看函数参数、局部变量声明和关键表达式的类型。AST dump 能证明“前端把源码理解成什么语法和类型”，不能证明“最终机器码怎样执行”，更不能证明“这段代码快不快”。
...
把前端报告写清楚，至少要回答四个问题：这个名字解析到哪个声明；这个表达式的类型是什么；是否发生隐式转换；是否触碰未定义行为或实现定义行为边界。比如 \code{std::uint32_t x = a + 3;} 和 \code{int x = a + 3;} 在机器上都可能生成加法，但语言层整数规则不同；\code{int* p; p + 1} 和 \code{char* p; p + 1} 在源码里都写 \code{+ 1}，但前端知道它们的步长不同。前端不是可有可无的“语法检查器”，它决定了后面优化器能合法假设什么。
...
\begin{lstlisting}[numbers=none,caption={add\_i32 的最小 IR 形态}]
...
\begin{itemize}
  \item \code{define i32 @add_i32(...)} 表示定义一个返回 \code{i32} 的函数，函数名是 \code{add_i32}。
  \item \code{\%a} 和 \code{\%b} 是 IR 里的虚拟值，不是 x86-64 的真实寄存器名。
  \item \code{entry:} 是基本块标签，表示一段顺序执行、只从末尾跳出的控制流片段。
  \item \code{add i32 \%a, \%b} 表示对两个 32 位整数虚拟值做加法。
  \item \code{ret i32 \%sum} 表示函数返回这个虚拟值。
\end{itemize}

IR 的价值在于把源码里的复杂语法压成更显式的值流、内存流和控制流。源码里写 \code{return a + b;}，IR 会显式说“有两个输入值、一个加法结果、一个返回”。后续优化器可以在这个层面问：这个加法结果是否真的被使用？这个函数是否可以内联？这个加法是否能常量折叠？这些问题在 C++ 语法树上也能问，但 IR 更适合大规模统一处理。
...
这里“可观察用途”不是口头感觉，而是语言规则中的边界。返回值被打印、写入外部可见内存、参与 \code{volatile} 访问、影响原子操作、触发异常或通过调用边界产生副作用，通常都可能成为可观察行为。只是在循环里算出一个局部变量，然后不再使用，优化器可以把它删掉。要证明循环没有被删，不能只看源码里还写着循环，必须保存 IR 或反汇编。

一个最低检查流程是：

\begin{lstlisting}[language=bash,caption={确认 benchmark 热循环没有消失}]
clang++ -std=c++20 -O3 -S -emit-llvm bench.cpp -o bench.ll
clang++ -std=c++20 -O3 -S -masm=intel bench.cpp -o bench.s
rg "sum|for.body|vector.body" bench.ll bench.s
\end{lstlisting}

如果 IR 和汇编里找不到循环形态，先怀疑结果没有被消费、输入是编译期常量、函数被内联后整体折叠，或者计时代码本身写错。不要直接拿时间数字下结论。
...
现在不要求你完全读懂每一项。先建立一个核心模型：可执行文件是操作系统加载器能理解的一种结构化文件；ELF、重定位、动态链接和加载器细节都可以沿这个模型继续细化。
...
\subsection{第一次读 ELF：先回答四个问题}

第一次看 \code{readelf} 输出，不要从第一行背到最后一行。先用它回答四个窄问题。

\begin{enumerate}
  \item 这个文件是什么身份：可执行文件、共享库还是目标文件？看 \code{readelf -h} 和 \code{file app}。
  \item 加载器会映射哪些范围，权限是什么？看 \code{readelf -l app} 里的 \code{LOAD} 和 \code{R/E/W} 权限。
  \item 链接和调试工具怎样组织代码、数据和符号？看 \code{readelf -S app} 里的 \code{.text}、\code{.rodata}、\code{.data}、\code{.bss}、\code{.symtab}。
  \item 动态运行还依赖谁？看 \code{readelf -d app} 里的 \code{NEEDED}，再用 \code{ldd ./app} 或 \filepath{/proc/<pid>/maps} 验证实际路径。
\end{enumerate}

注意 section 和 segment 的区别。section 是文件内部给链接器、调试器和分析工具看的分类；segment 是加载器实际映射进进程地址空间的单位。一个 \code{.text} section 通常会落进某个可读可执行的 \code{LOAD} segment。程序运行时，CPU 不会说“我要执行 \code{.text} section”，它只会从某个虚拟地址取指；这个地址所在的映射权限来自加载后的段和页表。

重定位也要用问题驱动来读。目标文件里的 \code{call 0} 不一定表示调用地址 0，而可能表示“这里还等链接器填地址”。\code{readelf -r add.o} 能告诉你哪些位置需要修补，\code{objdump -dr add.o} 能把反汇编和重定位放在一起看。若把目标文件占位地址当成最终运行地址，后面分析调用路径一定会错。
...
\subsection{虚拟地址不是物理内存编号}

指针里看到的地址通常是进程虚拟地址。它更像“这个进程内部的地址名字”，不是内存条上的真实物理编号。同一个虚拟地址数值，在两个不同进程中可以指向完全不同的物理页；同一个可执行文件运行两次，也可能因为 ASLR 得到不同的栈、堆和共享库地址。

\filepath{/proc/<pid>/maps} 是第一本书阶段最实用的观察入口。它的一行可能类似：

\begin{lstlisting}[numbers=none,caption={一行进程地址映射的读法}]
00400000-00452000 r-xp 00000000 08:01 123456 /tmp/app
\end{lstlisting}

这行可以拆成：虚拟地址范围是 \code{0x00400000} 到 \code{0x00452000}；权限是可读、可执行、私有映射；文件偏移从 \code{0} 开始；来源文件是 \filepath{/tmp/app}。如果 GDB 中 \register{rip} 落在这个范围内，说明当前正在执行这个映射中的代码。若某个崩溃地址落在没有映射的范围，可能是无效指针；若落在 \code{rw-p} 但当前在取指，可能是执行了不可执行内存。

这也说明为什么“地址每次变了”不等于“对象错了”。地址数值变化可能只是 ASLR 或动态库加载基址变化。要判断对象是否正确，要结合对象生命周期、指针来源、映射范围、权限和当前指令访问类型，而不是只看十六进制数字是否和上次一样。
...
读完本章后，不应只记住“源码会变成可执行文件”。更重要的是留下三类证据接口：机器码来自哪个二进制，地址属于哪个映射区间，符号和重定位怎样把多个目标文件接成一个进程。这三类接口会反复出现在调试、ABI 审计、崩溃分析和性能测量里。

分析汇编时，寄存器传参、栈对齐、返回地址、调用者保存和被调用者保存规则不是某个编译器的随意选择，而是二进制模块互相调用所需的约定。没有这些约定，不同源文件、不同库甚至不同语言写出的函数就无法在机器码层可靠协作。

做性能测量时，加载、运行时启动和进程资源必须从热路径里分离出来。计时区域不能混入动态链接、首次缺页、输出、输入准备、分配和结果验证。否则，统计方法再规范也只能放大错误实验边界。

高性能 kernel 往往以库函数、模板实例化、ISA 分发和动态链接的形式出现。报告必须确认当前机器执行的是哪一个实现，确认编译器是否生成了目标指令，确认数据是否按预期布局，确认测量没有混入加载和调度噪声。越往底层优化走，越要先确认“正在分析的进程”确实来自目标源码和目标二进制。
...
这个案例说明，程序不是一个抽象名词，而是一组具体产物和运行条件。底层学习从第一个实验开始就要求读者建立这种意识：任何现象都要先绑定到明确对象。后面读汇编、查 ABI、测性能，都要继承这个习惯。
...
缺少这些底线，所有底层分析都会变弱。你不能稳定生成同一类二进制，就无法比较汇编；不能确认加载库，就无法讨论 ABI；不能复现输入，就无法比较性能。可复现性不是发布工程才需要的事，从第一个实验开始就应训练。
...
身份确认不是慢动作，而是加速器。它让你少走弯路，也让报告更可信。这个习惯会一直延伸到可信测量部分：可信性能数据同样需要可信程序身份。
...
当实验结果需要交给别人复查时，身份清单就是最小交接协议。它让源码、构建、二进制、输入和运行环境之间的关系保持可追踪，也让每一条证据都有明确归属。
...
本章的验收不能只停在“知道有 ELF”。下面是一台 Linux x86-64 环境上对一个最小程序的证据片段。程序中保留一个 \code{noinline} 的 \code{add\_noinline}，目的是让符号和反汇编中有可追踪目标。
...
nm -C tiny:
  0000000000401180 T add_noinline
  0000000000401040 T main

objdump:
  40104d: e8 2e 01 00 00  call 401180 <add_noinline>

0000000000401180 <add_noinline>:
  401180: 8d 04 37  lea eax, [rdi+rsi]
  401183: c3        ret
\end{lstlisting}

这里可以把 C++、ABI 和机器指令连起来：System V x86-64 的前两个整数参数在 \register{rdi}/\register{rsi}，返回值在 \register{eax}/\register{rax}，\instruction{call} 把返回地址压栈并跳转，\instruction{ret} 返回调用点。若优化器把函数 inline 掉，\code{nm} 和 \code{objdump} 会出现不同证据；这不是工具坏了，而是优化状态变了。
...
只讲“源码变成汇编”仍然不够。真实系统事故经常发生在更深的连接处：C++ 抽象机给优化器提供了什么前提；IR 如何保存别名、异常和副作用；ELF 元数据如何影响加载、符号解析和回溯；运行时库、分配器、线程库和内核边界如何改变一次请求的成本。本节把这些深水区压成一张证据表，方便在具体问题中逐项核对。
...
每次解释一个底层问题，至少要跨三层。解释优化，必须有 C++ 规则、IR 或优化 remark、反汇编；解释链接，必须有符号、重定位、最终 ELF；解释运行时，必须有 \code{/proc/<pid>/maps}、动态库或 syscall/page fault 证据；解释崩溃，必须有寄存器、地址、对象生命周期和符号化栈。

这样做会让章节变厚，但厚度来自必要证据。系统训练不能满足于“知道一个概念”，而要在源码、二进制、运行时和机器状态之间来回闭合。
...
信号是必须单独建模的系统边界。控制流边界可以分成普通调用、系统调用、同步异常、异步中断、signal delivery、C++ unwind 和调度切换；本节用同一组问题看 signal。signal 位于这张表的交汇处：它可能由同步异常产生，例如非法内存访问；也可能由异步事件产生，例如终端、定时器或另一个进程；最终又通过内核改写用户态返回上下文，让用户代码先进入 handler。

因此，signal 不能被理解成“操作系统帮我调用一个 C++ 回调”。它不是普通函数调用，也不是用户态显式执行 \code{syscall} 的结果。内核在合适的返回用户态时机，把线程上下文改造成“先执行 signal handler，再回到原来位置或终止进程”。这就是异步控制流：它打断的可能是热循环、阻塞系统调用、库函数、内存分配器或 C++ 对象更新中间状态。

系统分析 signal 时要先问六件事：

\begin{enumerate}
  \item 信号是谁产生的：硬件异常、\code{kill}、终端、定时器、管道错误，还是内核事件。
  \item 信号面向进程还是具体线程，当前线程 mask 是否屏蔽它。
  \item 它何时从 pending 变成 delivery，内核选择了哪个线程。
  \item handler 使用普通用户栈还是 alternate signal stack。
  \item handler 返回后，原系统调用是成功、\code{EINTR}、重启，还是进程终止。
  \item 证据来自 \code{siginfo}、\register{rip}、fault address、\filepath{/proc/<pid>/status}、\code{strace} 还是 core dump。
\end{enumerate}
...
\subsubsection{signal 专用报告模板}

signal 章节的验收不是“知道几个信号名”，而是能把一次信号事故写成完整报告。最小模板如下：

\begin{lstlisting}[numbers=none,caption={signal 事故报告模板}]
signal_report:
  signal:
    signo:
    name:
    default_action:
    origin: hardware_fault / kill / terminal / timer / pipe / kernel

  target:
    process_or_thread:
    signal_mask_before:
    pending_state:
    disposition: default / ignore / handler / sigwait / signalfd

  delivery:
    delivered_thread:
    interrupted_state: user_hot_loop / blocking_syscall / library_code
    interrupted_rip:
    interrupted_rsp:
    stack: user_stack / alternate_signal_stack
    siginfo:
    ucontext:

  handler_contract:
    async_signal_safe_actions_only:
    writes_flag_or_fd:
    forbidden_work_observed:

  after_handler:
    rt_sigreturn:
    syscall_result: completed / EINTR / restarted / partial_progress
    process_result: resumed / terminated / core_dumped

  evidence:
    gdb_core:
    strace_signal_path:
    proc_status_masks:
    maps_and_fault_address:
    sanitizer_or_reproducer:
\end{lstlisting}

这个模板会逼出关键区别：\code{SIGSEGV} 来自当前线程的同步异常，通常有 fault address；\code{SIGTERM} 往往是外部管理器请求退出，可能投递给任意未屏蔽线程；\code{SIGPIPE} 是写路径的协议事件；\code{SIGALRM} 或 profiling 信号会周期性打断普通控制流。它们都叫 signal，但工程处理策略完全不同。
...
\section{第一册样板链路：FirstTrace-CPU}

前面的章节已经给出许多机制，但第一册要变强，必须有一条固定样板链路反复训练。这里定义一个极小实验 \term{FirstTrace-CPU}{第一册贯穿证据链实验}。它只包含两个翻译单元，却能同时暴露 C++ 语义、优化器视野、ABI、机器码、符号、重定位、链接和运行时边界。

这个实验故意不用复杂业务。复杂业务会让读者把注意力放在算法上；第一册的目标是先把“从源码到进程”的证据链钉牢。

\subsection{固定源码：必须拆成两个翻译单元}

第一个文件提供定义：

\begin{lstlisting}[language=C++,caption={add\_i32\_def.cpp}]
extern "C" int add_i32(int a, int b)
{
    return a + b;
}
...
\begin{lstlisting}[language=C++,caption={add\_i32\_caller.cpp}]
extern "C" int add_i32(int a, int b);

extern "C" int call_add(int x)
{
    return add_i32(x, 5);
}
\end{lstlisting}

拆成两个翻译单元不是形式主义。如果把定义和调用放在同一个文件里，优化器能直接看到 \code{add\_i32} 的函数体，\code{call\_add} 很可能被改写成：
...
这时你看不到外部调用、未定义符号和 \code{R\_X86\_64\_PLT32} 重定位。若本实验的目标是训练链接证据，单文件版本就是错误实验设计。

\begin{keyidea}
实验输入必须保留你要观察的机制。想观察跨翻译单元调用，就不能把函数体暴露给同一个翻译单元的优化器；想观察动态链接，就不能静态链接后还声称在看 lazy binding；想观察 page fault，就不能提前把页面全部预热。
\end{keyidea}

\subsection{第一层：C++ 语义合同}

源码层能证明的事实很少，但很关键：

\begin{itemize}
  \item \code{add\_i32} 接收两个 \code{int}，返回一个 \code{int}。
  \item \code{return a + b;} 的有符号溢出仍属于 C++ 未定义行为边界。
  \item \code{extern "C"} 只改变语言链接名，不改变函数调用 ABI、整数宽度或溢出规则。
  \item \code{call\_add} 的当前翻译单元只能看见 \code{add\_i32} 声明，看不见函数体。
\end{itemize}

这层不能证明参数在哪个寄存器，不能证明有 \instruction{call} 或 \instruction{jmp}，也不能证明最终有独立机器码。它只给后续层提供语言合同。

\subsection{第二层：LLVM IR 证据}

用 Clang 可以生成优化后的 LLVM IR：

\begin{lstlisting}[numbers=none,caption={生成 IR}]
clang++ --target=x86_64-linux-gnu -std=c++20 -O2 \
  -S -emit-llvm add_i32_def.cpp -o add_i32_def.ll

clang++ --target=x86_64-linux-gnu -std=c++20 -O2 \
  -S -emit-llvm add_i32_caller.cpp -o add_i32_caller.ll
\end{lstlisting}

定义方的关键 IR 形态可能是：

\begin{lstlisting}[numbers=none,caption={定义方 IR 的关键片段}]
define dso_local i32 @add_i32(i32 noundef %0, i32 noundef %1) {
  %3 = add nsw i32 %1, %0
  ret i32 %3
}
\end{lstlisting}
...
\begin{lstlisting}[numbers=none,caption={调用方 IR 的关键片段}]
define dso_local i32 @call_add(i32 noundef %0) {
  %2 = tail call i32 @add_i32(i32 noundef %0, i32 noundef 5)
  ret i32 %2
}
\end{lstlisting}

这里有两个训练点。第一，\code{add nsw} 里的 \code{nsw} 不是装饰，它表示 no signed wrap，来自 C++ 有符号溢出未定义这个语言前提。第二，调用方 IR 里仍是对 \code{add\_i32} 的调用，因为当前翻译单元没有函数体。若打开 LTO 或把定义放入头文件，IR 和最终机器码可能完全不同。

\subsection{第三层：汇编证据}

生成 x86-64 Intel 语法汇编：

\begin{lstlisting}[numbers=none,caption={生成 x86-64 汇编}]
clang++ --target=x86_64-linux-gnu -std=c++20 -O2 \
  -S -masm=intel add_i32_def.cpp -o add_i32_def.s

clang++ --target=x86_64-linux-gnu -std=c++20 -O2 \
  -S -masm=intel add_i32_caller.cpp -o add_i32_caller.s
\end{lstlisting}

定义方可能生成：

\begin{lstlisting}[numbers=none,caption={add\_i32 的典型汇编}]
add_i32:
  lea eax, [rdi + rsi]
  ret
\end{lstlisting}
...
\begin{lstlisting}[numbers=none,caption={call\_add 的典型汇编}]
call_add:
  mov esi, 5
  jmp add_i32       # TAILCALL
\end{lstlisting}

这不是普通 \instruction{call}，而是尾调用。因为 \code{call\_add} 调用 \code{add\_i32} 后没有剩余工作，编译器可以把它变成 \instruction{jmp}。这个结果非常适合作为第一册样板：读者会发现“跨函数调用”不一定在汇编里表现为 \instruction{call}；必须根据 ABI 和控制流语义判断。

\begin{warningbox}
不要把“源码里写了函数调用”机械翻译成“汇编里一定有 call”。优化器可能内联、尾调用、常量传播、删除或改变调用形态。报告里必须写“当前构建下观察到的调用形态”，不能把源码意图当机器事实。
\end{warningbox}

\subsection{第四层：目标文件、机器码和重定位}

生成目标文件：

\begin{lstlisting}[numbers=none,caption={生成目标文件并观察}]
clang++ --target=x86_64-linux-gnu -std=c++20 -O2 \
  -c add_i32_def.cpp -o add_i32_def.o

clang++ --target=x86_64-linux-gnu -std=c++20 -O2 \
  -c add_i32_caller.cpp -o add_i32_caller.o

llvm-objdump --triple=x86_64 -dr --x86-asm-syntax=intel add_i32_caller.o
llvm-nm -C add_i32_caller.o
readelf -r add_i32_caller.o
\end{lstlisting}

调用方目标文件的关键输出可以是：

\begin{lstlisting}[numbers=none,caption={调用方目标文件的机器码和 relocation}]
0000000000000000 <call_add>:
   0: be 05 00 00 00        mov esi, 0x5
   5: e9 00 00 00 00        jmp 0xa <call_add+0xa>
      0000000000000006: R_X86_64_PLT32 add_i32-0x4
\end{lstlisting}

符号表会显示：

\begin{lstlisting}[numbers=none,caption={调用方符号表}]
                 U add_i32
...
\begin{lstlisting}[numbers=none,caption={调用方重定位表}]
Relocation section '.rela.text' contains 1 entry:
  Offset     Type             Sym. Name + Addend
  00000006   R_X86_64_PLT32   add_i32 - 4
\end{lstlisting}

这三份输出共同证明：

\begin{itemize}
  \item \code{call\_add} 在这个目标文件中有独立机器码。
  \item \code{add\_i32} 在调用方目标文件中是未定义符号，需要链接器以后解析。
  \item \code{jmp} 后面的 4 字节全 0 不是最终目标地址，而是等待重定位修正的占位。
  \item \code{-0x4} 或 \code{-4} addend 来自 x86 rel32 以“下一条指令地址”为基准的编码规则。
\end{itemize}

如果报告只贴反汇编，不贴 \code{readelf -r}，就不能解释这个 0。若报告看到 \code{jmp 0xa} 就说“跳到函数末尾”，也是错误的，因为目标文件层的占位还没有被链接器修正。

\subsection{第五层：链接后的二进制}

在可链接的 x86-64 环境中，可以继续生成可执行文件：

\begin{lstlisting}[numbers=none,caption={链接后继续观察}]
clang++ -std=c++20 -O2 add_i32_def.cpp add_i32_caller.cpp driver.cpp -o app

file app
readelf -h -l -d app
nm -C app | grep -E 'add_i32|call_add|main'
objdump -drwC -Mintel app | grep -A8 -E '<add_i32>|<call_add>'
ldd ./app
\end{lstlisting}

链接后要重新解释证据。目标文件里的 \code{U add\_i32} 应该已经被某个定义或动态符号解决；\code{R\_X86\_64\_PLT32} 可能消失，也可能转换为动态链接相关的 PLT/GOT 路径；最终可执行文件有 ELF header、program headers、entry point、interpreter 和动态段。此时你可以讨论二进制文件如何被加载，但还不能声称某次运行已经触发了 lazy binding 或 page fault，除非继续保存运行时证据。

\subsection{第六层：进程和运行时证据}

运行时证据至少包括：

\begin{lstlisting}[numbers=none,caption={运行时证据命令}]
./app
cat /proc/$PID/maps
cat /proc/$PID/status | grep -E 'VmRSS|Threads|SigBlk|SigCgt'
LD_DEBUG=libs,bindings ./app 2>ld-debug.txt
strace -f -ttT -e trace=execve,mmap,mprotect,brk,munmap ./app
\end{lstlisting}

这些命令回答的是另一层问题：可执行文件被哪些虚拟地址映射；动态链接器加载了哪些共享库；是否发生 \code{mmap}、\code{mprotect}、\code{brk}；进程有多少线程；信号 mask 状态如何。它们不能替代 IR 或反汇编，但能解释启动、冷路径和系统边界。

如果当前实验环境不是 x86-64，仍然可以做两件事：第一，使用 \code{--target=x86\_64-linux-gnu} 生成 x86-64 目标文件，训练 IR、汇编、符号和重定位证据；第二，用当前目标生成可运行程序，训练 \code{/proc}、\code{strace}、loader 和 core dump 证据。报告要明确区分“交叉目标对象证据”和“当前目标运行证据”，不能混写。

\subsection{统一报告模板}

FirstTrace-CPU 的报告模板如下：

\begin{lstlisting}[numbers=none,caption={FirstTrace-CPU 报告模板}]
first_trace_cpu_report:
  source:
    files:
    why_two_translation_units:
    language_contract:
  ...
  assembly:
    def_asm:
    caller_asm:
    abi_argument_mapping:
    call_shape: call / tail_jmp / inline / deleted

  object:
    file_identity:
    symbols:
    disassembly_with_bytes:
    relocations:
    evidence_boundary:
...
这就是第一册的硬标准：每个结论都要标注它来自哪一层。语言层说 \code{add\_i32} 有两个 \code{int} 参数；IR 层说调用方是否仍保留调用；汇编层说是 \instruction{call} 还是尾 \instruction{jmp}；目标文件层说符号和重定位是否存在；链接层说最终地址和动态链接如何成立；运行层说进程怎样被加载和执行。

\subsection{第七层：运行时系统证据包}

很多报告在链接层就停住：有 ELF、有符号、有反汇编，于是以为“程序如何执行”已经讲完。真实运行还要经过动态加载器、C 运行时、线程库、调试信息、异常展开、系统调用、缺页和信号。第一册若只停在“源码到汇编/ABI”，读者能看懂机器码，却仍解释不了冷启动慢、GDB 看不到变量、C++ 析构为何能穿过异常、\code{clock\_gettime} 为什么有时不像系统调用、\code{SIGSEGV} 为什么不是普通回调。

运行时系统证据包要回答的是另一组问题：

\begin{lstlisting}[numbers=none,caption={RuntimeEvidencePack 的问题边界}]
RuntimeEvidencePack:
  loader:
    which interpreter mapped the program
    which DSOs were loaded
    which relocations were resolved before main
    whether lazy binding remains

  initialization:
    which code runs before main
    init_array and global constructors
    TLS setup and thread runtime setup

  debugger_view:
    line table
    variable locations
    frame unwind rules
    build-id and separated debug file identity
  ...
  kernel_boundary:
    libc wrapper
    syscall instruction or vDSO path
    errno and cancellation point
    page fault and signal delivery
\end{lstlisting}

这份清单把证据层补齐：源码语义说明“应该发生什么”；IR 说明优化器如何重写；目标文件说明还欠哪些符号和重定位；动态加载说明进程启动前后哪些地址被映射和修正；调试器说明机器状态怎样还原成源码视图；异常运行时说明栈怎样被展开；系统调用和异常说明控制流怎样越过用户态边界。

\subsubsection{动态加载：不是 readelf 静态输出的重复}

\code{readelf -d} 能说明二进制声明了哪些 \code{NEEDED} 库、解释器和动态段条目，但它不能证明某次运行中动态链接器实际搜索了哪些路径、加载了哪些具体文件、懒绑定是否发生、第一次调用是否触发了 resolver。运行证据必须来自运行时工具。

\begin{lstlisting}[numbers=none,caption={动态加载运行证据}]
# 静态身份：这个二进制要求哪个 interpreter 和哪些动态库。
readelf -l ./app | grep 'Requesting program interpreter'
readelf -d ./app | grep -E 'NEEDED|RUNPATH|RPATH|BIND_NOW|FLAGS'

# 运行路径：动态链接器实际装载和绑定了什么。
LD_DEBUG=libs,bindings ./app 2>ld-debug.txt
LD_BIND_NOW=1 ./app

# 进程地址空间：装载后真正映射到了哪些地址。
cat /proc/$PID/maps
\end{lstlisting}

结论要分层写。看到 \code{NEEDED libc.so.6}，只能说明二进制有这个动态依赖；看到 \code{LD\_DEBUG=libs} 输出，才能说明动态链接器在哪些目录找到具体文件；看到 \code{LD\_DEBUG=bindings}，才能讨论某个符号是否懒绑定；看到 \filepath{/proc/<pid>/maps}，才能讨论该次运行中各段地址、ASLR 和共享库映射。把这些混成一句“动态链接会加载库”，对性能分析不够。

\subsubsection{main 之前：初始化代码要单独计时}

程序不是从 \code{main} 第一行裸启动。内核把控制权交给 ELF interpreter 或入口代码；动态链接器装载依赖库并处理必要重定位；C 运行时建立调用 \code{main} 所需的环境；\code{.init\_array} 和 C++ 全局对象构造可能已经打开文件、分配内存、注册插件或初始化日志。若 benchmark 把进程启动时间当 kernel 时间，就会把这些成本算进算法。

\begin{lstlisting}[numbers=none,caption={main 之前成本的观察入口}]
readelf -x .init_array ./app
nm -C ./app | grep -E '__libc_start_main|_start|GLOBAL__sub_I'
strace -f -ttT -e trace=execve,mmap,mprotect,brk,openat,read ./app
perf stat -e page-faults,minor-faults,major-faults ./app
\end{lstlisting}

报告中至少要区分三段时间：进程启动到进入 \code{main}；\code{main} 中初始化到热循环开始；热循环本身。第一段受 loader、全局构造和冷 page fault 影响；第二段受业务初始化、内存分配和文件读取影响；第三段才适合分析指令、cache 和算法路径。若只给一个 elapsed time，读者无法判断慢在运行时系统还是核心代码。

\subsubsection{调试器视图：源码变量不是机器事实}

优化后调试最容易误判。GDB 显示 \code{optimized out}，不是因为变量“消失得不合理”，而是源码变量没有稳定机器位置。一个变量可能在某个 PC 范围内位于寄存器，另一个范围内变成常量，第三个范围内被拆成多个 SSA 值，最后完全没有可恢复位置。DWARF location list 才是调试器能否显示它的依据。

\begin{lstlisting}[numbers=none,caption={调试器视图证据}]
clang++ -O2 -g -fno-omit-frame-pointer app.cpp -o app
llvm-dwarfdump --debug-line app | head -80
llvm-dwarfdump --debug-info app | grep -E 'DW_TAG_variable|DW_AT_location'
readelf --debug-dump=frames app | head -120
objdump --dwarf=decodedline app | head -80
\end{lstlisting}

一份严肃报告不能只贴 GDB 截图。它要写清：当前二进制是否带 \code{-g}；是否省略 frame pointer；是否有 \code{.eh\_frame} 或 \code{.debug\_frame}；源码行和机器地址是否因为优化而多对一或一对多；变量位置是否有 location list。调试器是证据解释器，不是运行时事实本身。

\subsubsection{异常展开：析构不是自动发生的魔法}

C++ 异常路径是第一册必须补厚的运行时边界。源代码里 \code{throw} 看起来像控制流跳转，机器层面却要构造异常对象、调用运行时入口、沿栈查找 handler、运行 cleanup landing pad、执行析构，最后进入匹配的 \code{catch} 或调用 \code{terminate}。这条链依赖 ABI、unwind table 和运行时库。

\begin{lstlisting}[numbers=none,caption={异常展开证据包}]
clang++ -O2 -g exception_demo.cpp -o exception_demo
nm -C exception_demo | grep -E '__cxa_throw|personality|Unwind|terminate'
readelf -S exception_demo | grep -E 'eh_frame|gcc_except_table'
readelf --debug-dump=frames exception_demo | head -120
objdump -drwC -Mintel exception_demo | grep -E '__cxa_throw|landing|personality'
\end{lstlisting}

正确结论应该像这样写：源码层有一个离开作用域需要析构的对象；IR 或汇编层存在异常边或 cleanup 代码；ELF 层有展开信息和异常表；运行时层会通过 personality routine 判断 handler 和 cleanup；手写汇编或 C ABI 边界若没有可展开描述，就可能切断回溯或破坏析构。这样读者才会理解“禁止异常穿过某些边界”不是编码偏好，而是 ABI 合同。

\subsubsection{系统调用、vDSO 和 page fault：三种不同的内核边界}

用户态进入内核不只有显式 \instruction{syscall}。调用 \code{write} 常经过 libc wrapper 和 syscall ABI；调用 \code{clock\_gettime} 可能走 vDSO，避免真正陷入内核；访问尚未建立 PTE 的地址会触发 page fault，由内核修复或转成信号。这三条路径都可能出现在性能报告里，但含义完全不同。
...
路径 & 关键机制 & 证据边界 \\
\midrule
libc syscall wrapper & 普通函数 ABI 到 syscall ABI 的适配、\code{errno}、取消点 & \cmd{strace}、libc 反汇编、源码阅读 \\
vDSO & 内核映射只读数据和小段用户态代码，减少陷入内核 & \filepath{/proc/self/maps}、\code{linux-vdso.so.1}、\cmd{strace} 是否缺少调用 \\
minor page fault & 虚拟页合法，但 PTE 尚未建立或 COW 需要复制 & \cmd{perf stat -e minor-faults}、maps、触页实验 \\
major page fault & 需要等待磁盘或后端 I/O 取页 & \cmd{perf stat -e major-faults}、I/O trace \\
protection fault & 访问权限不允许，可能转成 \code{SIGSEGV} & fault address、\register{rip}、maps、core dump \\
\bottomrule
\end{tabular}
\end{center}

因此，报告里不能把“进入内核”写成一个桶。一次显式 \code{write} 的成本可能主要是用户态复制、VFS 和 page cache；一次 \code{clock\_gettime} 可能没有真正 syscall；一次普通 load 可能因为文件映射冷页而睡在缺页路径上。第一册读者必须学会把这些边界拆开，否则后面做 p99、I/O 和服务化时会把系统成本误判成算子成本。

\subsubsection{统一运行时报告模板}

把这组内容压成一份报告，能防止结论漂浮：

\begin{lstlisting}[numbers=none,caption={RuntimeEvidencePack 报告模板}]
runtime_evidence_pack:
  ...
  debugger_and_unwind:
    ...
    frame_pointer:
    eh_frame:
    variable_location_example:
    backtrace_quality:

  exception_path:
    throw_sites:
    cleanup_or_destructor_path:
    personality:
    exception_tables:
    boundary_where_exception_must_not_cross:

  kernel_boundary:
    syscall_trace:
    vdso_usage:
    page_fault_counts:
    signal_or_core_evidence:

  measurement_boundary:
    process_start_to_main:
    init_to_hot_loop:
    hot_loop_only:
    first_iteration_excluded:
...
这份模板比“源码语义 -> 汇编”更长，但它正是第一册走向完整系统书的缺口。第一册的最终能力不是背工具命令，而是知道每个工具证明哪一层事实，并能把语言、编译器、二进制、加载器、调试器、异常运行时和内核边界放进同一条因果链。

\subsection{五个必须写进报告的反例}

第一，单翻译单元反例。把定义和调用放进同一个文件，\code{call\_add} 可能变成 \code{lea eax, [rdi + 5]}。这证明优化器视野改变了实验对象。

第二，优化级别反例。用 \code{-O0} 可能看到保守栈槽和普通调用；用 \code{-O2} 可能看到尾调用。不能用 Debug 形态解释 Release 性能。

第三，重定位反例。目标文件中的 \code{e9 00 00 00 00} 不是最终跳转目标。必须结合 \code{R\_X86\_64\_PLT32 add\_i32-0x4} 解释。

第四，工具目标反例。在非 x86-64 主机上用默认 \code{clang++} 可能生成另一种目标对象；要观察 x86-64，就必须写清 \code{--target=x86\_64-linux-gnu} 和反汇编 triple。

第五，运行时反例。目标文件证据不能证明某次运行的动态链接、缺页或信号路径。运行事实必须来自 \code{/proc}、\code{strace}、\code{LD\_DEBUG}、GDB/core 或性能计数器。

到这里，第一册的训练对象就清楚了：不是“会背编译流程”，而是能把一个小函数在不同层中的身份逐层追踪，并知道每层证据能证明什么、不能证明什么。

...
第四，它要尽量保持长期稳定。随着项目发展，baseline 也可能修 bug 或改接口；这时要记录 baseline version。大型项目常常需要多个 baseline：reference baseline 用于正确性，simple baseline 用于解释机制，production baseline 用于工程比较。
...
硬编码常量适合入门示例，但 benchmark suite 应该能从命令行配置。
...
\section{四个基础 kernel 的使用边界}

前面已经给出 \code{sum_u64}、\code{copy_u64}、\code{fill_u64} 和 \code{dot_f32} 的派生指标口径。这里不再重复公式，只强调它们为什么要一起出现在同一个 benchmark suite 中：\code{sum_u64} 代表只读 streaming，\code{copy_u64} 代表读写流量，\code{fill_u64} 代表写密集初始化，\code{dot_f32} 代表读流量和浮点计算混合。四者一起测，才能区分计时框架问题、内存带宽问题、写分配问题和计算/归约问题。
...
这种写法一开始就限定了报告回答什么、不回答什么。不要把结论写成“优化版更快”这种无边界句子。
...
报告中可以只摘要关键字段，完整输出放进 \filepath{environment.txt}。摘要的作用是快速判断这份报告与另一台机器是否可比；完整文件的作用是后续复查。
...
第五，能同时服务基础分析和生产维护。前者需要看清变量如何影响结果，后者需要避免性能回归。二者都依赖同一个基础：协议清楚、数据可复核。
...
\section{从基础测量过渡到生产测量}

第一册的 benchmark 目标是基础可信性，不要求一开始就达到大型性能实验平台的复杂度。但基础实验应保留通向生产的接口：显式配置、raw 数据、summary、环境记录、二进制审计、正确性检查。这样面对真实项目时，不需要推翻方法，只需要扩大规模。
...
\section{深入讲解：为什么证据结构比图表更重要}

图表很直观，但证据结构决定图表是否可解释。没有问题陈述，图表不知道回答什么；没有 workload，横轴不知道代表什么；没有单位，纵轴无法比较；没有 raw 数据，图表无法复查；没有二进制证据，趋势无法解释；没有限制，结论会被过度推广。

固定结构的作用，是强迫作者在图表之前写清上下文。先写问题和结论，再放图表；先写协议，再放 summary；先写证据，再写解释。这样图表服务论证，而不是替代论证。
...
设想一个改动把 \code{sum_u64} 的循环手写展开四倍。作者在一次实验环境中单次运行显示快百分之八。按照本章标准，这还不能直接合入。第一步，确认候选实现和 reference 在空数组、一个元素、非四倍长度、大数组和随机数据上结果一致。第二步，确认编译器没有已经自动展开或向量化出等价代码。若 baseline 本来就很好，手写展开可能只增加代码大小。
...
测量基础教你把一次实验做可信，benchmark suite 设计教你把可信实验变成长期体系。长期体系的核心不是代码量，而是一致性：一致的输入描述，一致的计时协议，一致的正确性检查，一致的 raw schema，一致的报告格式，一致的 baseline 管理。只有一致，历史数据才有意义。
...
\section{benchmark suite 的收束标准}

一个第一册级别的收束 suite 不需要很大，但要完整。至少包含两个正确性 reference、三个输入规模、两种数据分布、raw 和 summary CSV、环境记录、反汇编 artifact、一个失败样本处理路径、一个 CI smoke 入口和一份证据结构。它应能在另一台同类机器上运行，并能说明哪些结果可比较、哪些不可比较。

收束标准还包括维护动作：新增 kernel 不复制计时代码，schema 变化有版本，baseline 更新有说明，异常结果有排查流程，报告能链接到 raw data。做到这些，suite 就不再是脚本，而是项目资产。
...
第一册最后应形成一个小而完整的性能分析闭环。它不追求大而全，只要求把本册的核心链路贯穿起来：C++ 合同定义语义，wrapper 清理输入边界，汇编 kernel 暴露 ABI 责任，测试确认结果正确，benchmark 保存 raw、summary、环境和反汇编，报告说明结论和限制。这个闭环可以只包含求和、拷贝、带条件求和三个 kernel，但每个 kernel 都应有 reference、candidate、correctness 和测量证据。
...
目录本身不是重点，重点是责任清楚。src 放实现，tests 放正确性，bench 放测量，results 放不可变结果，docs 放 ABI 合同、测量协议和证据结构。这样的结构能把第一册知识真正变成工程资产。
...
带着这些边界进入第二册，分析会更稳。此时已经知道如何确认对象、如何读二进制、如何检查 ABI、如何设计实验、如何写证据结构。后续主题会更难，但方法不变：定义问题，收集证据，建立模型，验证假设，说明限制。

\section{suite 维护者的责任}

benchmark suite 一旦成为项目资产，就需要有人维护。维护者不一定每天写新 benchmark，但要保护 schema、baseline、artifact、证据结构和 CI 入口不被随意破坏。新增 kernel 时，维护者要检查 correctness 是否接入；新增指标时，要检查单位和公式；更新 baseline 时，要要求原因；删除旧结果时，要确认是否仍有追溯价值。

维护者还要防止 suite 膨胀。每个 workload、每个指标、每个图表都应服务问题。没有人解释的图表会变成噪声；无人复查的 raw 会变成垃圾；过慢的 CI 会让大家绕过流程。好的 suite 是克制而稳定的，不是把所有想法都塞进去。

\section{从第一册到第二册的分析迁移}
...
如果第一册的闭环设计得好，第二册可以自然扩展它，而不是重写实验系统。比如把 \code{sum_u64} 扩展为 AVX2 版本，把 \code{dot_f32} 扩展为向量版本，把输入矩阵扩展到对齐和尾部，把证据字段扩展到 ISA dispatch。这样分析路径连续，工程资产也连续。

\section{进入第二册前的边界}

第一册到这里收束在三条能力上：能从源码追到二进制事实，能用 ABI 解释调用边界，能用可信测量支持性能结论。第二册会把同一套方法推进到 cache、TLB、SIMD、线程和更复杂的算子，但判断流程不变：先定义问题，再固定语义，再看机器形态，再设计证据。若这三条能力还不稳定，面对高级优化时很容易把术语当解释。

进入第二册前，应能从零构建并运行一个小项目，保存构建命令和二进制身份；能读一个优化构建的核心循环汇编，区分事实和推断；能解释至少一个结构体布局和数组地址公式；能为一个 C++ 调汇编函数写 ABI 合同和测试；能设计一个不把 setup 混进 timed region 的 benchmark；能保存 raw、summary、环境和反汇编 artifact；能写出结论限制，不把单机结果推广成普遍规律。

最终闭环可以压缩成一条证据链：C++ contract -> ABI mapping -> object file and relocation -> assembly control/data flow -> binary identity -> correctness reference -> benchmark evidence -> limitations。后续主题越复杂，越要保护这条链路。缺少语义合同，二进制解释会失去边界；缺少反汇编，性能假设会变成猜测；缺少 raw 和环境记录，测量结论无法复查。

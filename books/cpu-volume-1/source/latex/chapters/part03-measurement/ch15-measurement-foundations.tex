...
这里先讲最核心的机制：看到一个性能数字时，能判断它是否有资格作为工程证据。统计、噪声分析、benchmark suite 和 PMU 细节都必须服务这个判断，而不是堆叠工具名。
...
\begin{itemize}
  ...
  \item 是否因为 \code{-march=native} 使用了当前实验环境特定 ISA。
\end{itemize}
...
无论是否使用统计工具，报告都不要写得过度确定。可以写“在当前实验环境、当前输入和当前测量协议下，B 的 median 稳定低于 A，差异约为 8\%”，不要写“B 永远比 A 快”。如果差异很小，可以写“当前证据不足以判断 2\% 差异是否真实”。
...
下面给一个最小 harness。它不是要替代 Google Benchmark，而是为了看清楚每个部件为什么存在。
...
这些属于更完整的性能报告字段；当前实验若缺少它们，结论范围必须相应收窄。
...
这样的报告比假装环境可控更专业。性能工程不是必须每次都拥有完美实验室，而是要让证据边界保持清楚。
...
这些字段不需要每次都写成长文，但必须能在结果目录中找到。benchmark suite 章节会把它工程化；本章先要求你建立习惯。
...
这份报告没有使用复杂统计，但已经满足基本可信性。它说明了问题、机器、构建、正确性、workload、计时协议、二进制证据、数据和限制。raw 数据可以被复查，结论边界也足够清楚。
...
这份清单也是 benchmark suite 的准入条件。测量基础先让一次实验可信，suite 再把可信实验组织成可维护系统。若一次实验都不可信，自动化只会更快地产生不可信数字。
...
为了支持复核，报告要避免“个人路径”“手工操作”“临时环境”这类不可见条件。命令应尽量脚本化，输入应可下载或可生成，环境差异应记录。若某些条件无法共享，例如真实生产数据或专用机器，也要提供摘要和替代 workload。不能复核的结果可以作为探索，不能作为强决策依据。
...
作者也应把测量失败写进 PR。若某些 size 没有提升，说明；若 PMU 数据无法采集，说明；若结果只在单一实验环境稳定，说明。这些信息帮助团队决定是否合入、是否需要 feature flag、是否需要更多机器验证。性能 review 的目标不是证明作者厉害，而是保护项目做正确决策。

\section{测量章节的综合案例}

综合案例可以选一个很小的优化，例如把单累加器求和改成四个累加器。证据先写假设，再写测量协议，再写 raw 和 summary，再写反汇编，最后写结论限制。若结果不符合预期，也要解释可能原因并设计下一步。

这里的重点不是 speedup 大小，而是测量是否可信。一个没有提升但证据完整的结论，比一个快很多却无法复现的结论更可靠。性能工程首先是可信论证，其次才是优化技巧。
...
本章最后用下面问题收束：如果一个 benchmark 显示快百分之五，差异是否大于噪声；如果显示快十倍，是否排除了工作被删除；如果两个版本结果相反，输入、构建和环境是否一致；如果 PMU 事件支持某个假设，能否回到反汇编指出对应代码；如果 correctness 失败，性能结论是否已经停止。

这些问题比记住某个计时 API 更重要。计时 API 会变，硬件会变，工具会变；可信论证的方法不会变。benchmark suite 章节会把这种一次实验的方法扩展成长期体系。

\section{实验：修复一个会被优化器删除的 benchmark}
...
\section{实验：写一个最小可信 sum benchmark}
...
\section{实验：比较 \code{steady_clock} 与 TSC}
...
\section{实验：用 \code{perf stat} 建立硬件证据}
...
从现在开始，你不应该再接受“这段代码跑了几毫秒”这种孤立数字。性能工程的基本素养是：每个数字都必须带着上下文、方法、证据和限制。benchmark 输入、热路径、CSV 输出、参数扫描和报告格式都属于这个证据边界。

...
本册的读者不需要已经学过完整的计算机体系结构、操作系统、编译原理或高性能计算。但你必须愿意把一个程序拆开来看：源码是一层，编译器是一层，汇编和 ABI 是一层，CPU 执行是一层，测量方法又是一层。本册要做的事，就是把这些层按经典教材的方式连成一条能推导、能观察、能复现实验的路径。
...
学完第一册后，读者不需要能手写大型汇编库，但必须能读懂普通优化汇编，能从 C++ 函数签名推导 ABI 位置，能写小型手写汇编函数，并能用 \code{nm}、\code{readelf}、\code{objdump}、GDB 证明它真的按预期工作。

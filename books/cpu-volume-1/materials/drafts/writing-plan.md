...
本文档定义本书后续扩展顺序。它不是学习计划的替代品；学习节奏仍以 `books/cpu-volume-1/materials/x86_64_hpc_ai_16_week_bootcamp.md` 为准。本文档用于保证书稿持续按高密度、可实验、可验收的方式推进。
...
6. 写常见误区和验收标准。
7. 更新 `books/cpu-volume-1/materials/drafts/README.md` 链接。
8. 检查占位词和断链。

检查命令：

```bash
find books/cpu-volume-1/materials/drafts -maxdepth 1 -type f | sort
wc -l books/cpu-volume-1/materials/drafts/*.md
rg -n "TODO|TBD|FIXME|待补|占位" books/cpu-volume-1/materials/drafts README.md books/cpu-volume-1/materials/x86_64_hpc_ai_16_week_bootcamp.md
```

...
本附录不是节选代码，而是从 \filepath{books/compute-systems-engine-code/labs} 直接读入的完整工程源码。读者在 PDF 或 EPUB 中看到的清单，应当和本地可构建工程保持同源；如果源码修改，重建书籍后清单会随之更新。
...
这一组文件给出代码实践卷的顶层构建入口，以及第一条主线 \filepath{reference_pipeline} 的完整源码。它从输入身份、解析诊断、reference 结果、report、manifest、split 和 merge 开始，训练“先定义正确结果，再谈优化”的工程习惯。

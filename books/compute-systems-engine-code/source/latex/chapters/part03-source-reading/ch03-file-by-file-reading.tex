...
代码实践卷顶层只有三个入口文件，但它们决定后面所有源码能不能被复现。读者先看这些文件，目的不是背 CMake 语法，而是确认“这本代码卷到底构建哪些目标、测试怎样被挂进 CTest、PDF 清单从哪里读源码”。
...
\filepath{Makefile} & 给书稿构建提供 \code{check}、\code{pdf} 和 \code{clean}。 & 这是书籍交付入口，不是 C++ 工程测试入口；C++ 测试必须走 CMake/CTest。 \\
\end{longtable}

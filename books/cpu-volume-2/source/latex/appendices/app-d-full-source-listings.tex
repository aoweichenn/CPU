...
本附录完整收录 \filepath{books/cpu-volume-2/labs/compute_systems}。第二册主线工程从串行 reference、并行归约、同步原语、有界队列、wait channel、任务运行时一路推进到 futex 合同。阅读时不要跳过测试：并发代码最重要的证据往往在失败路径测试里。
...
测试文件是并发实验的核心证据。它不仅检查最终计数，还检查 close 唤醒、队列 drain、同池等待、CSV 表头、futex 错误路径等容易被漏掉的边界。

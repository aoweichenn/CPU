...
第二册主线工程是 \filepath{compute_systems} 运行时实验，再向 \filepath{reference_pipeline} 代码实践卷扩展。第一层先把单机并发和运行时合同做实：串行 reference、并行归约、计数器、有界队列、wait channel、TaskRuntime、futex probe。第二层再把输入身份、manifest、split、merge 和恢复报告接上。不要把这些看成散装 demo；它们共同回答一个问题：一个计算系统怎样证明任务没有丢、没有重、没有卡住，并且能发布可复查报告。

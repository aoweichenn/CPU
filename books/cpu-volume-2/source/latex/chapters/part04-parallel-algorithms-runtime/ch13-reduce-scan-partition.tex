...
本节继续把 事件计数、filter 和 shuffle partition 拆开。前面已经建立了基本概念，现在要进一步追问：并行输出位置不唯一，或者合并顺序改变结果 时，旧程序为什么失去解释能力，哪些对象必须独立出来，哪些实验能证明改进有效。这里不新增目录层级，只把本章最容易含糊的点逐个压实。

\topic{为什么 reduce 先证明结合律}

先把问题落到一个具体动作上。为什么 reduce 先证明结合律，不是为了多记一个术语，而是为了处理 并行输出位置不唯一，或者合并顺序改变结果 这样的具体场景。如果仍沿用 把 for 循环切块后直接 push 到共享 vector 或锁住全局 map，小输入可能看不出问题，可一旦出现并发、失败、倾斜或资源等待，状态就会散落在调用栈、临时文件、日志文本或线程局部变量里。本章要求读者把这些状态写回 identity、associativity、local partial、exclusive scan、partition manifest，让后续观察和恢复有共同语言。
...
实验报告要能独立阅读。输入摘要、命令、reference、关键 trace、资源指标、错误注入和未验证边界都要保留。源码或工具阅读则围绕 per cpu counter、sbitmap 和分层统计 展开，只解释与当前现象相关的路径。完成后，本章留下的材料应能自然交给下一章的 任务图运行时，而不是在章末断掉。

\topic{为什么 identity 不是可选参数}

这一点不能只靠定义说明。为什么 identity 不是可选参数，不是为了多记一个术语，而是为了处理 并行输出位置不唯一，或者合并顺序改变结果 这样的具体场景。如果仍沿用 把 for 循环切块后直接 push 到共享 vector 或锁住全局 map，小输入可能看不出问题，可一旦出现并发、失败、倾斜或资源等待，状态就会散落在调用栈、临时文件、日志文本或线程局部变量里。本章要求读者把这些状态写回 identity、associativity、local partial、exclusive scan、partition manifest，让后续观察和恢复有共同语言。
...
\topic{为什么 filter 需要 count 再 scan}

真正的判断来自反例。为什么 filter 需要 count 再 scan，不是为了多记一个术语，而是为了处理 并行输出位置不唯一，或者合并顺序改变结果 这样的具体场景。如果仍沿用 把 for 循环切块后直接 push 到共享 vector 或锁住全局 map，小输入可能看不出问题，可一旦出现并发、失败、倾斜或资源等待，状态就会散落在调用栈、临时文件、日志文本或线程局部变量里。本章要求读者把这些状态写回 identity、associativity、local partial、exclusive scan、partition manifest，让后续观察和恢复有共同语言。
...
\topic{为什么 partition 是后续阶段合同}

工程上更稳的路径是先固定语义。为什么 partition 是后续阶段合同，不是为了多记一个术语，而是为了处理 并行输出位置不唯一，或者合并顺序改变结果 这样的具体场景。如果仍沿用 把 for 循环切块后直接 push 到共享 vector 或锁住全局 map，小输入可能看不出问题，可一旦出现并发、失败、倾斜或资源等待，状态就会散落在调用栈、临时文件、日志文本或线程局部变量里。本章要求读者把这些状态写回 identity、associativity、local partial、exclusive scan、partition manifest，让后续观察和恢复有共同语言。
...
读者容易忽略的地方在边界。为什么浮点要写误差规则，不是为了多记一个术语，而是为了处理 并行输出位置不唯一，或者合并顺序改变结果 这样的具体场景。如果仍沿用 把 for 循环切块后直接 push 到共享 vector 或锁住全局 map，小输入可能看不出问题，可一旦出现并发、失败、倾斜或资源等待，状态就会散落在调用栈、临时文件、日志文本或线程局部变量里。本章要求读者把这些状态写回 identity、associativity、local partial、exclusive scan、partition manifest，让后续观察和恢复有共同语言。
...
把这个点写成实验时，要故意保留旧方案。为什么重做范围影响容错，不是为了多记一个术语，而是为了处理 并行输出位置不唯一，或者合并顺序改变结果 这样的具体场景。如果仍沿用 把 for 循环切块后直接 push 到共享 vector 或锁住全局 map，小输入可能看不出问题，可一旦出现并发、失败、倾斜或资源等待，状态就会散落在调用栈、临时文件、日志文本或线程局部变量里。本章要求读者把这些状态写回 identity、associativity、local partial、exclusive scan、partition manifest，让后续观察和恢复有共同语言。
...
\topic{为什么热点 key 要单独处理}

如果把它放进贯穿项目，接口就要承担更多责任。为什么热点 key 要单独处理，不是为了多记一个术语，而是为了处理 并行输出位置不唯一，或者合并顺序改变结果 这样的具体场景。如果仍沿用 把 for 循环切块后直接 push 到共享 vector 或锁住全局 map，小输入可能看不出问题，可一旦出现并发、失败、倾斜或资源等待，状态就会散落在调用栈、临时文件、日志文本或线程局部变量里。本章要求读者把这些状态写回 identity、associativity、local partial、exclusive scan、partition manifest，让后续观察和恢复有共同语言。
...
源码阅读只服务这个问题。为什么输出顺序是语义不是装饰，不是为了多记一个术语，而是为了处理 并行输出位置不唯一，或者合并顺序改变结果 这样的具体场景。如果仍沿用 把 for 循环切块后直接 push 到共享 vector 或锁住全局 map，小输入可能看不出问题，可一旦出现并发、失败、倾斜或资源等待，状态就会散落在调用栈、临时文件、日志文本或线程局部变量里。本章要求读者把这些状态写回 identity、associativity、local partial、exclusive scan、partition manifest，让后续观察和恢复有共同语言。
...
这一轮细读的目的，是让 事件计数、filter 和 shuffle partition 不再只是案例名，而成为贯穿本章的推导线。读者应能从 把 for 循环切块后直接 push 到共享 vector 或锁住全局 map 出发，解释它为什么合理、为什么失败、怎样引入 identity、associativity、local partial、exclusive scan、partition manifest，以及怎样用实验和源码证据验证。能做到这一点，本章内容才真正从概念变成工程知识。

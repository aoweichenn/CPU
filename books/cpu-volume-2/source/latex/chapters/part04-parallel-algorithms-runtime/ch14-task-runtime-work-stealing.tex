...
\begin{tikzpicture}[book diagram, x=1cm, y=1cm]
  \node[book data, minimum width=1.55cm] (a) {Task A};
  \node[book data, minimum width=1.55cm, below=0.7cm of a] (b) {Task B};
  \node[book hot, minimum width=1.55cm, right=1.15cm of a] (c) {Task C\\ready=2};
  \node[book hot, minimum width=1.55cm, right=1.15cm of b] (d) {Task D\\ready=1};
  \node[book state, minimum width=2.1cm, right=1.25cm of c] (ready) {Ready queue};
  \node[book state, minimum width=2.15cm, right=1.25cm of ready] (worker0) {Worker 0\\local deque};
  \node[book state, minimum width=2.15cm, below=0.72cm of worker0] (worker1) {Worker 1\\local deque};
  \node[book warnnode, minimum width=2.3cm, below=0.9cm of ready] (blocked) {禁止长期\\future.get()};

  \draw[book arrow] (a) -- (c);
  \draw[book arrow] (b) -- (c);
  \draw[book arrow] (b) -- (d);
  \draw[book arrow] (c) -- node[above, font=\footnotesize\sffamily] {计数归零} (ready);
  \draw[book arrow] (d) -- (ready);
  \draw[book arrow] (ready) -- (worker0);
  \draw[book arrow] (ready) -- (worker1);
  \draw[book weak arrow] (worker1.west) .. controls +(-1.05,0) and +(0,-0.6) .. (worker0.south)
    node[midway, right, font=\footnotesize\sffamily\color{BookMuted}] {steal};
  \draw[book weak arrow] (blocked.north) -- (ready.south);
\end{tikzpicture}
\end{systemdiagram}
...
项目可以提供一个 \cmd{engine trace summarize} 命令，输出 worker 利用率、最长任务、最长 ready 等待、steal 次数和关键路径估计。书中展示的是摘要，不是截图。这样既适合 PDF，也适合 EPUB 和手机阅读。

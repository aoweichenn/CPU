...
\item[reference] 参考实现。它可以慢，但必须清楚、确定、覆盖完整题意，用来给优化实现提供正确性对照。
\item[oracle] 判定器。它把 reference 和优化版放在同一批输入上比较，确认输出是否满足同一语义。
...
\item[GTest] GoogleTest，C++ 单元测试框架。它把样例、边界和反例变成可重复运行的测试，而不是只靠人工看代码。
\item[GMock] GoogleMock，GoogleTest 配套 mock 框架。复杂工程里用它替换外部依赖、固定交互和错误路径。
...
\item[TSan] ThreadSanitizer，线程检查器，用于发现数据竞争。它是并发代码的工具证据，不是算法正确性的完整证明。
...
\item[ABA] ABA problem，无锁结构里“值变回原样但对象事实已经改变”的问题。它提醒读者：并发结构不能只比较表面值，还要处理版本、所有权和回收时机。

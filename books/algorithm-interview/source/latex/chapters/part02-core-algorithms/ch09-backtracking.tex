...
剪枝必须是必要条件或支配关系，不能是直觉。组合总和中，候选全为正数且已排序，当前值大于剩余目标时，后续更大值也不能使用，这是合法剪枝；括号生成中，任意前缀右括号数不能超过左括号数，否则后面无法修复；复原 IP 地址中，剩余字符数量必须落在剩余段数到三倍剩余段数之间；N 皇后中，同列或同对角线冲突一定非法。没有证明的剪枝会制造隐藏漏解，尤其是在输入有负数、重复值或目标条件变化时。

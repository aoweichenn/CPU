...
\begin{lstlisting}[language=C++,caption={全排列：显式栈模拟回溯}]
...
\begin{lstlisting}[language=C++,caption={全排列 II：排序后按层去重}]
...
\begin{lstlisting}[language=C++,caption={子集：迭代扩展已有答案}]
...
\begin{lstlisting}[language=C++,caption={组合总和：显式状态保存起点和剩余目标}]
...
\begin{lstlisting}[language=C++,caption={括号生成：前缀合法性剪枝}]
...
\begin{lstlisting}[language=C++,caption={单词搜索：显式栈保存路径访问状态}]
...
N 皇后是回溯剪枝的代表。暴力方法在 \code{n \times n} 棋盘上选 \code{n} 个格子，再检查行、列、对角线是否冲突，搜索空间巨大。优化方式是按行放皇后：第 \code{row} 层只决定第 \code{row} 行皇后放在哪一列。这样行冲突天然消失，只需要维护列、主对角线和副对角线是否被占用。

\begin{lstlisting}[language=C++,caption={N 皇后：按行放置并维护三类占用}]
...
\begin{lstlisting}[language=C++,caption={组合总和 II：每个下标最多使用一次，同层跳过重复值}]
...
\begin{lstlisting}[language=C++,caption={分割回文串：预处理回文表后搜索切分点}]
...
\begin{lstlisting}[language=C++,caption={复原 IP 地址：段数和剩余长度剪枝}]

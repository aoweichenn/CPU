...
如果数组中有 0，这段代码仍然正确。当 \code{x == 0} 时，\code{dp[sum] += dp[sum]}，所有已有方案翻倍，正好对应给 0 加正号或负号都不改变总和但算作不同表达式。很多手写特判会在这里出错。计数 DP 还要关注结果范围；力扣题通常保证答案放进 int，工程中应考虑 \code{long long}。
...
\input{chapters/generated/generated-dp-greedy-cards}

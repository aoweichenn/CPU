...
数组题先不要急着找“模板”，要先看题目要求的候选对象是什么。候选对象通常只有四类：单个元素、两个元素组成的配对、连续区间、若干个切分点。单个元素题多半是扫描、计数或比较；配对题会走向哈希表、排序双指针或二分；连续区间题会走向前缀和、差分、滑动窗口、单调队列；切分点题会走向 DP、贪心或答案二分。只要候选对象没有分清，后面的优化就会变成背题号。

暴力枚举是数组题的起点，不是低质量答案。枚举所有左右端点，可以覆盖每个连续子数组；枚举所有下标对，可以覆盖每个二元关系；枚举每个切分点，可以覆盖每种分段。真正的问题在于暴力有没有重复工作。例如区间和每次重新累加，就是重复计算相邻区间共有的部分；三数之和每次重新找另外两个数，就是重复比较已经有序的候选；最小覆盖子串每次重新统计子串字符，就是重复维护高度重叠的窗口状态。优化的第一句话应该是“我把哪一类重复工作保存下来了”，而不是“我使用了某某算法”。

数组的物理基础也会影响算法表达。 \code{std::vector} 连续存储，支持下标随机访问，所以二分、前缀和、双指针和 DP 表都天然适合用它。连续内存让顺序扫描具有较好的缓存局部性，也让 \code{nums[i]} 这种访问能直接做地址计算。代价是中间插入删除要移动元素，尾部追加可能触发扩容，扩容后旧指针、引用和迭代器可能失效。刷题时我们很少在数组中间频繁插入；一旦题目需要稳定节点身份或常数时间移动节点，就应该想到链表或索引结构，而不是继续勉强使用数组。

区间语义必须统一。闭区间 \code{[left, right]} 的长度是 \code{right - left + 1}，右端最后一个元素真实参与窗口；半开区间 \code{[left, right)} 的长度是 \code{right - left}，右端表示下一个待处理位置。两种写法都可以，但一章代码里最好有主语义。前缀和天然适合半开区间，因为 \code{prefix[i]} 表示前 \code{i} 个元素，区间 \code{[left, right)} 的和就是 \code{prefix[right] - prefix[left]}。滑动窗口的代码如果写成闭区间，也要明确什么时候把右端加入状态，什么时候把左端移出状态。大多数 off-by-one 错误，都不是算术不好，而是区间含义没有先写清楚。

双指针不是“两个变量相向移动”的同义词。它成立需要一个排除理由。两端收缩型题目里，当前左右端点之外的候选已经处理过，移动哪一侧必须能证明不会丢掉更优答案；排序配对型题目里，和太小移动左端、和太大移动右端，依赖的是有序性；同向快慢指针里，慢指针通常代表已整理前缀的下一个写入位置，快指针代表正在读取的输入位置。三种模型的指针外形相似，但不变量完全不同。复盘时要写“被排除的候选为什么不可能成为答案”，否则只是把循环跑了一遍。

滑动窗口更严格。它需要三个条件同时成立：答案对象是连续区间；边界移动后状态能用常数或近似常数成本增删；移动策略不会漏掉合法答案。正整数数组的最短子数组和至少为目标，可以用窗口，因为右端扩展只会让和变大，左端收缩只会让和变小。含负数数组就不行，因为负数进入可能让和变小，负数离开可能让和变大，窗口的单调判断被破坏。无重复字符子串可以用窗口，不是因为字符问题天然窗口化，而是因为重复字符的上次出现位置能告诉左端至少要跳到哪里，且左端不会回退。

\begin{principlebox}{数组窗口题的推理顺序}
先写候选空间，再写暴力枚举；接着定位重复工作，判断它来自重复求和、重复查找、重复统计还是重复比较；然后选择保存状态的结构，说明状态如何随边界变化；最后证明每个下标为什么只会被处理常数次。若题目条件变化，例如数组从正数变成可负、从连续子数组变成子序列、从静态数组变成动态更新，原来的窗口或前缀模型必须重新审查。
\end{principlebox}

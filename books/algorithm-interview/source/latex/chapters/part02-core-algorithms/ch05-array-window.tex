...
\begin{lstlisting}[language=C++,caption={两数之和 II：有序数组配对搜索}]
...
\begin{lstlisting}[language=C++,caption={四数之和：排序、分层去重和长整型求和}]
...
\begin{lstlisting}[language=C++,caption={乘积小于 K 的子数组：窗口计数}]
...
\begin{lstlisting}[language=C++,caption={允许负数的最短子数组：前缀和加单调队列}]
...
\begin{lstlisting}[language=C++,caption={轮转数组：三次反转}]

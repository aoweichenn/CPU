...
\begin{lstlisting}[language=C++,caption={字母异位词分组：计数 key 带分隔符}]
...
\begin{lstlisting}[language=C++,caption={连续的子数组和：余数最早位置}]
...
\begin{lstlisting}[language=C++,caption={连续数组：0 转成 -1 后找相同前缀和}]
...
\begin{lstlisting}[language=C++,caption={快乐数：哈希集合检测循环}]

...
这题的关键不是取模代码，而是“同余差值可整除”的数学关系。若题目允许 \code{k == 0}，取模会出错，需要单独处理连续至少两个 0 或直接用前缀和相等判断；力扣当前约束通常给出非零 k，但面试中要先确认。前缀和如果可能很大，要用 \code{long long}；取模后余数范围有限，才适合放进哈希表。
...
本章力扣主线：1 两数之和、49 字母异位词分组、128 最长连续序列、202 快乐数、217 存在重复元素、560 和为 K 的子数组、523 连续的子数组和、525 连续数组、454 四数相加 II。复盘时必须说明哈希表保存的是值、下标、次数还是最早位置。

...
业务对象 & \code{PageLike \{ id, active, lru_node, dirty_node \}}\\
...
\begin{labbox}
实验：写一个最小侵入式链表。定义 \code{struct Item \{ int key; ListHead node; \}}，创建三个对象并把它们插入同一个链表。遍历时不要在 \code{ListHead} 里存 key，而是通过成员偏移回到 \code{Item}。再让 \code{Item} 增加第二个 \code{ListHead}，同时挂入另一个链表，观察同一对象如何拥有两种顺序。
\end{labbox}

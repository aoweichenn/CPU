...
动态规划题先问状态是否包含位置、容量、集合、区间、树节点、数字位或概率。背包看物品能用几次和方案是否区分顺序；区间 DP 看最后一步是切分还是选择最后处理对象，例如戳气球要选最后戳的气球来固定边界；状态压缩看 \code{n} 是否允许 \code{2^n}，还要看同一个集合是否需要记录最后位置；数位 DP 看是否受上界限制和前导零；优化 DP 看转移是否有窗口最值、决策单调、凸性或可离线。没有证明条件时，不要强行套优化。

...
\begin{principlebox}{从 LeetCode 案例进入高级专题}
...
LeetCode 1192 查找集群内的关键连接 & 删除每条边再搜太慢，\code{disc/low} 一次 DFS 判断桥。\\
\end{tabular}
\end{principlebox}
...
public:
    ...
    std::int64_t range_sum(int left, int right) const
    {
...
以 LeetCode 315 计算右侧小于当前元素的个数为例，题目给定数组 \code{nums}，要求对每个位置 \code{i} 返回它右侧严格小于 \code{nums[i]} 的元素个数；样例 \code{nums = [5,2,6,1]}，输出 \code{[2,1,1,0]}。暴力做法对每个位置向右扫描，复杂度 \complexity{O(n^2)}。从右往左处理时，已经加入树状数组的都是当前位置右侧元素；要知道有多少数小于 \code{nums[i]}，只需查询排名小于它的元素频次。由于值可能很大或为负，不能直接按值开数组，要先离散化成排名。优化来源不是神秘技巧，而是把“右侧历史元素集合”维护成一个支持排名前缀查询的数据结构。
...
public:
    explicit LazySegmentTree(const std::vector<std::int64_t>& values)
    ...
    void add(int left, int right, std::int64_t delta)
    {
        this->add(1, 0, this->n_ - 1, left, right, delta);
    }

    std::int64_t query_max(int left, int right)
    {
...
private:
    ...
    void apply(int node, std::int64_t delta)
    {
    ...
    void add(int node, int left, int right,
             int query_left, int query_right, std::int64_t delta)
    {
    ...
    std::int64_t query_max(int node, int left, int right,
                        int query_left, int query_right)
    {
        ...
        this->push(node);
        const int mid = left + (right - left) / 2;
        std::int64_t answer = std::numeric_limits<std::int64_t>::lowest();
        if (query_left <= mid) {
...
稀疏表适合静态数组的区间最值查询。它不能高效处理在线修改，但预处理后每次查询可以 \complexity{O(1)}。核心是 \code{st[k][i]} 保存从 \code{i} 开始、长度 \ensuremath{2^k} 的区间最值。查询 \code{[l,r]} 时，取 \code{k = floor(log2(r-l+1))}，用两个长度 \ensuremath{2^k} 的区间覆盖目标区间：一个从 \code{l} 开始，一个以 \code{r} 结尾。最小值、最大值这种幂等操作允许重叠覆盖，所以可以这么做；区间和不能用这种重叠方式直接查询。
...
LCA 最近公共祖先是树上查询的基础问题。暴力方法从两个节点不断向上走，最坏可能 \complexity{O(n)}。若查询很多，可以用倍增预处理。 \code{up[k][v]} 表示节点 \code{v} 向上跳 \ensuremath{2^k} 步到达的祖先。查询时先把两个节点跳到同一深度，再从大到小尝试同时跳，直到它们的父节点相同。优化来源是把“一步一步向上”拆成二进制跳跃。
...
数学算法的第一层是整数边界。很多题看似算法正确，却因为溢出、负数取模或除法方向出错。C++ 中 \code{int} 溢出是未定义行为，乘法取模前应使用 \code{std::int64_t} 或更宽类型。负数取模的结果可能仍为负，若要得到半开区间 $[0,\mathrm{mod})$ 范围，应按 $((x \bmod \mathrm{mod}) + \mathrm{mod}) \bmod \mathrm{mod}$ 归一化。这些不是细枝末节，而是数学题和工程代码之间的硬接口。

快速幂把 \ensuremath{a^n} 的线性连乘变成二进制分解。暴力乘 \code{n} 次是 \complexity{O(n)}；观察指数的二进制表示，每一位代表是否乘上当前底数，底数每轮平方，指数每轮右移。这个思想也用于矩阵快速幂、排列快速幂和函数复合快速幂。

\begin{lstlisting}[language=C++,caption={快速幂：支持取模的大整数乘方}]
std::int64_t mod_pow(std::int64_t base, std::int64_t exponent, std::int64_t mod)
{
    base %= mod;
...
std::int64_t extended_gcd(std::int64_t a, std::int64_t b,
                       std::int64_t& x, std::int64_t& y)
{
    ...
    const std::int64_t g = extended_gcd(b, a % b, next_x, next_y);
    x = next_y;
    y = next_x - (a / b) * next_y;
    return g;
}
...
public:
    CombinationMod(int n, std::int64_t mod)
    ...
    std::int64_t choose(int n, int k) const
    {
        if (k < 0 || k > n) {
            return 0;
        }
...
Matrix multiply(const Matrix& a, const Matrix& b, std::int64_t mod)
{
    const int n = static_cast<int>(a.size());
    const int m = static_cast<int>(b[0].size());
    const int inner = static_cast<int>(b.size());
    Matrix result(n, std::vector<std::int64_t>(m, 0));
    for (int i = 0; i < n; ++i) {
...
Matrix matrix_pow(Matrix base, std::int64_t exponent, std::int64_t mod)
{
    const int n = static_cast<int>(base.size());
    Matrix result(n, std::vector<std::int64_t>(n, 0));
    for (int i = 0; i < n; ++i) {
        result[i][i] = 1;
    }
    while (exponent > 0) {
        ...
        while (this->bfs(source, sink)) {
            ...
            while (true) {
                const std::int64_t pushed =
                    this->dfs(source, sink, std::numeric_limits<std::int64_t>::max());
                if (pushed == 0) {
                    break;
                }
                flow += pushed;
...
private:
    ...
    std::int64_t dfs(int node, int sink, std::int64_t flow)
    {
        if (node == sink) {
            return flow;
        }
...
数位 DP 处理“统计不超过 N 的数中有多少满足某种数字性质”。LeetCode 600 不含连续 1 的非负整数给定整数 \code{n}，要求统计 \code{[0,n]} 中二进制表示不包含连续 1 的整数个数；样例 \code{n = 5}，合法数是 \code{0(0), 1(1), 2(10), 4(100), 5(101)}，输出 5。暴力枚举所有数再检查二进制，若 \code{n} 很大不可行。数位 DP 把上界按二进制拆成位，从高位到低位构造数字。状态通常包括当前位置、是否贴着上界、是否已经开始、以及与题目性质相关的统计量。本题只需要知道上一位是否为 1，以及当前前缀是否仍贴着上界。
...
折半搜索处理的是 $2^n$ 太大但 $2^{n/2}$ 可接受的场景。LeetCode 1755 最接近目标值的子序列和给定数组和目标 \code{goal}，可以选择任意子序列，要求最小化 \code{|sum - goal|}；样例 \code{nums = [5,-7,3,5], goal = 6}，输出 0，因为可以选 \code{-7 + 3 + 5 + 5 = 6}。暴力枚举完整集合是 \complexity{O(2^n)}。把数组分成两半，分别枚举所有子集和，再通过排序、二分或双指针合并，复杂度降到 \complexity{O(2^{n/2} \log 2^{n/2})} 量级。优化来源不是剪枝，而是把全集选择分成两个独立半区。
...
IDA* 和 A* 都是带启发式的搜索。普通 BFS 保证最短步数，但状态爆炸；DFS 省空间但不保证最短。A* 用 \code{f = g + h} 排序，\code{g} 是已走代价，\code{h} 是对剩余代价的低估。用网格最短路理解：若只能上下左右走，曼哈顿距离不会超过真实剩余步数，是安全低估；如果把启发式写成曼哈顿距离的 3 倍，就可能高估，A* 可能过早放弃真正最短的绕路。若 \code{h} 从不高估真实剩余代价，A* 第一次弹出目标时能保证最优。IDA* 则用深度优先搜索配合逐渐增加的估价上界，常见于十五数码这类状态空间大但启发式较强的问题。面试中不一定手写它们，但要知道启发式必须可采纳，不能为了跑得快写一个会高估的 \code{h} 后还声称最优。

离线算法把所有查询先收集起来，重新排序后统一处理。它适合“答案不要求按输入过程实时输出”的问题。比如数组 \code{[5,1,4]} 上有查询“前缀里小于等于 3 的数有几个”和“小于等于 5 的数有几个”，在线做法可能每个查询扫一遍；离线做法把数组元素按值排序、查询按阈值排序，阈值推进到 3 时只加入 1，推进到 5 时再加入 4 和 5，然后用树状数组回答位置范围。并查集也常用于离线连通性，把边按权重或时间排序后逐步合并。离线的本质是改变处理顺序，用单调推进替代每个查询从头扫描。
...
计算几何的基本单位是向量。点积回答夹角和投影，叉积回答方向和面积。给三个点 \code{a,b,c}，向量 \code{b-a} 与 \code{c-a} 的叉积为正表示 \code{c} 在有向线段 \code{a->b} 的左侧，为负表示右侧，为零表示共线。线段相交、凸包、多边形面积都建立在这个判断上。整数坐标下，叉积可能溢出，应使用 \code{std::int64_t}。
...
{
    const std::int64_t x1 = b.x - a.x;
    const std::int64_t y1 = b.y - a.y;
    const std::int64_t x2 = c.x - a.x;
    const std::int64_t y2 = c.y - a.y;
    return x1 * y2 - y1 * x2;
}

bool between(std::int64_t a, std::int64_t b, std::int64_t x)
{
...
{
    const std::int64_t c1 = cross(a, b, c);
    const std::int64_t c2 = cross(a, b, d);
    const std::int64_t c3 = cross(c, d, a);
    const std::int64_t c4 = cross(c, d, b);
...
一致性哈希用于分布式系统中的节点扩缩容。普通取模 $hash(key) \bmod n$ 在节点数量变化时会让大量 key 重新映射；一致性哈希把节点和 key 都放到环上，key 归属顺时针遇到的第一个节点。增加或删除节点时，只影响环上相邻区间。虚拟节点用于缓解负载不均。这个结构不属于传统 LeetCode 高频内容，但它展示了哈希思想在工程系统中的另一种使用方式：目标不是单机 \complexity{O(1)} 查询，而是降低重分布成本。
...
LeetCode 778 & 对象是格子和高度阈值，目标是最小化路径最大高度，性质可以走 Dijkstra、二分可达或并查集开放节点。\\
\end{tabular}
\end{principlebox}
...
以 LeetCode 731 我的日程安排 II 为例，题目允许在线预订半开区间 \code{[start,end)}，要求任意时刻最多双重预订，不能出现三重预订；样例中依次调用 \code{book(10,20)}、\code{book(50,60)}、\code{book(10,40)} 返回 \code{true}，再调用 \code{book(5,15)} 返回 \code{false}，因为 \code{[10,15)} 会形成三重预订。暴力做法是把每次预订后所有时间点都扫描一遍，这在时间值很大时不可行。第一种结构化做法是差分扫描：每次临时把开始点加一、结束点减一，按时间顺序累计，若累计超过二就回滚。这说明本题本质是“动态事件点上的前缀最大值”。若预订次数不大，\code{std::map} 差分已经足够；若次数巨大且要极致性能，可以考虑动态线段树维护区间最大覆盖次数。
...
public:
    ...
    {
        constexpr std::uint64_t BASE = std::uint64_t{1'315'423'911};
        for (int i = 0; i < static_cast<int>(text.size()); ++i) {
            const auto byte =
    ...
    std::uint64_t range_hash(int left, int right) const
    {
...
\topic{动态规划专题深讲：状态、转移和优化边界}

高级 DP 的核心不是状态数量多，而是状态语义更抽象。基础 DP 的状态通常是位置或容量；高级 DP 会把集合、区间、树节点、数字位、概率、博弈局面和优化结构放进状态。写 DP 前先写一句话定义状态，定义里必须包含“处理到哪里”和“保留什么信息”。例如树形 DP 不能只写“\code{dp[u]} 是子树最优”，还要说明父节点是否偷会不会限制 \code{u}；状态压缩不能只写“\code{dp[mask]} 是最优”，还要说明集合内顺序是否已经不重要。如果这句话含糊，代码再短也不可靠。
...
\input{chapters/chapter-supplements/supplement-ch16-advanced-algorithms}

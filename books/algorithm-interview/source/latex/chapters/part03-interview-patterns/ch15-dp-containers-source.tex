...
long long num_distinct(const std::string& source, const std::string& target)
{
    std::vector<long long> dp(target.size() + 1, 0);
    dp[0] = 1;
...
\code{dp[0] = 1} 表示任何前缀都能用“什么都不选”的方式形成空串。倒序遍历的理由和 0-1 背包相同：当前字符只能贡献一次，如果正序更新，\code{dp[j - 1]} 可能已经使用了当前字符，会导致一个字符被重复使用。计数题还要注意答案范围，力扣某些题用 \code{int}，但教材里用 \code{long long} 更能表达安全边界。
...
void compute_partition_layer(const std::vector<long long>& previous,
                             ...
                             int opt_right)
{
    ...
    while (!tasks.empty()) {
        ...
        for (int split = task.opt_left; split <= upper; ++split) {
            const long long segment_sum = prefix[mid] - prefix[split];
...
long long min_merge_cost_knuth(const std::vector<int>& stones)
{
...
std::vector<long long> convex_hull_optimized_dp(
    const std::vector<long long>& prefix,
...
long long count_by_digit_sum(int upper_bound, int sum_limit)
{
    const std::string digits = std::to_string(upper_bound);
    using StateCounts = std::array<std::array<long long, 2>, 2>;
    ...
    for (int pos = 0; pos < static_cast<int>(digits.size()); ++pos) {
        for (StateCounts& state_counts : next) {
            for (std::array<long long, 2>& row : state_counts) {
                row.fill(0);
            }
        }
        ...
        for (int sum = 0; sum <= sum_limit; ++sum) {
            for (int tight = 0; tight <= 1; ++tight) {
                for (int started = 0; started <= 1; ++started) {
                    const long long ways = current[sum][tight][started];
                    if (ways == 0) {
                        continue;
                    }

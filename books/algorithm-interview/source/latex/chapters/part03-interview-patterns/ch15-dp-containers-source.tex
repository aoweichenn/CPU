...
这题要特别注意 0。若数组里有 0，给它加正号或负号都不改变和，但方案数应翻倍。上述 DP 能自然处理：当 \code{x == 0} 时，\code{dp[sum] += dp[sum]}，所有已有方案翻倍。很多手写特殊逻辑反而容易错。计数型 DP 要关注整型范围，力扣原题结果通常能放进 int，但工程中应考虑 \code{std::int64_t}。
...
\code{dp[0] = 1} 表示任何前缀都能用“什么都不选”的方式形成空串。倒序遍历的理由和 0-1 背包相同：当前字符只能贡献一次，如果正序更新，\code{dp[j - 1]} 可能已经使用了当前字符，会导致一个字符被重复使用。计数题还要注意答案范围，力扣某些题用 \code{int}，但教材里用 \code{std::int64_t} 更能表达安全边界。

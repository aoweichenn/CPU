...
\begin{lstlisting}[language=C++,caption={分割等和子集：0-1 背包可达性}]
...
\begin{lstlisting}[language=C++,caption={目标和：转换为 0-1 背包计数}]
...
\begin{lstlisting}[language=C++,caption={零钱兑换 II：完全背包组合计数}]
...
\begin{lstlisting}[language=C++,caption={买卖股票 I：维护历史最低价格}]
...
\begin{lstlisting}[language=C++,caption={含冷冻期股票：三状态 DP}]
...
\begin{lstlisting}[language=C++,caption={最长递增子序列：二次 DP 和路径还原}]
...
\begin{lstlisting}[language=C++,caption={最长递增子序列：tails 数组和二分优化}]
...
\begin{lstlisting}[language=C++,caption={不同路径：一维滚动数组}]
...
\begin{lstlisting}[language=C++,caption={最小路径和：边界初始化后主转移}]
...
\begin{lstlisting}[language=C++,caption={编辑距离：两行滚动数组}]
...
\begin{lstlisting}[language=C++,caption={最长公共子序列：一维数组和左上角缓存}]
...
\begin{lstlisting}[language=C++,caption={单词拆分：限制最大单词长度降低无效枚举}]
...
\begin{lstlisting}[language=C++,caption={戳气球：选择最后一个被戳的气球}]
...
\begin{lstlisting}[language=C++,caption={访问所有节点的最短路径：节点加集合状态 BFS}]
...
\begin{lstlisting}[language=C++,caption={划分为 K 个等和子集：集合状态 DP}]
...
\begin{lstlisting}[language=C++,caption={受限子序列和：单调队列优化转移最大值}]
...
\begin{lstlisting}[language=C++,caption={双堆延迟删除：滑动窗口中位数的核心结构}]
class DualHeap {
public:
...
\begin{lstlisting}[language=C++,caption={前缀和计数：查询和更新顺序不能反}]
...
\input{chapters/generated/generated-linux-labs}

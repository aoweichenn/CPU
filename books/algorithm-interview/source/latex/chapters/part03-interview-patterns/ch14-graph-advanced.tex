...
如果图中存在负边，Dijkstra 的贪心基础会崩塌。因为一个当前距离较大的节点，未来可能通过负边把另一个已经确定的节点拉得更小。Bellman-Ford 的想法更朴素：一条最短路径如果最多包含 \code{k} 条边，那么做 \code{k} 轮按边松弛就能得到它。经典最短路最多需要 \code{n - 1} 条边，因为无负环的最短简单路径不会重复经过节点。LeetCode 787 K 站中转内最便宜的航班 本质是限制边数的 Bellman-Ford。

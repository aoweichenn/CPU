...
\begin{lstlisting}[language=C++,caption={删除倒数第 N 个节点：虚拟头节点和快慢指针}]
struct ListNode {
    int value = 0;
...
\begin{lstlisting}[language=C++,caption={合并两个有序链表：尾指针接入节点}]
...
\begin{lstlisting}[language=C++,caption={K 个一组翻转链表：先找边界再局部反转}]
...
\begin{lstlisting}[language=C++,caption={最小栈：数据栈和最小值栈同步}]
class MinStack {
public:
    void push(int value)
    {
...
\begin{lstlisting}[language=C++,caption={柱状图最大矩形：单调递增栈}]
...
\begin{lstlisting}[language=C++,caption={滑动窗口最大值：队头保存当前最大值下标}]
...
\begin{lstlisting}[language=C++,caption={环形链表：快慢指针检测相遇}]
...
\begin{lstlisting}[language=C++,caption={LRU 缓存：哈希表定位节点，双向链表维护顺序}]
class LRUCache {
public:
...
\begin{lstlisting}[language=C++,caption={接雨水：双指针维护两侧最高柱}]
...
\begin{lstlisting}[language=C++,caption={基本计算器 II：栈处理乘除优先级}]
...
\begin{lstlisting}[language=C++,caption={相交链表：双指针走完两条路径}]
...
\begin{lstlisting}[language=C++,caption={回文链表：反转后半段后比较}]
...
\begin{lstlisting}[language=C++,caption={链表排序：自底向上迭代归并}]
...
\begin{lstlisting}[language=C++,caption={删除所有重复节点：识别重复段后整体跳过}]
...
\begin{lstlisting}[language=C++,caption={简化路径：栈保存有效目录段}]
...
\begin{lstlisting}[language=C++,caption={最长有效括号：栈保存边界下标}]

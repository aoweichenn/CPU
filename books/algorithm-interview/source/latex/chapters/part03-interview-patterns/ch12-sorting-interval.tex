...
\begin{lstlisting}[language=C++,caption={颜色分类：三指针维护四段区域}]
...
\begin{lstlisting}[language=C++,caption={合并区间：排序后只看结果尾部}]
...
\begin{lstlisting}[language=C++,caption={插入区间：前段、中段、后段}]
...
\begin{lstlisting}[language=C++,caption={无重叠区间：按结束时间贪心保留最多区间}]
...
\begin{lstlisting}[language=C++,caption={第 K 大元素：迭代快速选择}]
...
long long count_reverse_pairs(std::vector<int> nums)
{
...
\begin{lstlisting}[language=C++,caption={会议室 II：小顶堆保存房间结束时间}]
...
\begin{lstlisting}[language=C++,caption={航班预订统计：差分数组处理区间加法}]
...
\begin{lstlisting}[language=C++,caption={前 K 高频元素：桶按频次收集}]
...
\begin{lstlisting}[language=C++,caption={相对排序：计数后按给定顺序输出}]
...
\begin{lstlisting}[language=C++,caption={稳定划分：保持非零元素相对顺序}]
...
long long reverse_pairs_by_fenwick(const std::vector<int>& nums)
{
...
\begin{lstlisting}[language=C++,caption={最少箭引爆气球：按右端点贪心}]
...
\begin{lstlisting}[language=C++,caption={区间列表交集：移动较早结束的一方}]

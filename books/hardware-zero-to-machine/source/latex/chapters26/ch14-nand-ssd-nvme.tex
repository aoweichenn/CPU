...
层 & 材料/厚度 & 功能 \\
\midrule
P 型衬底 & 掺杂 B 的 Si & 提供 P 型沟道体；编程时表面反型形成 N 沟道；擦除时 P-well 施加高压 \\\tablerowrule
N+ 源/漏 & 重掺杂 As/P & 串（string）中相邻单元共享源/漏；提供电流通路端点 \\\tablerowrule
隧穿氧化层 & SiO$_2$, 7--9\,nm & 编程/擦除时电子 FN 隧穿的势垒层；厚度决定隧穿概率和保持时间；每 P/E 循环产生少量界面态损伤 \\\tablerowrule
浮栅 & 多晶硅, 30--50\,nm & 电荷存储体；导体，电子在其中自由分布；电荷量决定单元 $V_{th}$ \\\tablerowrule
IPD (ONO) & SiO$_2$ / Si$_3$N$_4$ / SiO$_2$, 共 $\sim$15\,nm & 层间介质（Inter-Poly Dielectric）；阻止浮栅电荷向控制栅泄漏；等效氧化层厚度（EOT）决定 CG 对 FG 的耦合比 \\\tablerowrule
...
\midrule
驱动电压 & $V_{CG}$=18--20\,V, 源/漏接地 & $V_{CG}$=10--12\,V, $V_D$=5--7\,V \\\tablerowrule
电流 & $J \sim \mu\mathrm{A/cm^2}$--$\mathrm{mA/cm^2}$；单元面积极小，单单元仅 fA--pA（低功耗） & $\mu$A/单元（高功耗） \\\tablerowrule
速度 & 慢（页编程 0.5--2\,ms） & 快（单字 5--20\,$\mu$s） \\\tablerowrule
粒度 & 整页同时编程 & 逐字节/逐字 \\\tablerowrule
...
\midrule
SLC & 2 & 1 & $\sim$5\,V & 1 & 100K & 25\,$\mu$s \\\tablerowrule
MLC & 4 & 2 & $\sim$2\,V & 3 & 3K--10K & 40\,$\mu$s \\\tablerowrule
TLC & 8 & 3 & $\sim$0.8\,V & 7 & 1K--3K & 60--90\,$\mu$s \\\tablerowrule
...
ISPP 参数 & 典型值（TLC） & 影响 \\
\midrule
$V_{pgm\_start}$ & 13--15\,V & 初始编程电压；太低则第一个脉冲无效 \\\tablerowrule
$\Delta V_{ISPP}$ & 0.2--0.5\,V & 步进越小→$V_{th}$ 分布越窄→但脉冲数越多→编程越慢 \\\tablerowrule
脉冲数/页 & 8--16（TLC 全窗口） & 取决于目标状态数和步进大小 \\\tablerowrule
单脉冲宽度 & 10--20\,$\mu$s & 隧穿需要时间积累 \\\tablerowrule
...
层级 & 典型大小 & 操作粒度 & 说明 \\
\midrule
Page & 16--36\,KiB & \hwkey{编程单位} & 一次 ISPP 编程操作写入一整页；页内所有单元共享同一条字线 \\\tablerowrule
Block & 4--16\,MiB & \hwkey{擦除单位} & 一次擦除操作清除一整块；2D MLC 块 = 128--256 页（$\sim$4--8\,MiB），3D TLC 块 = 字线层数 $\times$ 3 页（232 层约 700 页，$>$10\,MiB）\\\tablerowrule
Plane & 2--4 / die & 并行单位 & 每个 plane 有独立的 page register 和行译码，可同时操作 \\\tablerowrule
Die (LUN) & 1--2\,Tbit & 独立操作单位 & 一个 die 可独立执行命令；多 die 可并行 \\\tablerowrule
...
\midrule
字线层数 & 232（2$\times$116 堆叠） & 每根沟道上的存储单元数 \\\tablerowrule
沟道材料 & poly-Si 薄膜，壁厚 $\sim$10\,nm & 沟道孔 $\phi$$\sim$80--120\,nm；poly-Si 沉积于孔壁形成圆筒沟道；晶粒边界影响迁移率 \\\tablerowrule
字线材料 & W 或 Mo + TiN 阻挡层 & 低电阻金属字线（替代 2D 的 poly-Si） \\\tablerowrule
电荷俘获层 & SiN, $\sim$5--7\,nm & 替代浮栅；局域陷阱存储 \\\tablerowrule
隧穿氧化层 & SiO$_2$, $\sim$4--5\,nm & 比 2D 更薄，靠能带工程叠层兼顾擦除与保持 \\\tablerowrule
阻挡氧化层 & SiO$_2$ / high-$k$, $\sim$5--8\,nm & 阻止电荷向字线泄漏 \\\tablerowrule
...
\midrule
存储层材料 & 多晶硅（导体） & Si$_3$N$_4$（绝缘体） \\\tablerowrule
电子状态 & 自由移动，整层等电位 & 被局域陷阱捕获，固定不动 \\\tablerowrule
横向迁移 & 有：电子可沿 FG 横向扩散 & 无：陷阱位置固定 \\\tablerowrule
单点缺陷影响 & 致命：一个针孔→整层电荷泄漏 & 局部：只影响缺陷附近少量电荷 \\\tablerowrule
单元间耦合 & 显著：相邻 FG 电容耦合（$C_{coupling}$） & 极小：绝缘体无横向电场扩展 \\\tablerowrule
3D 工艺兼容性 & 差：poly-Si FG 在垂直结构中难以均匀沉积 & 好：SiN 可用 ALD/CVD 在深孔侧壁上共形（conformal，即各处厚度均匀贴合）沉积 \\\tablerowrule
适合 3D 堆叠 & 困难 & 天然适合 \\
\bottomrule
\end{longtable}
...
\begin{lstlisting}
// 注释（推进）：先检测失效并保存仍可信的数据，再切换到备用位置或重试路径。
// 注释（边界）：更新映射或元数据后必须校验；掉电重启应能判断最后有效版本。
...
类型 & 保持规格 & 条件 & 失效后果 \\
\midrule
SLC & 10 年（新片，常温） & 裸片宣称值；寿命末期的 JEDEC JESD218 要求：消费级 1 年 @ 30°C，企业级 3 个月 @ 40°C & $V_{th}$ 下移→读错误 \\\tablerowrule
MLC & 10 年 @ 55°C & 消费级 & 同上 \\\tablerowrule
TLC & 1--5 年 @ 40--55°C & 取决于 P/E 次数 & 高 P/E 后保持急剧退化 \\\tablerowrule
...
坏块类型 & 原因 & 标记方式 \\
\midrule
出厂坏块 & 制造缺陷（氧化层针孔、颗粒污染） & 出厂时在 spare area 写入坏块标记（0x00） \\\tablerowrule
...
参数 & 计算 & 说明 \\
\midrule
映射粒度 & 页级（4\,KiB 或 16\,KiB） & 每个逻辑页对应一个物理页 \\\tablerowrule
每条目大小 & 4 字节（PPA） & 物理页号 = die + block + page + plane \\\tablerowrule
1\,TB SSD 映射表 & 1\,TB / 4\,KiB $\times$ 4\,B = 1\,GiB & 需要 DRAM 或 HMB 存储 \\\tablerowrule
...
\midrule
7\% & 930\,GB & $\sim$3--5 & 消费级 SSD \\\tablerowrule
28\% & 720\,GB & $\sim$1.5--2.5 & 企业级 SSD \\\tablerowrule
...
NVMe 特性 & 参数 & 说明 \\
\midrule
Submission Queue (SQ) & 最多 64K 个队列 & 每个队列深度最多 64K 条目 \\\tablerowrule
Completion Queue (CQ) & 与 SQ 配对 & 控制器写入完成条目，触发中断 \\\tablerowrule
Doorbell 寄存器 & 每队列一对（SQ tail, CQ head） & 主机写 SQ tail doorbell 通知控制器有新命令 \\\tablerowrule
中断 & MSI-X, 最多 2048 个向量 & 每个 CQ 绑定一个中断向量→CPU 核；64K 是队列数上限，不是向量数上限 \\\tablerowrule
命令格式 & 64 字节（SQE） & 含 opcode, LBA, 长度, PRP/SGL 指针 \\\tablerowrule
...
并行层级 & 并行度 & 操作方式 & 带宽提升 \\
\midrule
Multi-plane & 2--4 plane/die & 同一 die 内多 plane 同时编程/读 & 2--4$\times$ \\\tablerowrule
Multi-die (LUN) & 2--4 die/pkg & 不同 die 独立执行命令 & 再 2--4$\times$ \\\tablerowrule
Multi-channel & 4--8 ch & 控制器多通道并行 & 再 4--8$\times$ \\\tablerowrule
...
\begin{lstlisting}
// 注释（推进）：逐点扫描延迟或阈值并记录连续通过窗口，不能只看单个通过点。
// 注释（边界）：训练结束取稳定窗口中心；没有有效窗口时进入明确失败路径。
...
特性 & BCH（旧） & LDPC（现代） \\
\midrule
纠错能力 & 24--72 bit/1KiB & 120--240 bit/1KiB \\\tablerowrule
译码方式 & 硬判决，代数译码 & 迭代译码（belief propagation） \\\tablerowrule
软判决支持 & 不支持 & 支持（利用 LLR 信息） \\\tablerowrule
译码延迟 & 低（$\mu$s 级） & 较高（迭代 10--50 次） \\\tablerowrule
适合 RBER & $<$10$^{-5}$ & 10$^{-3}$--10$^{-4}$（TLC/QLC 寿命末期） \\\tablerowrule
...
操作 & SLC & TLC & QLC \\
\midrule
页读（page read） & 25\,$\mu$s & 60--90\,$\mu$s & 90--130\,$\mu$s \\\tablerowrule
页编程（page program） & 200--300\,$\mu$s & 0.5--2\,ms & 1--3\,ms \\\tablerowrule
块擦除（block erase） & 1.5--2\,ms & 3--5\,ms & 4--7\,ms \\\tablerowrule
读重试（per retry） & +25\,$\mu$s & +60\,$\mu$s & +90\,$\mu$s \\\tablerowrule
P/E 寿命 & 100K & 1K--3K & 500--1K \\\tablerowrule
...
\toprule
设备 & 介质的物理特性 & 队列机制要解决什么 \\
\midrule
SSD & 读快写慢、擦除单位大、有寿命 & 写放大、FTL、磨损均衡 \\\tablerowrule
机械硬盘 & 寻道和旋转是毫秒级 & 调度合并、NCQ、顺序化 \\\tablerowrule
...
\midrule
SLC & 1 & 2 & 约 $10^{5}$ 次 & 设备缓存、企业特殊款 \\\tablerowrule
MLC & 2 & 4 & 约 $3\times10^{3}$-$10^{4}$ 次 & 早期主流，现多见于工业盘 \\\tablerowrule
TLC & 3 & 8 & 约 $10^{3}$-$3\times10^{3}$ 次 & 当前消费与数据中心主流 \\\tablerowrule
...
特殊约束集中在三个操作的粒度上，而且粒度由物理直接给定：

\begin{longtable}{L{0.12\linewidth}L{0.26\linewidth}L{0.2\linewidth}L{0.3\linewidth}}
\toprule
操作 & 粒度 & 时延量级 & 物理原因 \\
\midrule
读 & 页，4-16 KiB & 25-100 $\mu$s & 一页共享字线，整页同时选通 \\\tablerowrule
编程（写） & 页，只能 1 写成 0 & 数百 $\mu$s 到约 1 ms & 逐档校准阈值电压，TLC 调 8 档 \\\tablerowrule
...
出了题目：谁让擦除集中到少数块上，擦除越集中，少数块就越早达到寿命上限。另
还需要记录两项次要物理约束：同一块被读得太多会扰动相邻页
...
职责 & 对应的介质限制 & 做法 \\
\midrule
地址映射 & 不许原地改写 & 写新页、改映射、旧页标脏 \\\tablerowrule
空页供给 & 写前必须有已擦空块 & 预留空闲块池，随写随补 \\\tablerowrule
垃圾回收 & 擦按块、块内混有有效页 & 搬走有效页，再整块擦除 \\\tablerowrule
...
有效页比例 $v$ & WA & 直观画面 \\
\midrule
0 & 1 & 无页可搬，擦掉即可（顺序改写的尽头） \\\tablerowrule
0.25 & 约 1.33 & 搬 1 页换 3 页空位 \\\tablerowrule
0.5 & 2 & 搬一页换一页，写一倍送一倍 \\\tablerowrule
0.9 & 10 & 接近写满的盘做随机写 \\
\bottomrule
\end{longtable}
...
随机写为什么放大更严重，可以从有效页比例 $v$ 看出。顺序写把失
效集中到少数块上：文件被顺序覆盖，整块一起变脏，$v$ 趋
于 0、WA 趋于 1。均匀随机写使失效页分散到所有块：每个块
都会积累少量无效页，同时保留大量有效页，长期稳态下各块的 $v$
收敛到同一高位——盘越满，$v$ 越高。剩余空间只有一成的
盘，各块平均 $v\approx0.9$，按上式计算可得 WA 接近 10：主机每写
1 GiB，介质实际承受约 10 GiB。GC 挑块因此遵循贪心原则：
优先回收无效页最多的块，同样换回一个空块，$v=0.1$ 的块
只搬一成，$v=0.9$ 的块要搬九成。GC 的时机也影响体验：
空闲时做的后台 GC 通常不影响主机请求时延，写路径上由空块耗尽触发的
同步 GC 则直接增加当次写入时延——同一笔写放大，前者发生在空闲阶段，后者表现为请求时延尖峰。同一个“随机写”，在“冒险、异常与性能”一章只是时延
问题，在闪存这里同时是寿命问题：放大几倍，介质承担的 P/E 消耗就相应增加。工程上的缓解措施是预留空间（overprovisioning）：
盘内实装闪存多于标称容量，消费盘多留约 7\%，企业盘留到
28\% 上下，差额直接压低 $v$ 的有效值，把稳态 WA 降回个
位数。消费 TLC 盘还可使用 SLC 缓存：将一部分闪存配置为 SLC
模式，突发写入先进入缓存、空闲时再迁移至 TLC，缓存空间耗尽后的
缓存外持续写入速度才反映介质的持续写入能力。

\topic{磨损均衡与寿命}
写放大会消耗寿命预算，而 P/E 次数按块计：写入分布不
均，少数块先达到擦除寿命上限，整盘提前报废。磨损均衡（wear
leveling）的任务是把擦除次数在所有块之间摊平。动态磨损
均衡仅处理活跃数据：每次分配空块时挑擦除次数最少的，新写
入自然流向擦除次数较少的块。它对冷数据无效——一批文件写进去三年
不动，占着的块擦除次数恒为零，其余块继续承担擦写负载。静态磨
损均衡也会迁移冷数据：定期将长期不改写的数据迁移到擦除次数
多的块中长期存放，使擦除次数较少的块重新加入分配。消费级 SSD 通常采用动态磨损均衡，并辅以有
限的静态磨损均衡；企业级 SSD 则会更充分地实施两类均衡。需要注意的是，均衡搬运同样计入写放大，
均衡寿命分布本身需要额外写入。坏块管理同样依赖备用块池：
出厂坏块与使用过程中产生的坏块均由备用块替换；每替换一个坏块，
可用块数量与预留空间都会相应减少。

把擦除次数按块排成直方图，均衡与不均衡的差距清晰可见：
...
\end{pdfdiagram}
\diagramnote{磨损均衡前后的擦除次数分布（块按擦除次数排列）。只有动态均衡时，新写入总挑擦除次数最少的块，热数据块反复被分配、先达到 P/E 次数上限，而冷数据占着的块擦除次数恒为零——少数块先达到寿命上限，整盘提前报废。静态磨损均衡定期把长期不改写的冷数据搬入擦除次数较多的块中长期存放，使擦除次数较少的块重新加入分配，各块擦除次数被摊平；均衡搬运本身计入写放大，寿命均衡以额外写入为代价。}
...
每天写入量 & 预算天数 & 折合年限 \\
\midrule
20 GB & 7500 天 & 约 20.5 年 \\\tablerowrule
40 GB & 3750 天 & 约 10.3 年 \\\tablerowrule
100 GB & 1500 天 & 约 4.1 年 \\\tablerowrule
...
条曲线提供保持力保障，刷新本身也耗 P/E，又一笔主机看不见的磨损。
...
页是有效数据，GC 照样把它们作为有效数据搬移，写放大无谓增加。
...
\midrule
出厂态（新盘） & 全部块空闲 & 1 & 数万 \\\tablerowrule
稳态，有 TRIM 与预留 & GC 回收 & 2-3 & 万级 \\\tablerowrule
...
稳态性能还有队列一侧的条件，用“冒险、异常与性能”一章的 Little 定律直接
...
别把随机写当顺序写使。这三项措施不需要新增硬件，花的只是对介质物理约束
的理解。
...
Flash 约束
\exercisecheck{题目}{为什么 NAND 不能像 DRAM 一样原地覆盖任意字节？说明 page、block 与擦除的关系。}
\end{chapterexercise}

\begin{chapterexercise}{2}
FTL
\exercisecheck{题目}{顺序逻辑写经过 FTL 后一定形成顺序物理写吗？列出映射、垃圾回收和磨损均衡的影响。}
\end{chapterexercise}

\begin{chapterexercise}{3}
NVMe trace
\exercisecheck{题目}{从主机填写 SQ 项开始，按顺序追踪一次 NVMe 读到 CQ 完成，并标出 CPU 与控制器的 DMA 所有权交接。}
\end{chapterexercise}

\begin{chapterexercise}{4}
队列
\exercisecheck{题目}{SQ/CQ 为什么需要命令标识和 phase？队列环回时只比较数组下标有什么问题？}
\end{chapterexercise}

\begin{chapterexercise}{5}
超时恢复
\exercisecheck{题目}{命令超时后为什么不能立即释放数据页和命令标识？给出安全复位次序。}
\end{chapterexercise}

\begin{chapterexercise}{6}
持久化
\exercisecheck{题目}{普通写完成、FUA 写完成与 FLUSH 完成分别能支持什么推断？}
\end{chapterexercise}
...
Flash 约束
\answercheck{解答要点}{NAND 以 page 编程、以包含多个 page 的 block 擦除；改写旧数据通常写到新 page，再由垃圾回收搬迁有效页并擦除整块。}
\end{chapteranswer}

\begin{chapteranswer}{2}
FTL
\answercheck{解答要点}{不一定。逻辑到物理映射可重定向位置，垃圾回收搬移有效页，磨损均衡还会主动迁移数据；这些动作改变物理顺序和写放大。}
\end{chapteranswer}

\begin{chapteranswer}{3}
NVMe trace
\answercheck{解答要点}{主机写 SQ 与数据描述并同步，写 doorbell；控制器 DMA 取命令，访问 FTL/NAND，把数据 DMA 到主机页，写 CQ 与 phase；主机在中断/轮询后同步并回收。}
\end{chapteranswer}

\begin{chapteranswer}{4}
队列
\answercheck{解答要点}{命令标识把乱序完成归属到请求，phase 区分环回后的新旧 CQ 项；仅看下标会把上一圈残留内容误作新完成。}
\end{chapteranswer}

\begin{chapteranswer}{5}
超时恢复
\answercheck{解答要点}{控制器可能仍在读写旧页并返回迟到完成。先停新提交、屏蔽通知、取消或复位队列并确认 DMA 停止，再按 generation 处理旧请求，最后解映射和复用资源。}
\end{chapteranswer}

\begin{chapteranswer}{6}
持久化
\answercheck{解答要点}{普通完成可能只到易失缓存；FUA 要求该写跨过设备定义的非易失边界；FLUSH 要求此前相关写到达该边界。它们仍需上层正确安排数据与元数据顺序。}
\end{chapteranswer}

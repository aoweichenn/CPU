...
\begin{keyidea}
单周期、多周期和流水线并不是三套互不相干的机器，而是对硬件复用、关键路径和吞吐的三种组织方式。本章先建立执行组织与逐拍控制，再用整机和经典处理器案例核对这些机制，最后进入现代乱序执行。
\end{keyidea}

\section{一、单周期、多周期与流水线的共同状态边界}
...
事件 & 流水寄存器动作 & 不能丢失的状态 \\
\midrule
正常推进 & 下一级接收数据、控制与 valid & PC、目标寄存器、异常元数据 \\\tablerowrule
上游停顿 & PC 与相关前级保持，下游可排空 & 被阻塞指令及其控制位 \\\tablerowrule
插入气泡 & 下一级 valid 清零 & 老指令仍按原路径推进 \\\tablerowrule
分支冲刷 & 错误路径各级 valid 清零，PC 重定向 & 分支自身结果与正确目标 \\\tablerowrule
可变时延等待 & 请求级保持事务身份，响应级按 ID 接收 & 地址、byte enable、请求 ID、错误状态 \\\tablerowrule
精确异常 & 阻止年轻副作用并保存异常 PC/原因 & 老指令提交状态和异常指令身份 \\
\bottomrule
\end{longtable}

\section{四、多周期功能单元与可变时延存储器}
...
结构 & 索引/输入 & 保存状态 & 输出 \\
\midrule
BHT & 取指 PC 与可选历史 & 饱和计数器或模式状态 & 跳转方向预测 \\\tablerowrule
BTB & 取指 PC & tag、分支类型、目标地址 & 预测目标 \\\tablerowrule
RAS & call/return 事件 & 有限深度的返回地址栈 & 预测返回 PC \\\tablerowrule
恢复检查点 & 预测发生时的重命名/前端状态 & 可恢复指针或快照 & 误预测后的正确状态 \\
\bottomrule
\end{longtable}

\section{六、逐拍核对一次相关、停顿与重定向}
...
这张表有两条边界约定必须固定。第一，置位与清除都发生在拍末（时钟沿），比对用拍内的当前值，所以写回当拍的目标寄存器仍算 pending——多停一拍是保守但安全的约定。第二，pending 表只回答“能不能走”，不回答“值是多少”：它维护提交顺序，不维护数据本身；想减少停顿，需要上一专题的转发网络把未提交的值直接送往 ALU 输入。意义在于规模：8 个触发器加几组比较器，就把“某个结果现在是否可用”变成可查表的硬件事实；上一专题 \code{IDEX.memRead} 那条布尔式只是这张表“只看最近一条在途指令”的特例。理解了 pending 表，后面再看记分牌调度、寄存器重命名时，认出的都是同一思想的更细版本。
...
\section{八、早期微处理器：从 6502、Z80 到 8086}
...
Z80 的设计有两条主线。第一，寄存器越多，上下文切换越少访问内存：一组备用寄存器换来的是中断响应里整段存储器往返的消失。第二，刷新是“借用已有事务”的设计：DRAM 刷新本来要占用专门的总线时间，Z80 把它安排在每个取指周期里必然出现的空闲半段，系统里就再也看不见它。
...
286 保护模式用得少，原因正是表末两行。一是回程困难：286 没有提供把 PE 位清回去的手段，从保护模式回实模式只能靠硬件复位——PC/AT 的取巧做法是让键盘控制器拉复位线，BIOS 按事先约定的关机码恢复现场，慢而且别扭。二是没有分页：保护模式解决了越界检查和特权级，却没解决按页换出，而 DOS 生态的软件全按实模式固定，真正用 286 保护模式的只有 OS/2 1.x、XENIX 等少数系统。但它的历史位置不能跳过：GDTR/IDTR 的装载方式、选择子加描述符这层间接、“段寄存器不再是地址而是句柄”的观念转变，都是 80386 及其后所有 x86 保护模式的直接先例。“80386：32-bit、保护模式与地址转换”一节读 386 时，描述符表原样保留，只是基址扩到 32 位、访问权更细，再叠上分页这一层。
...
\midrule
取指队列 & PC、字节块、预测信息 & 指令 Cache 返回且队列有空位 & I-Cache miss、队列满或重定向 \\\tablerowrule
译码队列 & 指令边界、操作数编码、异常标记 & 后端可接受对应数量的微操作 & 复杂译码占用、重命名端背压 \\\tablerowrule
重命名端 & 架构源/目的、物理映射、ROB 序号 & 物理寄存器、ROB 和发射队列都有空位 & 任一资源不足时整组停顿 \\\tablerowrule
...
\midrule
I1 & \code{x1=x2+x3} & \code{p2,p3} & \code{x1->p32} & \code{p1} \\\tablerowrule
I2 & \code{x4=x1-1} & \code{p32} & \code{x4->p33} & x4 的旧映射 \\\tablerowrule
I3 & \code{x6=x7*x8} & x7、x8 的当前映射 & \code{x6->p34} & x6 的旧映射 \\\tablerowrule
...
\midrule
1 & I1 发射 & I4 发射 & I3 发射 & I2 等待 \code{p32} \\\tablerowrule
2 & I2 被唤醒并发射 & 空闲 & I3 第 2 拍 & I1、I4 已执行完成 \\\tablerowrule
3 & 空闲 & 空闲 & I3 第 3 拍 & I2 已执行完成 \\\tablerowrule
...
\midrule
地址已知且不重叠 & 可越过该 store 访问 Cache & 已证明不存在内存依赖 \\\tablerowrule
地址和数据已知且重叠 & 从 store queue 转发匹配字节 & 最新值尚未进入 Cache \\\tablerowrule
地址未知 & 等待，或推测越过并保留重放条件 & 可能存在尚未发现的同址写 \\\tablerowrule
...
\midrule
执行完成 & 物理结果写回，唤醒依赖者 & 结果已经成为不可回退状态 & 在 ROB 中记录异常或重放 \\\tablerowrule
ROB 队首完成 & 按退休宽度检查提交资格 & 较晚项也可越过异常提交 & 较早异常阻止本项退休 \\\tablerowrule
指令退休 & 架构映射确定，旧物理寄存器可回收 & store 已经到达内存介质 & store 只获得进入写缓冲资格 \\\tablerowrule
...
\section{十一、向量执行：一条指令怎样推进多个数据元素}
...
\section{十二、SMT：多个硬件线程怎样共享一个乱序核心}
...
\begin{chapterexercise}{综合练习：有效位至预测结构}
\begin{exercisesubproblem}{（a）有效位}
比较气泡与编码 NOP 的状态含义
\exercisecheck{检查要点}{气泡不携带真实工作或副作用}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）优先级}
同拍发生老指令异常和年轻 load 等待时给出控制顺序
\exercisecheck{检查要点}{精确异常优先且响应仍可归属}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）可变时延}
列出 load miss 等待期间必须保存的字段
\exercisecheck{检查要点}{地址、目标、宽度、ID 和错误同行}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）预测结构}
分别说明 BHT、BTB 与 RAS 的输入和输出
\exercisecheck{检查要点}{方向、普通目标、返回目标分开}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：计数器 trace 至 ADD trace}
\begin{exercisesubproblem}{（a）计数器 trace}
从弱跳转开始，对 T、T、N、N、N 更新二位计数器
\exercisecheck{检查要点}{每次只向实际结果移动一态}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）流水 trace}
写出 LOAD-use 停顿与随后分支误预测的 valid 变化
\exercisecheck{检查要点}{停顿保持，气泡插入，误预测清除}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）PC 取指}
写出 PC、指令存储器、IR 和 \code{pc_write/ir_write} 的周期关系
\exercisecheck{检查点}{能说明哪一拍保存指令}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）ADD trace}
为 \code{ADD R1,R2}（即 \code{R1=R1+R2}）列出读地址、ALU 操作、写回选择和写使能
\exercisecheck{检查点}{写回只在目标时钟沿发生}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：加载 trace 至条件码比较}
\begin{exercisesubproblem}{（a）加载 trace}
把加载指令拆成地址形成、存储读、MDR 保存、寄存器写回
\exercisecheck{检查点}{能处理存储器未 ready}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）存储 trace}
说明地址、写数据、字节使能、写控制和无寄存器写回
\exercisecheck{检查点}{能区分 RAM 写和 MMIO 副作用}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）JZ trace}
说明 Zero 标志如何影响下一 PC
\exercisecheck{检查点}{分支目标和顺序 PC 选择明确}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）条件码比较}
分别解释有符号小于和无符号小于使用哪些 ALU 标志
\exercisecheck{检查点}{不把 Carry、Sign 和 Overflow 混用}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：CALL/RET trace 至流水线冒险}
\begin{exercisesubproblem}{（a）CALL/RET trace}
把调用和返回拆成保存返回地址、更新栈指针、改写 PC 和恢复 PC
\exercisecheck{检查点}{能指出栈访问本质上是加载/存储事务}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）控制字}
设计一个包含 \code{alu_op/wb_sel/reg_write/mem_read/mem_write/pc_sel} 的控制字
\exercisecheck{检查点}{每一位能追到实际电路}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）微程序}
为加载指令写出 5 个微步骤和等待状态处理
\exercisecheck{检查点}{慢存储器只拖住访存阶段}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）流水线冒险}
说明结构、数据、控制三类冒险各破坏什么边界
\exercisecheck{检查点}{能提出停顿、转发或冲刷策略}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：SHL 微操作至 cmp+jcc 配对}
\begin{exercisesubproblem}{（a）SHL 微操作}
为 \code{0x7260}=\code{SHL R1, 8} 写出 T1--T5 各拍的微操作、控制线与提交对象
\exercisecheck{检查点}{不经过 T4，写回只在 T5 提交}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）三种组织拍数}
统计 ROM 前三条（\code{0x22F0}、\code{0x7260}、\code{0x3450}）在单周期、五拍多周期、理想五级流水下各用多少拍
\exercisecheck{检查点}{拍数与周期长度分开讨论}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）新增 SUB}
为 \code{SUB Rd, Rs}（\code{R[d]=R[d]-R[b]}）写控制字行和逐拍流程，说明数据通路是否要加新部件
\exercisecheck{检查点}{复用既有 ALU 减法路径}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）cmp+jcc 配对}
把 \code{JZ R2, +3} 改写成 x86-64 的 \code{test}/\code{jz} 两条指令，说明标志位在两条指令之间的角色
\exercisecheck{检查点}{标志是前写后读的约定状态}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：CPI 演算至唤醒与端口}
\begin{exercisesubproblem}{（a）CPI 演算}
...
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）pending 表读法}
...
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）乱序重命名}
...
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）唤醒与端口}
两条乘法的源都在同拍就绪，但核心只有一个乘法端口。说明 select 逻辑如何处理未选中的一条，以及为什么不能清除它的就绪位。
\exercisecheck{学习目标}{就绪不等于已经获得执行资源}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：存储转发至误预测恢复}
\begin{exercisesubproblem}{（a）存储转发}
...
\begin{chapteranswer}{综合练习：有效位至预测结构}
\begin{answersubproblem}{（a）有效位}
NOP 是有效指令且可能计入退休、异常和调试语义；气泡的 valid 为 0，所有架构写使能必须无效。
\answercheck{必须保留的检查点}{不用某个 opcode 代替所有权状态}
\end{answersubproblem}
\begin{answersubproblem}{（b）优先级}
保存老指令异常 PC/原因，撤销年轻请求或标记其响应不可提交；流水转入异常入口，同时保证已接受事务不会失去归属。
\answercheck{必须保留的检查点}{老状态精确、年轻副作用不可见}
\end{answersubproblem}
\begin{answersubproblem}{（c）可变时延}
至少保存 PC、目标寄存器、地址、宽度、符号扩展、请求 ID、异常和取消状态。
\answercheck{必须保留的检查点}{返回数据必须匹配原指令}
\end{answersubproblem}
\begin{answersubproblem}{（d）预测结构}
BHT 预测方向，BTB 给普通控制转移目标，RAS 给 return 目标；解析结果负责校验并恢复。
\answercheck{必须保留的检查点}{预测不等于提交}
\end{answersubproblem}
\end{chapteranswer}

\begin{chapteranswer}{综合练习：计数器 trace 至 ADD trace}
\begin{answersubproblem}{（a）计数器 trace}
弱跳转依次到强跳转、强跳转、弱跳转、弱不跳转、强不跳转。
\answercheck{必须保留的检查点}{强态需两次相反结果才翻转预测}
\end{answersubproblem}
\begin{answersubproblem}{（b）流水 trace}
LOAD 继续进 MEM，ADD 前级保持并向 EX 插气泡；数据可旁路后恢复。分支误预测沿清除更年轻 valid 并重定向 PC。
\answercheck{必须保留的检查点}{老指令不被误清除}
\end{answersubproblem}
\begin{answersubproblem}{（c）PC 取指}
PC 输出取指地址，指令存储器返回编码，\code{ir_write} 在取指边界保存编码；下一 PC 可由 \code{pc+size}、分支目标或异常入口选择。
\answercheck{必须保留的要点}{PC 和 IR 都是状态元件。}
\end{answersubproblem}
\begin{answersubproblem}{（d）ADD trace}
...
\end{answersubproblem}
\begin{answersubproblem}{（d）条件码比较}
...
\begin{chapteranswer}{综合练习：CALL/RET trace 至流水线冒险}
\begin{answersubproblem}{（a）CALL/RET trace}
...
\end{answersubproblem}
\begin{answersubproblem}{（b）三种组织拍数}
...
\end{answersubproblem}
\begin{answersubproblem}{（c）新增 SUB}
...
\end{answersubproblem}
\begin{answersubproblem}{（d）cmp+jcc 配对}
...
\begin{chapteranswer}{综合练习：CPI 演算至唤醒与端口}
\begin{answersubproblem}{（a）CPI 演算}
...
\end{answersubproblem}
\begin{answersubproblem}{（b）pending 表读法}
...
\end{answersubproblem}
\begin{answersubproblem}{（c）乱序重命名}
...
\end{answersubproblem}
\begin{answersubproblem}{（d）唤醒与端口}
...
\begin{chapteranswer}{综合练习：存储转发至误预测恢复}
\begin{answersubproblem}{（a）存储转发}
...
\end{answersubproblem}
\begin{answersubproblem}{（c）误预测恢复}
...
\end{answersubproblem}
\begin{answersubproblem}{（c）Z80 M/T 计数}

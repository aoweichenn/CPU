...
\begin{chapterexercise}{8086 地址形成}
用段寄存器和偏移计算一个物理地址
\exercisecheck{学习目标}{能说明 20-bit 地址来自硬件加法路径}
\end{chapterexercise}

\begin{chapterexercise}{复用总线}
写出 8086 读周期中地址、ALE、数据方向和 READY 的阶段
\exercisecheck{学习目标}{能指出何时锁存地址、何时采样数据}
\end{chapterexercise}

\begin{chapterexercise}{字节存储体}
解释 A0 与 \(\overline{\mathrm{BHE}}\) 如何选择低/高字节
\exercisecheck{学习目标}{能处理奇地址字访问}
\end{chapterexercise}

\begin{chapterexercise}{80386 分页}
把一个 32-bit 线性地址拆成目录索引、页表索引和页内偏移
\exercisecheck{学习目标}{能说明异常发生在真正访存前}
\end{chapterexercise}

\begin{chapterexercise}{保护模式入口}
列出 GDTR/IDTR、CR0、CR3 和跳转刷新各自作用
\exercisecheck{学习目标}{能说明状态切换顺序}
\end{chapterexercise}

\begin{chapterexercise}{复位向量}
比较 6502、Z80/8051、8086、80386 后续 x86 的复位入口
\exercisecheck{学习目标}{能说明第一条指令必须由非易失存储响应}
\end{chapterexercise}

\begin{chapterexercise}{平台分层}
说明 CPU 本地总线、内存/cache 控制、高速扩展、低速 I/O 和固件层职责
\exercisecheck{学习目标}{能说明等待来自整个平台}
\end{chapterexercise}

\begin{chapterexercise}{SoC 演进}
说明把内存控制器、I/O 和安全启动搬进片内的收益与代价
\exercisecheck{学习目标}{能指出片上互连和调试复杂度}
\end{chapterexercise}

\begin{chapterexercise}{主板结构}
画出 CPU、DDR、PCH、UEFI Flash、PCIe、NVMe、GPU 的连接关系
\exercisecheck{学习目标}{能说明供电、时钟、复位和枚举不是 CPU 内部问题}
\end{chapterexercise}
...
\begin{chapterexercise}{6502 寻址与拍数}
用零页寻址和 (zp),Y 间接变址写数组求和循环并逐拍计数
\exercisecheck{学习目标}{理解拍数就是总线事务序列}
\end{chapterexercise}

\begin{chapterexercise}{Z80 交换与刷新}
说明 EXX 怎样加速中断响应、R 寄存器怎样把刷新藏进取指
\exercisecheck{学习目标}{寄存器组交换=不访存的上下文切换}
\end{chapterexercise}

\begin{chapterexercise}{486/Pentium 集成}
列出 80486 和 Pentium 各自搬进片内的部件及对应收益
\exercisecheck{学习目标}{集成把等待从板级移到片内}
\end{chapterexercise}

\begin{chapterexercise}{6502 五拍 trace}
写出 \code{(zp),Y} 间接变址读一个字节的 5 个时钟拍：每拍地址总线上是什么、CPU 内部在做什么
\exercisecheck{学习目标}{拍数能还原成总线事务序列}
\end{chapterexercise}

\begin{chapterexercise}{Z80 M/T 计数}
数出 \code{LD A,(HL)} 和 \code{EXX} 各占几个机器周期、几个 T 状态，并指出 DRAM 刷新藏在哪一拍
\exercisecheck{学习目标}{M1 后半拍兼做刷新，不另占总线事务}
\end{chapterexercise}

\begin{chapterexercise}{页表 walk 演算}
...
\begin{chapterexercise}{BIOS 四阶段}
把 PC 从复位到交出启动扇区控制权的过程拆成 POST、设备初始化、启动设备选择、签名移交四个阶段，写出每阶段完成的边界
\exercisecheck{学习目标}{每阶段有明确的完成标志和失败现象}
\end{chapterexercise}

\begin{chapterexercise}{RISC-V/x86 对照}
用同一段求和循环说明定长 load/store 与变长内存操作数在译码和编译器上的差别
\exercisecheck{学习目标}{复杂度放在译码前端还是编译器调度里}
\end{chapterexercise}

\section*{参考解答与要点提示}

\begin{chapteranswer}{8086 地址形成}
物理地址由 \code{segment*16+offset} 得到，内部编程模型保持 16-bit，外部地址能力扩展到 20-bit。
\answercheck{必须掌握的要点}{分段是地址形成硬件。}
\end{chapteranswer}

\begin{chapteranswer}{复用总线}
T1 输出地址并用 ALE 锁存；T2 改变总线方向并给出读控制；T3/Tw 等待目标驱动数据且 READY 可延长；T4 采样并结束。
\answercheck{必须掌握的要点}{复用引脚必须分阶段解释。}
\end{chapteranswer}

\begin{chapteranswer}{字节存储体}
偶地址低字节由 A0 选择，奇地址高字节由 \(\overline{\mathrm{BHE}}\) 参与选择；奇地址字访问可能拆成两个周期。
\answercheck{必须掌握的要点}{位宽不等于任意地址一次完成。}
\end{chapteranswer}

\begin{chapteranswer}{80386 分页}
4 KiB 页下，高 10 bit 选 PDE，中 10 bit 选 PTE，低 12 bit 是页内偏移；PDE/PTE 不存在或权限不允许时触发异常。
\answercheck{必须掌握的要点}{地址转换也是硬件路径。}
\end{chapteranswer}

\begin{chapteranswer}{保护模式入口}
进入保护模式前要准备描述符表、装载 GDTR/IDTR、设置 CR0、刷新取指路径；分页还需准备页目录/页表、装载 CR3、打开分页位。
\answercheck{必须掌握的要点}{状态切换顺序错误会让取指和异常入口失效。}
\end{chapteranswer}

\begin{chapteranswer}{复位向量}
6502 从高端向量取入口，Z80/8051 常从 0 地址开始，8086 从 \code{0xFFFF0} 附近，后续 x86 从更高物理地址附近，SoC 可从 Boot ROM 进入。
\answercheck{必须掌握的要点}{启动入口是硬件规格。}
\end{chapteranswer}

\begin{chapteranswer}{平台分层}
CPU 本地总线处理核心事务，内存/cache 控制处理 DRAM 和回填，高速扩展处理图形/磁盘/网络，低速 I/O 处理串口/键盘/RTC，固件负责初始化和启动选择。
\answercheck{必须掌握的要点}{主频之外还有平台等待。}
\end{chapteranswer}

\begin{chapteranswer}{SoC 演进}
片上集成缩短常见路径、减少引脚和功耗，但引入片上互连、一致性、时钟/电源域、安全启动和调试复杂度。
\answercheck{必须掌握的要点}{低时延来自边界移动，不是所有路径变快。}
\end{chapteranswer}

\begin{chapteranswer}{主板结构}
内存控制器在 CPU 内，CPU 直连 DDR 内存和 PCIe Root（显卡、NVMe 等高速设备）；PCH 经 DMI 与 CPU 相连，挂低速 I/O（串口、键盘、RTC）与固件 Flash；固件上电先执行并完成初始化。
\answercheck{必须掌握的要点}{平台分层是把不同速度的路径分开组织。}
\end{chapteranswer}
...
\begin{chapteranswer}{6502 寻址与拍数}
LDA 零页 3 拍，(zp),Y 间接变址 5 拍（跨页 6 拍）；求和循环每轮用 (zp),Y 读一个元素，接 INY、BNE，拍数可手工核对。
\answercheck{必须掌握的要点}{简单数据通路让逐拍 trace 可行。}
\end{chapteranswer}

\begin{chapteranswer}{Z80 交换与刷新}
\code{EX AF,AF'} 交换 AF 一对，\code{EXX} 一条指令交换 BC、DE、HL 三对，省去压栈；M1 取指后半段把 R 输出到地址总线完成 DRAM 刷新。
\answercheck{必须掌握的要点}{交换和借用都是减少总线事务。}
\end{chapteranswer}

\begin{chapteranswer}{486/Pentium 集成}
486 搬入 cache、FPU、流水线；Pentium 搬入双发射、BTB 预测、分离 cache 和 64-bit 数据总线。
\answercheck{必须掌握的要点}{每一代都在移动等待的位置。}
\end{chapteranswer}

\begin{chapteranswer}{6502 五拍 trace}
拍 1 取 opcode，拍 2 取零页操作数，拍 3、拍 4 从零页相邻两字节读出 16-bit 指针的低、高字节，拍 5 用指针加 Y 的有效地址读数据；低字节加法跨页时多 1 拍修正高字节，共 6 拍。
\answercheck{必须掌握的要点}{5 拍 = 2 拍取指、2 拍取指针、1 拍读数。}
\end{chapteranswer}

\begin{chapteranswer}{Z80 M/T 计数}
\code{LD A,(HL)} 为 2 个机器周期共 7 个 T 状态（M1 取指 4T，存储器读 3T）；\code{EXX} 只有 M1，共 4 个 T 状态。刷新藏在每个 M1 的 T3、T4：指令字节已入 CPU，地址总线让给 R 寄存器输出刷新行地址。
\answercheck{必须掌握的要点}{T 状态计数能直接核对数据手册的时序图。}
\end{chapteranswer}

\begin{chapteranswer}{页表 walk 演算}
...
\begin{chapteranswer}{BIOS 四阶段}
阶段一，POST 加电自检：检查 CPU、ROM 校验和与芯片组基本寄存器，初始化并测试内存，失败时以蜂鸣码或诊断码报告；阶段二，设备初始化：执行视频等 option ROM，初始化键盘、磁盘控制器，并建立中断向量表与 BIOS 数据区；阶段三，启动设备选择：BIOS 按启动顺序把候选设备的第一个扇区（512 字节）读到 \code{0x0000:0x7C00}；阶段四，签名移交：检查扇区末尾 \code{0x55AA} 签名，有效则跳转到 \code{0x7C00}，交出控制权。
\answercheck{必须掌握的要点}{每阶段交出确定状态，下一段才有约定可依。}
\end{chapteranswer}

\begin{chapteranswer}{RISC-V/x86 对照}
RISC-V 定长 32-bit、load/store、31 个自由寄存器：译码边界固定，循环中加载与加法各一条指令，编译器调度加载以隐藏等待；x86-64 变长 1--15 字节、16 个寄存器、运算可带内存操作数：同一循环指令更少更短，但译码器要先确定指令边界，内存操作数在片内被拆成加载微操作和加法微操作。
\answercheck{必须掌握的要点}{ISA 风格决定复杂度由译码前端还是编译器调度承担。}
\end{chapteranswer}
...
使用的栈 & 同一条用户栈 & 切到本进程内核栈 \\\tablerowrule
...
\midrule
保存现场 & \code{rip}→\code{rcx}、\code{rflags}→\code{r11}、切内核栈、建 \code{pt\_regs} & 处理器核心相关章节“保存故障 PC 和原因” \\\tablerowrule

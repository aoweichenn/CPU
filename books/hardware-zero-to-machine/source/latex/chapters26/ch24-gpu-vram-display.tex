...
存储与设备事务部分讲 DMA 时说过它只擅长"整块、连续、大批"
...
和处理器核心部分的寄存器组对照最直观。最小 CPU 里 8 到
...
这个思想本书已经见过一次。NVMe 与存储 I/O 两章讲 NVMe 队列
...
工程含义与存储与设备事务部分 cache 友好遍历是同一类动作：按
...
这就是 GPU 版的空间局部性。存储与设备事务部分讲 cache 行与
...
补一块——这个队列行为和NVMe 与存储 I/O 两章的命令队列是同
一张脸。
...
是算术。与NVMe 与存储 I/O 两章 NVMe 一次读的 trace 对照着看
...
。这正是存储与设备事务部分描述符 DMA 的那张图，只是执行体从磁盘控
...
GPU 与 CPU 的协作不靠函数调用，而靠一条与NVMe 与存储 I/O 两章 NVMe
...
这条提交—执行—回收的环路画出来，与存储与设备事务部分、第十三
章是同一副骨架：
...
把这段与存储与设备事务部分的描述符环并排，是同一张图的第三次出现：
环不动、指针动；doorbell 只通知、不携带数据；设备按头
尾差补读，偶尔错过一次通知也无妨。与NVMe 与存储 I/O 两章 NVMe 提交
...
以NVMe 与存储 I/O 两章"先写命令项、再写 doorbell，中间隔一道屏障"
...
把 GPU 命令环与NVMe 与存储 I/O 两章的 NVMe 提交队列逐项对照，结构
...
\end{pdfdiagram}
\diagramnote{命令缓冲与命令环的二级结构。应用先把命令录进命令缓冲这种可整块复用的对象（录一次），提交时再把整块挂到主存命令环上——一帧里不变的绘制序列录一次、提交多次，驱动省掉重新编码的开销。环只是搬运通道：doorbell 照样只带新尾指针，GPU 前端经 DMA 取命令。构造的成本与提交的成本分开，贵的只做一次，与存储与设备事务部分"描述符预先建好、传输时只改指针"是同一招。}
...
令一个完成项，而是一道"到此为止"的水线。与NVMe 与存储 I/O 两章
...
章中断合并、NVMe 与存储 I/O 两章轮询模式（如高性能 NVMe 驱动 polling
...
这张表有三处值得停下来看。第一，第 1 到第 3 步与NVMe 与存储 I/O 两章 NVMe
的提交一模一样：备好数据、写命令、doorbell——存储与设备事务部分"描述符先
...
步 & 动作 & 耗时量级 & 执行者 \\
\midrule
1 & 把 a、b 放入锁页（pinned）缓冲（存储与设备事务部分：DMA 要求物理地址稳定） & 一次性 & CPU \\
...
把这次 trace 与NVMe 与存储 I/O 两章的 NVMe 读并排摆开，是同一个骨架穿了两
套衣服：

\begin{longtable}{L{0.12\linewidth}L{0.38\linewidth}L{0.34\linewidth}}
\toprule
维度 & 内核启动 & NVMe 读（NVMe 与存储 I/O 两章） \\
\midrule

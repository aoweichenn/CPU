...
显存 VRAM & 数 GB 至数十 GB & 约四到八百拍 & 分配器管空间，带宽见前文 \\\tablerowrule
...
\begin{chapterexercise}{内核 trace 排序}
...
\begin{chapteranswer}{时延与吞吐取向}
A：$4\times5\times10^{9}=2\times10^{10}$ 次/秒；B：$128\times1.5\times10^{9}=1.92\times10^{11}$ 次/秒，约为 A 的 9.6 倍。但 B 的单 lane 每拍只 1 次操作且时钟更低，指针追逐这类串行依赖代码在 B 上只有一条 lane 干活，时延比 A 差几十倍；数组加法没有依赖，B 的吞吐直接兑现。取向由负载的依赖结构决定
\answercheck{必须掌握的要点}{吞吐方案赢在可并行负载，串行负载仍归时延方案}
\end{chapteranswer}

\begin{chapteranswer}{占用率演算}
...
\begin{chapteranswer}{内核 trace 排序}

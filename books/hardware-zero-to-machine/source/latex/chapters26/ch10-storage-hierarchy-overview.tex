...
\begin{keyidea}
存储系统不是一排容量和速度数字，而是一条由不同粒度、协议和完成语义组成的访问路径。本章建立从 load 到块 I/O 的全局坐标系；后续各章分别承担 Cache、地址转换、DRAM 与块设备的首次完整教学。
\end{keyidea}
...
\section{四、存储层次、地址转换和设备驱动 trace（执行轨迹）}
...
STORE 对流水线来说可以很早“执行完成”：地址和数据已经形成，写事务进入 store buffer，后续独立指令可以继续执行。但这个 STORE 什么时候对其他核心、DMA 或设备可见，要看 store buffer 排队、cache 状态、内存类型和屏障规则。本章后文将从 DMA 可见性进入多核内存模型；这里先固定一条直接相关的规则：普通 RAM 写可以合并和延迟；MMIO 写通常要求更强顺序；DMA 描述符写在通知设备前必须已经对设备可见。
...
\midrule
store 执行 & 地址、数据和字节掩码形成 & 写请求内容确定 & 该写不会因异常撤销 \\\tablerowrule
store 退休 & 指令越过精确异常边界 & 写属于正确程序路径 & 其他核心已经看到 \\\tablerowrule
取得写权限 & 其他共享副本失效或降级 & 本核可以修改该 line & 写已经持久化 \\\tablerowrule
观察者读取 & 观察者按内存模型取得某个版本 & 该观察者的可见顺序确定 & 介质掉电后仍存在 \\\tablerowrule
...
核心 0 & 动作 & 核心 1 & 动作 \\
\midrule
S0 & \code{x=1} & S1 & \code{y=1} \\\tablerowrule
...
生产者 & 顺序约束 & 消费者 & 顺序约束 \\
\midrule
\code{data=42} & 普通写可先在缓冲中推进 & \code{r=load\_acquire(ready)} & 后续访问不能越过成功 acquire \\\tablerowrule
...
\midrule
编译器屏障 & 编译器的指令与访问排序 & 编译阶段不跨越指定点移动 & 不会自动生成硬件同步事务 \\\tablerowrule
硬件内存屏障 & load/store、原子操作与观察顺序 & 按 ISA 规定限制重排或等待完成 & 不等于清空全部 Cache \\\tablerowrule
Cache clean & 脏 Cache line & 把修改推进到指定一致性点 & 不一定到达持久介质 \\\tablerowrule
Cache invalidate & 本地有效副本 & 后续读取重新取得数据 & 未先 clean 可能丢失脏数据 \\\tablerowrule

...
\topic{接收环与发送环}网卡与内存之间走的就是存储与设备事务部分的描述符环，
...
字，最后由 TCP 重传表现为"网速偶尔抖一下"。存储与设备事务部分"缓冲区何时
...
AF\_XDP，业务主体仍走内核栈加零拷贝。存储与设备事务部分那句提醒在这里同样
...
\topic{NVMe 的队列模型}
存储与设备事务部分讲描述符环时给过一般结构：环不动、指针动，doorbell 只通知、不
...
机回收 SQ，完成信息也不挤在命令项里写回。这是比存储与设备事务部分通用环更彻底
的分工。

一次提交的完整序列有七步，逐步对应存储与设备事务部分的每一条约定：
...
项、再发中断，主机在中断处理里读到的一定是已经写好的 CQ 项。存储与设备事务部分
...
是存储与设备事务部分"描述符先可见、再通知"的约定；顺序反了，设备可能读到半写
...
队列拥塞 & 深队列高负载下排队，“冒险、异常与性能”一章的 $1/(1-\rho)$ & 百微秒级 & 深度过大或负载接近饱和 \\
...
三类设备共享同一张检查表不是巧合：存储与设备事务部分的描述符环与 DMA、“冒险、异常与性能”一章的

...
\topic{Pentium 用双发射和分支预测重组执行结构}Pentium 的性能提升不再主要来自时钟频率，而是来自执行结构的重组。它有 U、V 两条整数流水线，满足配对规则的简单指令可以在同一拍内双发射；分支目标缓冲（BTB）预测分支方向，猜错才冲刷流水线；指令 cache 和数据 cache 分开，各 8 KiB，取指和读数不再争同一个端口；外部数据总线扩到 64 bit，一次总线事务搬运的数据翻倍。Pentium Pro 又往前走了一步：x86 指令先被译成微操作，由乱序执行核心按数据就绪顺序执行，寄存器重命名消除假依赖，最后按程序顺序提交，从而在乱序执行下保持精确异常。执行可以和提交分离，是这一代留下的主线：处理器核心部分讲的执行/提交区分，在这里成为真实芯片的基本组织方式。

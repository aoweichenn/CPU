...
\begin{keyidea}
组合逻辑只能计算当前输入，状态元件才能保存过去。本章用采样边界把锁存器、触发器、寄存器、跨时钟域和状态机连成一条连续通路。
\end{keyidea}

“数字硬件基础”部分讲的是组合逻辑：输入稳定后，输出由当前输入唯一决定。它能计算，却不能保存过去。真正的机器必须能把某个结果留下来，等下一个步骤继续使用；也必须能规定“什么时候更新状态”，否则反馈路径会让硬件在一个周期内连续变化，机器无法分步骤推进。
...
两个反相器互相连接时，每一侧都在“支持”另一侧的当前电平。若某个扰动让左侧电压略有上升，左侧反相器把中间节点电压压低，右侧反相器再把这个低电平反相为高，反馈回来支持左侧的高电平。这个正反馈过程让电路迅速落入两个稳定点之一。数字电路把这两个稳定点命名为 0 和 1，但底层仍然是晶体管、电容、电阻和电源共同决定的物理平衡。
...
\midrule
0 & 0 & 禁用 & 两个 NAND 的输入都有 0，Q 和 $\overline{Q}$ 都被迫为 1，互补关系被破坏；这是非法输入，不是“存了 1” \\\tablerowrule
0 & 1 & 1 & 上侧 NAND 因 $\overline{S}=0$ 输出 1；下侧 NAND 两输入全 1 输出 0，反馈回去维持 Q=1。set 生效 \\\tablerowrule
1 & 0 & 0 & 与上一行对称：$\overline{R}=0$ 把 $\overline{Q}$ 拉成 1，反馈把 Q 拉成 0。reset 生效 \\\tablerowrule
1 & 1 & 保持 & 无外部请求：若 Q=1，下侧输出 0 支持上侧输出 1；若 Q=0 则反过来。反馈环自己维持旧值 \\
\bottomrule
\end{longtable}

禁行还要单独说一句“释放之后发生什么”。若 $\overline{S}$、$\overline{R}$ 从 0,0 同时回到 1,1，两个 NAND 的旧输出都是 1，于是两个门都打算把输出改成 0——谁先翻转，反馈就支持谁，最终状态取决于两个门的实际延迟差。这就是为什么正文说“稳定在 0 还是 1 取决于门延迟和释放顺序”：禁行退出瞬间是一场不可预测的竞争。可靠设计要么不产生 0,0，要么让两个输入不同时释放。
...
\midrule
0 & 0 & 1 & 1 & 无请求：反馈环保持旧值 \\\tablerowrule
1 & 0 & 0 & 1 & 置位：Q 被写成 1 \\\tablerowrule
0 & 1 & 1 & 0 & 复位：Q 被写成 0 \\\tablerowrule
...
到这里，四种保存 1 bit 的结构可以排成一条线：双反相器反馈用两个稳定点保存 1 bit，但没有独立写入控制；SR 锁存器允许用 set/reset 命令改变状态，代价是 set 与 reset 不能给出冲突请求；D 锁存器用一个数据输入和 enable 表达写入，代价是 enable 有效期间输入可以穿透到输出；D 触发器只在时钟边沿采样并保存，代价是必须满足建立时间和保持时间。
...
总线阶段 & 锁存器动作 & 设计含义 \\
\midrule
地址有效 & 锁存窗口打开，地址进入锁存器 & 允许复用总线先表达地址 \\\tablerowrule
窗口关闭 & 内部反馈保持地址 & 外部存储器继续看到稳定地址 \\\tablerowrule
数据传输 & 总线引脚改作数据或状态用途 & 地址不会随复用引脚变化而丢失 \\\tablerowrule
...
从这个案例可以看出，锁存器适合用在协议已经提供清晰电平窗口的地方，例如地址锁存、外部总线相位分离、显示输出暂存等。若协议没有给出可靠窗口，或者多个状态级之间存在组合反馈，透明锁存器反而会让调试变难。
...
\midrule
D 在窗口打开前已稳定 & Q 早已跟随，窗口关闭时保存 & 沿到来时采样，沿后 Q 更新 \\\tablerowrule
D 在窗口中段才变化（晚到数据） & Q 随之更新，保存的是晚到的值 & 距沿超过建立时间则正常采样，但 Q 只在沿后才变 \\\tablerowrule
D 在窗口内出现毛刺 & 毛刺经传播延迟出现在 Q 上 & 毛刺不落在沿附近窗口内则完全不可见 \\\tablerowrule
D 在窗口外任意变化 & 无影响 & 无影响 \\
\bottomrule
\end{longtable}
...
这并不是说透明锁存器一无是处。前面 8086 地址锁存的案例已经说明，协议给出清晰电平窗口的地方正是锁存器的用武之地；“触发器、时钟周期与采样窗口”一节还会看到，主从结构正是用两个窗口错开的锁存器拼出一个触发器。工程结论因此是分层而不是否决：跨相位保持用锁存器，状态回路的提交边界用触发器。
...
\begin{lstlisting}
// 注释（推进）：右侧先由旧状态计算，所有状态只在有效时钟沿同时更新。
// 注释（边界）：条件未命中或处于两个时钟沿之间时，寄存器保持原值。
on clock edge:
  Q = D
between clock edges:
  Q holds
...
J & K & 下一 Q & 含义 \\
\midrule
0 & 0 & Q & 无请求：保持旧值 \\\tablerowrule
1 & 0 & 1 & 置位请求：沿后 Q=1 \\\tablerowrule
0 & 1 & 0 & 复位请求：沿后 Q=0 \\\tablerowrule
...
\midrule
第 1 级 Q$_0$ & 输入时钟 CLK & $f/2$ & 二分频 \\\tablerowrule
第 2 级 Q$_1$ & Q$_0$ & $f/4$ & 四分频 \\\tablerowrule
...
\midrule
时钟到输出延迟 & Q 何时能作为后级输入使用 & 后级过早采样旧值或过渡值 \\\tablerowrule
建立/保持时间 & D 在边沿前后要稳定多久 & 偶发采错、亚稳态或重复触发 \\\tablerowrule
最小时钟脉宽 & CP 高低电平是否足够长 & 窄脉冲被忽略或产生异常采样 \\\tablerowrule
...
项目 & 示例数值 & 读法 \\
\midrule
\code{tCQ_max} & 1.2ns & 源寄存器输出最晚 1.2ns 后有效 \\\tablerowrule
\code{tcomb_max} & 5.8ns & 组合逻辑最坏传播时间 \\\tablerowrule
\code{twire_max} & 0.9ns & 远距离连线和负载引入延迟 \\\tablerowrule
\code{tsetup} & 0.6ns & 目标寄存器采样前要求 \\\tablerowrule
\code{tskew+margin} & 0.5ns & 偏斜和工程余量 \\\tablerowrule
...
把这几类约束放在一起看，修复手段完全不同：建立约束失败说明最慢路径赶不上采样沿，要缩短组合逻辑、流水线化、降频或优化布局；保持约束失败说明最快路径太早冲进采样窗口，要增加最小延迟、调整时钟偏斜或改变布线；复位释放失败是状态元件退出复位不一致，要异步拉入、同步释放并限定复位域；跨时钟输入失败是采样沿附近输入不受控，要用两级同步、握手、异步 FIFO（first in, first out，先进先出队列，读写两侧各用自己的时钟）或协议化采样。用建立的修法去修保持，或者用降频去解决复位和跨边界问题，都会把故障掩盖成偶发错误。
\vspace{1.0em plus 0.35em}
...
关键问题 & 异步复位 & 同步复位 \\
\midrule
何时生效 & 引脚有效立即生效，不问时钟 & 下一个有效沿才生效 \\\tablerowrule
无时钟时能否工作 & 能：上电、停振时都有效 & 不能：沿不来状态不变 \\\tablerowrule
数据路径代价 & 不占 D 路径，使用专用引脚 & 选择逻辑串入 D 路径，增加一级延迟 \\\tablerowrule
释放时刻 & 仍是异步事件，可能违反恢复时间 & 由建立/保持分析覆盖，时刻确定 \\\tablerowrule
...
最常见的单 bit 同步结构是两级触发器串联。第一级直接采样异步输入，可能进入亚稳态；第二级在下一个时钟沿再采样第一级输出，给第一级更多时间恢复到明确 0 或 1。两级同步不能从数学上消灭亚稳态，但能显著降低亚稳态传播到后级控制逻辑的概率。多 bit 数据不能简单地每一位各接两级同步后直接使用，因为各位可能在不同周期被采到，形成混合值；多 bit 跨边界通常要使用握手、有效位、格雷码计数器或异步 FIFO。
...
放一个数量级感受“恢复时间买可靠性”的杠杆有多大。亚稳态未恢复概率随恢复时间按指数 $e^{-t/\tau}$ 下降：恢复窗口每多一个器件时间常数 $\tau$，失败概率约下降 2.7 倍；每多约 $2.3\tau$，失败概率大约下降一个数量级。假设某触发器 $\tau=0.1$ns，第一级留约 2ns（约 $20\tau$）恢复窗口，单次采样的失败概率约 $e^{-20}$，即 $2\times10^{-9}$ 量级；异步输入平均每秒变化 1000 次，则大约 $5\times10^5$ 秒（约 6 天）才出现一次可见失败。若只留 0.3ns（约 $3\tau$）——比如同步器后面紧跟一条长组合路径再吃一级触发器——恢复窗口少了约 17 个 $\tau$，失败概率抬升约 $e^{17}$（约 7 个数量级），达到约 $5\times10^{-2}$，同样输入下每秒失败约 50 次。这些数字是示意量级，真实参数必须查器件手册；但结论形态是普适的：\hwkey{可靠性按指数购买，时钟越快、事件越频繁、恢复窗口越短，买到的越少}。

\begin{longtable}{L{0.22\linewidth}L{0.36\linewidth}L{0.32\linewidth}}
\toprule
MTBF 影响因素 & 对可靠性的影响 & 设计含义 \\
\midrule
目标时钟频率 & 采样次数越多，遇到危险窗口机会越多 & 高频域更需要规范同步结构 \\\tablerowrule
异步事件频率 & 输入变化越频繁，触发亚稳态机会越多 & 高频事件应使用握手或 FIFO \\\tablerowrule
恢复时间 & 两级触发器之间时间越充足，传播概率越低 & 同步器后不应接长组合路径再采样 \\\tablerowrule
...
跨边界对象 & 推荐处理 & 不当处理的风险 \\
\midrule
单 bit 按钮/中断 & 两级同步后再进入状态机 & 亚稳态扩散或一次事件被识别多次 \\\tablerowrule
多 bit 数据总线 & 使用握手、valid/ready 或 FIFO & 各 bit 来自不同采样时刻，形成错误编码 \\\tablerowrule
复位释放 & 异步拉入、同步释放 & 状态机部分触发器先运行 \\\tablerowrule
...
单 bit 做法本章已经展开：两级触发器串联，第一级留给亚稳态恢复，第二级输出干净电平，前面也已经算过 MTBF 的账——恢复窗口从约 2ns 缩到 0.3ns，失败间隔就从约 6 天缩到 20 毫秒级（每秒约 50 次）。这里只补一条数量级推论：因为失败概率随恢复时间按指数 $e^{-t/\tau}$ 下降，MTBF 对余量极端敏感，余量每多一个器件时间常数 $\tau$，失败间隔大约拉长 2.7 倍；每多约 $2.3\tau$，大约拉长 10 倍。在两级之后再添第三级，恢复窗口近似翻倍，MTBF 随之再抬高若干个数量级，从“产品寿命内偶见”推到“远远超出产品寿命”；反过来，同步器后面紧接一条长组合路径再吃一级触发器，等于把恢复窗口削掉大半，MTBF 可能从按年计跌回按天计。结论很朴素：可靠性按指数购买，买多买少由恢复窗口决定，而恢复窗口几乎不花钱——只是触发器和布线。
...
跨域对象与做法 & 机制 & 代价与适用 \\
\midrule
单 bit 电平或事件：两级同步器 & 第一级吸收亚稳态，第二级给出稳定电平 & 延迟一两拍；适用按钮、中断、状态标志 \\\tablerowrule
多 bit 批量数据：请求/应答握手 & 源先放好数据再发请求，数据保持稳定直到应答返回 & 每次传输数拍；适用低速、非连续传输 \\\tablerowrule
...
安全启动控制板中的 \code{start_request} 可以作为例子。按钮原始触点先经过默认电平、保护和消抖，得到板内稳定请求；若后级控制器是同步状态机，这个请求还要在控制器时钟域内同步，状态机只能使用同步后的 \code{start_request_sync}。这样“数字硬件基础”部分的“可靠 bit”与本章的“可靠采样边界”才真正接上：前者解决电平和路径，后者解决保存时刻。
...
对硬件工程而言，“外部输入已经变成 0/1”不是终点；只有当它被目标时钟域可靠采样、被事件协议保持、被状态机按安全条件消费之后，它才成为可用于控制机器动作的内部事实。
...
寄存器动作 & 下一值表达 & 硬件含义 \\
\midrule
保持 & \code{next = current} & 写回旧 Q 或关闭写使能 \\\tablerowrule
装载 & \code{next = input} & 在边界保存外部或组合结果 \\\tablerowrule
清零 & \code{next = 0} & 同步清零或受控复位路径 \\\tablerowrule
置位 & \code{next = all\_ones} & 标志位、掩码或诊断状态写入 \\\tablerowrule
...
\midrule
CP/CLK & 哪个边界提交新计数值 & 单步时钟、系统时钟、分频后时钟 \\\tablerowrule
ENP/ENT 等使能 & 本周期是否执行加一 & PC 加一、循环计数、定时等待 \\\tablerowrule
并行装载 & 是否用外部值覆盖加一结果 & 跳转地址、预置计数、状态恢复 \\\tablerowrule
清零/复位 & 初始状态如何确定 & 上电入口、错误恢复、实验复位 \\\tablerowrule
进位输出 & 更宽计数器何时进位 & 级联 8-bit、16-bit 或更宽计数器 \\
\bottomrule
\end{longtable}

把两个 4-bit 计数器级联成 8-bit 计数器时，低 4 位的进位不能被理解为普通数据输出。它是高 4 位是否在同一边界加一的控制条件，必须满足高位计数器的使能时序。若把低位 Q 经过随意组合逻辑再驱动高位时钟，就把同步计数器退化成脉动计数器（也叫\hwrisk{纹波计数器}，ripple counter），时序关系会变得难以分析。同步级联的要点是：所有位共享同一个时钟，进位只作为下一状态选择条件。

\topic{专题：纹波计数器与同步计数器对照}

上一段提醒过：级联计数器时若把低位 Q 经过随意组合逻辑去驱动高位时钟，同步结构就退化成了纹波计数器。它和同步计数器完成同一个功能，但时钟的组织方式完全不同，值得放在一起对照。

纹波计数器的做法极为节省：每个触发器接成翻转模式（把 D 触发器的 $\overline{Q}$ 接回 D），第一级由外部时钟驱动，之后每一级的时钟取自前一级的输出。取 Q 还是 $\overline{Q}$ 决定计数方向：加法计数要求高位在低位由 1 变 0 时翻转，而上升沿触发的触发器只在 0 变 1 时动作，低位由 1 变 0 时其 $\overline{Q}$ 恰好给出上升沿，所以加法计数器要把时钟接到前一级的 $\overline{Q}$ 上；若改接前一级的 Q，低位由 0 变 1 时高位跟着翻转，得到的便是减法计数。第 0 位在时钟沿之后先翻转；第 1 位看到第 0 位变化后才跟着翻转；如此逐级传递，第 $n$ 位要等前面各级依次翻完才轮到本级，延迟随位号一级一个 $t_{CQ}$ 累加。变化像波浪一样从低位传到高位，这正是“纹波”二字的来历。
...
\midrule
0ns & 外部时钟沿到达，各级尚未更新 & 0111（旧值 7） \\\tablerowrule
1ns & 第 0 位翻转完成 & 0110（读数 6，中间值） \\\tablerowrule
2ns & 第 1 位跟随翻转 & 0100（读数 4，中间值） \\\tablerowrule
3ns & 第 2 位跟随翻转 & 0000（读数 0，中间值） \\\tablerowrule
...
\midrule
纹波计数器 & 每级时钟取自前一级 Q & 低位 1 个 $t_{CQ}$ 起，逐级累加 & 稳定前出现短暂中间值 & 低速分频链、定时链、读数不被同域逻辑消费的场合 \\\tablerowrule
...
时钟沿序号 & 沿后稳定值 & 说明 \\
\midrule
0 & 0000 & 初始值 \\\tablerowrule
1 & 0001 & 正常加一 \\\tablerowrule
$\cdots$ & $\cdots$ & 依次递增 \\\tablerowrule
9 & 1001 & 最后一个合法值（十进制 9） \\\tablerowrule
...
1 & 0001 & 正常加一 \\\tablerowrule
$\cdots$ & $\cdots$ & 依次递增 \\\tablerowrule
9 & 1001 & 译码门检出末态，本拍内拉起装载控制 \\\tablerowrule
...
\midrule
复位法 & 检出 $N$（如 1010） & 异步清零，不等时钟沿 & 有，$N$ 短暂出现 & 清零脉冲仅数纳秒，各级清零不齐时可能清不干净 \\\tablerowrule
...
\midrule
二进制 & 16 & 最多 4 位同时变 & 全译码，变化途中可能出中间值 & 通用计数、地址生成 \\\tablerowrule
格雷码 & 16 & 恒为 1 位 & 译码无中间态 & 跨时钟域指针、角度与位置编码 \\\tablerowrule
环形 & 4 & 2 位（一位退、一位进） & 无需译码门，Q 直接当状态标志 & 独热状态机、简单轮转控制 \\\tablerowrule
...
\midrule
0 & 0001 & 0 & 种子值，必须非零 \\\tablerowrule
1 & 0010 & 0 & \\\tablerowrule
2 & 0100 & 1 & \\\tablerowrule
3 & 1001 & 1 & \\\tablerowrule
4 & 0011 & 0 & \\\tablerowrule
5 & 0110 & 1 & \\\tablerowrule
6 & 1101 & 0 & \\\tablerowrule
...
\midrule
\code{SHCP} 边沿 & 串行 bit 进入内部移位寄存器 & 外部输出可保持不变 \\\tablerowrule
连续 8 次移位 & 内部移位状态逐步形成一个字节 & 中间状态不应直接驱动负载 \\\tablerowrule
\code{STCP} 边沿 & 内部字节提交到输出锁存器 & Q0 到 Q7 一起更新 \\\tablerowrule
...
流水线寄存器保存的不是单个数，而是一组需要一起前进的状态：数据、控制位、valid 位、异常标志和目的寄存器编号。若只有数据向前移动而控制位没有同步移动，后级就会把一个周期的数据解释成另一个周期的操作。移位寄存器是最简单的“状态随边界推进”模型；CPU 流水线是同一思想在更宽状态集合上的应用。

\topic{专题：串并转换与动态扫描显示}

把 74HC595 真正用到板子上，要把每个引脚的职责和一种典型负载——多位数码管——连起来看，才能理解为什么这颗芯片几乎成了“输出扩展”的代名词。
...
引脚 & 职责 & 关键问题 \\
\midrule
SER（DS） & 串行数据输入，给出待移入的一位 & 该位必须在 SRCLK 有效沿前稳定 \\\tablerowrule
SRCLK（SHCP） & 移位时钟，驱动内部移位寄存器 & 移位期间外部输出不变，这正是设计目的 \\\tablerowrule
RCLK（STCP） & 锁存时钟，把整字节提交到输出 & 8 次移位完成后才给，过早会输出半成品字节 \\\tablerowrule
OE（低有效） & 输出使能，无效时输出高阻 & 上电期间保持无效，避免显示乱闪 \\\tablerowrule
...
扫描步骤 & 动作 & 检查点 \\
\midrule
消隐 & 关闭当前位选（或令 OE 无效） & 位选与段码不能同时切换 \\\tablerowrule
送段码 & 段码线（或 595）给出下一位图案 & 段码必须在位选打开前稳定 \\\tablerowrule
选通 & 打开下一位位选 & 同一时刻只有一位被选通 \\\tablerowrule
保持 & 维持点亮约 1 至 2ms & 整帧周期小于 20ms，即帧率大于 50Hz \\\tablerowrule
...
当前状态 & 条件 & 下一状态 \\
\midrule
IDLE & start=1 & LOAD \\\tablerowrule
LOAD & ready=1 & RUN \\\tablerowrule
RUN & count\_done=1 & DONE \\\tablerowrule
DONE & clear=1 & IDLE \\
\bottomrule
\end{longtable}
...
状态 & 本周期动作 & 关键输出 & 离开条件 \\
\midrule
IDLE & 等待同步后的启动请求 & 关闭写使能与外部驱动 & \code{start_pending=1} \\\tablerowrule
LOAD & 装载输入寄存器或初始计数 & \code{load_we=1} & 输入 ready 或装载完成 \\\tablerowrule
RUN & 执行运算、计数或移位 & \code{run_en=1} & \code{count_done=1} \\\tablerowrule
...
\midrule
\code{S_LOW} & 上一拍 sig 为 0 & 0 & \code{S_EDGE} & \code{S_LOW} \\\tablerowrule
\code{S_EDGE} & 刚采到上升沿 & 1 & \code{S_HIGH} & \code{S_LOW} \\\tablerowrule
...
\midrule
\code{S_LOW} & 0 & 0 & \code{S_LOW} \\\tablerowrule
\code{S_LOW} & 1 & 1 & \code{S_HIGH} \\\tablerowrule
\code{S_HIGH} & 0 & 0 & \code{S_LOW} \\\tablerowrule
...
维度 & Moore & Mealy \\
\midrule
本例状态数 & 3 & 2 \\\tablerowrule
输出决定因素 & 仅当前状态 & 当前状态加输入 \\\tablerowrule
响应时刻 & 采到条件后下一拍 & 采到条件的当拍 \\\tablerowrule
输出稳定度 & 整拍稳定，沿后至多一级门 & 受输入路径影响，沿后 $t_{CQ}$ 加组合延迟 \\\tablerowrule
毛刺风险 & 输出无输入直接路径 & 输入毛刺可穿透到输出 \\\tablerowrule
...
真实工程中必须处理四个经常被低估的问题：时钟如何送达，复位如何跨越边界，状态编码如何影响安全性，验证如何证明边界确实成立。这四件事不改变“组合逻辑准备下一值、状态元件保存下一值”的基本模型，却决定一个设计能否从纸面状态表进入可靠机器。

首先区分\hwkey{写使能}和\hwrisk{门控时钟}。写使能的做法是让时钟继续到达触发器，但在时钟沿到来时决定是否接收新 D 值；若写使能为 0，寄存器保持原值。门控时钟则是让某段时钟停止翻转，以降低功耗或隔离无用活动。两者在功能描述上都可能表现为“保持不变”，但硬件风险完全不同。普通数据通路优先使用写使能；真正需要门控时钟时，应使用专用时钟门控单元，并保证 enable 在安全窗口内被锁存。不能随意用一个 AND 门把 \code{clk} 和 \code{enable} 相与后送给触发器，因为 enable 的毛刺或变化时刻可能制造短脉冲、窄时钟或额外时钟沿。
...
这些检查会直接连接到 CPU 设计。PC 是寄存器，指令寄存器是状态，流水线寄存器是阶段边界，控制器是状态机，异常和中断入口依赖同步事件，写回使能必须在正确周期打开。真正的 CPU 不是一张组合逻辑图，而是一组由时钟、复位、状态编码、控制使能和波形验证共同约束的同步系统。

\topic{经典案例：交通灯控制器的完整设计}

设计一个十字路口的交通灯控制器：东西向绿灯亮 30 秒，转黄灯 5 秒，然后双向全红 2 秒；接着南北向绿灯 30 秒、黄灯 5 秒，再全红 2 秒，随后回到东西向绿灯，如此循环。全红段不是装饰：在一个方向放行之前，两个方向都必须先看到红灯，给已经进入路口的车辆留出清空时间。这 2 秒是用一个专门状态换来的安全余量，删掉它，状态机更简单，路口更危险。
...
状态 & 含义 & 装载值 \\
\midrule
\code{EW\_GREEN} & 东西向绿灯，南北向红灯 & 30s \\\tablerowrule
\code{EW\_YELLOW} & 东西向黄灯，南北向红灯 & 5s \\\tablerowrule
\code{ALL\_RED\_EW} & 东西向刚结束，双向全红 & 2s \\\tablerowrule
\code{NS\_GREEN} & 南北向绿灯，东西向红灯 & 30s \\\tablerowrule
\code{NS\_YELLOW} & 南北向黄灯，东西向红灯 & 5s \\\tablerowrule
...
当前状态 & 条件 & 下一状态 & 动作 \\
\midrule
\code{EW\_GREEN} & \code{timer\_done}=1 & \code{EW\_YELLOW} & 装载 5s，重启定时器 \\\tablerowrule
\code{EW\_YELLOW} & \code{timer\_done}=1 & \code{ALL\_RED\_EW} & 装载 2s，重启定时器 \\\tablerowrule
\code{ALL\_RED\_EW} & \code{timer\_done}=1 & \code{NS\_GREEN} & 装载 30s，重启定时器 \\\tablerowrule
\code{NS\_GREEN} & \code{timer\_done}=1 & \code{NS\_YELLOW} & 装载 5s，重启定时器 \\\tablerowrule
\code{NS\_YELLOW} & \code{timer\_done}=1 & \code{ALL\_RED\_NS} & 装载 2s，重启定时器 \\\tablerowrule
...
状态 & 东西向 & 南北向 \\
\midrule
\code{EW\_GREEN} & 绿 & 红 \\\tablerowrule
\code{EW\_YELLOW} & 黄 & 红 \\\tablerowrule
\code{ALL\_RED\_EW} & 红 & 红 \\\tablerowrule
\code{NS\_GREEN} & 红 & 绿 \\\tablerowrule
\code{NS\_YELLOW} & 红 & 黄 \\\tablerowrule
...
\midrule
\code{EW\_GREEN} & 000 & 000 & 000001 \\\tablerowrule
\code{EW\_YELLOW} & 001 & 001 & 000010 \\\tablerowrule
\code{ALL\_RED\_EW} & 010 & 011 & 000100 \\\tablerowrule
\code{NS\_GREEN} & 011 & 010 & 001000 \\\tablerowrule
\code{NS\_YELLOW} & 100 & 110 & 010000 \\\tablerowrule
...
编码 & 触发器数 & 译码逻辑 & 适用场合 \\
\midrule
二进制 & 3 & 最深，需综合全部编码位 & 状态很多、触发器昂贵的 ASIC \\\tablerowrule
格雷 & 3 & 深度与二进制相当 & 相邻转移无中间码，跨时钟域计数指针常用 \\\tablerowrule
...
状态 & 含义 & 已匹配前缀 \\
\midrule
\code{S0} & 没有可用前缀（起始或刚失配） & （空） \\\tablerowrule
\code{S1} & 尾巴是 1 & 1 \\\tablerowrule
\code{S2} & 尾巴是 11 & 11 \\\tablerowrule
\code{S3} & 尾巴是 110 & 110 \\
\bottomrule
\end{longtable}
...
\midrule
\code{S0} & \code{S0}/0 & \code{S1}/0 \\\tablerowrule
\code{S1} & \code{S0}/0 & \code{S2}/0 \\\tablerowrule
\code{S2} & \code{S3}/0 & \code{S2}/0 \\\tablerowrule
...
重叠检测正是靠“匹配后退回 \code{S1} 而非 \code{S0}”实现的。以输入 11011 为例：第 4 位到达时检测到第一个 1101，机器停在 \code{S1}；第 5 位的 1 随即把它推进 \code{S2}——第二个匹配的前缀 11 已经备好。若输入继续 01，即完整序列 1101101，第二个 1101 在第 7 位被认出，它与第一个匹配共享了第 4 位的那个 1。倘若错误地让机器匹配后回 \code{S0}，这个共享位就被不必要地丢弃，1101101 中的第二个 1101 将被漏检。

最后说明为什么用 Mealy 型：检测输出依赖“状态加当前输入”，比 Moore 型少一个状态（Moore 需专门增加一个输出为 1 的状态，如同前文边沿检测例中的 \code{S_EDGE}），同拍给出结果，响应快一个周期。代价是 \code{y} 直接跟着 \code{x} 走，\code{x} 上的毛刺会原样出现在 \code{y} 上。若 \code{y} 只被后级在时钟沿采样，提前一拍是净收益；若 \code{y} 要驱动写使能这类危险控制线，应当再寄存一拍，用稳定性换掉这一拍的提前量。四个状态用 2 位二进制编码恰好用完，没有空洞编码。

\topic{专题：从时序约束到检查方法}

同步设计不能仅以“这个状态机功能正确”作为结论。功能正确说明下一状态逻辑在稳定输入下给出正确答案；时序正确说明这些答案能在规定边界前到达，并且不会过早冲入保持窗口。二者缺一不可。分析一条关键路径，应当把它写成四个对象：源状态元件，即哪个触发器在发射沿（即前文所说的更新沿）后改变 Q，不能只写信号名而不写状态边界；数据路径，即结果经过哪些门、mux、加法器、译码和连线，不能只看逻辑级数而忽略布线和负载；目标状态元件，即哪个触发器在捕获沿（即采样沿）采样 D，不能没有采样窗口；时钟关系，即周期、偏斜、抖动和工程余量，不能用理想同相时钟代替真实时钟树。
...
现象 & 优先检查 & 不应直接假设 \\
\midrule
高频下偶发错误 & 建立约束、关键路径、温度和电压角落 & 逻辑表达式一定写错 \\\tablerowrule
任何频率都偶发错误 & 保持约束、异步输入、复位释放、亚稳态 & 降低频率一定有效 \\\tablerowrule
复位后偶发失控 & 复位释放同步、非法状态恢复、安全默认输出 & 只要复位电平正确就足够 \\\tablerowrule
...
器件 & 本章对应概念 & 搭建时的重点 \\
\midrule
74HC14 & 施密特输入、慢边沿整形、按钮消抖前后边界 & 输出反相；语义校正信号取自第一级，需要与输入同相的低有效信号时再串联第二级 \\\tablerowrule
74HC74 & 正沿 D 触发器、两级同步器、事件保持位 & 异步置位/复位脚必须接到确定无效电平或受控复位 \\\tablerowrule
74HC161/74HC163 & 4-bit 同步计数器、当前状态加一、进位级联 & 区分异步复位和同步复位，计数使能与并行装载不能悬空 \\\tablerowrule
74HC157 & 数据选择器、写使能保持、\code{D=mux(we,new,Q)} & 让时钟连续运行，用数据路径决定写入或保持 \\\tablerowrule
74HC595 & 串入并出移位寄存器、输出锁存、三态输出 & 区分移位时钟和锁存时钟，输出负载要受限 \\\tablerowrule
...
物料 & 用途 & 检查要点 \\
\midrule
稳定直流电源 & 给所有逻辑芯片提供同一电源域 & 电压在芯片允许范围内，实验板各处共地 \\\tablerowrule
0.1uF 去耦电容 & 抑制芯片开关瞬间的局部电源扰动 & 每片芯片靠近 VCC/GND 放置一颗 \\\tablerowrule
上拉/下拉电阻 & 给按钮、复位和未驱动输入确定默认值 & 松开按钮或开关断开时节点不悬空 \\\tablerowrule
LED 与限流电阻 & 低速观察 Q、同步级和计数输出 & LED 电流不超过芯片输出能力 \\\tablerowrule
按钮与 RC 滤波 & 产生人工事件和复位请求 & 按钮边沿进入 74HC14 前已有默认电平 \\\tablerowrule
...
检查项 & 必须确认 & 常见后果 \\
\midrule
电源脚 & 每片芯片的 VCC、GND 与数据手册一致 & 芯片发热、无输出或永久损坏 \\\tablerowrule
去耦 & 每片芯片电源脚旁有 0.1uF 电容 & 时钟沿或输出翻转时产生偶发错误 \\\tablerowrule
低有效脚 & \code{reset_n/MR_n/OE_n} 等处于确定有效或无效电平 & 电路长期复位、输出高阻或随机启动 \\\tablerowrule
未用输入 & 接到确定 0 或 1，不能悬空 & LED 随机闪烁，计数器误跳变 \\\tablerowrule
输出负载 & LED 串电阻，不能多个推挽输出硬短接 & 输出电流过大或总线冲突 \\\tablerowrule
...
实验 & 观察信号 & 预期现象 \\
\midrule
按钮同步器 & \code{raw/pressed/meta/sync/reset_n/clk} & 后级只在时钟边界看到同步后的事件 \\\tablerowrule
4-bit 计数器 & \code{clk/reset_n/enable/Q0..Q3} & 关使能时状态保持，开使能时每周期加一 \\\tablerowrule
74HC595 输出链路 & \code{DS/SHCP/STCP/MR_n/OE_n/Q0..Q7} & 移位期间输出不乱跳，锁存边界后一起更新 \\\tablerowrule
...
失败现象 & 优先检查 & 修复方向 \\
\midrule
按一次计多次 & 按钮抖动、手动时钟、74HC14 整形 & 增加 RC 消抖，整形后再进时钟域 \\\tablerowrule
LED 随机闪烁 & 悬空输入、复位脚、输出使能脚、电源轨 & 给所有输入确定默认电平并检查共地 \\\tablerowrule
复位后假事件 & 同步器复位值、pending 位默认值、释放边界 & 异步拉入、同步释放，事件位复位为 0 \\\tablerowrule
74HC595 输出不变 & \code{OE_n/MR_n/STCP/SHCP} 与数据稳定时间 & 区分移位时钟和锁存时钟，固定控制脚电平 \\\tablerowrule
写使能保持失败 & mux 选择脚、Q 反馈路径、D 采样边界 & 观察 \code{write_enable/D/Q/clk} 的周期关系 \\\tablerowrule
...
数据手册项目 & 读法 & 设计用途 \\
\midrule
绝对最大额定值 & 超过可能损伤器件 & 用来划红线，不用来定正常工作点 \\\tablerowrule
推荐工作条件 & 厂商承诺正常工作的范围 & 决定 VCC、电压域、温度和输入边沿要求 \\\tablerowrule
\code{VIH/VIL} & 输入被承认为 1 或 0 的电压门限 & 判断 3.3V 输出能否驱动 5V 输入 \\\tablerowrule
\code{VOH/VOL} & 在给定输出电流下仍能保证的输出电平 & 判断下一级是否能可靠识别高低电平 \\\tablerowrule
输出电流 & 单脚和整片芯片的源/灌电流限制 & 决定 LED 电阻、负载数量和是否需要驱动器 \\\tablerowrule
传播延迟 & 输入变化到输出有效的最坏时间 & 计入时序预算，不能只看典型值 \\\tablerowrule
最小时钟脉宽 & CP、SHCP、STCP 等高/低电平至少持续多久 & 判断按钮、NE555 或逻辑脉冲是否太窄 \\\tablerowrule
...
负载类型 & 风险 & 正确处理 \\
\midrule
多个 CMOS 输入 & 电容变大，边沿变慢 & 降低频率、缩短连线，必要时加缓冲器 \\\tablerowrule
LED 观察负载 & 输出电流过大拉低高电平或抬高低电平 & 串较大限流电阻，优先低电流观察 \\\tablerowrule
长杜邦线 & 串扰、反射、接触不良和地线回路 & 低速实验、短线、共地、必要时示波器确认 \\\tablerowrule
继电器或电机 & 电流和反灌电压远超逻辑芯片能力 & 使用驱动管、续流二极管、独立供电和隔离策略 \\\tablerowrule
...
时钟来源 & 适合用途 & 额外检查 \\
\midrule
按钮单步 & 教学观察、单周期推进 & 必须消抖并经施密特整形，不直接驱动 CP \\\tablerowrule
NE555 & 低速自动运行 & 检查占空比、电源去耦、边沿和最小脉宽 \\\tablerowrule
晶振模块 & 稳定周期时钟 & 频率、电压域、输出负载和面包板布线余量 \\\tablerowrule
微控制器 GPIO（通用输入输出引脚） & 可编程时钟或测试脉冲 & 电压兼容、脉宽、地线和软件抖动 \\
\bottomrule
\end{longtable}
...
安全边界也要明确。本章实验只验证低压逻辑信号，不验证功率驱动。若要让逻辑电路控制继电器、电机、灯带或其它外部负载，必须增加驱动晶体管或 MOSFET、栅极/基极电阻、续流二极管或 TVS（瞬态电压抑制二极管）、独立电源去耦、共地或隔离策略，并重新评估电流、热、反灌电压和失效状态。把 74HC595、74HC74 或 74HC161 的输出脚直接当作功率开关，是把逻辑输出误用为能量输出。
...
报告项 & 应记录的内容 & 判断依据 \\
\midrule
物料与版本 & 芯片完整型号、逻辑系列、电源电压 & 可回到数据手册复查电气条件 \\\tablerowrule
连接约定 & VCC/GND、复位、置位、输出使能、未用输入 & 没有悬空控制脚和危险负载 \\\tablerowrule
时钟与复位 & 时钟来源、频率范围、复位拉入和释放方式 & 状态只在边界变化，复位后无伪事件 \\\tablerowrule
异步输入 & 原始引脚、消抖、施密特整形、同步级数 & 后级只使用同步后的信号 \\\tablerowrule
观测结果 & LED、逻辑分析仪或示波器看到的关键节点 & 现象能解释到具体边界和信号名 \\\tablerowrule
故障处理 & 失败现象、定位步骤、修复前后对比 & 修复不是偶然重插线，而有明确原因 \\
\bottomrule
\end{longtable}

\topic{专题：setup/hold 不是公式，而是采样窗口（小结与扩展）}
触发器的 setup（即前文所说的建立时间，手册常记作 \code{tSU} 或 \code{t_setup}）和 hold（保持时间，\code{tH} 或 \code{t_hold}）约束经常被背成两个时间参数，但它们本质上是在说明采样窗口。时钟沿到来前，D 输入要提前稳定一段时间；时钟沿之后，D 输入还要继续保持一小段时间。若输入在窗口内变化，触发器内部再生过程可能无法快速决定 0 或 1，输出延迟变长，甚至进入亚稳态。

\begin{longtable}{L{0.18\linewidth}L{0.42\linewidth}L{0.3\linewidth}}
\toprule
时序参数 & 原理含义 & 违反后果 \\
\midrule
\code{t_setup} & 时钟沿前 D 必须稳定的最短时间 & 采样值可能错误或输出延迟异常 \\\tablerowrule
\code{t_hold} & 时钟沿后 D 必须保持的最短时间 & 新数据可能穿透到本周期采样 \\\tablerowrule
\code{t_clkq} & 时钟沿后 Q 输出稳定所需时间 & 后级组合逻辑可用时间减少 \\\tablerowrule
\code{t_pd} & 组合逻辑最坏传播延迟 & 决定下一级 setup 是否满足 \\\tablerowrule
...
检查项 & 代入组合 & 计算 & 结论与余量 \\
\midrule
建立时间 & 最坏组合：\code{tCQ\_max}、组合最大延迟、\code{tSU}、不利偏斜 & 3+9+2+1=15ns，周期 20ns & 通过，余量 +5ns \\\tablerowrule
保持时间 & 最好组合：\code{tCQ\_min}、组合最小延迟约 0、\code{tH}、不利偏斜 & 1.5ns 对 0.5+1=1.5ns & 边缘，余量 0ns，建议加最小延迟 \\\tablerowrule
...
设计选择 & 作用 & 仍需注意 \\
\midrule
两级同步器 & 给第一级亚稳态多一个周期衰减 & 只能同步单 bit 电平，不能同步多 bit 总线值 \\\tablerowrule
握手协议 & 用 valid/ready 或请求/应答跨边界 & 吞吐降低，但语义清楚 \\\tablerowrule
异步 FIFO & 用双端口存储和格雷码指针跨时钟 & 指针同步和空满判断必须严格验证 \\\tablerowrule
...
\topic{复位要区分上电初始化、错误恢复和运行控制（小结与扩展）}
...
复位对象 & 建议策略 & 检查要点 \\
\midrule
控制状态机 & 复位到明确安全状态 & 复位释放后第一拍输出是否安全 \\\tablerowrule
PC/入口状态 & 装入复位向量或启动状态 & 第一条取指地址是否确定 \\\tablerowrule
数据寄存器 & 可按需要不复位，由控制路径保证无效时不使用 & 是否有 valid 位防止读旧值 \\\tablerowrule
外设接口 & 先复位设备内部，再开放总线响应 & 复位期间是否会误写 MMIO（存储器映射 I/O） \\\tablerowrule
...
验证项 & 要检查什么 & 失败风险 \\
\midrule
复位覆盖 & 所有关键状态寄存器进入已知状态 & 上电随机进入危险路径 \\\tablerowrule
非法状态恢复 & 未使用编码能回到安全状态或错误入口 & 状态机锁死或误写设备 \\\tablerowrule
输出默认值 & 每个状态未显式赋值的控制线有安全默认 & 锁存器推断或保持旧控制线 \\\tablerowrule
输入同步 & Mealy 输入是否来自同步边界 & 毛刺直接影响输出控制线 \\\tablerowrule
进度性 & 等待状态是否有超时或退出条件 & 外设无响应时永久等待 \\
\bottomrule
\end{longtable}

\topic{消抖和边沿检测要分成两件事（小结与扩展）}
...
阶段 & 原理 & 预期现象 \\
\midrule
同步 & 两级触发器降低亚稳态传播概率 & raw 改变时内部不会产生多 bit 混合状态 \\\tablerowrule
消抖 & 输入持续稳定 N 个周期才接受新电平 & 抖动期间 \code{stable_level} 不变化 \\\tablerowrule
边沿检测 & 当前稳定电平与上一稳定电平比较 & 一次按下只产生一个事件脉冲 \\\tablerowrule
...
经典系统教材通常会把组合逻辑和状态逻辑分开建模：组合逻辑计算下一值，状态元件在边界保存下一值。
\end{classicnote}
\section{六、四槽异步 FIFO：多 bit 数据怎样跨越两个时钟域}
...
\midrule
复位完成 & 无 & \code{000 -> 000} & empty，非 full & 两域先确认共同空闲代际 \\\tablerowrule
...
\begin{chapterexercise}{综合练习：入门：画 Q 波形至入门：主从结构}
\begin{exercisesubproblem}{（a）入门：画 Q 波形}
给定一段 D 和 CLK 波形，分别画出 D 锁存器（LE=CLK）和 D 触发器（上升沿）的 Q
\exercisecheck{预期结果}{锁存器 Q 在 CLK=1 全程跟随 D，触发器 Q 只在上升沿变化}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）入门：填 SR 真值表}
补全 $\overline{S}$/$\overline{R}$ 四种组合下 Q 的值，并标出禁行
\exercisecheck{预期结果}{禁行说明互补被破坏，不是“存了 1”；释放顺序决定恢复结果}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）入门：画计数波形}
从初值 9 开始画 6 个周期的 Q[3:0] 总线带和 Q0 波形
\exercisecheck{预期结果}{每个上升沿整体 +1，Q0 恰好是 CLK 二分频}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）入门：主从结构}
给定 D 与 CLK，填出主锁存器 M 和输出 Q 在各阶段的值
\exercisecheck{预期结果}{M 只在 CLK=0 跟随 D，Q 只在上升沿复制 M}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：触发器路径时序分析至复位释放}
\begin{exercisesubproblem}{（a）触发器路径时序分析}
为一条寄存器到寄存器路径列出 \code{tCQ}、组合延迟、连线延迟、建立时间、偏斜和余量
\exercisecheck{预期结果}{能判断目标周期是否通过建立约束}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）保持违例}
构造一条最小延迟过短的路径，并写出修复方法
\exercisecheck{预期结果}{能区分建立问题和保持问题}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）锁存器透明窗口}
说明两个锁存器共用 enable 时可能出现的穿透风险
\exercisecheck{预期结果}{能解释为什么大系统更偏好清晰时钟边界}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）复位释放}
为一个三状态控制器写复位表和释放策略
\exercisecheck{预期结果}{复位期间危险输出关闭，释放在同步边界发生}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：两级同步至事件保持}
\begin{exercisesubproblem}{（a）两级同步}
说明外部按钮或中断线进入时钟域的处理流程
\exercisecheck{预期结果}{原始异步信号不能直接驱动状态机}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）MTBF 指标}
说明为什么同步器只能降低亚稳态风险，并列出影响 MTBF 的因素
\exercisecheck{预期结果}{能把时钟频率、事件频率、恢复时间和器件特性联系起来}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）外部事件链路}
为一个按钮或传感器写出从原始引脚到状态机消费的完整信号链
\exercisecheck{预期结果}{能区分电气处理、同步、边沿检测和事件保持}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）事件保持}
设计 \code{pending/ack} 结构，说明何时置位、何时清除
\exercisecheck{预期结果}{窄事件不能因状态机暂时不可接收而丢失}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：多 bit 跨域至复位域}
\begin{exercisesubproblem}{（a）多 bit 跨域}
说明为什么 8-bit 总线不能逐位两级同步后直接使用
\exercisecheck{预期结果}{给出握手、FIFO 或格雷码方案}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）脉冲同步}
设计一个不会漏采窄脉冲的跨域事件方案
\exercisecheck{预期结果}{能说明事件保持到被确认}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）时钟门控}
比较写使能、数据 mux 保持、专用时钟门控和普通门控时钟
\exercisecheck{预期结果}{能说明为什么普通门电路不应直接切时钟}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）复位域}
为两个相关时钟域写复位释放和 ready 等待策略
\exercisecheck{预期结果}{能说明复位跨域也需要同步和安全默认值}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：状态编码至按钮同步器实作}
\begin{exercisesubproblem}{（a）状态编码}
比较二进制、one-hot 和格雷编码的用途与风险
\exercisecheck{预期结果}{能为未使用编码写安全恢复规则}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）波形验证}
列出验证一个同步小系统必须观察的信号
\exercisecheck{预期结果}{能从波形判断写使能、复位、pending/ack 是否正确}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）CPU 连接}
说明 PC、流水线寄存器、控制器和中断入口分别对应本章哪个概念
\exercisecheck{预期结果}{能把“状态、时钟与时序逻辑”一章边界模型映射到后续 CPU}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）按钮同步器实作}
用 74HC14 和 74HC74 写出按钮到同步电平的连接方案
\exercisecheck{预期结果}{能区分原始引脚、消抖整形、第一级同步和第二级同步}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：计数器实作至实验板物料}
\begin{exercisesubproblem}{（a）计数器实作}
用 74HC161 或 74HC163 设计 4-bit LED 计数器
\exercisecheck{预期结果}{能说明时钟、复位、计数使能、并行装载和输出负载}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）移位输出实作}
用 74HC595 说明串行输入、移位时钟、锁存时钟和输出使能
\exercisecheck{预期结果}{能解释为什么八个输出在锁存边界一起更新}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）写使能实作}
用 74HC157 和 74HC74 搭一个可保持的 1-bit 寄存器
\exercisecheck{预期结果}{能说明为什么数据 mux 保持优于普通门控时钟}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）实验板物料}
为本章低速逻辑实验列出最小 BOM（物料清单）和用途
\exercisecheck{预期结果}{能覆盖电源、去耦、默认电平、观察负载和测量工具}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：接线检查至电压与扇出}
\begin{exercisesubproblem}{（a）接线检查}
写出上电前检查表，覆盖低有效脚、未用输入、电压域和输出负载
\exercisecheck{预期结果}{能防止悬空、误复位、输出短路和电平不兼容}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）故障定位报告}
针对一个失败现象写出定位步骤和修复验证
\exercisecheck{预期结果}{能从现象追到具体信号边界，而不是只重插线}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）数据手册读取}
给出 74HC 实验前必须查的参数表和用途
\exercisecheck{预期结果}{能区分绝对最大额定值、推荐工作条件、输入输出电平和时序参数}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）电压与扇出}
判断一个输出能否可靠驱动后级输入、LED 或多路负载
\exercisecheck{预期结果}{能用 \code{VOH/VOL/VIH/VIL} 和输出电流说明余量}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：时钟与面包板限制至芯片案例}
\begin{exercisesubproblem}{（a）时钟与面包板限制}
说明按钮、NE555、晶振模块和面包板布线的边界
\exercisecheck{预期结果}{能解释最小脉宽、边沿质量、寄生效应和低速验证范围}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）非法状态恢复}
为状态机未使用编码指定下一状态和输出
\exercisecheck{预期结果}{非法状态不产生危险使能}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）状态机输出}
比较只依赖状态的输出和依赖输入的输出
\exercisecheck{预期结果}{能说明稳定性与响应速度的取舍}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（d）芯片案例}
用 74HC74、74HC161 或移位寄存器说明一个本章概念
\exercisecheck{预期结果}{绑定到真实引脚、复位、时钟和输出负载}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：纹波与同步计数器至模 12 计数器}
\begin{exercisesubproblem}{（a）纹波与同步计数器}
比较 4-bit 纹波计数器与 4-bit 同步计数器的时钟接法、延迟来源和译码风险
\exercisecheck{预期结果}{能说明纹波逐级延迟累积、译码有毛刺窗口，同步计数器把代价移到组合进位链}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）LFSR 前八态}
4 位 LFSR 每拍 \code{Q3} 取旧 \code{Q2}、\code{Q2} 取旧 \code{Q1}、\code{Q1} 取旧 \code{Q0}、\code{Q0} 取旧 \code{Q3} 与旧 \code{Q2} 的异或，从 0001 起列出前 8 个状态
\exercisecheck{预期结果}{序列正确，并能说明 0000 为何必须排除}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）模 12 计数器}
用 4-bit 寄存器与加一逻辑设计模 12 计数器（0 到 11 循环）
\exercisecheck{预期结果}{归零发生在同步边界，无瞬时非法态}
\end{exercisesubproblem}
\end{chapterexercise}

\begin{chapterexercise}{综合练习：Moore 与 Mealy 区分至格雷码相邻性}
\begin{exercisesubproblem}{（a）Moore 与 Mealy 区分}
判断交通灯控制器与 1101 序列检测器各属哪类，并说明输出稳定性差异
\exercisecheck{预期结果}{分类依据写成“输出是否依赖当前输入”}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（b）交通灯转移表}
按东西绿 30s、黄 5s、全红 2s、南北绿 30s、黄 5s、全红 2s 的需求补全状态转移表与输出表
\exercisecheck{预期结果}{六状态闭环，\code{timer\_done} 为唯一转移条件，输出只依赖状态}
\end{exercisesubproblem}
\begin{exercisesubproblem}{（c）格雷码相邻性}
核对交通灯六状态格雷编码 000、001、011、010、110、100 的每对相邻状态（含首尾闭环）是否只差一位
\exercisecheck{预期结果}{六次转移逐一核对，全部只差一位}
\end{exercisesubproblem}
\end{chapterexercise}

\section*{参考解答与要点}

\begin{chapteranswer}{综合练习：入门：画 Q 波形至入门：主从结构}
\begin{answersubproblem}{（a）入门：画 Q 波形}
锁存器 Q 在 CLK=1 期间完全复制 D 的每次变化；触发器 Q 只在每个上升沿复制当时刻的 D，其余时间保持。
\answercheck{必须保留的要点}{触发器不是“更快的锁存器”，采样时刻完全不同。}
\end{answersubproblem}
\begin{answersubproblem}{（b）入门：填 SR 真值表}
0,0 禁用；0,1 置 1；1,0 置 0；1,1 保持。从 0,0 同时释放到 1,1 时结果不可预测。
\answercheck{必须保留的要点}{禁行是非法输入；可靠设计不产生 0,0 或不同时释放。}
\end{answersubproblem}
\begin{answersubproblem}{（c）入门：画计数波形}
Q[3:0] 总线带：9、10、11、12、13、14 逐沿跳变；Q0：1、0、1、0、1、0。
\answercheck{必须保留的要点}{总线带整体跳变，不是逐位爬；Q0 每周期翻转。}
\end{answersubproblem}
\begin{answersubproblem}{（d）入门：主从结构}
M：CLK=0 期间等于 D，CLK=1 期间保持上升沿时刻的 D；Q：每个上升沿等于 M 当时值。
\answercheck{必须保留的要点}{D 在 CLK=1 期间的变化只影响 M 保持的旧值，不影响 Q。}
\end{answersubproblem}
\end{chapteranswer}

\begin{chapteranswer}{综合练习：触发器路径时序分析至复位释放}
\begin{answersubproblem}{（a）触发器路径时序分析}
示例：\code{Tclk >= 0.8ns + 3.6ns + 0.7ns + 0.4ns + 0.5ns = 6.0ns}。若目标周期为 5.0ns，则建立约束失败。
\answercheck{必须保留的要点}{使用最大延迟和不利偏斜，不得只用典型门延迟。}
\end{answersubproblem}
\begin{answersubproblem}{（b）保持违例}
示例：最快路径 0.13ns 到达，而目标保持窗口要求 0.28ns 后才允许变化，存在保持违例。修复可插入保持缓冲、调整布局或修正时钟树。
\answercheck{必须保留的要点}{降频不能可靠修复保持违例。}
\end{answersubproblem}
\begin{answersubproblem}{（c）锁存器透明窗口}
enable 有效期间，前一级输入变化可能穿过第一级锁存器、组合逻辑和第二级锁存器，使一次更新跨越多个预期阶段。
\answercheck{必须保留的要点}{必须指出透明期间没有清晰提交边界。}
\end{answersubproblem}
\begin{answersubproblem}{（d）复位释放}
控制状态复位到 \code{IDLE} 或 \code{RESET_WAIT}，写使能和危险输出为 0；复位可异步拉入，但在本时钟域用触发器同步释放。
\answercheck{必须保留的要点}{复位值、安全输出、释放边界三者都要写明。}
\end{answersubproblem}
\end{chapteranswer}

\begin{chapteranswer}{综合练习：两级同步至事件保持}
\begin{answersubproblem}{（a）两级同步}
外部按钮先经过电气默认值和消抖，再进入两级触发器，状态机只使用 \code{button_sync}。
\answercheck{必须保留的要点}{两级同步降低亚稳态扩散概率，但不替代消抖。}
\end{answersubproblem}
\begin{answersubproblem}{（b）MTBF 指标}
MTBF 表示亚稳态导致可见失败的平均时间尺度。目标时钟越快、异步事件越频繁，风险越高；第一级到第二级之间恢复时间越长，风险越低；器件恢复参数也会影响结果。
\answercheck{必须保留的要点}{同步器是可靠性设计，不是绝对功能保证。}
\end{answersubproblem}
\begin{answersubproblem}{（c）外部事件链路}
示例链路：\code{raw -> filtered -> pressed -> meta -> sync -> event -> pending -> FSM}。其中 raw/filtered 属于电气与消抖，meta/sync 属于时钟域同步，event/pending 属于目标时钟域事件协议。
\answercheck{必须保留的要点}{每一层信号名应对应一个可测量、可观察的边界。}
\end{answersubproblem}
\begin{answersubproblem}{（d）事件保持}
\code{start_event} 置位 \code{start_pending}，状态机在 \code{IDLE} 且安全条件满足时产生 \code{start_ack} 清除 pending；复位或故障可按安全策略清除或拒绝事件。
\answercheck{必须保留的要点}{单周期事件不应直接作为危险动作使能。}
\end{answersubproblem}
\end{chapteranswer}

\begin{chapteranswer}{综合练习：多 bit 跨域至复位域}
\begin{answersubproblem}{（a）多 bit 跨域}
8-bit 总线逐位同步会采到混合时刻的 bit。若是数据传输，应使用 valid/ready 握手或异步 FIFO；若是计数指针，可用格雷码减少一次变化的 bit 数。
\answercheck{必须保留的要点}{多 bit 数据必须有整体有效性约定。}
\end{answersubproblem}
\begin{answersubproblem}{（b）脉冲同步}
可把源域窄脉冲转换为保持电平或翻转事件位，目标域同步后检测变化，再用确认信号让源域清除事件。
\answercheck{必须保留的要点}{事件必须持续到目标域可见，不应仅依赖单周期脉冲宽度。}
\end{answersubproblem}
\begin{answersubproblem}{（c）时钟门控}
写使能让时钟继续到达触发器，只控制是否保存新值；专用时钟门控用于功耗控制并要求 enable 无毛刺和安全锁存；普通 AND 门切时钟会产生短脉冲、额外边沿和不可控偏斜。
\answercheck{必须保留的要点}{普通数据通路优先用写使能，不应随意组合门控时钟。}
\end{answersubproblem}
\begin{answersubproblem}{（d）复位域}
每个时钟域内部同步释放复位；跨域依赖通过 ready、同步器或状态握手确认。同步器、pending 位和危险输出在复位期间应进入不会伪触发的默认值。
\answercheck{必须保留的要点}{复位释放顺序属于设计约定，不能假设所有域同时恢复。}
\end{answersubproblem}
\end{chapteranswer}

\begin{chapteranswer}{综合练习：状态编码至按钮同步器实作}
\begin{answersubproblem}{（a）状态编码}
二进制编码节省触发器但译码较复杂；one-hot 译码直接但触发器多；格雷编码适合相邻状态或跨域指针。无论哪种编码，未使用编码都应导向安全状态并关闭危险输出。
\answercheck{必须保留的要点}{状态编码选择要绑定资源、速度、跨域和安全要求。}
\end{answersubproblem}
\begin{answersubproblem}{（b）波形验证}
至少观察 \code{clk/reset/source_q/dest_d/write_enable/state/sync1/sync2/pending/ack}，检查 D 是否在采样沿前稳定、写使能是否只在目标周期有效、复位释放是否产生伪事件。
\answercheck{必须保留的要点}{只看最终寄存器值不足以证明同步边界正确。}
\end{answersubproblem}
\begin{answersubproblem}{（c）CPU 连接}
PC 是寄存器，流水线寄存器是阶段边界，控制器是状态机，写回使能是状态控制线，中断入口来自同步后的外部或内部事件。
\answercheck{必须保留的要点}{后续 CPU 仍遵守“组合准备、边界提交”的模型。}
\end{answersubproblem}
\begin{answersubproblem}{（d）按钮同步器实作}
按钮节点用上拉或下拉给出默认电平，经过 RC 和 74HC14 得到整形后的 \code{button_pressed}；再接入 74HC74 两级触发器，后级只使用第二级 Q。复位脚进入不会伪触发的默认状态。
\answercheck{必须保留的要点}{必须写清楚每一级信号名，不能把 raw 引脚直接送入状态机。}
\end{answersubproblem}
\end{chapteranswer}

\begin{chapteranswer}{综合练习：计数器实作至实验板物料}
\begin{answersubproblem}{（a）计数器实作}
74HC161/74HC163 的 CP 接低速时钟，计数使能接受控有效电平，并行装载保持无效或由实验开关控制，Q0 到 Q3 通过限流电阻接 LED。复位极性按数据手册连接。
\answercheck{必须保留的要点}{计数器体现同一时钟边界上的 4-bit 状态更新。}
\end{answersubproblem}
\begin{answersubproblem}{（b）移位输出实作}
74HC595 的 \code{DS} 在 \code{SHCP} 边沿前稳定，8 次移位后再给 \code{STCP} 边沿，Q0 到 Q7 一起更新；\code{MR_n} 和 \code{OE_n} 保持在确定电平。
\answercheck{必须保留的要点}{必须区分移位寄存器内部状态和输出锁存边界。}
\end{answersubproblem}
\begin{answersubproblem}{（c）写使能实作}
74HC157 在旧 Q 和 \code{new_bit} 之间选择，输出送入 74HC74 的 D；时钟持续送入触发器，\code{write_enable=0} 时写回旧 Q，\code{write_enable=1} 时写入新值。
\answercheck{必须保留的要点}{不应使用普通与门直接门控时钟作为正式方案。}
\end{answersubproblem}
\begin{answersubproblem}{（d）实验板物料}
最小 BOM 应包含稳定 3.3V 或 5V 电源、面包板、目标 74HC/74HCT 芯片、0.1uF 去耦电容、上拉/下拉电阻、LED 与限流电阻、按钮、RC 滤波电容、杜邦线和万用表；逻辑分析仪或示波器用于观察边沿和短脉冲。
\answercheck{必须保留的要点}{物料清单必须对应实验中的电源、输入、状态输出和测量需求。}
\end{answersubproblem}
\end{chapteranswer}

\begin{chapteranswer}{综合练习：接线检查至电压与扇出}
\begin{answersubproblem}{（a）接线检查}
上电前检查 VCC/GND、低有效复位和输出使能、未用输入默认值、LED 限流、输出是否短接、电压域是否兼容。发现芯片发热或输出异常时应立即断电复查。
\answercheck{必须保留的要点}{检查表应能解释常见硬件失败，而不是只列器件名称。}
\end{answersubproblem}
\begin{answersubproblem}{（b）故障定位报告}
示例：按一次计数器跳多格时，先观察按钮 raw，再看 74HC14 输出，最后看计数器 CP；若 raw 抖动而整形后仍有多个边沿，应增加消抖或避免按钮直接作为时钟。报告要写出修复前后观测差异。
\answercheck{必须保留的要点}{失败分析必须对应到可观察信号和修复后的波形。}
\end{answersubproblem}
\begin{answersubproblem}{（c）数据手册读取}
必须先查推荐工作条件确认 VCC、电压和输入边沿，再查 \code{VIH/VIL/VOH/VOL} 判断电平兼容，查输出电流判断 LED 和负载，查传播延迟、最高频率与最小脉宽判断时钟是否合格；绝对最大额定值只用于损伤边界。
\answercheck{必须保留的要点}{不得把绝对最大额定值当作正常工作条件。}
\end{answersubproblem}
\begin{answersubproblem}{（d）电压与扇出}
若驱动器 \code{VOH_min} 小于接收器 \code{VIH_min} 加余量，高电平不可靠；若 LED 电流过大，输出电平会被拉坏；一个输出驱动多个 CMOS 输入时还要考虑电容负载和边沿变慢。
\answercheck{必须保留的要点}{判断必须同时看电压余量、电流限制和负载电容。}
\end{answersubproblem}
\end{chapteranswer}

\begin{chapteranswer}{综合练习：时钟与面包板限制至芯片案例}
\begin{answersubproblem}{（a）时钟与面包板限制}
按钮单步必须消抖和施密特整形，NE555 要检查占空比和脉宽，晶振模块要检查频率和电压域；面包板只适合低速概念验证，不能证明高频 CPU 级可靠性。
\answercheck{必须保留的要点}{状态边界必须来自合格边沿，不来自慢爬升或多次抖动。}
\end{answersubproblem}
\begin{answersubproblem}{（b）非法状态恢复}
未使用编码的下一状态导向 \code{IDLE}、\code{FAULT} 或复位等待态，输出逻辑默认关闭写使能、驱动使能和片选。
\answercheck{必须保留的要点}{非法状态的输出也要安全，不应只写下一状态。}
\end{answersubproblem}
\begin{answersubproblem}{（c）状态机输出}
Moore 输出只依赖状态，通常更稳定；Mealy 输出还依赖输入，响应更快但可能把输入毛刺传到控制线。危险输出宜寄存或只由稳定状态产生。
\answercheck{必须保留的要点}{取舍必须结合后级是否采样或驱动危险对象。}
\end{answersubproblem}
\begin{answersubproblem}{（d）芯片案例}
以 74HC74 为例，D 是待保存数据，CLK 是采样边界，Q 是保存结果，异步复位/置位必须按数据手册极性连接；未用输入不能悬空。
\answercheck{必须保留的要点}{真实芯片要检查引脚、电源、复位极性、时钟边沿和负载。}
\end{answersubproblem}
\end{chapteranswer}

\begin{chapteranswer}{综合练习：纹波与同步计数器至模 12 计数器}
\begin{answersubproblem}{（a）纹波与同步计数器}
纹波计数器用低位 Q 作高位时钟，第 k 位比第 0 位晚 k 个 \code{tCQ} 翻转，最高位在第 4 个 \code{tCQ} 才稳定，与第 0 位相差 3 个 \code{tCQ}；翻转窗口内译码器会看到瞬时错值，输出毛刺真实存在。同步计数器所有位共用同一时钟沿、同时更新，代价是进位链（高位使能等于低位全 1）随位数变长，成为组合关键路径。
\answercheck{必须保留的要点}{纹波的风险在时钟不同步，同步的代价在进位逻辑深度。}
\end{answersubproblem}
\begin{answersubproblem}{（b）LFSR 前八态}
0001、0010、0100、1001、0011、0110、1101、1010。逐拍按规则计算：高三位左移接收邻位，\code{Q0} 取旧 \code{Q3} 与旧 \code{Q2} 的异或。全 0 态的反馈恒为 0，一旦进入便永远锁死，故初态必须非零。
\answercheck{必须保留的要点}{每拍只由旧态决定新态；0000 是陷阱态。}
\end{answersubproblem}
\begin{answersubproblem}{（c）模 12 计数器}
现态为 1011（即 11）时，下一状态逻辑选 0000 而非加一：用比较器译码 1011 控制数据 mux 即可。若改用异步清零计数器（如 74HC161），需译码 1100 去拉清零脚，会产生短暂的 1100 毛刺态，且各位清零时刻不一致；同步清零（如 74HC163）译码 1011 后在时钟沿干净归零。
\answercheck{必须保留的要点}{归零必须发生在时钟边界；异步清零方案要指出毛刺态。}
\end{answersubproblem}
\end{chapteranswer}

\begin{chapteranswer}{综合练习：Moore 与 Mealy 区分至格雷码相邻性}
\begin{answersubproblem}{（a）Moore 与 Mealy 区分}
交通灯的灯色只由当前状态决定，属 Moore 型，输出在整个状态期间稳定；1101 检测器在 \code{S3} 且输入为 1 的转移上给出输出，属 Mealy 型，同拍响应但输入毛刺可直达输出。
\answercheck{必须保留的要点}{分类看输出函数的自变量，与状态个数无关。}
\end{answersubproblem}
\begin{answersubproblem}{（b）交通灯转移表}
\code{EW\_GREEN} 到 \code{EW\_YELLOW} 到 \code{ALL\_RED\_EW} 到 \code{NS\_GREEN} 到 \code{NS\_YELLOW} 到 \code{ALL\_RED\_NS} 再回到 \code{EW\_GREEN}，转移条件均为 \code{timer\_done}=1，装载值依次为 5、2、30、5、2、30 秒；输出依次为绿/红、黄/红、红/红、红/绿、红/黄、红/红。\code{timer\_done}=0 时保持原状态。
\answercheck{必须保留的要点}{状态数、转移条件、输出必须与需求逐条对应。}
\end{answersubproblem}
\begin{answersubproblem}{（c）格雷码相邻性}
000 到 001 差最低位；001 到 011 差中间位；011 到 010 差最低位；010 到 110 差最高位；110 到 100 差中间位；100 到 000 差最高位。六次转移均只翻转一位。
\answercheck{必须保留的要点}{闭环的最后一边（100 回到 000）也必须核对，不能只看前五次。}
\end{answersubproblem}
\end{chapteranswer}

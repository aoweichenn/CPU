...
连续 8 次移位 & 内部移位状态逐步形成一个字节 & 中间状态不应直接驱动负载 \\
...
外设接口 & 先复位设备内部，再开放总线响应 & 复位期间是否会误写 MMIO（存储器映射 I/O） \\
...
消抖 & 输入持续稳定 N 个周期才接受新电平 & 抖动期间 \code{stable_level} 不变化 \\
...
CPU 连接 & 说明 PC、流水线寄存器、控制器和中断入口分别对应本章哪个概念 & 能把“状态、时钟与时序逻辑”一章边界模型映射到后续 CPU \\
...
实验板物料 & 为本章低速逻辑实验列出最小 BOM（物料清单）和用途 & 能覆盖电源、去耦、默认电平、观察负载和测量工具 \\

...
第二，\code{\$signed} 与比较时的扩展规则。\code{\$signed(x)} 强制把 \code{x} 按补码解释。比较运算的规则是：两个操作数都是 \code{signed}，先符号扩展到同宽，再按补码比较；只要有一个是 \code{unsigned}，两个都按无符号处理。最后这条是著名的反直觉规则，给出一个容易出错的示例：

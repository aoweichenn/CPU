...
\code{\$display} 一族 & 打印机与终端 & 忽略或报错 & testbench \\\tablerowrule
...
\begin{chapterexercise}{module 读图}
...
为本章“testbench 与仿真阅读”一节的 \code{sap1\_ram} 写自检 testbench 的核心部分：向地址 7 写入 \code{8'hA5} 再读回，比对不符时用 \code{\$fatal} 报告，要求不依赖人眼读波形。列出生成时钟、驱动写读、自检三段各自的写法
\exercisecheck{学习目标}{预期+比对+报错才算自检}
\end{chapterexercise}

\section*{参考解答与要点}

\begin{chapteranswer}{module 读图}
...
时钟段沿用“testbench 与仿真阅读”一节骨架：\code{always \#5 clk = \~{}clk;}。驱动段写在 \code{initial} 里：\code{addr = 4'd7; din = 8'hA5; we = 1'b1;}，等一个上升沿后 \code{we = 1'b0;}。自检段：再隔一拍读回，\code{if (dout !== 8'hA5) \$fatal(1, "RAM FAIL"); else \$display("PASS"); \$finish;}。激励按拍推进，预期值 \code{8'hA5} 写进比对语句，严格不等 \code{!==} 让 X 也判失败
\answercheck{必须掌握的要点}{预期值+逐项比对+\code{\$fatal} 三件套；眼看波形不算自检}
\end{chapteranswer}

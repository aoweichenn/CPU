...
\begin{pdfdiagram}
  ...
  \node[hw label, text width=3.4cm, align=center] at (0,0) {4-bit 模 16 数环\\外圈无符号\\内圈补码};
\end{pdfdiagram}
\diagramnote{补码的数环读法：同一个 bit 模式，外圈是无符号读数，内圈是补码读数。加法器只按模 16 进位，两种读法共享同一条进位链——这就是为什么加法和减法能共用一套硬件。}

“取反加一”也不再是口诀，而是一行证明：对任意 $x$，都有 $x + \sim x = \code{1111} = -1$（模 16 下各位互补，相加必得全 1），所以 $-x = \sim x + 1$。硬件只需要一排 NOT 门和一条加法进位链，减法就被归并成了加法。
...
\topic{定点数把小数点位置变成硬件合同}定点数把“小数点在第几位”固定下来。常见写法是 Q 格式（沿用早期 DSP 手册用字母 Q 标记小数位数的习惯）：Q4.4 表示 8-bit 总宽度中有 4 个整数相关位和 4 个小数位，实际值等于原始整数除以 $2^4$。例如 \code{00011000} 的无符号整数值是 24；若按 Q4.4 解释，它的实际值是 $24/16=1.5$。同一串 bit 并没有改变，改变的是缩放合同。
...
把这条公式落到最常见的格式上。IEEE 754 单精度用 32 个 bit，分成三段：
...
把“提前计算”真正展开一次。以 4-bit 为例，把 $C_{i+1} = G_i \lor (P_i C_i)$ 逐位代入，每个进位都改写成只含 G、P 和 \code{C0} 的形式：
...
注意右边只依赖 A、B 和 \code{C0}：\code{C4} 不再等 \code{C3}，四个进位可以同时开算。这就是“超前进位”的全部含义——不是让进位消失，而是把进位链的串行代入改成并行的与或逻辑。
...
\end{waveform}
\diagramnote{三周期 \code{R3=R1+R2} 的对齐波形：控制线只在规定周期为高；状态元件只在上升沿提交。注意三条控制线的互斥关系——任何一拍 \code{reg\_write} 提前变高，R3 都会被未成熟的值污染；任何一拍 \code{ALU\_out\_write} 忘记关闭，下一次计算边界就会混入旧值。}
...
ALU 表面上像一个会做很多运算的黑盒，内部通常更像若干条专用组合路径加一个结果选择器。加法/减法依赖进位链，逻辑运算依赖逐位门，移位依赖多级选择网络，比较往往复用减法证据。理解这一点之后，读者就不会把 \code{ADD}、\code{AND}、\code{SLL}、\code{SLT} 当成同等代价的“软件操作”。

\begin{longtable}{L{0.16\linewidth}L{0.36\linewidth}L{0.38\linewidth}}
\toprule
ALU 功能 & 常见硬件来源 & 关键代价或风险 \\
\midrule
加法 & 全加器链、超前进位或前缀进位网络 & 进位传播决定关键路径，位宽越大越明显 \\
减法 & \code{B} 逐位取反并令最低位 \code{carry_in=1} & Carry 和 Overflow 的解释必须按减法规则重新定义 \\
按位逻辑 & 每一位独立的 AND/OR/XOR/NOT & 延迟短，但结果选择 mux 会增加负载 \\
无符号小于 & 减法后的借位或 Carry 证据 & 不能直接看符号位 \\
有符号小于 & 减法结果符号与 Overflow 共同决定 & \code{0x8000-1} 这类边界最容易出错 \\
移位 & 桶形移位器或多周期逐位移位 & 单周期桶形移位器面积和 mux 级数较大 \\
\bottomrule
\end{longtable}
...
\[
G_i=A_iB_i,\qquad P_i=A_i\oplus B_i,\qquad C_{i+1}=G_i\lor(P_iC_i)
\]
...
乘法和除法不是“更大的加法按钮”。一个 \(n\) 位乘法可以看成 \(n\) 个部分积的移位求和；一个除法可以看成反复试减、恢复或不恢复余数；浮点运算还要经历对阶、尾数运算、规格化和舍入。若所有步骤都做成单周期组合逻辑，面积和延迟会迅速失控。

\begin{longtable}{L{0.18\linewidth}L{0.44\linewidth}L{0.28\linewidth}}
\toprule
运算 & 可拆分的硬件步骤 & 常见控制方式 \\
\midrule
无符号乘法 & 判断乘数最低位、条件加被乘数、移位、计数 & 多周期移位加法，或阵列/树形乘法器 \\
补码乘法 & 处理符号扩展和负权部分积 & Booth 编码减少连续 1 的部分积数量 \\
无符号除法 & 左移余数、试减除数、决定商位、必要时恢复余数 & 多周期状态机，逐位产生商 \\
定点乘法 & 原始整数相乘后调整缩放位数 & 乘积位宽翻倍，截断和舍入必须规定 \\
浮点加法 & 对阶、尾数加减、规格化、舍入、异常标志 & 分流水级或由微码/协处理器完成 \\
\bottomrule
\end{longtable}
...
\midrule
1 & \code{0000,1110} & $0-2<0$ & 0 & 恢复 R=\code{0000} \\
2 & \code{0001,1100} & $1-2<0$ & 0 & 恢复 R=\code{0001} \\
3 & \code{0011,1000} & $3-2=1$ & 1 & 保留 R=\code{0001} \\
4 & \code{0011,0001} & $3-2=1$ & 1 & 保留 R=\code{0001} \\
\bottomrule
\end{longtable}
...
浮点的难点更隐蔽。两个指数不同的浮点数相加时，较小数的尾数要右移对齐；右移时丢掉的位会进入 guard、round、sticky 证据——guard 是被移出的最高一位，round 是次一位，sticky 是其余被移出位按 OR 合并出的“是否还有 1”，舍入电路用这三个额外证据决定末位该不该进 1。尾数相加后可能需要左规或右规；最后按舍入模式提交。如果读者只把浮点看成“整数加法多一个小数点”，就无法解释为什么 \((a+b)+c\) 和 \(a+(b+c)\) 可能不同，也无法解释 NaN、Inf、非规格化数和异常标志。

\topic{专题：ALU 验收向量要覆盖边界值}
ALU 最怕只用随机正常值验收。随机测试当然有用，但边界值才会逼出进位、溢出、符号扩展、移位宽度和比较解释错误。一个合格的验收表至少应覆盖零、最大正数、最小负数、全 1、单 bit、交替 bit、进位跨字节、符号位翻转和移位距离边界。

\begin{longtable}{L{0.18\linewidth}L{0.26\linewidth}L{0.46\linewidth}}
\toprule
向量类型 & 示例 & 主要检查点 \\
\midrule
零值 & \code{0+0}，\code{0-0} & Zero 标志、无额外进位、写回不遗漏 \\
无符号进位 & \code{0xFF+0x01} & 结果回绕和 CarryOut 同时正确 \\
有符号溢出 & \code{0x7F+0x01} & Sign 为 1 不等于“合法负数”，Overflow 必须置位 \\
最小负数 & \code{0x80-0x01}，\code{-128/-1} & 补码非对称范围和除法溢出 \\
比较混用 & \code{0xFF < 0x01} & 无符号为假，有符号为真 \\
移位边界 & \code{1<<0}，\code{1<<7}，\code{1<<8} & 移位距离截断或非法规则必须明确 \\

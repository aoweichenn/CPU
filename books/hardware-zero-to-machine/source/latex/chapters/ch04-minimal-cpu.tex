...
把这次走读填上具体数值，"路径"就变成"账目"。设 \code{R1=5}、\code{R2=7}、指令长 2 字节：
...
取指后 & \code{IR = ADD R3,R1,R2} & 编码来自 \code{MEM[100]} \\
译码后 & \code{alu\_op=ADD}，读地址 R1/R2，写地址 R3，\code{wb\_sel=ALU}，\code{reg\_write=1} & 控制线与 opcode 一致 \\
读寄存器 & \code{read\_a=5}，\code{read\_b=7} & 寄存器堆当前值 \\
执行后 & \code{ALU\_out=12}，写回 mux 输出 12 & 加法路径和写回选择正确 \\
时钟沿 & \code{R3 <- 12}，\code{PC <- 102} & 两个状态同时提交，R1/R2 不变 \\
\bottomrule
\end{longtable}

任何一格对不上，故障范围就锁死了：\code{read\_a} 不是 5，查寄存器堆读地址；\code{ALU\_out} 不是 12，查 \code{alu\_op} 和进位链；12 没到写数据口，查 \code{wb\_sel}；R3 没变或 PC 没变，查 \code{reg\_write}、\code{pc\_write} 和时钟沿本身。这就是 trace 比"检查一下"可靠的地方。
...
这张表可以直接变成验收清单。若某条 LOAD 指令读到了错误数据，不应只问“内存为什么错”，而要逐项检查：PC 是否取到正确编码，译码是否识别为 LOAD，ALU 是否形成正确地址，MEM 阶段是否访问正确目标，读回数据是否进入 MDR（memory data register，存储器数据寄存器——访存返回数据在进入写回路径之前的暂存，写回 mux 的“存储器数据”候选就是从它引出），WB 阶段是否选择了 MDR 而不是 ALUOut。这样的排查方式比凭感觉改控制器可靠得多。
...
把控制字压到这台具体 ISA 上，每条指令一行，全部 7 条覆盖：
...
\code{LDI} & 直送 & 立即数 & — & ALU & 1 & 0 & 0 & 顺序 \\
\code{LOAD} & ADD & 寄存器 & off4 扩展 & 存储器 & 1 & 1 & 0 & 顺序 \\
\code{STORE} & ADD & 寄存器 & off4 扩展 & — & 0 & 0 & 1 & 顺序 \\
\code{JZ} & 判零 & 寄存器 & — & — & 0 & 0 & 0 & Z?目标：顺序 \\
\code{JMP} & — & — & — & — & 0 & 0 & 0 & 目标 \\
\code{SLI} & 左移 & 寄存器 & imm4 & ALU & 1 & 0 & 0 & 顺序 \\
\bottomrule
\end{longtable}
}

这张表还能再压成布尔式。以 \code{reg\_write} 为例，它只在四条指令下为 1：

\begin{lstlisting}
reg_write = (op == ADD) OR (op == LDI) OR (op == LOAD) OR (op == SLI)
mem_read  = (op == LOAD)
mem_write = (op == STORE)
...
\topic{先规定一个极小 ISA，程序才能进入 ROM}为了让整机实验真正运行，需要把“指令”压成固定 bit。下面是一种 16-bit 教学编码：高 4 bit 是 opcode，中间字段选择寄存器，低位可解释为寄存器号、立即数或分支偏移。寄存器字段 3 bit，即 R0–R7，其中 R0 固定为 0。它不是任何真实 ISA 的复制品，但足以把编码、译码、控制字和 ROM 内容连接起来。
...
\code{LDI} & \code{0010 d x imm8} & \code{R[d]=zero_extend(imm8)} & 立即数扩展、写回 \\
\code{LOAD} & \code{0011 d a off4 --} & \code{R[d]=MEM[R[a]+off4]} & 地址形成、读存储、写回 \\
\code{STORE} & \code{0100 s a off4 --} & \code{MEM[R[a]+off4]=R[s]} & 地址形成、写数据、字节使能 \\
...
\code{SLI} & \code{0111 d a imm4 --} & \code{R[d]=R[a]<<imm4}，低位补 0 & 移位器、写回 \\
\bottomrule
\end{longtable}

两个语义细节必须先定死，后面的程序才跑得上。第一，跳转偏移都相对\hwkey{下一条指令的地址}计算：跳转目标 = 本条地址 + 1 + 偏移，偏移按补码解释，可正可负。第二，\code{LDI} 只能装 8 位立即数，高地址必须靠移位拼出来：\code{LDI R1, 0xF0} 得到 \code{R1=0x00F0}，再 \code{SLI R1, R1, 8} 左移 8 位得到 \code{R1=0xF000}。\code{SLI} 的字段布局与 \code{ADD} 相同（d、a、低 4 位立即数），所以译码路径几乎免费。

这个编码能直接写入 ROM。若 \code{GPIO_IN=0xF004}、\code{GPIO_OUT=0xF000}，程序先用 \code{LDI}+\code{SLI} 拼出 MMIO 基址 \code{0xF000}，再循环读取按键、条件跳转、写 LED。下面左栏是 ROM 中的真实 16-bit 字，右栏是对应汇编和语义：

\begin{longtable}{L{0.1\linewidth}L{0.13\linewidth}L{0.24\linewidth}L{0.45\linewidth}}
\toprule
地址 & 内容 & 汇编 & 语义 \\
\midrule
0 & \code{0x22F0} & \code{LDI R1, 0xF0} & \code{R1 = 0x00F0} \\
1 & \code{0x7260} & \code{SLI R1, R1, 8} & \code{R1 = 0xF000}（MMIO 基址） \\
2 & \code{0x3450} & \code{LOAD R2, [R1+4]} & \code{R2 = MEM[0xF004]}，读 GPIO\_IN \\
3 & \code{0x5403} & \code{JZ R2, +3} & \code{R2==0} 时跳到地址 7（$3+1+3$） \\
4 & \code{0x2601} & \code{LDI R3, 1} & \code{R3 = 1} \\
5 & \code{0x4640} & \code{STORE R3, [R1+0]} & \code{MEM[0xF000] = 1}，点亮 LED \\
6 & \code{0x6FFB} & \code{JMP -5} & 跳回地址 2（$6+1-5$） \\
7 & \code{0x2600} & \code{LDI R3, 0} & \code{R3 = 0} \\
8 & \code{0x4640} & \code{STORE R3, [R1+0]} & \code{MEM[0xF000] = 0}，熄灭 LED \\
9 & \code{0x6FF8} & \code{JMP -8} & 跳回地址 2（$9+1-8$） \\
\bottomrule
\end{longtable}

注意三个可验证事实。第一，偏移账是平的：地址 3 的 \code{JZ +3} 落到地址 7，地址 6 的 \code{JMP -5} 和地址 9 的 \code{JMP -8} 都落回地址 2。第二，\code{0xF004} 这条地址在 ISA 里是\textbf{可以形成}的——不是注释里的愿望，而是 \code{0x00F0 << 8 + 4} 的真实计算。第三，ROM 里每个 16-bit 字都能被译码器逐字段读出：拿 \code{0x7260} 验证，高 4 位 \code{0111} 是 \code{SLI}，d=\code{001}、a=\code{001}、imm4=\code{1000}=8。
...
用三条指令验证这张图确实能跑通全部指令类型：\code{ADD} 走“寄存器堆双读 → ALU → 写回 mux 选 ALU 结果”；\code{LDI} 走“imm8 零扩展 → mux A → ALU 直送 → 写回”；\code{LOAD} 走“基址寄存器加 off4 符号扩展 → ALU 形成地址 → 数据存储 → 写回 mux 选存储器数据”；\code{STORE} 复用 LOAD 的地址路径，但数据从寄存器堆流向数据存储；\code{JZ} 的 Zero 决定 \code{pc mux} 选 +1 加法器还是 +off 加法器；\code{SLI} 走“mux A → ALU 内置移位路径 → 写回”。没有一条指令需要图外的部件——这是数据通路完整性的最低验收。
...
这一圈账值得逐行核对：偏移算术（沿 4 的 $3+1+3$、沿 7 和 12 的回跳）、MMIO 地址形成（沿 3 和 8 的 \code{0xF000+4}）、条件跳转两次走向相反（沿 4 跳、沿 9 不跳）、以及程序效果本身——LED 跟随按键状态循环亮灭。能写出这张表，才说明编码、译码、数据通路、控制字和 PC 更新在同一条账上；任何一格对不上，回到控制真值表和数据通路图逐格反查即可。
...
\end{waveform}
\diagramnote{复位后前三个周期的时序，对照§二 ROM 程序开头三条编码（\code{0x22F0}=\code{LDI R1,0xF0}、\code{0x7260}=\code{SLI R1,R1,8}、\code{0x3450}=\code{LOAD R2,[R1+4]}）。复位把 PC 压在 \code{0x0000}、写使能压在安全值；释放后每个周期内组合路径完成取指、译码、执行，周期末上升沿才提交新状态——\code{0x00F0} 在第二个上升沿、\code{0xF000} 在第三个上升沿写进 R1。这就是“状态只在时钟沿改变”的波形证据，单步调试时应逐格核对。}
...
先说明口径：这个案例使用 32-bit 宽数据和 \code{ADDI/SUBI/JNZ} 等示意助记符，比前面的 16-bit 极小 ISA 宽一档，目的是让数值关系更醒目；两者的 trace 方法完全相同——读旧状态、经组合路径、在时钟沿提交。
...
\begin{pdfdiagram}
  ...
  \node[hw memory, minimum width=3.8cm, minimum height=1.0cm] (f2) at (1.9,1.25) {保存的 R5（sum2 写入）};
  \node[hw memory, minimum width=3.8cm, minimum height=1.0cm] (f3) at (1.9,0.25) {向下可继续增长};
  % SP 位置标注（指向内存条右边界的三条分界线）
  \draw[hw bus] (4.6,2.75) node[right, hw label]{调用前的 SP} -- (3.8,2.75);
  \draw[hw bus] (4.6,1.75) node[right, hw label]{CALL 保存返回地址后} -- (3.8,1.75);
  \draw[hw bus] (4.6,0.75) node[right, hw label]{sum2 再减 4 后（帧底）} -- (3.8,0.75);
\end{pdfdiagram}
\diagramnote{函数调用案例对应的栈帧内存布局。CALL 先把返回地址压入，SP 下移一格；\code{sum2} 序言再减 SP 并把 R5 保存到新帧里；尾声按相反顺序 LOAD 恢复、SP 加回。RET 硬件只认“返回地址”这一格：若某条越界 STORE 改写了它，RET 就把被污染的值装进 PC，程序表现为“返回后跑飞”。}
...
ADD trace & 读端口选择 R1/R2，ALU 选择 ADD，写回 mux 选择 ALUOut，写地址为 R3，\code{reg_write=1} 只在写回边界有效。示例账：\code{R1=5,R2=7} 时 \code{read\_a=5}、\code{read\_b=7}、\code{ALU\_out=12}，时钟沿 \code{R3<-12}、\code{PC=100->102}。 & 算对不等于写对。 \\
...
JZ trace & 比较或 ALU 结果产生 Zero，控制器根据 Zero 选择顺序 PC 或分支目标；PC 只在提交边界更新。示例账（本章 ROM 程序）：地址 3 的 \code{JZ +3} 在 \code{R2=0} 时 \code{PC=3+1+3=7}；同一指令在 \code{R2=1} 时顺序取地址 4。 & 条件输入必须在采样前稳定。 \\
...
CALL/RET trace & CALL：返回地址先经栈保存（\code{SP 减后 STORE}），再 \code{PC=目标}；RET：从栈读回返回地址（\code{LOAD}），恢复 SP，\code{PC=返回地址}。栈访问本质就是 LOAD/STORE 事务，SP 是普通寄存器加约定。 & 调用链是事务序列，不是语法。 \\

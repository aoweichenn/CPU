...
口把物理几何直接暴露给软件——第七章 int 13h 的调用约定
...
坏块替换、掉电重建的完整软件，配的 DRAM 比第八章整台教
...
稳态性能还有队列一侧的条件，用第十二章的 Little 定律随手
...
面，扇区号等盘片转到第几段。第七章 int 13h 的寄存器约定
...
之一。第十二章的 Amdahl 定律在这里换成一句大白话：整个阵列的随机
吞吐，等于盘数乘以每盘那约 100 次。
...
按 8b/10b 编码折算有效带宽约 600 MB/s（第九章讨论过这类编码开销
...
位；iodepth 就是在途请求数，与第十二章 Little 定律里的在途数是同一
个量。
...
度为 1 时没有重排空间，调度无能为力。这正是第十二章 Little 定律的
...
或盘内在做重映射。第十二章"用测量代替猜测"的原则在外设上格外省
...
了，按第十二章 Little 定律，吞吐随之改变。AHCI 只有一条 32 深的队
...
超时 T & 距上次中断满 T $\mu$s 即发 & 调大（几十 $\mu$s） & 调小或置 0 \\
...
级。单队列网卡插上万兆链路，瓶颈不在线，在环。用第十二章 Little
...
间接代价。用第十二章 roofline 的话说：每次无谓的拷贝都在消耗全机
共享的带宽屋顶，留给应用的屋顶相应变矮。
...
而是根本没有共享——第十二章谈多核时说过，把共享拆开比把锁做快更值钱，
...
第十二章的 Little 定律给出该维持多少；用满上限买到的是排队而不是吞
吐——本节第四主题会把这笔账算到具体数字。
...
\topic{尾延迟：P99 从哪里来}
前面反复出现的"典型时延 100 $\mu$s"只是中位数附近的值。第十二章已
...
队列拥塞 & 深队列高负载下排队，第十二章的 $1/(1-\rho)$ & 百微秒级 & 深度过大或负载接近饱和 \\
SMR 重写 & 区域写满触发整段重写（第三节） & 十毫秒级至秒级 & 随机写命中已写区域 \\
...
平均时延把这些全藏了起来。算一遍第十二章的同款例子：100 个请求里 99
...
这条曲线的形状与第十二章的 roofline 是同一族：拐点左侧被深度（在途
...
或浅队列，避免排在批量流量后面——用第十二章的语言说，浅队列买的是时
...
三类设备共享同一张检查表不是巧合：第五章的描述符环与 DMA、第十二章的
...
任务 & 要求 & 学习目标 \\
\midrule
扇区对齐 & 某 4 KB 扇区硬盘的分区从 63 号逻辑（512 B）扇区开始，文件系统按 4 KB 块写入。求分区起始字节偏移是否为 4 KB 的整数倍；说明每次文件系统块写要触及几个物理扇区、代价是什么，并给出修复做法 & 分区边界与物理扇区对齐 \\
...
中断聚合权衡 & 不聚合：$1.488\times10^{6}\times5\ \mu\text{s}\approx7.4$ CPU 秒/秒，约需 8 个核专职处理中断，系统实际早已丢包。聚合 32 帧一次：中断率 $1.488\times10^{6}/32\approx4.65\times10^{4}$ 次/秒，CPU 开销 $\approx4.65\times10^{4}\times5\ \mu\text{s}\approx0.23$ 秒/秒，约一个核的 23\%。代价：帧要等够一批或等到聚合定时器超时，平均时延增加约半个聚合窗口（$T/2$） & 聚合把到达率除以批大小；吞吐与时延的交换 \\

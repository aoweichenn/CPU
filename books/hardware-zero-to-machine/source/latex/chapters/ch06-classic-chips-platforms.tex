...
接上调试器（QEMU 内置 monitor 或 GDB 调试桩）后，可以从复位后的第一条指令开始单步，看 BIOS 怎样建立段描述符和中断向量，看 boot 扇区怎样用 \code{int 0x13} 读盘；可以在任意时刻打印全部寄存器和任意一段物理内存，改一个字节再继续运行，直接观察”数据段复制错了”会长成什么样；还可以故意让代码访问未映射地址，看仿真器报告总线错误或目标机异常，而不必担心烧坏任何东西。

...
PC/AT 平台 & 把 BIOS ROM、8259、8254、8237、键盘控制器、UART、扩展总线放入同一整机说明 & 能说明每个对象对应启动、中断、定时、DMA、I/O 或扩展资源 \\
端口资源表 & 为一个早期 x86 平台写出 I/O 端口、中断线、DMA 通道、内存窗口和启动顺序规格 & 能解释平台兼容性为什么依赖这些硬件合同 \\
...
386/486 平台层次 & 说明 CPU 本地总线、内存/cache 控制、高速扩展、低速 I/O 和固件层各自职责 & 能说明主频之外的等待来自 DRAM、桥片、仲裁和设备 \\
PCIe 设备事务 & 说明配置空间、BAR/MMIO、DMA、MSI/MSI-X 和错误报告的硬件含义 & 能把现代扩展设备仍然还原成请求、响应、完成和错误 \\
SoC 集成 & 说明 cache、内存控制器、片上互连、集成外设和安全启动如何降低常见路径时延 & 能同时指出新增的一致性、时钟/电源域和安全边界 \\
\bottomrule
\end{longtable}
...
PC/AT 平台 & BIOS ROM 提供复位后的第一段固件；8259A 汇总中断；8253/8254 提供定时节拍；8237A 执行 DMA；键盘控制器接收扫描码并可参与复位/门控；UART 和并口提供外设连接；扩展总线允许插卡响应 I/O、内存窗口或中断线。 & 平台是 CPU、固件、控制器和扩展资源共同形成的机器合同。 \\
端口资源表 & 端口表要列出设备寄存器的端口号、读写语义和译码宽度；中断表说明 IRQ 来源、屏蔽和 EOI；DMA 表说明通道、方向、地址和长度；内存窗口说明显存、Option ROM、缓冲区或 MMIO；启动顺序说明固件初始化和枚举路径。 & 兼容性依赖稳定的端口、中断、DMA 和启动约定。 \\
...
386/486 平台层次 & CPU 本地总线负责核心访存事务；内存/cache 控制负责 DRAM、二级 cache、写回和等待；高速扩展路径负责图形、磁盘、网络或总线桥；低速 I/O 控制负责串口、并口、键盘、RTC 等；固件层负责 BIOS、Option ROM、设备初始化和启动选择。 & 整机等待可能来自内存、桥片、仲裁、设备和固件，而不仅是 CPU 主频。 \\
PCIe 设备事务 & 配置空间暴露设备身份、能力和 BAR；BAR/MMIO 把设备寄存器映射到物理地址；DMA 让设备成为内存事务发起者；MSI/MSI-X 通过内存写投递中断消息；错误报告记录链路、权限、超时或设备内部错误。 & 现代互连形式复杂，但仍可按请求、响应、完成、中断和错误分析。 \\
SoC 集成 & SoC 把 CPU 核、cache、内存控制器、片上互连、外设和安全启动收进一颗或少数芯片。常见路径变短，数据和转换更靠近执行核心，外设通过片上互连和队列协作；新增问题是一致性、QoS、时钟/电源域、调试权限和安全启动边界。 & 低时延来自平台集成与层级存储，不是所有访问都同样快。 \\
\bottomrule
\end{longtable}

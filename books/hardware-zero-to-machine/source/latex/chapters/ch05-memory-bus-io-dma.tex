...
以软盘控制器常用的通道 2 为例，把 8192 字节从设备搬到物理地址 \code{0x23450} 的完整配置序列如下。页号 \code{0x02}、页内偏移 \code{0x3450}，结束地址 \code{0x2544F}，不跨页边界；计数写 $8192-1=8191=$ \code{0x1FFF}。
...
\begin{pdfdiagram}
  ...
  % index → 槽
  \draw[hw bus] (5.2,4.0) -- node[hw label, right, pos=0.55]{index 译码选槽} (5.2,3.18);
  \node[hw state, minimum width=4.8cm] (s0) at (5.2,2.9) {槽 168：tag，V，64 B 数据};
  \node[hw state, minimum width=4.8cm] (s1) at (5.2,2.05) {槽 169：tag=0x00010，V=1，数据};
  \node[hw state, minimum width=4.8cm] (s2) at (5.2,1.2) {槽 170：tag，V，64 B 数据};
...
\begin{pdfdiagram}
  ...
  \draw[hw bus] (2.3,4.9) -- (8.0,4.9);
  \node[hw label, below] at (5.15,4.9) {index 译码：同一组的两路同时选中};
  \draw[hw bus] (2.3,4.9) -- (2.3,3.91);
  \draw[hw bus] (8.0,4.9) -- (8.0,3.91);
  % offset 选行内字节
  \node[hw label, below] at (6.7,5.6) {offset 选行内字节};
  % 组内两路
  \node[hw state, minimum width=3.4cm] (way0) at (2.3,3.6) {路 0：tag，V，64 B 数据};
  \node[hw state, minimum width=3.4cm] (way1) at (8.0,3.6) {路 1：tag，V，64 B 数据};
  % 2 选 1 mux
  \node[hw compute, minimum width=1.9cm] (mux) at (5.15,3.6) {2 选 1 mux};
  ...
  % 两路 tag 并行比较
  \node[hw compute, minimum width=2.4cm] (cmp0) at (2.3,2.35) {tag 比较（= ?）};
  \node[hw compute, minimum width=2.4cm] (cmp1) at (8.0,2.35) {tag 比较（= ?）};
  \draw[hw bus] (way0.south) -- (cmp0.north);
  \node[hw label, anchor=west] at (2.35,2.72) {槽内 tag};
  \draw[hw bus] (way1.south) -- (cmp1.north);
  \node[hw label, anchor=west] at (8.05,2.72) {槽内 tag};
  % 地址 tag 总线：并行送两个比较器

...
但单纯反馈还不足以形成可用存储单元。
\begin{circuit}[x=1.4cm,y=1cm]
  \node[not gate US, draw, scale=1.0] (inv1) at (0,0) {};
  \node[not gate US, draw, scale=1.0] (inv2) at (2.5,0) {};
  \node[left] at ($(inv1.input)+(-0.5,0)$) {X};
  \node[right] at ($(inv2.output)+(0.5,0)$) {Y};
  \draw (inv1.output) -- ++(0.3,0) |- ($(inv2.input)+(-0.3,0)$) -- (inv2.input);
  \draw (inv2.output) -- ++(0.3,0) |- ($(inv1.input)+(-0.3,0)$) -- (inv1.input);
  \node[below=0.3cm of inv1, font=\scriptsize, BookTeal] {0};
  \node[below=0.3cm of inv2, font=\scriptsize, BookTeal] {1};
\end{circuit}
\diagramnote{两个反相器首尾相接形成双稳态。}

若无法可靠地把它从 0 改成 1
\begin{block}[x=1.5cm,y=1cm]
  \node[draw, rectangle, minimum width=1.8cm, minimum height=1.6cm, fill=BookCodeBg] (sym) at (0,0) {};
  \node[above] at (sym.north) {D-Latch};
  \node[left] at ($(sym.west)+(-0.1,0.4)$) {D};
  \node[left] at ($(sym.west)+(-0.1,-0.4)$) {LE};
  \node[right] at ($(sym.east)+(0.1,0.4)$) {Q};
  \node[right] at ($(sym.east)+(0.1,-0.4)$) {Q'};
\end{block}
\diagramnote{D 锁存器方框符号。}，或从 1 改成 0，它只是一个稳定反馈环。因此需要写入控制。锁存器可以理解为带写使能的 1-bit 存储单元：写使能有效时，输入 D 可以影响输出 Q；写使能无效时，反馈通路维持原来的 Q。
...
把 SR 锁存器变成 D 锁存器的意义
\begin{circuit}[x=1.6cm,y=1.1cm]
  \node[nand gate US, draw, logic gate inputs=nn, scale=1.1] (n1) at (0,0) {};
  \node[nand gate US, draw, logic gate inputs=nn, scale=1.1] (n2) at (3.5,0) {};
  \node[left] at ($(n1.input 1)+(-0.7,0)$) {$\overline{S}$};
  \node[left] at ($(n1.input 2)+(-0.7,0)$) {$\overline{R}$};
  \node[right, font=\small\bfseries] at ($(n1.output)+(0.5,0)$) {Q};
  \node[right, font=\small\bfseries] at ($(n2.output)+(0.5,0)$) {$\overline{Q}$};
  \draw ($(n1.input 1)+(-0.7,0)$) -- (n1.input 1);
  \draw ($(n1.input 2)+(-0.7,0)$) -- (n1.input 2);
  \draw (n1.output) -- ++(0.5,0) |- ($(n2.input 1)+(-0.5,0)$) -- (n2.input 1);
  \draw (n2.output) -- ++(0.5,0) |- ($(n1.input 2)+(-0.5,0)$) -- (n1.input 2);
\end{circuit}
\diagramnote{交叉耦合 NAND 门构成 SR 锁存器。}，在于把两个可能冲突的命令合并为一个数据输入 D 和一个写使能 enable。enable 有效且 D=1 时，相当于请求 set；enable 有效且 D=0 时，相当于请求 reset；enable 无效时，set 和 reset 都不被触发，反馈保持旧值。这样，外部控制逻辑不再同时生成 set/reset 两条危险命令，而是生成“是否写入”和“写入什么值”两个更清晰的问题。
...
on clock edge:
  Q = D
between clock edges:
\begin{block}[x=1.5cm,y=0.9cm]
  \node[hw compute, minimum width=1.6cm] (mux) {MUX};
  \node[hw state, right=0.8cm of mux, minimum width=1.4cm] (ff) {D-FF};
  \draw[hw bus] (ff.east) -- ++(0.6,0) node[right] (q_out) {Q};
  \draw[hw bus] (q_out) |- ++(0,0.7) -| (mux.south) node[below right, hw label, pos=0.3]{反馈};
  \draw[hw bus] (mux) -- (ff) node[above, hw label, pos=0.5]{D};
  \draw[hw bus] (mux.west) -- ++(-0.6,0) node[left]{D$_{in}$};
  \node[hw control, below=0.5cm of mux, minimum width=1.0cm] (we) {WE};
  \draw[hw bus] (we) -| (mux.north) node[above right, hw label]{选择};
  \node[below=0.1cm of we, hw label] {0=保持, 1=写入};
\end{block}
...
触发器采样也不是数学瞬间
\begin{waveform}[x=1pt,y=1.2ex]
  \draw[BookRule!30,step=20pt] (0,-3ex) grid (280pt,8ex);
  \draw[BookAccent,thick] (0,4ex) -- (0,6ex) -- (20pt,6ex) -- (20pt,4ex) -- (40pt,4ex) -- (40pt,6ex) -- (60pt,6ex) -- (60pt,4ex) -- (80pt,4ex) -- (80pt,6ex) -- (100pt,6ex) -- (100pt,4ex) -- (120pt,4ex) -- (120pt,6ex) -- (140pt,6ex) -- (140pt,4ex) -- (160pt,4ex) -- (160pt,6ex) -- (180pt,6ex) -- (180pt,4ex) -- (200pt,4ex) -- (200pt,6ex) -- (220pt,6ex) -- (220pt,4ex) -- (240pt,4ex) -- (240pt,6ex) -- (260pt,6ex) -- (260pt,4ex) -- (280pt,4ex);
  \node[BookAccent,left] at (0,5ex) {CLK};
  \draw[BookAmber,thick] (0,0) -- (14pt,0) -- (14pt,3ex) -- (46pt,3ex) -- (46pt,0) -- (280pt,0);
  \node[BookAmber,left] at (0,1.5ex) {D};
  \draw[BookTeal,thick] (0,-3ex) -- (20pt,-3ex) -- (28pt,-3ex) -- (28pt,0) -- (68pt,0) -- (68pt,-3ex) -- (280pt,-3ex);
  \node[BookTeal,left] at (0,-1.5ex) {Q};
  \draw[BookRed,thick,->] (20pt,-3ex) -- (28pt,-3ex) node[below,font=\scriptsize,BookRed,midway] {tCQ};
  \draw[BookAmber,thick,->] (28pt,1ex) -- (60pt,1ex) node[above,font=\scriptsize,BookAmber,midway] {组合逻辑};
  \draw[BookTeal,thick,->] (60pt,1ex) -- (68pt,1ex) node[above,font=\scriptsize,BookTeal,midway] {tSU};
\end{waveform}
\diagramnote{tCQ + 组合延迟 + tSU = 最小时钟周期。}。输入 D 必须在时钟沿到来前稳定一小段时间，这称为建立时间；时钟沿之后还要继续稳定一小段时间，这称为保持时间。
...
\begin{waveform}[x=1pt,y=1.2ex]
  ...
  \draw[BookAccent,thick] (0,4ex) -- (0,6ex) -- (20pt,6ex) -- (20pt,4ex) -- (40pt,4ex) -- (40pt,6ex) -- (60pt,6ex) -- (60pt,4ex) -- (80pt,4ex) -- (80pt,6ex) -- (100pt,6ex) -- (100pt,4ex) -- (120pt,4ex) -- (120pt,6ex) -- (140pt,6ex) -- (140pt,4ex) -- (160pt,4ex) -- (160pt,6ex) -- (180pt,6ex) -- (180pt,4ex) -- (200pt,4ex) -- (200pt,6ex) -- (220pt,6ex) -- (220pt,4ex) -- (240pt,4ex) -- (240pt,6ex) -- (260pt,6ex) -- (260pt,4ex) -- (280pt,4ex);
  \node[BookAccent,left] at (0,5ex) {CLK};
  \draw[BookAmber,thick] (0,0) -- (14pt,0) -- (14pt,3ex) -- (46pt,3ex) -- (46pt,0) -- (84pt,0) -- (84pt,3ex) -- (126pt,3ex) -- (126pt,0) -- (164pt,0) -- (164pt,3ex) -- (206pt,3ex) -- (206pt,0) -- (244pt,0) -- (244pt,3ex) -- (280pt,3ex);
  \node[BookAmber,left] at (0,1.5ex) {D};
  \draw[BookTeal,thick] (0,-3ex) -- (20pt,-3ex) -- (20pt,0) -- (60pt,0) -- (60pt,-3ex) -- (100pt,-3ex) -- (100pt,0) -- (140pt,0) -- (140pt,-3ex) -- (180pt,-3ex) -- (180pt,0) -- (220pt,0) -- (220pt,-3ex) -- (260pt,-3ex) -- (260pt,0) -- (280pt,0);
  \node[BookTeal,left] at (0,-1.5ex) {Q};
  \foreach \x in {14,54,94,134,174,214,254} {\fill[BookRed!15] (\x pt,4.5ex) rectangle (\x+6 pt,6.5ex);}
  \foreach \x in {20,60,100,140,180,220,260} {\fill[BookAmber!15] (\x pt,4.5ex) rectangle (\x+6 pt,6.5ex);}
  \node[BookRed,right,font=\scriptsize] at (20pt,7.5ex) {$t_{su}$};
  \node[BookAmber,right,font=\scriptsize] at (26pt,7.5ex) {$t_h$};
\end{waveform}
\diagramnote{建立/保持时间窗口。}
...
\begin{block}[x=1.5cm,y=0.8cm]
  \node[hw compute, minimum width=1.8cm] (tcq) {tCQ};
  \node[hw compute, right=0.3cm of tcq, minimum width=2.8cm] (tcomb) {组合逻辑};
  \node[hw compute, right=0.3cm of tcomb, minimum width=1.4cm] (twire) {连线};
  \node[hw compute, right=0.3cm of twire, minimum width=1.2cm] (tsu) {tSU};
  \node[hw compute, right=0.3cm of tsu, minimum width=1.0cm] (tskew) {偏斜+余量};
  \node[hw label, below=0.1cm of tcq] {0.8};
  \node[hw label, below=0.1cm of tcomb] {3.6};
  \node[hw label, below=0.1cm of twire] {0.7};
  \node[hw label, below=0.1cm of tsu] {0.4};
  \node[hw label, below=0.1cm of tskew] {0.5};
  \draw[hw bus] (tcq.west) -- ++(-0.6,0) node[left,hw label]{CLK};
  \draw[hw bus] (tskew.east) -- ++(0.6,0) node[right,hw label]{CLK};
  \draw[BookMuted, decorate, decoration={brace,amplitude=4pt,raise=2pt}] (tcq.north west) -- (tskew.north east) node[midway,above=8pt,BookAccent,font=\small\bfseries] {Tclk $\geq$ 6.0ns};
\end{block}
\diagramnote{时钟周期预算条。}
\end{lstlisting}
...
复位本身也要讲边界。
\begin{block}[x=1.5cm,y=0.9cm]
  \node[draw, dashed, rounded corners, inner sep=5pt] (rst_in) {外部复位};
  \node[hw state, right=0.8cm of rst_in] (ff1) {FF$_1$};
  \node[hw state, right=0.8cm of ff1] (ff2) {FF$_2$};
  \node[hw control, right=0.8cm of ff2] (ctrl) {同步};
  ...
  \node[below=0.15cm of ff1, hw label] {D=1};
  \node[below=0.15cm of ff2, hw label] {D=Q$_1$};
\end{block}
\diagramnote{异步拉入/同步释放复位电路。}
...
最常见的单 bit 同步结构是两级触发器串联
\begin{waveform}[x=1pt,y=1.2ex]
  \draw[BookRule!30,step=20pt] (0,-4ex) grid (280pt,8ex);
  \draw[BookAccent,thick] (0,4ex) -- (0,6ex) -- (20pt,6ex) -- (20pt,4ex) -- (40pt,4ex) -- (40pt,6ex) -- (60pt,6ex) -- (60pt,4ex) -- (80pt,4ex) -- (80pt,6ex) -- (100pt,6ex) -- (100pt,4ex) -- (120pt,4ex) -- (120pt,6ex) -- (140pt,6ex) -- (140pt,4ex) -- (160pt,4ex) -- (160pt,6ex) -- (180pt,6ex) -- (180pt,4ex) -- (200pt,4ex) -- (200pt,6ex) -- (220pt,6ex) -- (220pt,4ex) -- (240pt,4ex) -- (240pt,6ex) -- (260pt,6ex) -- (260pt,4ex) -- (280pt,4ex);
  \node[BookAccent,left] at (0,5ex) {CLK};
  \draw[BookAmber,thick] (0,0) -- (50pt,0) -- (50pt,3ex) -- (150pt,3ex) -- (150pt,0) -- (280pt,0);
  \node[BookAmber,left] at (0,1.5ex) {async};
  \draw[BookRed,thick] (0,-3ex) -- (20pt,-3ex) -- (20pt,0) -- (50pt,0);
  \draw[BookRed,dashed,thick] (50pt,0) .. controls (58pt,2ex) and (62pt,-2ex) .. (70pt,0);
  \draw[BookRed,thick] (70pt,0) -- (100pt,0) -- (100pt,-3ex) -- (150pt,-3ex);
  \draw[BookRed,dashed,thick] (150pt,-3ex) .. controls (158pt,-1ex) and (162pt,1ex) .. (170pt,0);
  \draw[BookRed,thick] (170pt,0) -- (280pt,0);
  \node[BookRed,left] at (0,-1.5ex) {FF$_1$};
  \draw[BookTeal,thick] (0,-5.5ex) -- (60pt,-5.5ex) -- (60pt,-3ex) -- (180pt,-3ex) -- (180pt,-5.5ex) -- (280pt,-5.5ex);
  \node[BookTeal,left] at (0,-4.5ex) {FF$_2$};
  \node[BookRed,font=\scriptsize,align=center] at (60pt,2.5ex) {亚稳态};
  \node[BookTeal,font=\scriptsize] at (120pt,-5ex) {稳定};
\end{waveform}
\diagramnote{两级同步器波形：第一级可能亚稳态，第二级输出稳定。}。第一级直接采样异步输入，可能进入亚稳态；第二级在下一个时钟沿再采样第一级输出，给第一级更多时间恢复到明确 0 或 1。两级同步不能从数学上消灭亚稳态，但能显著降低亚稳态传播到后级控制逻辑的概率。多 bit 数据不能简单地每一位各接两级同步后直接使用，因为各位可能在不同周期被采到，形成混合值；多 bit 跨边界通常要使用握手、有效位、灰码计数器或异步 FIFO。
...
\begin{block}[x=1.6cm,y=0.9cm]
  \node[draw, dashed, rounded corners, inner sep=6pt, label={[hw label]above:异步域}] (async) {输入};
  \node[hw state, right=1.0cm of async, label={[hw label]above:第一级}] (ff1) {FF$_1$};
  \node[hw state, right=1.0cm of ff1, label={[hw label]above:第二级}] (ff2) {FF$_2$};
  \node[draw, dashed, rounded corners, inner sep=6pt, right=1.0cm of ff2, label={[hw label]above:目标域}] (sync) {sync};
  \draw[hw bus] (async) -- (ff1) node[above,hw label,pos=0.5]{metastable};
  \draw[hw bus] (ff1) -- (ff2) node[above,hw label,pos=0.5]{recovery};
  \draw[hw bus] (ff2) -- (sync) node[above,hw label,pos=0.5]{stable};
  \node[below=0.15cm of ff1, BookRed, font=\scriptsize] {meta};
  \node[below=0.15cm of ff2, BookTeal, font=\scriptsize] {sync};
\end{block}
...
\begin{block}[x=1.5cm,y=1cm]
  \node[draw, rectangle, minimum width=1.6cm, minimum height=1.4cm, fill=BookCodeBg] (latch) at (0,0) {};
  \node[above=0.05cm of latch, font=\scriptsize\bfseries] {D-Latch};
  \node[left] at ($(latch.west)+(-0.1,0.3)$) {D};
  \node[left] at ($(latch.west)+(-0.1,-0.3)$) {E};
  \node[right] at ($(latch.east)+(0.1,0.3)$) {Q};
  \node[right=0.5cm of latch, scale=1.3] {$\Rightarrow$};
  \node[draw, rectangle, minimum width=1.6cm, minimum height=1.4cm, fill=BookCodeBg, right=2.5cm of latch] (ff) {};
  \node[above=0.05cm of ff, font=\scriptsize\bfseries] {D-FF};
  \node[left] at ($(ff.west)+(-0.1,0.3)$) {D};
  \node[left] at ($(ff.west)+(-0.1,-0.3)$) {CLK};
  \node[right] at ($(ff.east)+(0.1,0.3)$) {Q};
  \draw[BookAccent, thick] ($(ff.west)+(-0.05,-0.3)$) -- ++(0.15,0) -- ++(0,0.15);
  \node[below=0.1cm of ff, font=\scriptsize, BookAccent] {上升沿};
\end{block}
\diagramnote{锁存器(透明) vs 触发器(边沿)。}
...
on clock edge:
\begin{block}[x=1.5cm,y=0.9cm]
  \node[hw compute, minimum width=1.8cm] (add) {+1};
  \node[hw state, right=0.8cm of add, minimum width=1.6cm] (ff) {4-bit 寄存器};
  \draw[hw bus] (add) -- (ff) node[above, hw label, pos=0.5]{next};
  \draw[hw bus] (ff.east) -- ++(0.5,0) node[right] (q) {Q[3:0]};
  \draw[hw bus] (q) |- ++(0,0.8) -| (add.north) node[above right, hw label, pos=0.3]{当前};
  \node[hw control, below=0.5cm of add, minimum width=1.0cm] (en) {EN};
  \draw[hw bus] (en) -| (add.south) node[below right, hw label]{使能};
  \node[below=0.1cm of en, hw label] {0=保持, 1=加一};
\end{block}
...
next[1] = current[0]
next[0] = serial_in
\begin{block}[x=1.4cm,y=0.9cm]
  \node[hw state, minimum width=
2.4cm, label={[hw label]above:8-bit 移位}] (shift) {};
  \node[hw memory, right=0.6cm of shift, minimum width=2.4cm, label={[hw label]above:输出锁存}] (latch) {};
  \node[right=0.4cm of latch] (q) {Q$_0$..Q$_7$};
  \draw[hw bus] (shift) -- (latch) node[above, hw label, pos=0.5]{8-bit};
  \draw[hw bus] (latch) -- (q);
  \draw[hw bus] (shift.west) -- ++(-0.5,0) node[left]{DS};
  \node[below=0.15cm of shift, hw label] {SHCP};
  \node[below=0.15cm of latch, hw label] {STCP};
\end{block}
\diagramnote{74HC595：移位寄存器 + 输出锁存。}
\end{lstlisting}
...
有限状态机由三部分组成：当前状态、输入条件、下一状态逻辑。
\begin{block}[x=1.5cm,y=1.0cm]
  \node[hw state, minimum width=1.6cm] (state) {状态寄存器};
  \node[hw compute, right=0.8cm of state, minimum width=1.8cm] (next) {下一状态逻辑};
  \node[hw control, right=0.8cm of next, minimum width=1.6cm] (out) {输出逻辑};
  \node[left=0.6cm of state] (clk) {CLK};
  \node[left=0.6cm of state, below=0.3cm of clk] (inputs) {输入};
  \node[right=0.6cm of out] (outputs) {输出};
  \draw[hw bus] (clk) -- (state);
  \draw[hw bus] (state) -- (next) node[above, hw label, pos=0.5]{状态};
  \draw[hw bus] (inputs) |- (next);
  \draw[hw bus] (inputs) |- (out);
  \draw[hw bus] (next) -- (state) node[left, hw label, pos=0.5]{next};
  \draw[hw bus] (out) -- (outputs);
  \draw[hw bus] (state) |- ++(0,-0.6) -| (out) node[below, hw label, pos=0.3]{状态};
\end{block}
\diagramnote{有限状态机结构。}当前状态保存在寄存器里，组合逻辑根据当前状态和输入条件决定下一状态。
...
\begin{block}[x=1.5cm,y=1.2cm]
  \node[draw, circle, minimum size=1.2cm, fill=BookCodeBg] (idle) {IDLE};
  \node[draw, circle, minimum size=1.2cm, fill=BookCodeBg, right=2cm of idle] (load) {LOAD};
  \node[draw, circle, minimum size=1.2cm, fill=BookCodeBg, right=2cm of load] (run) {RUN};
  \node[draw, circle, minimum size=1.2cm, fill=BookCodeBg, right=2cm of run] (done) {DONE};
  ...
  \draw[hw bus, bend right=30] (idle) to node[below, hw label]{start=0} (idle);
  \draw[hw bus, bend left=30] (done) to node[below, hw label]{clear=0} (done);
\end{block}
\diagramnote{四状态控制器状态图。}
...
其次，复位也有“域”。
\begin{block}[x=1.5cm,y=1.0cm]
  \node[hw compute, minimum width=1.4cm] (mux) {MUX};
  \node[hw state, right=0.6cm of mux, minimum width=1.2cm] (ff) {D-FF};
  \draw[hw bus] (mux) -- (ff);
  \draw[hw bus] (ff.east) -- ++(0.4,0) |- (mux.south);
  \draw[hw bus] (mux.west) -- ++(-0.4,0) node[left]{D};
  \node[hw control, below=0.4cm of mux] (we) {WE};
  \draw[hw bus] (we) -- (mux);
  \node[below=0.1cm of ff, hw label] {时钟始终存在};
  \node[right=0.8cm of ff, scale=1.5] (arr) {$\Rightarrow$};
  \node[hw control, right=0.8cm of arr, minimum width=1.0cm] (andg) {AND};
  \node[hw state, right=0.6cm of andg, minimum width=1.2cm] (ff2) {D-FF};
  \draw[hw bus] (andg) -- (ff2);
  \draw[hw bus] (ff2.east) -- ++(0.4,0) node[right]{Q};
  \draw[hw bus] (andg.west) -- ++(-0.4,0) node[left]{CLK};
  \node[hw control, below=0.4cm of andg] (en) {EN};
  \draw[hw bus] (en) -- (andg);
  \node[below=0.1cm of ff2, hw label] {EN 毛刺 → 时钟毛刺};
\end{block}
\diagramnote{写使能 vs 门控时钟。}

...
\bottomrule
\end{longtable}
\begin{block}[x=2.2cm,y=1.5cm]
  \node[hw state, minimum width=3.8cm, label={[hw label]above:8-bit 移位寄存器}] (shift) {};
  \node[hw memory, right=1.0cm of shift, minimum width=3.8cm, label={[hw label]above:8-bit 输出锁存}] (latch) {};
  \node[right=0.6cm of latch] (q) {Q$_0$..Q$_7$};
  \draw[hw bus] (shift) -- (latch) node[above, hw label, pos=0.5]{8-bit};
  \draw[hw bus] (latch) -- (q);
  \draw[hw bus] (shift.west) -- ++(0.8,0) node[left]{DS};
  \node[below=0.25cm of shift, hw label] {SHCP 移位};
  \node[below=0.25cm of latch, hw label] {STCP 锁存};
\end{block}
...
\bottomrule
\end{longtable}
\begin{block}[x=2.5cm,y=1.6cm]
  \node[hw state, minimum width=2.4cm] (state) {状态寄存器};
  \node[hw compute, right=1.2cm of state, minimum width=3.0cm] (next) {下一状态逻辑};
  \node[hw control, right=1.2cm of next, minimum width=2.4cm] (out) {输出逻辑};
  \node[left=1.0cm of state] (clk) {CLK};
  \node[left=1.0cm of state, below=0.5cm of clk] (inputs) {输入};
  \node[right=1.0cm of out] (outputs) {控制输出};
  ...
  \draw[hw bus] (state) |- ++(0,-0.8) -| (out) node[below, hw label, pos=0.3]{当前状态};
\end{block}
...
\bottomrule
\end{longtable}
\begin{block}[x=2.5cm,y=1.8cm]
  \node[draw, circle, minimum size=1.8cm, fill=BookCodeBg] (idle) {IDLE};
  \node[draw, circle, minimum size=1.8cm, fill=BookCodeBg, right=3.0cm of idle] (load) {LOAD};
  \node[draw, circle, minimum size=1.8cm, fill=BookCodeBg, right=3.0cm of load] (run) {RUN};
  \node[draw, circle, minimum size=1.8cm, fill=BookCodeBg, right=3.0cm of run] (done) {DONE};
  ...
  \draw[hw bus, bend right=40] (idle) to node[below, hw label]{start=0} (idle);
  \draw[hw bus, bend left=40] (done) to node[below, hw label]{clear=0} (done);
\end{block}
...
\begin{block}[x=2.5cm,y=1.6cm]
  ...
  \node[hw state, right=1.0cm of mux, minimum width=2.0cm] (ff) {D-FF};
  \draw[hw bus] (mux) -- (ff);
  \draw[hw bus] (ff.east) -- ++(0.6,0) |- (mux.south);
  \draw[hw bus] (mux.west) -- ++(-0.6,0) node[left]{D};
  \node[hw control, below=0.6cm of mux] (we) {WE};
  \draw[hw bus] (we) -- (mux);
  \node[below=0.2cm of ff, hw label] {时钟始终存在};
  \node[right=1.2cm of ff, scale=2.0] (arr) {$\Rightarrow$};
  \node[hw control, right=1.2cm of arr, minimum width=1.8cm] (andg) {AND};
  \node[hw state, right=1.0cm of andg, minimum width=2.0cm] (ff2) {D-FF};
  \draw[hw bus] (andg) -- (ff2);
  \draw[hw bus] (ff2.east) -- ++(0.6,0) node[right]{Q};
  \draw[hw bus] (andg.west) -- ++(-0.6,0) node[left]{CLK};
  \node[hw control, below=0.6cm of andg] (en) {EN};
  \draw[hw bus] (en) -- (andg);
  \node[below=0.2cm of ff2, hw label] {EN 毛刺 时钟毛刺};
\end{block}

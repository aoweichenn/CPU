...
\end{longtable}
\vspace{3.0em plus 1.0em}
\begin{circuit}[x=1.6cm,y=1.6cm]
  \node[nand gate US, draw, logic gate inputs=nn, scale=1.3] (n1) at (0,3) {};
  \node[nand gate US, draw, logic gate inputs=nn, scale=1.3] (n2) at (0,0) {};
  % 外部输入
  \draw (n1.input 1) -- ++(-1.4,0) node[left] {$\overline{S}$};
  \draw (n2.input 2) -- ++(-1.4,0) node[left] {$\overline{R}$};
  % 输出
  \draw (n1.output) -- ++(1.2,0) node[right,BookTeal] {Q};
  \draw (n2.output) -- ++(1.2,0) node[right,BookTeal] {$\overline{Q}$};
  % 交叉耦合 —— 对角线画出经典 X
  \draw[BookAccent,line width=0.55pt] (n1.output) -- ++(0.5,0) -- ++(-2.0,-2.5) -- (n2.input 1);
  \draw[BookAccent,line width=0.55pt] (n2.output) -- ++(0.35,0) -- ++(-1.85,2.5) -- (n1.input 2);
\end{circuit}

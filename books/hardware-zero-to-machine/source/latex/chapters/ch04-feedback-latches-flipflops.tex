...
\end{longtable}
\vspace{3.0em plus 1.0em}
\begin{block}[x=2.2cm,y=1.8cm]
  \node[draw, rectangle, minimum width=2.8cm, minimum height=2.4cm, fill=BookCodeBg, rounded corners=3pt] (sym) at (0,0) {};
  \node[above=0.15cm of sym, font=\small\color{BookNavy}] {D-Latch};
  \node[left] at ($(sym.west)+(-0.15,0.6)$) {D};
  \node[left] at ($(sym.west)+(-0.15,-0.6)$) {LE};
  \node[right] at ($(sym.east)+(0.15,0.6)$) {Q};
  \node[right] at ($(sym.east)+(0.15,-0.6)$) {Q'};
  \node[below=0.4cm of sym, font=\footnotesize, BookAccent] {LE=1: 透明};
  \node[below=0.75cm of sym, font=\footnotesize, BookTeal] {LE=0: 保持};
\end{block}
...
\vspace{3.0em plus 1.0em}
\begin{block}[x=2.2cm,y=1.8cm]
  \node[draw, rectangle, minimum width=2.6cm, minimum height=2.2cm, fill=BookCodeBg, rounded corners=3pt] (ff) at (0,0) {};
  \node[above=0.15cm of ff, font=\small\color{BookNavy}] {D-FF};
  \node[left] at ($(ff.west)+(-0.15,0.6)$) {D};
  \node[left] at ($(ff.west)+(-0.15,-0.6)$) {CLK};
  \node[right] at ($(ff.east)+(0.15,0.6)$) {Q};
  \node[right] at ($(ff.east)+(0.15,-0.6)$) {Q'};
  \draw[BookAccent, very thick] ($(ff.west)+(-0.08,-0.6)$) -- ++(0.25,0) -- ++(0,0.25);
  \node[below=0.4cm of ff, font=\footnotesize, BookAccent] {上升沿采样};
\end{block}
...
\begin{waveform}[x=2pt,y=5ex]
  ...
  \draw[BookRed,line width=1.2pt,<->] (32pt,19ex) -- (44pt,19ex);
  \draw[BookAmber,line width=1.2pt,<->] (44pt,19ex) -- (96pt,19ex);
  \draw[BookTeal,line width=1.2pt,<->] (96pt,19ex) -- (108pt,19ex);
\end{waveform}
...
\begin{block}[x=2.0cm,y=1.5cm]
  \node[hw state, minimum width=2.2cm] (state) {状态寄存器};
  \node[hw compute, right=1.2cm of state, minimum width=2.6cm] (next) {下一状态逻辑};
  \node[hw control, right=1.2cm of next, minimum width=2.2cm] (out) {输出逻辑};
  \node[left=0.8cm of state] (clk) {CLK};
  \node[below=0.6cm of next] (inputs) {输入};
  ...
  \draw[hw bus] (inputs) -| (out.south);
  \draw[hw bus, bend right=40] (next.south) to node[below, hw label, pos=0.5]{next state} (state.south);
  \draw[hw bus] (out) -- (outputs);
  \draw[hw bus] (state.east) |- ++(0.3,-0.8) -| (out.south) node[below, hw label, pos=0.6]{当前状态};
\end{block}
...
\vspace{3.0em plus 1.0em}
\begin{block}[x=1.8cm,y=1.5cm]
  \node[hw compute, minimum width=2.0cm] (mux) {MUX};
  \node[hw state, right=0.8cm of mux, minimum width=1.8cm] (ff) {D-FF};
  \draw[hw bus] (mux) -- (ff);
  \draw[hw bus] (ff.east) -- ++(0.5,0) |- (mux.south);
  \draw[hw bus] (mux.west) -- ++(-0.5,0) node[left]{D};
  \node[hw control, below=0.5cm of mux] (we) {WE};
  \draw[hw bus] (we) -- (mux);
  \node[below=0.35cm of ff, hw label, align=center] {时钟始终存在};
  \node[right=1.5cm of ff, font=\LARGE\bfseries, BookAccent] (arr) {$\Rightarrow$};
  \node[hw control, right=1.5cm of arr, minimum width=1.4cm] (andg) {AND};
  \node[hw state, right=0.8cm of andg, minimum width=1.8cm] (ff2) {D-FF};
  ...
  \draw[hw bus] (en) -- (andg);
  \node[below=0.35cm of ff2, hw label] {EN 毛刺$\to$时钟毛刺};
\end{block}

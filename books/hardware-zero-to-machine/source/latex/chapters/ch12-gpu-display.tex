...
这个思想本书已经见过一次。第十三章讲 NVMe 队列
时算过：设备时延固定，吞吐靠在途请求数撑起来，
是第十二章 Little 定律的直接应用。GPU 把同一公式
...
是算术。与第十三章 NVMe 一次读的 trace 对照着看
...
量级（每次数微秒）——传输必须攒大块，这与第十二章"批量摊薄固定成本"
...
显存 VRAM & 数 GB 至数十 GB & 约四到八百拍 & 分配器管空间，带宽见前文 \\
...
现在把本节全部数字收进第十二章的 roofline。判定式只有一
...
GPU 与 CPU 的协作不靠函数调用，而靠一条与第十三章 NVMe
...
尾差补读，偶尔错过一次通知也无妨。与第十三章 NVMe 提交
...
以第十三章"先写命令项、再写 doorbell，中间隔一道屏障"
...
把 GPU 命令环与第十三章的 NVMe 提交队列逐项对照，结构
...
章中断合并、第十三章轮询模式（如高性能 NVMe 驱动 polling
...
做的浮点操作在 $10^{10}$ 到 $10^{11}$ 量级（标量到向量峰值），连片元着色这一级都扛不住。这就
...
这张表有三处值得停下来看。第一，第 1 到第 3 步与第十三章 NVMe
...
锁死——管线吞吐由瓶颈级决定，这与第十二章的利用率分析是同一条
规律。
...
步 & 动作 & 耗时量级 & 执行者 \\
\midrule
1 & 把 a、b 放入锁页（pinned）缓冲（第五章：DMA 要求物理地址稳定） & 一次性 & CPU \\
...
7 & DMA 把 4 MiB 结果搬回主存 & 约 266 $\mu$s & GPU DMA \\
...
作每字节；按第十二章 roofline，峰值算力 10 TFLOPS、带宽 400 GB/s
...
把这次 trace 与第十三章的 NVMe 读并排摆开，是同一个骨架穿了两
套衣服：
...
第十二章定量方法给出的决策顺序：先测负载各段占多少时间，再决定
把哪一段交给专用硬件。

...
前面三章已经解释了 bit 如何稳定、组合逻辑如何计算、状态元件如何保存历史、ALU 和控制器如何让数据通路在每个时钟边界提交新状态。若停在这里，读者只能得到一台“会在片内寄存器之间搬数”的小机器。真正的整机还需要大容量主存、可寻址设备、外部事件、等待和错误路径。本章专门补上这条桥：CPU 怎样把一个地址变成一次存储器或设备事务，设备怎样把完成事件通知 CPU，DMA 又怎样在不让 CPU 逐字搬运的情况下读写主存。
...
现代互连 & 用 valid/ready 描述一次读请求和读响应 & 能说明背压和 completion 的作用 \\
\bottomrule
\end{longtable}
